...
'**
'**       作  成  日    :  2004/02/20
'**       変  更  日    :  2004/05/12  By CIS - バグ対応（仕入先が未登録時、異常終了）
...
'   CSVﾌｧｲﾙ出力先
    mCsvName = Mid(CisFun.WinRoot, 1, 3) & "入荷一覧表.csv"
    mCsvKey = "CXH0250"
...
'   仕入先
    ...
    lb_msg.Caption = "【 入荷データ 抽出中 】"
    DoEvents

    gSL_Select = ""
    gSL_Select = gSL_Select & "select"
    gSL_Select = gSL_Select & "  nyt.*"
    ...
    gSL_Select = gSL_Select & ", isnull(hnm.表示品番, nyt.品番) 表示品番"
    gSL_Select = gSL_Select & ", isnull(hct.契約先, '') 契約先"
    gSL_Select = gSL_Select & " from"
    gSL_Select = gSL_Select & "  入荷テーブル nyt"
    gSL_Select = gSL_Select & "  left outer join 発注テーブル hct"
    gSL_Select = gSL_Select & "    on hct.発注管理NO = nyt.発注管理NO"
    gSL_Select = gSL_Select & "  left outer join 取引先マスタ trm"
    gSL_Select = gSL_Select & "    on trm.取引先CD = nyt.仕入先"
    gSL_Select = gSL_Select & "   and trm.取引先区分 = 1"
    gSL_Select = gSL_Select & "  left outer join 品番マスタ hnm"
    gSL_Select = gSL_Select & "    on hnm.品番 = nyt.品番"
    gSL_Select = gSL_Select & " where nyt.発注管理NO >= 0"
'   >> 仕入先
    If RTrim(iH1_Torcd) <> "" Then
        gSL_Select = gSL_Select & "   and nyt.仕入先 = '" & RTrim(iH1_Torcd) & "'"
    End If
'   >> 入荷日
    If Trim(iH1_SYmd) <> "" And CisFun.Val2(iH1_SYmd.cDay) = 0 Then
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(nyt.入荷日, 1, 6) = '" & iH1_SYmd & "'"
    Else
        '> 開始日のみ
        If Trim(iH1_SYmd) <> "" And Trim(iH1_EYmd) = "" Then
            gSL_Select = gSL_Select & "   and nyt.入荷日 = '" & Trim(iH1_SYmd) & "'"
        End If
        '> 終了日のみ
        If Trim(iH1_SYmd) = "" And Trim(iH1_EYmd) <> "" Then
            gSL_Select = gSL_Select & "   and nyt.入荷日 <= '" & Trim(iH1_EYmd) & "'"
        End If
        '> 開始日＆終了日
        If Trim(iH1_SYmd) <> "" And Trim(iH1_EYmd) <> "" Then
            gSL_Select = gSL_Select & "   and nyt.入荷日 between '" & Trim(iH1_SYmd) & "'" _
                                                         & " and '" & Trim(iH1_EYmd) & "'"
        End If
    End If
    gSL_Select = gSL_Select & " order by"
    gSL_Select = gSL_Select & "  nyt.仕入先"
    gSL_Select = gSL_Select & ", nyt.品番"
    gSL_Select = gSL_Select & ", nyt.背番号"
...
Private Function PrintStart() As Boolean
    PrintStart = False
    If Op_Out(0).Value Or Op_Out(1).Value Then
        lb_msg.Caption = "【 入荷データ 印刷中 】"
        DoEvents
        With CisPrtCXH0250
            .cDataMaxCnt = CisDB.RecordCount
            Set .iPrtForm = CXH0250L
            .dpMSG = "入荷一覧表　印刷中"
