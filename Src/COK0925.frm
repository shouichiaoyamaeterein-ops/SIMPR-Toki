...
Begin VB.Form COK0925 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "COK0925.frx":0000
         Height          =   5565
         Left            =   45
         OleObjectBlob   =   "COK0925.frx":0015
         TabIndex        =   3
         Top             =   60
         Width           =   14190
      End
...
Private Function BodySet() As Boolean
    ...
    BodySet = False
    gSL_Select = "SELECT SUBSTRING(KJ.実績年月,1,4) + '/' + SUBSTRING(KJ.実績年月,5,2) 実績年月,"
    gSL_Select = gSL_Select & "CASE WHEN RTRIm(KJ.検収日) <> '' THEN "
    gSL_Select = gSL_Select & "          SUBSTRING(KJ.検収日,1,4) + '/' + SUBSTRING(KJ.検収日,5,2) + '/' + SUBSTRING(KJ.検収日,7,2) "
    gSL_Select = gSL_Select & "     ELSE '' END 編集検収日,"
    gSL_Select = gSL_Select & "KJ.契約先,KJ.検収日,KJ.納入番号,KJ.品番,"
    gSL_Select = gSL_Select & "CASE WHEN KJ.有無償区分 = 0 THEN '有償' ELSE '無償' END 有無償区分,"
    gSL_Select = gSL_Select & "CASE WHEN HN.表示品番 IS NOT NULL THEN HN.表示品番 ELSE KJ.品番 END 編集品番,"
    gSL_Select = gSL_Select & "KJ.数量,KJ.単価,KJ.金額,KJ.識別子,KJ.出庫者,KJ.検収管理NO,  "
    gSL_Select = gSL_Select & "ISNULL(SY.値名称,'') 識別子名,  "
    gSL_Select = gSL_Select & "ISNULL(TR1.略称,'') 仕入先名, "
    gSL_Select = gSL_Select & "ISNULL(TR2.略称,'') 支給先名 "
    ...
    mSL_OrderA = " Order By KJ.実績年月,KJ.納入番号,KJ.品番,KJ.検収日,KJ.検収管理NO "
    mSL_OrderD = " Order By KJ.実績年月 Desc,KJ.納入番号 Desc,KJ.品番 Desc,KJ.検収日 Desc,KJ.検収管理NO Desc"
' 選択条件セット
    TorWhere = " 赤黒区分 = 0 AND KJ.実績年月 < '" & RTrim(RV_YM) & "'"
    mSL_SelectW = ""
    With CisFun
        .Where_CreateKB = NewDocument
        .Where_Create AlphaMe, "KJ.契約先", iH1_Torcd
        .Where_Create AlphaMe, "KJ.実績年月", iH1_KYm
        .Where_Create AlphaMe, "KJ.納入番号", iH1_Nouno
