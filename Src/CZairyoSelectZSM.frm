...
Begin VB.Form CZairyoSelectZSM 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料選択画面"
   ClientHeight    =   3495
   ...
   Begin VB.PictureBox Picture 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CZairyoSelectZSM.frx":0000
         Height          =   2520
         Left            =   30
         OleObjectBlob   =   "CZairyoSelectZSM.frx":0015
         TabIndex        =   4
         Top             =   30
         Width           =   9525
      End
   ...
   End
End
Attribute VB_Name = "CZairyoSelectZSM"
...
'**********************************************************'
'**
'**       フォーム名    :  受給/発注/自社/材質　品番選択画面
'**       フォームID    :  CZairyoSelectZSM
'**       処理概要      :
'**
'**       作  成  日    :  2009/09/29  By CIS
...
Private Sub Form_Activate()
    ...
    mSL_Select = ""
    mSL_Select = mSL_Select & " SELECT ZT.材料管理番号, ZT.受給品番, ZT.発注品番,ZT.自社品番,"  ' 2009.09.29 upd : 自社品番を追加
    mSL_Select = mSL_Select & "        ZT.材質, ZM.板厚, ZM.幅, ZM.長さ, ZT.契約先,"            ' 2009.10.01 add : 契約先を追加
...
'   mSL_Select = mSL_Select & "  WHERE ZT.背番号 = '" & RTrim(RV_Seban) & "'"                   ' 2009.09.29 del
'   mSL_Select = mSL_Select & "    AND ZT.取引先 = '" & RTrim(RV_Torcd) & "'"                   ' 2009.09.29 del
    mSL_Select = mSL_Select & "  WHERE ZT.取引先 = '" & RTrim(RV_Torcd) & "'"                   ' 2009.09.29 add : 上の２文を差し替え
    If RTrim(RV_KTorcd) <> "" Then                                                              ' 2009.10.01 add
       mSL_Select = mSL_Select & "    AND ZT.契約先 = '" & RTrim(RV_KTorcd) & "'"               ' 2009.10.01 add
    End If                                                                                      ' 2009.10.01 add
'    If RVI_Hinmoku <> 9 Then
'       mSL_Select = mSL_Select & "    AND 品目 = " & RVI_Hinmoku
'    End If
    mSL_Select = mSL_Select & "  ORDER BY ZT.材質, ZT.受給品番, ZT.発注品番, ZT.自社品番"       ' 2009.09.29 upd : 自社品番を追加
...
'   H1lb_Seban = RTrim(RV_Seban)        ' 背番号        ' 2009.09.30 del
    H1lb_Torcd = RTrim(RV_Torcd)        ' 取引先コード  ' 2009.09.30 add
...
Private Sub RtnSet()
    With Adodc1
        RV_KTorcd = .Recordset("契約先")        ' 2009.10.01 add
        RV_Zaist = .Recordset("材質")
        RV_JHinbn = .Recordset("受給品番")
        RV_HHinbn = .Recordset("発注品番")
        RV_Zainm = .Recordset("自社品番")       ' 2009.09.29 add : 自社品番を RV_Zainm で代用
