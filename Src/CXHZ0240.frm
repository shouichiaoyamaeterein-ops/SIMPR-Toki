...
Begin VB.Form CXHZ0240 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料・入荷画面入力 [単品]-品番入力"
   ClientHeight    =   9945
   ClientLeft      =   3375
   ClientTop       =   1350
   ClientWidth     =   14715
      ...
      Begin Cis3D_v60.CIS3D UC_3D4 
         ...
         Begin Cis3D_v60.CIS3D BLB_DD 
            ...
            Begin vsFlexLib.vsFlexArray vsGrid 
               ...
               Appearance      =   1
               ConvInfo        =   1418783674
               FormatString    =   "№ |背番号  |品  番  /  材質-寸法　　　  　　              |収容数    |枚数 |端   　数 |入荷数   　|単位| |"
               Rows            =   11
               Cols            =   23
               BackColor       =   12648384
               ...
               Begin Cis3D_v60.CIS3D Input_Area 
                  ...
                  cFont3DColor2   =   8421504
                  cBoderStyle     =   1
                  Begin Cis3D_v60.CIS3D B1lb_Sunpo 
                     Height          =   210
                     Left            =   6660
                     Top             =   285
                     Visible         =   0   'False
                     ...
                     _ExtentY        =   370
                     BackColor       =   16777152
                     Caption         =   "XXXXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXX"
                     BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
                        Name            =   "ＭＳ ゴシック"
                        Size            =   11.25
                        Charset         =   128
                     ...
                     cPositionX      =   30
                  End
                  Begin Cis3D_v60.CIS3D B1lb_Zaisitu 
                     Height          =   210
                     Left            =   6660
                     Top             =   90
                     Visible         =   0   'False
                     ...
                     cPositionX      =   30
                  End
                  Begin Cis3D_v60.CIS3D B1lb_ZSunpo 
                     Height          =   210
                     Left            =   1050
                     Top             =   285
                     Width           =   5610
...
'**********************************************************'
'**
'**       フォーム名    :   材料入荷画面入力 [単品]-品番入力
'**       フォームID    :   CXHZ0240
'**       処理概要      :
'**
'**       作  成  日    :   2004/02/24  By CIS SIMPR-A
'**       変  更  日    :   2004/05/28  By CIS 背番号半角ﾁｪｯｸ追加
'**       変  更  日    :   2005/11/17  By CIS オプション追加（稼働日カレンダー）
'**                                             入荷日チェック=1:仕入先カレンダー
'**       変  更  日    :   2008/04/11  By CIS 背番号８桁対応
'**       変  更  日    :   2009/01/16  By CIS 手配区分：直送時　直送向け支給伝票発行
'**       変  更  日    :   2009/02/24  By CIS バグ修正
'**       変  更  日    :   2009/03/14  By CIS 実績管理ﾃｰﾌﾞﾙ出力時に材料ﾏｽﾀ参照(原因不明ﾊﾞｸﾞ対応)
'**       変  更  日    :   2009/06/25  By CIS 支給伝票の発行をﾌﾟﾙｰﾌﾘｽﾄ発行後に変更
'**
'**********************************************************'
...
Private Sub PB_DSP_Click()
    ...
    CXHZ0231.Show vbModal
    Unload CXHZ0231
    Set CXHZ0231 = Nothing
...
Private Sub iB1_Seban_LostFocus()
    ...
    If RTrim(iB1_Seban) = "" And RTrim(iB1_Hinbn) = "" Then
        B1lb_Syuyo = ""
        iB1_Maisu = ""
        iB1_HSuryo = ""
        B1lb_Syuyo = ""
...
Private Sub iB1_Hinbn_LostFocus()
    ...
    vsGrid.TextMatrix(CisFun.Val2(Input_Area.Tag), 2) = iB1_Hinbn & vbCr & B1lb_ZSunpo
    vsGrid.TextMatrix(CisFun.Val2(Input_Area.Tag), 22) = iB1_Hinbn
...
'日付チェック
    If RTrim(iH1_Ymd) = "" Then
        CisFun.MB_MSG(1) = "     　【　入荷日 必須！ 】     "
        CisFun.MB_MSG(3) = "     入荷日 を入力して下さい。      "
        CisFun.MBOX
        iH1_Ymd.SetFocus
        Exit Function
    End If
    If Not iH1_Ymd.cChkResult Then
        CisFun.MB_MSG(1) = "     　【　入荷日 異常！ 】     "
        CisFun.MB_MSG(3) = "     入荷日を再入力して下さい。      "
        CisFun.MBOX
        iH1_Ymd.SetFocus
        Exit Function
    End If
    If RTrim(iH1_Ymd) > Format(Now(), "yyyymmdd") Then
        CisFun.MB_MSG(1) = "     入荷日が未来の日付です。     "
...
' マスタ入力 SELECT文 ｾｯﾄ
    If OpH1_HinKB(0) Then
       gSL_Select = " SELECT ZW.*,ISNULL(SY1.値名称,'')手配単位名,ZTM.受給品番 品番,ZNM.表示寸法"
    Else
       gSL_Select = " SELECT ZW.*,ISNULL(SY1.値名称,'')手配単位名,ZTM.発注品番 品番,ZNM.表示寸法"
    End If
    gSL_Select = gSL_Select & " FROM 材料入荷画面入力ワーク ZW"
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 材料マスタ ZNM"
    gSL_Select = gSL_Select & "   ON  ZNM.材料管理番号 = ZW.材料管理番号"
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 材料取引先マスタ ZTM"
    gSL_Select = gSL_Select & "   ON ZTM.材料管理番号 = ZW.材料管理番号"
    gSL_Select = gSL_Select & "   AND ZTM.取引先 = ZW.仕入先"
    gSL_Select = gSL_Select & "   AND ZTM.受入 = ZW.受入"
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 名称マスタ SY1"
    gSL_Select = gSL_Select & "   ON  SY1.区分名称 = '単位区分'"
    gSL_Select = gSL_Select & "   AND SY1.値 = ZNM.手配単位"
    ...
    gSL_Select = gSL_Select & "   AND ZW.受入 = '" & RTrim(iH1_Ukeir) & "'"
...
'
        For gCnt1 = 1 To .Rows - 1
            If RTrim(.TextMatrix(gCnt1, 1)) = "" And _
                 RTrim(.TextMatrix(gCnt1, 22)) = "" And _
            ...
            End If
        '<< 重複チェック >>
            If RTrim(.TextMatrix(gCnt1, 22)) <> "" Then
                For gCnt2 = gCnt1 + 1 To .Rows - 1
                    If RTrim(.TextMatrix(gCnt1, 22)) = RTrim(.TextMatrix(gCnt2, 22)) Then
            ...
            End If
        ' 品番入力チェック
            If RTrim(.TextMatrix(gCnt1, 1)) <> "" And RTrim(.TextMatrix(gCnt1, 22)) = "" Then
                With CisFun
                     .MB_Lines = 4
                     .MB_MSG(2) = "    品番を入力して下さい            "
                     .MB_Title = ""
        ...
        '<< 材料取引先マスタ存在チェック >>
            ...
            If OpH1_HinKB(0) Then
               gSL_Select = gSL_Select & " WHERE 受給品番 = '" & RTrim(.TextMatrix(gCnt1, 22)) & "'"
            Else
               gSL_Select = gSL_Select & " WHERE 発注品番 = '" & RTrim(.TextMatrix(gCnt1, 22)) & "'"
            End If
...
'''                GoTo Body1Chk_Err
'''            End If
            .TextMatrix(gCnt1, 8) = ZTM.板厚
            .TextMatrix(gCnt1, 9) = ZTM.幅
            .TextMatrix(gCnt1, 10) = ZTM.長さ
            
            .TextMatrix(gCnt1, 11) = ZTM.メーカー
            .TextMatrix(gCnt1, 14) = ZTM.手配区分
            .TextMatrix(gCnt1, 15) = ZTM.SK区分
            .TextMatrix(gCnt1, 16) = ZTM.メーカー受入
            .TextMatrix(gCnt1, 17) = ZTM.契約先
            .TextMatrix(gCnt1, 18) = ZTM.材料管理番号
        '<< かんばん部品の時、枚数チェック >>
            If ZTM.SK区分 = 0 And CisFun.Val2(.TextMatrix(gCnt1, 4)) = 0 Then
        ...
        '<< 入荷数チェック >>
            If CisFun.Val2(.TextMatrix(gCnt1, 6)) = 0 Then
                CisFun.MB_MSG(1) = "    " & gCnt1 & "行目【　入荷数　必須！】            "
                CisFun.MB_MSG(3) = "    入荷数を入力して下さい。            "
                CisFun.MBOX
...
'   マスタ項目移送
    For gIndex = 1 To vsGrid.Rows - 1
        If RTrim(vsGrid.TextMatrix(gIndex, 22)) <> "" Then
            Call ItemsClearZNG
            With ZNG
                ...
                .材質 = RTrim(vsGrid.TextMatrix(gIndex, 20))
                .板厚 = CisFun.Val2(vsGrid.TextMatrix(gIndex, 8))
                .幅 = CisFun.Val2(vsGrid.TextMatrix(gIndex, 9))
                .長さ = CisFun.Val2(vsGrid.TextMatrix(gIndex, 10))
                ...
                .納入先 = CisFun.Val2(vsGrid.TextMatrix(gIndex, 11))
                .納入先受入 = CisFun.Val2(vsGrid.TextMatrix(gIndex, 16))
...
Private Sub Seban_Set()
    ...
    Call ItemsClearZTM
    gSL_Select = "SELECT ZT.*,ISNULL(SY.値名称,'')手配単位名,ZM.表示寸法  FROM 材料取引先マスタ ZT"
    ...
    With ZTM
        If OpH1_HinKB(0) Then
           iB1_Hinbn = RTrim(.受給品番)
        Else
           iB1_Hinbn = RTrim(.発注品番)
        End If
        
        B1lb_ZSunpo = RTrim(ZTM.材質) & " " & RTrim(ZTM.表示寸法)
        B1lb_Zaisitu = RTrim(ZTM.材質)
        B1lb_Sunpo = RTrim(ZTM.表示寸法)
        
        B1lb_Tani = RTrim(.手配単位名)
        iB1_Hinbn.Tag = .材料管理番号
    End With
'収容数表示
    Call Syuyo_Set
End Sub
...
Private Sub Hinbn_Set()
    ...
    gSL_Select = gSL_Select & "   AND 値 = ZM.手配単位"
    If OpH1_HinKB(0) Then
       gSL_Select = gSL_Select & " WHERE ZT.受給品番 = '" & RTrim(iB1_Hinbn) & "'"
    Else
       gSL_Select = gSL_Select & " WHERE ZT.発注品番 = '" & RTrim(iB1_Hinbn) & "'"
    End If
    gSL_Select = gSL_Select & "   AND ZT.取引先 = '" & RTrim(iH1_Torcd) & "'"
    gSL_Select = gSL_Select & "   AND ZT.受入 = '" & RTrim(iH1_Ukeir) & "'"
    If OpH1_HinKB(0) Then
       gSL_Select = gSL_Select & "   AND ISNULL(ZT.受給品番    ,'') <> ''"
    Else
       gSL_Select = gSL_Select & "   AND ISNULL(ZT.発注品番    ,'') <> ''"
    End If
    If Not ZTMRead(gSL_Select, 1) Then Exit Sub
    With ZTM
        iB1_Seban = RTrim(ZTM.背番号)
        If OpH1_HinKB(0) Then
           iB1_Hinbn = RTrim(ZTM.受給品番)
        Else
           iB1_Hinbn = RTrim(ZTM.発注品番)
        End If
...
Private Sub Syuyo_Set()
    ...
    If RTrim(ZTM.変更日) <> "" Then
        If RTrim(ZTM.変更日) & RTrim(ZTM.変更便) > RTrim(mNYmd) & RTrim(mNBin) Then
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 8, 1, "背番")
        Call .FixedSet(中中, 左中, あり, 46, 2, "品番 / 材質:寸法")
        Call .FixedSet(中中, 右中, あり, 10, 3, "収容数")
        Call .FixedSet(中中, 右中, あり, 5, 4, "枚数")
        Call .FixedSet(中中, 右中, あり, 10, 5, "端　数")
        Call .FixedSet(中中, 右中, あり, 13, 6, "入荷数")
        Call .FixedSet(中中, 中中, あり, 4, 7, "単位")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, なし, 0, 8, "板厚")
        Call .FixedSet(中中, 左中, なし, 0, 9, "幅")
        Call .FixedSet(中中, 左中, なし, 0, 10, "長さ")
        Call .FixedSet(中中, 左中, なし, 11, 11, "納入先")
        ...
        Call .FixedSet(中中, 左中, なし, 0, 16, "納入先受入")
        Call .FixedSet(中中, 左中, なし, 0, 17, "契約先")
        Call .FixedSet(中中, 左中, なし, 0, 18, "材料管理番号")
        Call .FixedSet(中中, 左中, なし, 0, 19, "Select")
        Call .FixedSet(中中, 左中, なし, 0, 20, "材質")
        Call .FixedSet(中中, 左中, なし, 0, 21, "表示寸法")
        Call .FixedSet(中中, 左中, なし, 0, 22, "品番")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not ZNG_RDSTS
        ...
        With vsGrid
            ...
            .TextMatrix(ID, GridCol(1)) = RTrim(ZNG.背番号)
            .TextMatrix(ID, GridCol(2)) = RTrim(ZNG.品番) & vbCr & RTrim(ZNG.材質) & " " & RTrim(ZNG.表示寸法)
            If ZNG.収容数 <> 0 Then
                .TextMatrix(ID, GridCol(3)) = Format(ZNG.収容数, "#####0.000")
            End If
            .TextMatrix(ID, GridCol(7)) = Format(ZNG.枚数, "####")
            If ZNG.端数 <> 0 Then
                .TextMatrix(ID, GridCol(5)) = Format(ZNG.端数, "#####0.000")
            End If
            If ZNG.発注数 <> 0 Then
                .TextMatrix(ID, GridCol(6)) = Format(ZNG.発注数, "#####0.000")
            End If
            .TextMatrix(ID, GridCol(7)) = RTrim(ZNG.手配単位名)
        
            .Redraw = True
            .TextMatrix(ID, 20) = RTrim(ZNG.材質)
            .TextMatrix(ID, 21) = RTrim(ZNG.表示寸法)
            .TextMatrix(ID, 22) = RTrim(ZNG.品番)
            .Redraw = True
        End With
        Call ZNGReadNext(1)
    Loop
...
Private Sub TextToVsGrid(tRow As Integer)
    With vsGrid
        ...
        .TextMatrix(tRow, 2) = RTrim(iB1_Hinbn) & vbCr & RTrim(B1lb_ZSunpo)
        
        If CisFun.Val2(B1lb_Syuyo) <> 0 Then
            .TextMatrix(tRow, 3) = Format(CisFun.Val2(B1lb_Syuyo), "###0.000")
        Else
            .TextMatrix(tRow, 3) = ""
        End If
        ...
        End If
        If CisFun.Val2(iB1_HSuryo) <> 0 Then
            .TextMatrix(tRow, 6) = Format(CisFun.Val2(iB1_HSuryo), "###0.000")
        Else
            .TextMatrix(tRow, 6) = ""
        End If
        If CisFun.Val2(B1lb_Hasu) <> 0 Then
            .TextMatrix(tRow, 5) = Format(CisFun.Val2(B1lb_Hasu), "###0.000")
        Else
            .TextMatrix(tRow, 5) = ""
        End If
        
        .TextMatrix(tRow, 7) = RTrim(B1lb_Tani)
    
        .TextMatrix(tRow, 20) = RTrim(B1lb_Zaisitu)
        .TextMatrix(tRow, 21) = RTrim(B1lb_Sunpo)
        .TextMatrix(tRow, 22) = RTrim(iB1_Hinbn)
...
Private Sub VsGridToText(tRow As Integer)
    With vsGrid
        ...
        '品番
        iB1_Hinbn = RTrim(.TextMatrix(tRow, 22))
        '材質/寸法
        B1lb_ZSunpo = RTrim(.TextMatrix(tRow, 20)) & " " & RTrim(.TextMatrix(tRow, 21))
        '収容数
        B1lb_Syuyo = RTrim(.TextMatrix(tRow, 3))
        '枚数
        iB1_Maisu = CisFun.Val2(.TextMatrix(tRow, 4))
        '端数
        B1lb_Hasu = RTrim(.TextMatrix(tRow, 5))
        '入荷数
        iB1_HSuryo = RTrim(.TextMatrix(tRow, 6))
        '単位
        B1lb_Tani = RTrim(.TextMatrix(tRow, 7))
        
        B1lb_Zaisitu = RTrim(.TextMatrix(tRow, 20))
        B1lb_Sunpo = RTrim(.TextMatrix(tRow, 21))
...
Private Sub VSGrid_DblClick()
    ...
    End With
    iB1_Seban = "": iB1_Hinbn = ""
    B1lb_Syuyo = "": iB1_Maisu = "": iB1_HSuryo = "": B1lb_Hasu = "": B1lb_Tani = ""
    B1lb_Zaisitu = "": B1lb_Sunpo = "": B1lb_ZSunpo = ""
    Call TextToVsGrid(Val(Input_Area.Tag))
...
'入力件数確認
    gSL_Select = "SELECT COUNT(*) 件数 FROM 材料入荷画面入力ワーク"
    gSL_Select = gSL_Select & " WHERE 処理端末 = HOST_NAME()"
    If Not ZNGRead(gSL_Select, 1) Then Exit Sub
    If ZNG.件数 = 0 Then Exit Sub
...
'プルーフリスト出力先確認
    With CisSimplePrint
         .PrintTitle = "材料入荷入力プルーフリスト"
         .SaveKey = gRegKey
         .Preservation = "CXHZ0240"
...
'入荷プルーフリスト
    If OpH1_HinKB(0) Then
       If Not CXHZ0240LPrint(0, PrtOut) Then Exit Sub
    Else
       If Not CXHZ0240LPrint(1, PrtOut) Then Exit Sub
    End If
...
'入力データ削除
    gSL_Select = "DELETE FROM 材料入荷画面入力ワーク"
    gSL_Select = gSL_Select & " WHERE 処理端末 = HOST_NAME()"
...
Private Function Tyokuso_Sy()
    ...
    gSL_Select = "SELECT ZN.* " & vbCr
    gSL_Select = gSL_Select & " FROM 材料入荷画面入力ワーク ZN " & vbCr
    ...
    gSL_Select = gSL_Select & " AND ZN.受入 = ZT.受入 " & vbCr              '2009/02/24
    ...
    gSL_Select = gSL_Select & " ORDER BY ZN.契約先,ZN.仕入先,ZN.受入,ZN.発注区分,ZN.納入先,ZN.納入日,ZN.納入便,"
    gSL_Select = gSL_Select & "          ZN.手配区分 , ZN.SK区分, ZN.材質, ZN.板厚, ZN.幅, ZN.長さ"
    
    wNouNo = ""
    wGyoNo = 0
    If ZNGRead(gSL_Select) Then
...
'              Key_NYmd <> ZNG.納入日 Or Key_NBin <> ZNG.納入便 Or Key_TEKb <> ZNG.手配区分 Or Key_SKKb <> ZNG.SK区分) Then
          If (wGyoNo > 3) Or _
             (Key_KTorcd <> ZNG.契約先 Or Key_Torcd <> ZNG.仕入先 Or Key_Uke <> ZNG.受入 Or _
              Key_HCKb <> ZNG.発注区分 Or Key_NTorcd <> ZNG.納入先 Or _
              Key_NYmd <> ZNG.納入日 Or Key_NBin <> ZNG.納入便 Or Key_TEKb <> ZNG.手配区分 Or Key_SKKb <> ZNG.SK区分) Then
             If Not Tyokuso_Nouno_Get Then Exit Function      '納入番号獲得
             wNouNo = gStr
             wGyoNo = 1
          ...
          wSL_Select = wSL_Select & " WHERE 発注管理NO = " & ZNG.発注管理NO
          With CisDB
              .SQL = wSL_Select
...
' 2009/03/14 Start
    gSL_Select = "SELECT 板厚,幅,長さ FROM 材料マスタ "
    gSL_Select = gSL_Select & " WHERE 材料管理番号 = " & ZNG.材料管理番号
    Call ZRMRead(gSL_Select, 1, 1)
' 2009/03/14 End
