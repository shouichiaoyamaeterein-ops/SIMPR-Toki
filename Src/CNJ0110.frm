...
Private Sub Form_Activate()
    ...
    If SYMRead(gSL_Select, 1) Then
        iH1_YM = SYM.英数字1
        CNJ0110_Naiji = Mid(SYM.英数字2, 1, 2)
    Else
        CNJ0110_Naiji = "定常"
    End If
    Op_OutKB(SYM.数字1).Value = True
    Op_OutKB(0).Caption = CNJ0110_Naiji & "＜初回＞"
...
Private Function Head1Chk() As Boolean
    ...
    If Op_OutKB(0).Value Or Op_OutKB(1).Value Then       ' 初回・計変
       ...
       gSL_Select = gSL_Select & "  NJ.コメント,ISNULL(HN.品番編集,'') 品番編集,"
'       gSL_Select = gSL_Select & "  NT.*"                      ' 内示TEXTテーブルより発行
    ...
    If Op_OutKB(2).Value Then       ' 全て
       gSL_Select = "Select TK.契約先,TR.正式名称,Cast(TK.オプション3 as Tinyint) 内示形態,"
       gSL_Select = gSL_Select & "  NV.コメント,ISNULL(HN.品番編集,'') 品番編集,"
'       gSL_Select = gSL_Select & "  NT.*"                      ' 内示TEXTテーブルより発行
       gSL_Select = gSL_Select & "  NV.*,NV.工区 置換工区 "      ' 内示テーブルより発行
       gSL_Select = gSL_Select & "  From 内示TEXTテーブル NT"
       gSL_Select = gSL_Select & "  Left Outer Join 内示テーブル NV"
       gSL_Select = gSL_Select & "    On  NV.内示年月   = NT.内示年月"
       gSL_Select = gSL_Select & "    And NV.登録FCODE = NT.FILECODE"
       gSL_Select = gSL_Select & "    And NV.登録H作成日 = NT.HEAD作成日"
       gSL_Select = gSL_Select & "    And NV.登録HSEQ = NT.HEADSEQ"
       gSL_Select = gSL_Select & "    And NV.登録連番 = NT.連番"
       gSL_Select = gSL_Select & "  Left Outer Join TEXT管理テーブル TK"
       gSL_Select = gSL_Select & "    On TK.FILECD = NT.FILECODE"
       gSL_Select = gSL_Select & "  Left Outer Join 取引先マスタ TR"
       gSL_Select = gSL_Select & "    On TR.取引先CD = TK.契約先"
       ...
       gSL_Select = gSL_Select & "    On HN.品番 = NT.品番"
        With CisFun
            .Where_CreateKB = NewDocument
            .Where_Create AlphaMe, "NV.内示年月", "", NotNul, Create
            .Where_Create AlphaMe, "ISNULL(NV.内示年月,'')", "", 等しくない, Create
        End With
    End If
'
    With CisFun
        ...
        .Where_Create AlphaMe, "置換工区", iH1_Kouku
        wSl_Where = .Where_Phrase
    End With
    
    gSL_Select = gSL_Select & wSl_Where
    gSL_Select = gSL_Select & " Order By 置換工区,TK.契約先,TK.オプション3,NT.品番,NT.納入先,NT.受入"
...
Private Function CsvPut() As Boolean
    ...
    ' 翌月・翌々月を求める
        ...
        OutCnt = 0
        OutStr = "№,品番,納入先,受入,整備,担当,納区,背番号,納入回数,収容数,管理番号,"
        OutStr = OutStr & wYM & "当初仕掛内示,"
        OutStr = OutStr & wYMy & "計画変更数,"
        OutStr = OutStr & wYMyy & "引取内示,"
        OutStr = OutStr & wYM & "仕掛内示,"
        OutStr = OutStr & wYMy & "仕掛内示,"
        OutStr = OutStr & wYMyy & "仕掛内示,"
        OutStr = OutStr & "適用開始月,旬,"
        OutStr = OutStr & "適用終了月,旬,"
        
        OutStr = OutStr & "コメント,カード発行枚,かんばん枚数,+/-前回との差,"
        For gInt = 1 To End_Day
            OutStr = OutStr & gInt & "日"
        ...
        Do Until Not NTT_RDSTS
            ...
            OutStr = OutStr & RTrim(.納入先) & ","
            OutStr = OutStr & RTrim(.受入) & ","
            OutStr = OutStr & RTrim(.整備) & ","
            OutStr = OutStr & RTrim(.担当) & ","
            OutStr = OutStr & RTrim(.納入区分) & ","
            ...
            OutStr = OutStr & .当初内示数 & ","
            If .調整数符号 = "-" Then .調整数 = .調整数 * -1
            OutStr = OutStr & .調整数 & ","
            If .変更数符号 = "-" Then .変更数 = .変更数 * -1
            OutStr = OutStr & .変更数 & ","
            OutStr = OutStr & .内示数 & ","
            OutStr = OutStr & .翌月内示数 & ","
            OutStr = OutStr & .翌々月内示数 & ","
            
            If Trim(.適用開始年月日) <> "" Then
                OutStr = OutStr & Mid(.適用開始年月日, 5, 2) & ","
                gLong = CisFun.Val2(Mid(.適用開始年月日, 7, 2))
                If gLong > 0 And gLong < 11 Then OutStr = OutStr & "1,"
                If gLong > 10 And gLong < 21 Then OutStr = OutStr & "2,"
                If gLong > 20 Then OutStr = OutStr & "3,"
            Else
                OutStr = OutStr & ",,"
            End If
            If Trim(.適用終了年月日) <> "" Then
                OutStr = OutStr & Mid(.適用終了年月日, 5, 2) & ","
                gLong = CisFun.Val2(Mid(.適用終了年月日, 7, 2))
                If gLong > 0 And gLong < 11 Then OutStr = OutStr & "1,"
                If gLong > 10 And gLong < 21 Then OutStr = OutStr & "2,"
                If gLong > 20 Then OutStr = OutStr & "3,"
            Else
            ...
            OutStr = OutStr & RTrim(.コメント) & ","
            OutStr = OutStr & .納品書発行枚数 & ","
            OutStr = OutStr & .かんばん回転枚数 & ","
            If .かんばん増減符号 = "-" Then
                .かんばん増減枚数 = .かんばん増減枚数 * -1
            End If
            OutStr = OutStr & .かんばん増減枚数 & ","
            For gLong = 1 To End_Day
                OutStr = OutStr & .内示数量(gLong)
