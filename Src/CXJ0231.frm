...
Begin VB.Form CXJ0231 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CXJ0231.frx":0000
         Height          =   5550
         Left            =   90
         OleObjectBlob   =   "CXJ0231.frx":0015
         TabIndex        =   2
         Top             =   60
         Width           =   8550
      End
...
Private Function BodySet() As Boolean
    ...
    BodySet = False
    gSL_Select = "SELECT HG.受注区分,HG.納入先,TR.略称,HG.受入,"
    gSL_Select = gSL_Select & "HG.納入日,HG.納入便,HG.納入番号,  "
    gSL_Select = gSL_Select & "ISNULL(SY.値名称,'')編集受注区分, "
    ...
    gSL_Select = gSL_Select & " FROM 出荷画面入力ワーク HG "
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 取引先マスタ TR "
    gSL_Select = gSL_Select & "   ON  TR.取引先区分 = 1"
    gSL_Select = gSL_Select & "   AND TR.取引先CD = HG.納入先"
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 名称マスタ SY "
    gSL_Select = gSL_Select & "   ON  SY.区分名称 = '受注区分'"
    gSL_Select = gSL_Select & "   AND SY.区分タイプ = ''"
    gSL_Select = gSL_Select & "   AND SY.値 = HG.受注区分"
    mSL_Group = " GROUP By HG.受注区分,HG.納入先,HG.受入,HG.納入日,HG.納入便,HG.納入番号,TR.略称,SY.値名称 "
    mSL_OrderA = " Order By HG.受注区分,HG.納入先,HG.受入,HG.納入日,HG.納入便,HG.納入番号 "
    mSL_OrderD = " Order By HG.受注区分 Desc,HG.納入先 Desc,HG.受入 Desc,HG.納入日 Desc,HG.納入便 Desc"
' 選択条件セット
    mSL_SelectW = ""
    With CisFun
        .Where_CreateKB = NewDocument
        .Where_Create AlphaMe, "HG.納入先", iH1_Torcd
...
Private Sub RtnSet()
    With Adodc1
        RV_Kubun = .Recordset("受注区分")
        RV_Torcd = .Recordset("納入先")
