   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日 　|仕入先 |仕  入  先  名 |納 番 |頁|手配|品　番　　               |収容数  |枚数|発注数 |"
            Rows            =   21
            Cols            =   14
            BackColor       =   12648447
...
'**       変  更  日    :   2005/05/11  発注区分=5:指示も発行対象とする
'**       変  更  日    :   2007/09/20  By CIS TEC B-848 印刷モジュール追加
'**                                           （切替）gIniName [Option] - 現品票 = 0:ﾚｰｻﾞｰ、1:TEC
'**
'**********************************************************'
...
Private Sub Form_Activate()
    ...
    Call GetCryOption("G", "Option", "現品票", mPrintType)
    If RTrim(mPrintType) = "" Then
        mPrintType = "0"
        Call GetCryOption("P", "Option", "現品票", mPrintType)
    End If
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=10      Else 1=110
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            If mPrintType = "0" Then
                Call CXH0350LPrint(wPrintKB, wPrintOut)
            Else
                Call CXH0351LPrint(wPrintKB, wPrintOut)
            End If
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 10"
        gSL_Select = gSL_Select & "   and hct.発注管理NO = cht.管理NO"
'2005/05/11修正        gSL_Select = gSL_Select & "   and hct.SK区分 = 1"
        gSL_Select = gSL_Select & "   and ( hct.SK区分 = 1 "
        gSL_Select = gSL_Select & "    or ( hct.作成種別 = 'G' AND hct.SK区分 = 0 AND hct.発注区分 = 5 ) )"
    Else
'   ### 再発行既存 ###
'2005/05/11修正        gSL_Select = gSL_Select & " where hct.SK区分 = 1"
        gSL_Select = gSL_Select & " where ( hct.SK区分 = 1 "
        gSL_Select = gSL_Select & "    or ( hct.作成種別 = 'G' AND hct.SK区分 = 0 AND hct.発注区分 = 5 ) )"
    End If
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(12))) = "X" Then
                ...
                CHT.帳票区分 = 110
                CHT.管理NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(11)))
                Call CHTInsert
            End If
        Next ID
    End With
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 110"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 10, 1, "納 入 日")
        Call .FixedSet(中中, 左中, あり, 6, 2, "仕入先")
        Call .FixedSet(中中, 左中, あり, 12, 3, "仕入先名")
        Call .FixedSet(中中, 左中, あり, 6, 4, "納 番")
        Call .FixedSet(中中, 右中, あり, 2, 5, "頁")
        Call .FixedSet(中中, 左中, あり, 4, 6, "手配")
        Call .FixedSet(中中, 左中, あり, 25, 7, "品　　　番")
        Call .FixedSet(中中, 右中, あり, 7, 8, "収容数")
        Call .FixedSet(中中, 右中, あり, 5, 9, "枚数")
        Call .FixedSet(中中, 右中, あり, 7, 10, "発注数")
        Call .FixedSet(中中, 左中, あり, 0, 11, "発注管理NO")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
Private Sub HctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '品番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(HCT.表示品番)
        '収容数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = Format(HCT.収容数, "#,###")
        '箱数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = Format(HCT.枚数, "#,###")
        '発注数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = Format(HCT.発注数, "#,###")
        '発注管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = HCT.発注管理NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
