...
Begin VB.Form CXJ0540 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "ノーかんばんエフ　再発行"
   ClientHeight    =   10980
   ClientLeft      =   75
   ClientTop       =   1500
   ClientWidth     =   15315
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日　-便|納入先 |納入先名            |頁|背番号|品　番　　               |収容数  |箱数  |納入数  ||"
            Rows            =   21
            Cols            =   15
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=51      Else 1=151
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            Call CXJ0130LPrint(wPrintKB, wPrintOut)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 51"
        gSL_Select = gSL_Select & "   and jyt.受注管理NO = cht.管理NO"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where jyt.受注管理NO > 0"
    End If
'   >> 納入先
...
'   >> 品番
    If RTrim(iH1_Hinbn) <> "" Then
        If InStr(1, iH1_Hinbn, "*") = 0 Then
            gSL_Select = gSL_Select & "   and jyt.品番 = '" & RTrim(iH1_Hinbn) & "'"
        Else
            gSL_Select = gSL_Select & "   and jyt.品番 like '" & RTrim(Replace(iH1_Hinbn, "*", "%")) & "'"
        End If
    End If
'   >> 背番号
    If RTrim(iH1_Seban) <> "" Then
        If InStr(1, iH1_Seban, "*") = 0 Then
            gSL_Select = gSL_Select & "   and jyt.背番号 = '" & RTrim(iH1_Seban) & "'"
        Else
            gSL_Select = gSL_Select & "   and jyt.背番号 like '" & RTrim(Replace(iH1_Seban, "*", "%")) & "'"
        End If
    End If
'   >> 納入日
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(11))) = "X" Then
                ...
                CHT.帳票区分 = 151
                CHT.管理NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(9)))
                CHT.管理行NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(10)))
                Call CHTInsert
            End If
        Next ID
    End With
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 151"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 7, 2, "納入先")
        Call .FixedSet(中中, 左中, あり, 20, 3, "納　入　先　名")
        ...
        Call .FixedSet(中中, 右中, あり, 8, 6, "収容数")
        Call .FixedSet(中中, 右中, あり, 8, 7, "箱　数")
        Call .FixedSet(中中, 右中, あり, 9, 8, "納入数")
        Call .FixedSet(中中, 左中, あり, 0, 9, "出荷指示書NO")
        Call .FixedSet(中中, 左中, あり, 0, 10, "出荷指示書行NO")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
    ...
    '   >> 納入日＆便
        If Trim(JYT.納入日) <> "" Then
            If RTrim(JYT.納入便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(JYT.納入日), "0000/00/00")
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(JYT.納入日), "0000/00/00") & "-" & RTrim(JYT.納入便)
            End If
        End If
    '   >> 納入先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = RTrim(JYT.納入先)
    '   >> 納入先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = RTrim(JYT.納入先名)
    '   >> 背番号
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(JYT.背番号)
    '   >> 品番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(JYT.表示品番)
    '   >> 収容数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = Format(JYT.収容数, "#,###")
    '   >> 箱数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = Format(JYT.枚数, "#,###")
    '   >> 納入数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = Format(JYT.納入数, "#,###")
    '   >> 出荷指示書NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = JYT.出荷指示書NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = JYT.出荷指示書行NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
