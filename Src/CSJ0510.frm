...
Begin VB.Form CSJ0510 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CSJ0510.frx":0000
         Height          =   7740
         Left            =   60
         OleObjectBlob   =   "CSJ0510.frx":0015
         TabIndex        =   9
         Top             =   60
         Width           =   14475
      End
...
'+---------------------+
    With CisDBGrid
        ...
        .LayOutName = CisFun.WSID & "SJ0510.Lay"       ' ﾚｲｱｳﾄ情報格納先、ﾌｧｲﾙ名
        .PrintHeadLine = あり
        .PrintBodyLine = あり
        .PrintBodyGyo = 10
...
'       表示条件 セット
       ...
       With CisFun
            ...
            ' 部署
            .Where_Create AlphaMe, "ZS.部門", iH1_Torcd
            ' 品番
            .Where_Create AlphaMe, "ZS.品番", iH1_Hinbn
            ' 作業者
            .Where_Create AlphaMe, "ZS.作業者", iH1_Sagyo
'
            ' 材質
            .Where_Create AlphaMe, "ZS.材質", iH1_Zaist
            ' 板厚
            .Where_Create AlphaMe, "ZS.板厚", Format(iH1_Itatu, "###0.00")
            ' 幅
            .Where_Create AlphaMe, "ZS.幅", Format(iH1_Width, "###0.00")
            ' 長さ
            .Where_Create AlphaMe, "ZS.長さ", Format(iH1_Long, "###0.00")
    ...
    '-- データ存在確認 -------
    gSL_Select = ""
    gSL_Select = gSL_Select & "    SELECT ZS.材料使用量管理NO, ZS.材料使用量管理行,"
    gSL_Select = gSL_Select & "       SUBSTRING(ZS.作業日,1,4) + '/' + SUBSTRING(ZS.作業日,5,2) + '/' + SUBSTRING(ZS.作業日,7,2) AS 作業日,"
'
    gSL_Select = gSL_Select & "       CASE WHEN ISNULL(ZS.部門, '') = '' THEN ''"
    gSL_Select = gSL_Select & "            ELSE ISNULL(ZS.部門,  '') + ':' + ISNULL(TM.正式名称,'')"
    gSL_Select = gSL_Select & "       END  部門  ,"
    gSL_Select = gSL_Select & "       CASE WHEN ISNULL(ZS.作業者, '') = '' THEN ''"
    gSL_Select = gSL_Select & "            ELSE ISNULL(ZS.作業者,'') + ':' + ISNULL(M1.値名称  ,'')"
    gSL_Select = gSL_Select & "       END  作業者,"
    gSL_Select = gSL_Select & "       ISNULL(H1.表示品番,'') AS 品番,"
    gSL_Select = gSL_Select & "       RTRIM(材質) + ': ' + RTRIM(CAST(板厚 AS CHARACTER)) + 'x ' + RTRIM(CAST(幅 AS CHARACTER)) + 'x ' + RTRIM(CAST(長さ AS CHARACTER)) AS 材質,"
    gSL_Select = gSL_Select & "       REPLACE(CONVERT(VARCHAR, CAST(ZS.加工数 AS MONEY), 1), '.00', '') AS 表示加工数,"
    gSL_Select = gSL_Select & "       CONVERT(VARCHAR, CAST(ZS.単重   AS decimal(10,3)), 1)AS 表示単重 ,"
    gSL_Select = gSL_Select & "       CONVERT(VARCHAR, CAST(ZS.使用量 AS decimal(10,3)), 1)AS 表示使用重量 ,"
    gSL_Select = gSL_Select & "       RTRIM(CAST(ラベル重量1 AS CHARACTER)) + 'x ' + RTRIM(CAST(ラベル本数1 AS CHARACTER)) AS 表示ラベル1,"
    gSL_Select = gSL_Select & "       RTRIM(CAST(ラベル重量2 AS CHARACTER)) + 'x ' + RTRIM(CAST(ラベル本数2 AS CHARACTER)) AS 表示ラベル2,"
    gSL_Select = gSL_Select & "       RTRIM(CAST(ラベル重量3 AS CHARACTER)) + 'x ' + RTRIM(CAST(ラベル本数3 AS CHARACTER)) AS 表示ラベル3,"
    gSL_Select = gSL_Select & "       RTRIM(CAST(ラベル重量4 AS CHARACTER)) + 'x ' + RTRIM(CAST(ラベル本数4 AS CHARACTER)) AS 表示ラベル4,"
    gSL_Select = gSL_Select & "       CASE WHEN 戻し重量 = 0 THEN '無' ELSE '有' END 表示戻し,備考,"
    gSL_Select = gSL_Select & "       CONVERT(VARCHAR, CAST(ZS.合計重量 AS decimal(15,3)), 1)AS 表示合計重量"
...
' 分類内容(Order By句) ｾｯﾄ
    With CisDBGrid
        ...
        End If
    End With
  ''gSL_Select = gSL_Select & CisFun.Where_Phrase & CisDBGrid.Select_OrderBy
...
' Call .GridInitSetCsv(ﾏｰｼﾞ処理,分類処理,ﾃﾞｰﾀ連結名,印刷見出し,印刷桁数,合計印刷)
    With CisDBGrid
         ...
         Call .GridInitSet(True, , "材質", "材　質 / 寸　法", 50)
         Call .GridInitSet(, , "表示加工数", "加工数", 10, True)
         Call .GridInitSet(, , "表示単重", "単重", 10)
         Call .GridInitSet(, , "表示使用重量", "使用重量", 15, True)
         Call .GridInitSet(, , "表示ラベル1", "ラベル重量１", 50)
         Call .GridInitSet(, , "表示ラベル2", "ラベル重量２", 50)
         Call .GridInitSet(, , "表示ラベル3", "ラベル重量３", 50)
         Call .GridInitSet(, , "表示ラベル4", "ラベル重量４", 50)
         Call .GridInitSet(, , "表示戻し", "戻し", 4)
         Call .GridInitSet(, , "表示合計重量", "合計重量", 20, True)
         Call .GridInitSet(True, , "備考", "備　　　　考", 70)
    End With
End Sub
...
Private Sub TotalDsp(JCol As Long)
    ...
    Dim wcnt3           As Integer
    
    gSL_Select = "SELECT SUM(加工数), SUM(使用量), COUNT(*) FROM 材料使用量テーブル "
'
    Select Case JCol
      Case -1
           wWhere = ""
      Case 0, 1, 2, 3, 4    ' , 5
           ...
           If JCol = 1 Or JCol = 2 Then     ' 部門、作業者
...
'-------------------------------------------------------------------------------------------' 2009.12.28 add start
           ElseIf JCol = 0 Then
              ...
              wWhere = DBGrid1.Columns(JCol).DataField & " = '" & wStr & "'"
'-------------------------------------------------------------------------------------------' 2009.12.28 add end
           ElseIf JCol = 3 Then             ' 品番
              wWhere = DBGrid1.Columns(JCol).DataField & " = '" & Replace(Trim(DBGrid1.Columns(JCol).Value), "-", "") & "'"
           ElseIf JCol = 4 Then             ' 材質/寸法
              wStr = "": wStr1 = "": wStr2 = "": wStr3 = ""
              wCnt = InStr(Trim(DBGrid1.Columns(JCol).Value), ":")
              If wCnt > 1 Then
                 wStr = Trim(DBGrid1.Columns(JCol).Value)
                 wStr1 = Mid(wStr, 1, wCnt - 1)
                 wStr = Trim(Mid(wStr, wCnt + 1))
                 wCnt = InStr(wStr, "x")
                 wStr2 = Mid(wStr, 1, wCnt - 1)
                 wStr = Trim(Mid(wStr, wCnt + 1))
                 wCnt = InStr(wStr, "x")
                 wStr3 = Mid(wStr, 1, wCnt - 1)
                 wStr4 = Trim(Mid(wStr, wCnt + 1))
              End If
              wWhere = DBGrid1.Columns(JCol).DataField & " = '" & wStr1 & "'" _
                       & " AND 板厚 = " & wStr2 & " AND 幅 = " & wStr3 & " AND 長さ = " & wStr4
           Else
...
'
    ...
    With CisDB
        .SQL = gSL_Select
        If .DBRead(1, 0) Then
            lb_TKSu = Format(.Recordset(0), "#,##0.000")
            lb_TFSu = Format(.Recordset(1), "#,##0.000")
