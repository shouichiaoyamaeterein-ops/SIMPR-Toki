...
Begin VB.Form CXHZ0501 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料・入出庫入力データ検索"
   ClientHeight    =   7740
   ClientLeft      =   240
   ClientTop       =   1470
   ClientWidth     =   10515
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CXHZ0501.frx":0000
         Height          =   5550
         Left            =   60
         OleObjectBlob   =   "CXHZ0501.frx":0015
         TabIndex        =   0
         Top             =   60
         Width           =   10080
      End
...
'**       変  更  日    :   2008/07/10  By CIS - ボタン押下での検索ボタンが出ていなかったので修正
...
'▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼ 2008/07/10 INSERT START
           Case vbKeyF7:       If PB_Look.Visible Then Call PB_Look_Click  '【検索】
'▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼▲▼ 2008/07/10 INSERT END
...
Private Function BodySet() As Boolean
    ...
    gSL_Select = gSL_Select & "    when zjk.実績区分 = 1 then '入庫'"
    gSL_Select = gSL_Select & "    when zjk.実績区分 = 2 then '出庫'"
    gSL_Select = gSL_Select & "    when zjk.実績区分 = 3 then '支給'"
    ...
    gSL_Select = gSL_Select & ", case when zjk.実績元 = '' then '' else zjk.実績元 + ':' + isnull(tr2.略称,'') end 実績元名"
    ...
    gSL_Select = gSL_Select & "    on tr2.取引先CD = zjk.実績元"
...
'   gSL_Select = gSL_Select & " where zjk.データ区分 = 0"
'*--------------------------------------------------<< 2004/06/29 >> Delete End
'   >> 選択条件セット
    If RTrim(iH1_Ymd) <> "" Then
        If CisFun.Val2(iH1_Ymd.cDay) = 0 Then
            gSL_Select = gSL_Select & "   and substring(zjk.実績日,1,6) = '" & iH1_Ymd & "'"
        Else
            gSL_Select = gSL_Select & "   and zjk.実績日 = '" & iH1_Ymd & "'"
        End If
    End If
    If H1Ck_JKbn(1) = 1 Or _
       H1Ck_JKbn(2) = 1 Or _
       H1Ck_JKbn(3) = 1 Then
        gSL_Select = gSL_Select & "   and zjk.実績区分 in ("
        gStr = ""
        If H1Ck_JKbn(1) = 1 Then gSL_Select = gSL_Select & gStr & "1": gStr = ","
        If H1Ck_JKbn(2) = 1 Then gSL_Select = gSL_Select & gStr & "2": gStr = ","
        If H1Ck_JKbn(3) = 1 Then gSL_Select = gSL_Select & gStr & "3": gStr = ","
        gSL_Select = gSL_Select & ")"
    End If
'   >> 集計
...
'   >> 表示順
    ...
    Else
        gSL_Select = gSL_Select & " order by"
        gSL_Select = gSL_Select & "  zjk.実績区分 desc"
