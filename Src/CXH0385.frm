...
Begin VB.Form CXH0385 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "支給チケット　ファイル再出力"
   ClientHeight    =   10980
   ClientLeft      =   1050
   ClientTop       =   4035
   ClientWidth     =   15315
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |作成ファイル名 　 　　       |納 入 日 　-便|仕入先 |   仕  入  先  名   |受入|納 番 |頁|発注区|"
            Rows            =   21
            Cols            =   15
            BackColor       =   12648447
...
'**********************************************************'
'**
'**       フォーム名    :   支給チケット　ファイル再出力
'**       フォームID    :   CXH0385
'**       処理概要      :
'**
'**       作  成  日    :   2005/02/24  By CIS
...
'-----------------------------------------------< 2006/02/02 ADD START >
            ...
            wPrintKB = 1                ' 帳票区分：0=19 Else 1=119
            Call CXH0380TFile(wPrintKB)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 19"
        ...
        gSL_Select = gSL_Select & "   and hch.発行種類 = 4"
        gSL_Select = gSL_Select & "   and hch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and hch.TEXTNAME <> ''"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where hch.発行種類 = 4"
        gSL_Select = gSL_Select & "   and hch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and hch.TEXTNAME <> ''"
    End If
'   >> 納入番号
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 119"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'       Call .FixedSet(見出し位置設定,列位置設定,ﾏｰｼﾞ処理,ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 0, 9, "納品書管理NO")
        Call .FixedSet(中中, 左中, あり, 0, 10, "カード")
...
Private Sub HchInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '納品書管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = HCH.納品書管理NO
        'カード
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = HCH.カード
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
