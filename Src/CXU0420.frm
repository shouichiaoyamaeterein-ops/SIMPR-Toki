...
'                      ｾﾙ桁数,列番号,見出し内容)
        ...
        Call .FixedSet(中中, 左中, あり, 29, 4, "品番")
        Call .FixedSet(中中, 左中, なし, 8, 5, "納入No")
...
' 表示条件 セット
    With CisFun
         ...
         .Where_Create AlphaMe, "UJ.実績年月", iH1_SYM
         ...
         ' 契約先
         .Where_Create AlphaMe, "UJ.契約先", iH1_Torcd
         ' 車型
         .Where_Create AlphaMe, "HN.車型", iH1_Syakei
         ' 品番
         .Where_Create AlphaMe, "UJ.品番", iH1_Hinbn
    End With

    gSL_Select = " SELECT SUBSTRING(UJ.実績年月,1,4) + '/' + SUBSTRING(UJ.実績年月,5,2) 実績年月,"
    gSL_Select = gSL_Select & " CASE WHEN ISNULL(TR.略称,'') <> '' THEN "
    gSL_Select = gSL_Select & "      UJ.契約先 + ':' + TR.略称 "
    gSL_Select = gSL_Select & " ELSE UJ.契約先 END 契約先名,"
    gSL_Select = gSL_Select & " UJ.契約先,"
    gSL_Select = gSL_Select & " CASE WHEN ISNULL(HN.表示品番,'') = '' THEN UJ.品番 Else HN.表示品番 End 品番,"
    gSL_Select = gSL_Select & " 納入番号,UJ.納入数, UJ.金額,"
    gSL_Select = gSL_Select & " CASE WHEN UJ.入力区分 = '' THEN HN.車型 ELSE UJ.車型 END 車型"
    gSL_Select = gSL_Select & " From 売上明細テーブル UJ"
    gSL_Select = gSL_Select & "  Left Outer Join 品番マスタ HN"
    gSL_Select = gSL_Select & "   ON HN.品番 = UJ.品番"
    gSL_Select = gSL_Select & "    LEFT Outer Join 取引先マスタ TR"
    gSL_Select = gSL_Select & "    ON TR.取引先CD = UJ.契約先"
    gSL_Select = gSL_Select & "    And TR.取引先区分 = 0"
' 抽出文　作成
    gSL_Select = gSL_Select & CisFun.Where_Phrase
    
    gSL_Select = gSL_Select & " ORDER BY UJ.実績年月,UJ.契約先,UJ.車型,UJ.品番,UJ.納入番号 "
    
    If Not UJTRead(gSL_Select) Then GoTo BodySet_ED1
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not UJT_RDSTS
        With VSGrid1
            ...
            End If
            If Trim(UJT.実績年月) = "" Then
                .TextMatrix(ID, GridCol(1)) = "    "
            Else
                .TextMatrix(ID, GridCol(1)) = UJT.実績年月
            End If
            If Trim(UJT.契約先名) = "" Then
                .TextMatrix(ID, GridCol(2)) = "    "
            Else
                .TextMatrix(ID, GridCol(2)) = UJT.契約先名
            End If
            If Trim(UJT.車型) = "" Then
                .TextMatrix(ID, GridCol(3)) = "    "
            Else
                .TextMatrix(ID, GridCol(3)) = UJT.車型
            End If
            If Trim(UJT.品番) = "" Then
                .TextMatrix(ID, GridCol(4)) = "    "
            Else
                .TextMatrix(ID, GridCol(4)) = UJT.品番
            End If
            If Trim(UJT.納入番号) = "" Then
                .TextMatrix(ID, GridCol(5)) = "    "
            Else
                .TextMatrix(ID, GridCol(5)) = UJT.納入番号
            End If
            .TextMatrix(ID, GridCol(6)) = CisFun.RSetFld(UJT.納入数, 7, "###,###")
            .TextMatrix(ID, GridCol(7)) = CisFun.RSetFld(UJT.金額, 10, "##,###,###")
            ID = ID + 1
        End With
        Call UJTReadNext
    Loop
