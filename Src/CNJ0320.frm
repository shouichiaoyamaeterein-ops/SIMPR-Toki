...
'**
'**       作  成  日    :  2004/03/24
'**       変  更  日    :  2005/02/10 - CSV出力の修正（納入先受入の出力）
'**
'**********************************************************'
...
'   ( 読み込み順 )
    gSL_Select = gSL_Select & " ORDER BY HJ.手配先, HJ.受入, HJ.品番, 納期" & vbCrLf
...
Private Function PrintStart() As Boolean
    PrintStart = False
    If Op_Out(0).Value Or Op_Out(1).Value Then
        lb_msg.Caption = "【 発注内示データ 印刷中 】"
        DoEvents
        With CisPrt
             Set .iPrtForm = CNJ0320L
             .dpMSG = "発注内示表　印刷中"
...
'                   End If
                    OutStr = OutStr & RTrim$(.直送先) & ","
                    OutStr = OutStr & DBSelect_HTM(.品番, .手配先, .受入, 1, .直送先) & ","
