      ...
      Begin Cis3D_v60.CIS3D UC_3D4 
         ...
         Begin Cis3D_v60.CIS3D BLB_DD 
            ...
            Begin vsFlexLib.vsFlexArray vsGrid 
               ...
               Appearance      =   1
               ConvInfo        =   1418783674
               FormatString    =   "№ |背番号|品　番　　　　　　　　　 |収容数  |枚　数 |端　数 |発注数 　|打切残数 |"
               Rows            =   11
               Cols            =   20
               BackColor       =   12648384
...
'日付チェック
    ...
    If Not OpH1_HKbn(1).Value And gSYSOp_CLKb = 1 Then
        gInt = Kdou_Check(iH1_Ymd, 1, iH1_Torcd, iH1_Ukeir)           '仕入先カレンダー      2007/07/13MNT
    Else
...
''''            End If
        ...
        '<< 発注打切チェック >>
            ...
            If RTrim(gStr) = "1" Then
                gSL_Select = "SELECT SUM(発注数)発注数 FROM 発注画面入力ワーク"
                gSL_Select = gSL_Select & " WHERE 処理端末 = HOST_NAME()"
                gSL_Select = gSL_Select & "   AND 処理区分 = 0"
                gSL_Select = gSL_Select & "   AND 品番 = '" & RTrim(.TextMatrix(gCnt1, 2)) & "'"
                Call HGWRead(gSL_Select, 1)
                If HUT.設定残数 < CisFun.Val2(vsGrid.TextMatrix(gCnt1, 6)) + HGW.発注数 Then
