...
'-----------------------------------------------< 2006/02/02 ADD START >
            ...
            wPrintKB = 1                ' 帳票区分：0=6 Else 1=106
            Call CXH0320TFile(wPrintKB)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 6"
        ...
        gSL_Select = gSL_Select & "   and hch.発行種類 = 1"
        gSL_Select = gSL_Select & "   and hch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and hch.TEXTNAME <> ''"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where hch.発行種類 = 1"
        gSL_Select = gSL_Select & "   and hch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and hch.TEXTNAME <> ''"
    End If
'   >> 伝票種類
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 106"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 25, 12, "X品番")
...
Private Sub HchInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = RTrim(HCH.手配区分名)
        'SK
        ...
        End Select
        '納品書管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(13)) = HCH.納品書管理NO
        'カード
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(14)) = HCH.カード
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
