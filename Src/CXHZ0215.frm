...
Begin VB.Form CXHZ0215 
   ...
   Begin vsFlexLib.vsFlexArray vsGridS 
      ...
      Appearance      =   1
      ConvInfo        =   1418783674
      FormatString    =   "№|材質                     |寸法                    |背番号  |収容数     |箱数 |端数       |納入数       |納品書管理NO|"
      Rows            =   6
      BackColor       =   12648447
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 23, 1, "材     質")
        Call .FixedSet(中中, 左中, あり, 24, 2, "寸　　法")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE START
        'Call .FixedSet(中中, 左中, あり, 6, 3, "背番号")
        Call .FixedSet(中中, 左中, あり, 8, 3, "背番号")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        Call .FixedSet(中中, 右中, あり, 11, 4, "収容数")
        Call .FixedSet(中中, 右中, あり, 5, 5, "箱数")
        Call .FixedSet(中中, 右中, あり, 11, 6, "端数")
        Call .FixedSet(中中, 右中, あり, 13, 7, "発注数")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, なし, 0, 8, "納品書管理NO")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not ZCT_RDSTS
        With vsGridS
            ...
            '
            .TextMatrix(ID, GridCol(1)) = ZCT.材質
            .TextMatrix(ID, GridCol(2)) = ZCT.表示寸法
            .TextMatrix(ID, GridCol(3)) = ZCT.背番号
            .TextMatrix(ID, GridCol(4)) = Format(ZCT.収容数, "#,##0.000")
            .TextMatrix(ID, GridCol(5)) = Format(ZCT.枚数, "###")
            .TextMatrix(ID, GridCol(6)) = Format(ZCT.端数, "#,##0.000")
            .TextMatrix(ID, GridCol(7)) = Format(ZCT.発注数, "#,##0.000")
       
            .TextMatrix(ID, GridCol(8)) = ZCT.納品書管理NO
...
Private Sub ReturnPress()
    With vsGridS
        ...
        Rvc_NKNo = CisFun.Val2(.TextMatrix(.Row, 8))
