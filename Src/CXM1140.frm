...
'**                                             未設定分の表示
'**                         2008/01/10  By CIS 代行→納入先等大幅変更　PXM0140より以降
'**********************************************************'
    ...
    Dim mNTorcd()     As String
    Dim mNTorkj()     As String
    Dim mNUkeir()       As String
    Dim mNTorNm()     As String
    Dim mJTorcd()     As String
    Dim mCyokuKb()     As Byte
...
Private Sub PB_Back_Click()
    ...
    gSL_Select = gSL_Select & " TH.支給元,TH.支給元2,TH.直直区分,TH.納入先,TH.納入先工場,TH.納入先受入,TH.納入先CD,TH.納入先名称"
    ...
    gSL_Select = gSL_Select & "   AND TH.代行契約先 = '" & RTrim(THM.代行契約先) & "'"
    ...
    gSL_Select = gSL_Select & " TH.支給元,TH.支給元2,TH.直直区分,TH.納入先,TH.納入先工場,TH.納入先受入,TH.納入先CD,TH.納入先名称"
    gSL_Select = gSL_Select & " ,TR.略称,TR2.被自社 ,TR2.被自社工場 "
...
Private Sub PB_Hinbn_Click()
    ...
    RV_STorcd2 = RTrim(iB1_TyokTorcd)                       '支給元2
    RV_KTorcd = RTrim(iB1_KTorcd)                           '契約先ｺｰﾄﾞ
    RV_KTorkj = RTrim(iB1_KTorKj)                           ' "    工場
    RV_CyokuKb = B1chk_Tyoku.Value                          '直直区分
    RV_NTorcd = RTrim(iB1_DTorcd)                           '納入先ｺｰﾄﾞ
    RV_NTorkj = RTrim(iB1_DTorKj)                           ' "    工場
    RV_NUkeir = RTrim(iB1_DUke)                             ' "    受入
    RV_NTorNm = RTrim(iB1_DTorNm)                           ' "    名称
...
'契約先
    ...
    Else
        gStr = RTrim(TRM.被自社)
        If Trim(TRM.被自社工場) <> "" Then
           gStr = gStr & "-" & RTrim(TRM.被自社工場)
        End If
        mHiJisya = RTrim(gStr)
    End If
'代行契約先
...
'   未設定分を品番取引先マスタより表示
    If ChkH1_Disp.Value = 1 Then
        ...
        gSL_Select = gSL_Select & "  '' 支給元,'' 支給元2,0 直直区分,'' 納入先,'' 納入先工場,'' 納入先受入,'' 納入先CD,'' 納入先名称,"
        gSL_Select = gSL_Select & "  TR.略称,0 品番件数  ,TR2.被自社 契約先被自社,TR2.被自社工場 契約先被工場"
    ...
    For gInt = 1 To vsGrid.Rows - 1
        If RTrim(vsGrid.TextMatrix(gInt, 1)) <> "" And RTrim(vsGrid.TextMatrix(gInt, 9)) = "" Then
            ...
            With THM
                ...
                .直直区分 = CisFun.Val2(vsGrid.TextMatrix(gInt, 12))
'''                .支給元工場2 = RTrim(vsGrid.TextMatrix(gInt, 13))
'''                .支給元2受入 = RTrim(vsGrid.TextMatrix(gInt, 17))
                .納入先 = RTrim(vsGrid.TextMatrix(gInt, 14))
                .納入先工場 = RTrim(vsGrid.TextMatrix(gInt, 15))
                .納入先CD = RTrim(vsGrid.TextMatrix(gInt, 14)) & RTrim(vsGrid.TextMatrix(gInt, 15))
                .納入先受入 = RTrim(vsGrid.TextMatrix(gInt, 7))
                .納入先名称 = RTrim(vsGrid.TextMatrix(gInt, 8))
                .作成者 = gTanto
            End With
        ' ( 追加処理 )
            Call THMInsert
            ...
            If RTrim(vsGrid.TextMatrix(gInt, 1)) <> "" And RTrim(vsGrid.TextMatrix(gInt, 9)) <> "" Then
               If RTrim(vsGrid.TextMatrix(gInt, 1)) = mTorcd(gInt) Then                                     ' 2006/09/05
                   With HTM
                        ...
                        gSL_Select = gSL_Select & " 支給元2 = '" & RTrim(iB1_TyokTorcd) & "',"
                        ...
                        gSL_Select = gSL_Select & " 直直区分 = " & CisFun.Val2(vsGrid.TextMatrix(gInt, 12)) & ","
'''                        gSL_Select = gSL_Select & " 支給元工場2 = '" & RTrim(vsGrid.TextMatrix(gInt, 13)) & "',"
'''                        gSL_Select = gSL_Select & " 支給元2受入 = '" & RTrim(vsGrid.TextMatrix(gInt, 17)) & "',"
                        gSL_Select = gSL_Select & " 納入先 = '" & RTrim(vsGrid.TextMatrix(gInt, 14)) & "',"
                        gSL_Select = gSL_Select & " 納入先工場 = '" & RTrim(vsGrid.TextMatrix(gInt, 15)) & "',"
                        gSL_Select = gSL_Select & " 納入先CD = '" & RTrim(vsGrid.TextMatrix(gInt, 14)) & RTrim(vsGrid.TextMatrix(gInt, 15)) & "',"
                        gSL_Select = gSL_Select & " 納入先受入 = '" & RTrim(vsGrid.TextMatrix(gInt, 7)) & "',"
                        gSL_Select = gSL_Select & " 納入先名称 = '" & RTrim(vsGrid.TextMatrix(gInt, 8)) & "',"
                        ...
                        gSL_Select = gSL_Select & "   AND 支給元2 = '" & RTrim(mTyokTorcd) & "'"
                        gSL_Select = gSL_Select & "   AND 直直区分 = " & RTrim(mCyokuKb(gInt)) & ""
'''                        gSL_Select = gSL_Select & "   AND 支給元工場2 = '" & RTrim(mSTorkj2(gInt)) & "'"
'''                        gSL_Select = gSL_Select & "   AND 支給元2受入 = '" & RTrim(mSTorUke2(gInt)) & "'"
                        gSL_Select = gSL_Select & "   AND 納入先 = '" & RTrim(mNTorcd(gInt)) & "'"
                        gSL_Select = gSL_Select & "   AND 納入先工場 = '" & RTrim(mNTorkj(gInt)) & "'"
                        gSL_Select = gSL_Select & "   AND 納入先受入 = '" & RTrim(mNUkeir(gInt)) & "'"
                        gSL_Select = gSL_Select & "   AND 納入先名称 = '" & RTrim(mNTorNm(gInt)) & "'"
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, あり, 9, 5, "直直区分")                  '2006/03/15
        Call .FixedSet(中中, 中中, あり, 9, 6, "納入先")
        Call .FixedSet(中中, 中中, あり, 3, 7, "納入先受入")
        Call .FixedSet(中中, 左中, あり, 25, 8, "納入先名称")
        Call .FixedSet(中中, 中中, あり, 3, 9, "品")                '品番件数
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 中中, あり, 0, 10, "x契約取引先")
        Call .FixedSet(中中, 中中, あり, 0, 11, "x契約取引工場")
        Call .FixedSet(中中, 中中, あり, 0, 12, "x直直")
        Call .FixedSet(中中, 中中, あり, 0, 13, "xX2")
        Call .FixedSet(中中, 中中, あり, 0, 14, "x納入先")
        Call .FixedSet(中中, 中中, あり, 0, 15, "x納入先工場")
        Call .FixedSet(中中, 左中, なし, 0, 16, "Select")
        Call .FixedSet(中中, 中中, あり, 0, 17, "xX3")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    ReDim mCyokuKb(Ttl_Kensu + 1)
    
    ReDim mSTorcd2(Ttl_Kensu + 1)
    ReDim mSTorkj2(Ttl_Kensu + 1)
    ReDim mSTorUke2(Ttl_Kensu + 1)
    ReDim mNTorcd(Ttl_Kensu + 1)
    ReDim mNTorkj(Ttl_Kensu + 1)
    ReDim mNUkeir(Ttl_Kensu + 1)
    ReDim mNTorNm(Ttl_Kensu + 1)
...
'
            If THM.直直区分 <> 0 Then
                .TextMatrix(ID, GridCol(5)) = "対象"
            End If
            
            If RTrim(THM.納入先工場) <> "" Then
                .TextMatrix(ID, GridCol(6)) = THM.納入先 & Space(5 - CisFun.Len2(THM.納入先)) _
                                      & "-" & THM.納入先工場 + Space(2 - CisFun.Len2(THM.納入先工場))
            Else
                .TextMatrix(ID, GridCol(6)) = THM.納入先 & Space(5 - CisFun.Len2(THM.納入先)) & Space(3)
            End If
            .TextMatrix(ID, GridCol(7)) = THM.納入先受入 + Space(2 - CisFun.Len2(THM.納入先受入))
            .TextMatrix(ID, GridCol(8)) = RTrim$(THM.納入先名称)
            
            .TextMatrix(ID, GridCol(9)) = Format(THM.品番件数, "##")
            
            .TextMatrix(ID, GridCol(10)) = RTrim$(THM.契約取引先)
            .TextMatrix(ID, GridCol(11)) = RTrim$(THM.契約工場)
            .TextMatrix(ID, GridCol(12)) = THM.直直区分
            .TextMatrix(ID, GridCol(14)) = RTrim$(THM.納入先)
            .TextMatrix(ID, GridCol(15)) = RTrim$(THM.納入先工場)
        End With
        '初期情報退避
        ...
        mCyokuKb(ID) = THM.直直区分
        mNTorcd(ID) = RTrim(THM.納入先)
        mNTorkj(ID) = RTrim(THM.納入先工場)
        mNUkeir(ID) = RTrim(THM.納入先受入)
        mNTorNm(ID) = RTrim(THM.納入先名称)
...
'       >> 直直区分
        If B1chk_Tyoku.Value <> 0 Then
            .TextMatrix(tRow, 5) = "対象"
        Else
            .TextMatrix(tRow, 5) = ""
        End If
...
'       >>
        ...
        .TextMatrix(tRow, 12) = B1chk_Tyoku.Value
...
Private Sub VsGridToText(tRow As Integer)
    With vsGrid
        ...
        '>> 直直
        If CisFun.Val2(.TextMatrix(tRow, 12)) <> 0 Then
           B1chk_Tyoku.Value = 1
        Else
           B1chk_Tyoku.Value = 0
        End If
