   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日　-便|仕入先|仕 入 先 名 |納 番 |頁|手配|背番号|材　質　　               |収容数  |箱数|発注数   |端数    |"
            Rows            =   21
            Cols            =   16
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=31      Else 1=131
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            Call CXHZ0360LPrint(wPrintKB, wPrintOut)
...
'   ### 再発行既存 ###
    ...
    gSL_Select = gSL_Select & "   and sym.値 = zct.手配区分"
    If H1Op_SKbn(1) Then
'   ### 再発行最新 ###
        gSL_Select = gSL_Select & " where cht.端末番号 = substring(host_name(),1,20)"
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 31"
        gSL_Select = gSL_Select & "   and zct.発注管理NO = cht.管理NO"
        gSL_Select = gSL_Select & "   and zct.端数区分 = 1"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where zct.端数区分 = 1"
    End If
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
...
'   >> 材質
    If RTrim(iH1_Zaist) <> "" Then
        If InStr(1, iH1_Zaist, "*") = 0 Then
            gSL_Select = gSL_Select & "   and zct.材質 = '" & RTrim(iH1_Zaist) & "'"
        Else
            gSL_Select = gSL_Select & "   and zct.材質 like '" & RTrim(Replace(iH1_Zaist, "*", "%")) & "'"
        End If
    End If
'   >> 背番号
    If RTrim(iH1_Seban) <> "" Then
        If InStr(1, iH1_Seban, "*") = 0 Then
            gSL_Select = gSL_Select & "   and zct.背番号 = '" & RTrim(iH1_Seban) & "'"
        Else
            gSL_Select = gSL_Select & "   and zct.背番号 like '" & RTrim(Replace(iH1_Seban, "*", "%")) & "'"
        End If
    End If
'   >> 納入日
    ...
    gSL_Select = gSL_Select & ", zct.納入番号"
    gSL_Select = gSL_Select & ", zct.納品書頁"
    gSL_Select = gSL_Select & ", zct.納品書行"
'
End Sub
'**************************************
'*                                    *
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 131"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE STAT
        'Call .FixedSet(中中, 左中, あり, 6, 7, "背番号")
        Call .FixedSet(中中, 左中, あり, 8, 7, "背番号")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        Call .FixedSet(中中, 左中, あり, 25, 8, "材　　　質")
        Call .FixedSet(中中, 右中, あり, 8, 9, "収容数")
        Call .FixedSet(中中, 右中, あり, 4, 10, "箱数")
        Call .FixedSet(中中, 右中, あり, 9, 11, "発注数")
        Call .FixedSet(中中, 右中, あり, 8, 12, "端数")
...
Private Sub ZctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '納番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(ZCT.納入番号)
        '頁
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = Format(ZCT.納品書頁, "#,###")
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(ZCT.手配区分名)
        '背番号
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(ZCT.背番号)
        '材質
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = RTrim(ZCT.材質)
        '収容数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = Format(ZCT.収容数, "##0.000")
        '箱数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = Format(ZCT.枚数, "###")
        '発注数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = Format(ZCT.発注数, "##0.000")
        '端数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = Format(ZCT.端数, "##0.000")
        '発注管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(13)) = ZCT.発注管理NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
