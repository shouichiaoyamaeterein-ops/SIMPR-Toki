...
Private Sub Form_Load()
    ...
    With CisPrtCXM1150
        .iPrtTitle = "構成マスタ照会一覧"
        .iLineMax = 40                                  ' 2019/07/31 UPD：30→40
        .iLinePitch = 6
        .iPaperSize = p_A4
        ...
        If Not .PrintIni Then
           ...
           End With
           Exit Sub
'        .cLine = 0         ' 2019/07/26 DEL
'        .cPage = 0         ' 2019/07/26 DEL
        End If
        .cLine = 0          ' 2019/07/26 ADD
        .cPage = 0          ' 2019/07/26 ADD
    End With
...
Private Function PrintProc()
    ...
    Call CisPrtCXM1150.ClrLstFld("B1")


    Call HeadWrite(1)                       ' 2019/07/30 ADD


'*---------------------------------*
'*  入荷テーブル読込み
'*---------------------------------*
    Dim iCnt As Long                        ' 2019/07/29 ADD
  ''For iCnt = 1 To iRdCnt                  ' 2019/08/01 DEL    ' 2019/07/29 ADD
    For iCnt = 1 To iTotalLineCnt           ' 2019/08/01 ADD
...
'        End If
'        Call KOLRead(gSL_Select, , 1)
'        '===================================================================================' 2019/07/29 ADD END
 
  
  
'    Do Until Not KOL_RDSTS                     ' 2019/08/01 DEL
        ...
        With KOL
            ' レベル
          ''If L_wsHKbn = 0 Then                '2009/11/10             ' 2019/07/31 DEL
              ''Select Case KOL.レベル
                Select Case wRdKOL(2, iCnt)
                    Case 0: B1_Level = "0"
                    Case 1: B1_Level = ".1"
                    Case 2: B1_Level = "..2"
                    Case 3: B1_Level = "...3"
                    Case 4: B1_Level = "....4"
                    Case Else
                        B1_Level = "....." & CisFun.Mid2(Format(KOL.レベル, "##"), 1, 6)    ' 2019/07/30 UPD：CisFun.Mid2で桁数調整
                End Select
'GoTo TEST_JMP_1

'===============================================================================================================================' 2019/07/30 DEL START
'                If .手配先 <> "" Then
'                   'B1_Tehai = RTrim(.手配先)
'            '=========================== 2013/03/15 Start
...
'End With
                ...
                gSL_Select = ""
              ''If KOL.区分 = "" Then
                If Trim(wRdKOL(10, iCnt)) = "" Then             ' 10：区分
                    B1_KB = ""
                    gSL_Select = "SELECT HT.*,"
                    gSL_Select = gSL_Select & vbCrLf & " TR.略称 取引先名,"
                    gSL_Select = gSL_Select & vbCrLf & " SY.値名称 手配区分名,"
                    gSL_Select = gSL_Select & vbCrLf & " HN.品名"
                    gSL_Select = gSL_Select & vbCrLf & " FROM 品番取引先マスタ HT "
                    gSL_Select = gSL_Select & vbCrLf & " LEFT OUTER JOIN 品番マスタ HN "
                    gSL_Select = gSL_Select & vbCrLf & "   ON HN.品番 = HT.品番 "
                    gSL_Select = gSL_Select & vbCrLf & " LEFT OUTER JOIN 取引先マスタ TR "
                    gSL_Select = gSL_Select & vbCrLf & "   ON TR.取引先CD = HT.取引先 "
                    gSL_Select = gSL_Select & vbCrLf & "  AND TR.取引先区分 = HT.品目 "
                    gSL_Select = gSL_Select & vbCrLf & " LEFT OUTER JOIN 名称マスタ SY "
                    gSL_Select = gSL_Select & vbCrLf & "   ON SY.区分名称 = '手配区分' "
                    gSL_Select = gSL_Select & vbCrLf & "  AND SY.値 = HT.手配区分"
                    gSL_Select = gSL_Select & vbCrLf & "  AND SY.区分タイプ = ''"
                    
                    If L_wsHKbn = 0 Then
                      ''gSL_Select = gSL_Select & vbCrLf & " WHERE HT.品番 = '" & RTrim(.TextMatrix(pRow, SCol(9))) & "'"
                        If wRdKOL(2, iCnt) = 0 Then         ' <<親品番>>
                            gSL_Select = gSL_Select & vbCrLf & " WHERE HT.品目 = 0"
                            gSL_Select = gSL_Select & vbCrLf & "   AND HT.品番 = '" + Replace(wRdKOL(0, iCnt), "-", "") + "'"
                        Else                                ' <<子品番>>
                            gSL_Select = gSL_Select & vbCrLf & " WHERE HT.品目 = 1"
                            gSL_Select = gSL_Select & vbCrLf & "   AND HT.品番 = '" + Replace(wRdKOL(4, iCnt), "-", "") + "'"
                        End If
                        
                    Else
                        gSL_Select = gSL_Select & vbCrLf & " WHERE HT.品目 = 1"
                        gSL_Select = gSL_Select & vbCrLf & "   AND HT.品番 = '" + Replace(wRdKOL(3, iCnt), "-", "") + "'"
                    End If
                    
                    gSL_Select = gSL_Select & vbCrLf & " ORDER BY HT.取引先"
                    
                    Call Str_Monitor(gSL_Select)        ' for DEBUG
                    If HTMRead(gSL_Select, 1, 3) Then
                        gStr = HTM.取引先 & ":" & RTrim(HTM.取引先名)               ' RTrim(CIS.手配先コード) & RTrim(CIS.手配先工場コード) & ":" & RTrim(TRM.略称)
                        B1_Tehai = CisFun.Mid2(gStr, 1, 22)
                        B1_Seban = CisFun.Mid2(RTrim(HTM.背番号), 1, 8)
                        If HTM.変更日 <> "" And Format(Now(), "yyyyMMdd") >= HTM.変更日 Then
                            B1_Syuyo = Format(HTM.収容数2, "#,###")
                        Else
                ...
                Else
                    B1_KB = "材"
                    gSL_Select = "SELECT ZR.*,"
                    gSL_Select = gSL_Select & vbCrLf & " TR.略称 手配先名,"
                    gSL_Select = gSL_Select & vbCrLf & " SY.値名称 手配区分名,"
                    gSL_Select = gSL_Select & vbCrLf & " ZR.材料名"
                    gSL_Select = gSL_Select & vbCrLf & " FROM 材料マスタ ZR "
                    gSL_Select = gSL_Select & vbCrLf & " LEFT OUTER JOIN 材料取引先マスタ ZT "
                    gSL_Select = gSL_Select & vbCrLf & "   ON ZR.材料管理番号 = ZT.材料管理番号 "
                    gSL_Select = gSL_Select & vbCrLf & " LEFT OUTER JOIN 取引先マスタ TR "
                    gSL_Select = gSL_Select & vbCrLf & "   ON TR.取引先CD = ZT.取引先 "
                    gSL_Select = gSL_Select & vbCrLf & "  AND TR.取引先区分 = 1 "
                    gSL_Select = gSL_Select & vbCrLf & " LEFT OUTER JOIN 名称マスタ SY "
                    gSL_Select = gSL_Select & vbCrLf & "   ON SY.区分名称 = '手配区分' "
                    gSL_Select = gSL_Select & vbCrLf & "  AND SY.値 = ZT.手配区分"
                    gSL_Select = gSL_Select & vbCrLf & "  AND SY.区分タイプ = ''"
                  ''gSL_Select = gSL_Select & vbCrLf & " WHERE ZR.材料管理番号 = " & .TextMatrix(pRow, SCol(9))
                  ''gSL_Select = gSL_Select & vbCrLf & " WHERE ZR.材料管理番号 = " & Str(KOL.材料管理番号)
                    gSL_Select = gSL_Select & vbCrLf & " WHERE ZR.材料管理番号 = " & Str(wRdKOL(14, iCnt))
                    gSL_Select = gSL_Select & vbCrLf & " ORDER BY ZT.取引先"
                    Call Str_Monitor(gSL_Select)        ' for DEBUG
                    If ZTMRead(gSL_Select, 1, 3) Then
                        gStr = ZTM.取引先 & ":" & RTrim(ZTM.手配先名)               ' RTrim(CIS.手配先コード) & RTrim(CIS.手配先工場コード) & ":" & RTrim(TRM.略称)
                        B1_Tehai = CisFun.Mid2(gStr, 1, 22)
                        B1_Seban = ""
                        If ZTM.変更日 <> "" And Format(Now(), "yyyyMMdd") >= ZTM.変更日 Then
                            B1_Syuyo = Format(ZTM.収容数2, "#,###")
                        Else
                            B1_Syuyo = Format(ZTM.収容数1, "#,###")
                        End If
                    End If
                End If
                ...
                gStr = wRdKOL(11, iCnt)
                If gStr = "" Then
                    If wRdKOL(2, iCnt) = 0 Then
                        gStr = wRdKOL(0, iCnt)
                    Else
                        gStr = wRdKOL(4, iCnt)
                    End If
                End If
                B1_Hinbn = CisFun.Mid2(RTrim(gStr), 1, 20)
                B1_Sunpou = CisFun.Mid2(RTrim(wRdKOL(12, iCnt)), 1, 25)
                B1_Hinnm = CisFun.Mid2(RTrim(wRdKOL(13, iCnt)), 1, 25)
              ''B1_Hinbn = CisFun.Mid2(RTrim(.表示品番), 1, 20)
              ''B1_Sunpou = CisFun.Mid2(RTrim(.表示寸法), 1, 25)
...
'   印刷終了
    Call CisPrtCXM1150.PrintEndDoc
...
Private Function HeadWrite(KB As Byte)
    ...
    With CisPrtCXM1150
    '   １頁内印刷行数ﾁｪｯｸ
      ''If .iLineMax >= .cLine + 1 Then Exit Function                   ' 2019/07/31 DEL
        If KB = 0 And .iLineMax >= .cLine + 1 Then Exit Function        ' 2016/07/31 ADD
    ...
    '============================== 2014/02/13 Start
        If L_wsHKbn = 0 Then
            H1_JyuBiko = RTrim(CXM1150.lb_JyutyuBiko)
        Else
            H1_JyuBiko = ""
        End If
    '============================== 2014/02/13 End
