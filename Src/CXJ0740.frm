...
Begin VB.Form CXJ0740 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "【愛知機械】出荷データ変換"
   ClientHeight    =   11190
   ClientLeft      =   -540
   ClientTop       =   1020
   ClientWidth     =   15360
...
'**
'**       作  成  日    :   2010/03/17
'**       変  更  日    :   2010/05/31  出荷日指定,名称マスタ参照位置修正
'**
'**********************************************************'
...
'------------------- 履歴処理開始
    ...
    Text_YMD = RTrim(iH1_YMD)           '2010/05/31
...
Private Function Convert_ListProc() As Boolean
    ...
    If Not Text_CvtList(Text_Proc) Then
       ...
       Exit Function
    End If
    If Text_ProcKB <> 0 Then lb_msg.Caption = "出荷変換リスト発行": DoEvents
    
    ' ログ出力
    gStr = "【 テキスト出荷変換リスト出力 開始 】"
    Call Text_LogPut(gStr, "", 1)
...
' * 変換エラーリスト
    ...
    If THNRead(gSL_Select) Then
        ...
        lb_msg = "【 テキスト出荷 】 変換リスト 印刷中"
        DoEvents
        With Text_Prt
             Set .iPrtForm = CXJ0740L
...
'        .値名称 = "テキスト受注[愛知機械]"
    ...
    Call ItemsClearSYM
    gSL_Select = "Select * From 名称マスタ "
    gSL_Select = gSL_Select & " Where 区分名称   = 'テキスト管理[愛知機械]'"
    gSL_Select = gSL_Select & " And   区分タイプ = ''"
'    gSL_Select = gSL_Select & " And   値        = 'JY'"
    gSL_Select = gSL_Select & " And   値        = 'SY'"             '2010/05/31
    Call SYMRead(gSL_Select, 1)
