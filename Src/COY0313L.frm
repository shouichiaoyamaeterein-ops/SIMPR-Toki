VERSION 5.00
Begin VB.Form COY0313L 
   Caption         =   "îÑè„ó\ëzï\Åyå^îÔ"
   ClientHeight    =   3330
   ClientLeft      =   675
   ClientTop       =   2115
   ClientWidth     =   15420
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      Caption         =   "Å¶ îÑè„ó\ëzï\Åyå^îÔÅz Å¶"
      BeginProperty Font 
         Name            =   "ÇlÇr ñæí©"
         Size            =   18
         Charset         =   128
...
'**********************************************************'
'**
'**       ÉtÉHÅ[ÉÄñº    :  îÑè„ó\ëzï\Åyå^îÔÅEñæç◊Åz
'**       ÉtÉHÅ[ÉÄID    :  COY0313L
'**       èàóùäTóv      :
'**
'**       çÏ  ê¨  ì˙    :  2004/10/06
...
'**********************************************************'
    ...
    Dim wAKinGk(15)      As Currency     'é‘å^åv
    Dim wTKinGk(15)      As Currency     'å_ñÒêÊåv
    Dim wSKinGk(15)      As Currency     'ëççáåv
...
Private Function DataRead()
    ...
    If Not UYKRead(gSL_Select, 0) Then
        Call UYKClose
        Exit Function
    End If
...
Private Function PrintProc()
    ...
    CisPrt.cLine = 99
    wInCnt = 0: wSyaCnt = 0
    KeyTorcd = "": KeySyakei = "": KeyHinbn = ""
    Erase wAKinGk: Erase wTKinGk: Erase wSKinGk
    ...
    Do Until Not UYK_RDSTS
        With UYK
            ...
            For gInt = 1 To 12
                gLong = .ã‡äz(gInt) / 1000
                B1_KataHi(gInt) = Format(Format(gLong, "#,##0;-#,##0;#"), String(9, "@"))
            Next
            gLong = .è„ä˙åv / 1000
            B1_KataHi(13) = Format(Format(gLong, "#,##0;-#,##0;#"), String(9, "@"))
            gLong = .â∫ä˙åv / 1000
            B1_KataHi(14) = Format(Format(gLong, "#,##0;-#,##0;#"), String(9, "@"))
            gLong = .ä˙çáåv / 1000
            B1_KataHi(15) = Format(Format(gLong, "#,##0;-#,##0;#"), String(9, "@"))
            ...
            For gInt = 1 To 12
                wAKinGk(gInt) = wAKinGk(gInt) + .ã‡äz(gInt)
            Next
            wAKinGk(13) = wAKinGk(13) + .è„ä˙åv
            wAKinGk(14) = wAKinGk(14) + .â∫ä˙åv
            wAKinGk(15) = wAKinGk(15) + .ä˙çáåv
...
'
    ...
    With CisPrt
        ...
        With UYK
            ...
            H1_NJYy = Mid(.ó\ëzîNåé, 1, 4)
            H1_NJMm = Mid(.ó\ëzîNåé, 5, 2)
            H1_TORCD = RTrim(.å_ñÒêÊ)
            H1_TORNM = RTrim(.å_ñÒêÊñº)
            gLong = 0
            For gInt = 1 To 12
                H1_Mm(gInt) = Format(DateAdd("m", gInt - 1, CDate(Left(.äJénîNåé, 4) & "/" & Mid(.äJénîNåé, 5, 2) & "/01")), "mm") & "åé"
                H1_Yy(gInt) = Format(DateAdd("m", gInt - 1, CDate(Left(.äJénîNåé, 4) & "/" & Mid(.äJénîNåé, 5, 2) & "/01")), "yyyy") & "îN"
                If H1_Yy(gInt) = H1_Yy(gLong) Then
                    H1_Yy(gInt) = ""
                Else
                    gLong = gInt
                End If
...
Private Sub TotalWrite(Optional Kb As Integer = 0)
    With CisPrt
        ...
        For gInt = 1 To 15
            wTKinGk(gInt) = wTKinGk(gInt) + wAKinGk(gInt)
        Next
        Erase wAKinGk: wSyaCnt = 0
    End With
    ...
    With CisPrt
        ...
        For gInt = 1 To 15
            wSKinGk(gInt) = wSKinGk(gInt) + wTKinGk(gInt)
        Next
        Erase wTKinGk
    End With
