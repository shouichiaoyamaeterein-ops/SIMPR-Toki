...
'**       変  更  日    :   2004/04/20  型費作成追加
'**       変  更  日    :   2004/04/22  計算方法による計算追加
'**                                     売上実績テーブルには集計データ作成
'**                                     売上明細テーブルには明細データ作成
'**       変  更  日    :   2004/11/15  売上元データ出荷Or検収に対応
'**       変  更  日    :   2005/02/10  Logo表示の修正（システム環境設定が反映されない！）
'**       変  更  日    :   2005/06/09  実績年月チェック変更（確認ＭＳＧ表示後、処理出来るように）
'**       変  更  日    :   2011/02/28  トランザクション廃止
'**
'**********************************************************'
...
'    gSL_Select = gSL_Select & "KN.品番,KN.納入先,KN.検収日,SUM(KN.検収数) 検収数," & vbCr
'    gSL_Select = gSL_Select & " ISNULL(TR1.略称,'') 納入先名," & vbCr
'    gSL_Select = gSL_Select & " ISNULL(TR2.略称,'') 契約先名" & vbCr
'    gSL_Select = gSL_Select & "  From 売上単価未決ワーク TW" & vbCr
'    gSL_Select = gSL_Select & "   Left Outer Join 検収テーブル KN" & vbCr
'    gSL_Select = gSL_Select & "    ON KN.検収管理NO = TW.検収管理NO" & vbCr
'    gSL_Select = gSL_Select & "    AND KN.検収管理行NO = TW.検収管理行NO" & vbCr
'    gSL_Select = gSL_Select & "   LEFT OUTER JOIN 品番マスタ HN" & vbCr
'    gSL_Select = gSL_Select & "    ON HN.品番 = KN.品番" & vbCr
'    gSL_Select = gSL_Select & "   LEFT OUTER JOIN 品番取引先マスタ HT" & vbCr
'    gSL_Select = gSL_Select & "    ON HT.品番 = KN.品番" & vbCr
'    gSL_Select = gSL_Select & "    And HT.取引先 = KN.納入先" & vbCr
'    gSL_Select = gSL_Select & "    And HT.受入 = KN.受入" & vbCr
'    gSL_Select = gSL_Select & "   LEFT OUTER JOIN 取引先マスタ TR1" & vbCr
'    gSL_Select = gSL_Select & "    ON TR1.取引先CD = KN.納入先" & vbCr
...
'
'    gSL_Select = gSL_Select & " GROUP BY HN.表示品番,HT.契約先,KN.品番,KN.納入先,KN.検収日,TR1.略称,TR2.略称" & vbCr
'    gSL_Select = gSL_Select & " Order By KN.納入先,HN.表示品番,KN.検収日" & vbCr
'+---------------------------------------------------- 2004/11/15 Delete End
'+---------------------------------------------------- 2004/11/15 Insert
' 検収 Or 出荷
    Call ItemsClearIRN
    gCompany = ""
...
'   < 印刷データ読み込み >
    gSL_Select = "SELECT HN.表示品番,HT.契約先," & vbCr
    gSL_Select = gSL_Select & "KN.品番,KN.納入先," & vbCr
    If Mid(IRN.oyian, 1, 1) <> "1" Then
       gSL_Select = gSL_Select & "KN.検収日,SUM(KN.検収数) 検収数," & vbCr
    Else
       gSL_Select = gSL_Select & "SJ.出荷日 検収日,SUM(SJ.出荷数) 検収数," & vbCr
    End If
    gSL_Select = gSL_Select & " ISNULL(TR1.略称,'') 納入先名," & vbCr
    gSL_Select = gSL_Select & " ISNULL(TR2.略称,'') 契約先名" & vbCr
    gSL_Select = gSL_Select & "  From 売上単価未決ワーク TW" & vbCr
    If Mid(IRN.oyian, 1, 1) <> "1" Then
        ...
        gSL_Select = gSL_Select & "    AND KN.検収管理行NO = TW.検収管理行NO" & vbCr
    Else
        gSL_Select = gSL_Select & "   Left Outer Join 受注テーブル KN" & vbCr
        gSL_Select = gSL_Select & "    ON KN.出荷指示書NO = TW.検収管理NO" & vbCr
        gSL_Select = gSL_Select & "    AND KN.出荷指示書行NO = TW.検収管理行NO" & vbCr
        gSL_Select = gSL_Select & "   Left Outer Join 出荷テーブル SJ" & vbCr
        gSL_Select = gSL_Select & "    ON SJ.出荷指示書NO = TW.検収管理NO" & vbCr
        gSL_Select = gSL_Select & "    AND SJ.出荷指示書行NO = TW.検収管理行NO" & vbCr
    End If
    gSL_Select = gSL_Select & "   LEFT OUTER JOIN 品番マスタ HN" & vbCr
    gSL_Select = gSL_Select & "    ON HN.品番 = KN.品番" & vbCr
    gSL_Select = gSL_Select & "   LEFT OUTER JOIN 品番取引先マスタ HT" & vbCr
    ...
    If Mid(IRN.oyian, 1, 1) <> "1" Then
       ...
       gSL_Select = gSL_Select & " Order By KN.納入先,HN.表示品番,KN.検収日" & vbCr
    Else
       gSL_Select = gSL_Select & " GROUP BY HN.表示品番,HT.契約先,KN.品番,KN.納入先,SJ.出荷日,TR1.略称,TR2.略称" & vbCr
       gSL_Select = gSL_Select & " Order By KN.納入先,HN.表示品番,SJ.出荷日" & vbCr
    End If
'+---------------------------------------------------- 2004/11/15 Insert End
    If Not KNTRead(gSL_Select) Then
       Call KNTClose
       Exit Sub
    End If
...
'+---------------------------------------------------------------- 2004/11/15 Insert
        If Mid(IRN.oyian, 1, 1) <> "1" Then
           ...
           .pHeadItemSet 3, 右, "検収数"
        Else
           .pHeadItemSet 2, 中, "出 荷 日"
           .pHeadItemSet 3, 右, "出荷数"
        End If
'+---------------------------------------------------------------- 2004/11/15 Insert End
        ...
        Do Until (Not KNT_RDSTS)
            ...
            gStr = KNT.納入先
            If Trim(KNT.納入先名) <> "" Then gStr = gStr & "：" & CisFun.Mid2(KNT.納入先名, 1, 20)
            .pBodyValueSet 0, gStr
            If Trim(KNT.表示品番) = "" Then
               .pBodyValueSet 1, RTrim$(KNT.品番)
            Else
               .pBodyValueSet 1, RTrim$(KNT.表示品番)
            End If
            With CisFun
                .DateE_BefVal = KNT.検収日
                .DateEdit
            End With
            .pBodyValueSet 2, CisFun.DateE_AfVal
            .pBodyValueSet 3, Format(KNT.検収数, "#,###")
            gStr = KNT.契約先
...
'+------------------------------------------------------------------------------   2005/06/09 Insert
    If RTrim$(iH1_YM) < RTrim$(mZenYm) Then
        With CisFun
            .MB_Lines = 4
            .MB_MSG(2) = "    今回処理年月が前回処理年月より過去の年月ですが         "
            .MB_MSG(3) = "    本当に処理を開始してもよろしいですか？         "
            .MB_Button = OK_CAN
            If Not .MBOX Then
                iH1_YM.SetFocus
                Exit Function
...
Private Function Proc_Start() As Boolean
    ...
    lb_msg = "【 売上実績処理中 】しばらくお待ち下さい": DoEvents
    
    On Error GoTo Proc_Start_Err
' 売上実績未決テーブル削除
    With CisDB
        .SQL = "DELETE 売上単価未決ワーク "
        .DBExec
    End With
'======================================='
'   【検収】
'======================================='
    ...
    If Not JIssekiProc Then Exit Function
...
'''       gSL_Select = " SELECT 締日 FROM 取引先マスタ "
'''       gSL_Select = gSL_Select & " WHERE 取引先CD = '" & iH1_Torcd & "'"
...
'''    End If
'''' 締日 全て
'''    If H1Op_Shime(0).Value Then
'''       If H1Op_Proc(1).Value Then
'''          With CisDB
'''               .SQL = "DELETE 売上実績テーブル Where 実績年月 = '" & iH1_YM & "'"
'''               .DBExec
'''               .SQL = "Update 検収テーブル Set 実績年月 = '' Where 実績年月 = '" & iH1_YM & "'"
'''               .DBExec
'''          End With
'''       End If
'''' 締日未設定 処理
...
'''      If Not JIssekiProc Then Exit Function
'''' 締日設定済みの処理
'''      gSL_Select = "SELECT HT.契約先,ISNULL(締日,0) 締日 FROM 品番取引先マスタ HT"
'''      gSL_Select = gSL_Select & "     LEFT OUTER JOIN 取引先マスタ TR"
'''      gSL_Select = gSL_Select & "         ON HT.契約先 = TR.取引先CD"
'''      gSL_Select = gSL_Select & "         AND TR.取引先区分 = 0 "
'''      gSL_Select = gSL_Select & "     WHERE 締日 <> 0 "
'''      gSL_Select = gSL_Select & "       AND HT.契約先 <> '' "
'''      gSL_Select = gSL_Select & "     GROUP BY HT.契約先,締日"
'''
'''      If TRMRead(gSL_Select, , 1) Then
'''         Do Until Not TRM_RDSTS
'''            If TRM.締日 = 0 Then
'''               mShime = 99
'''            Else
'''               mShime = TRM.締日
...
'''    End If
'''' 締日 個別
'''    If H1Op_Shime(1).Value Then
'''' 締日設定済みの処理
'''       mShime = CisFun.Val2(iH1_Shime)
'''       If mShime = 0 Then mShime = 99
'''       mTorcd = "%"
'''       If Not JIssekiProc Then Exit Function
'''       GoTo Proc_Start_ED
'''    End If
'''' 締日 個別(実績更新済を除くすべて)
'''    If H1Op_Shime(2).Value Then
'''       wwShime = CisFun.Val2(iH1_Shime)
'''       If wwShime = 99 Then wwShime = 0
'''       gSL_Select = "SELECT HT.契約先,ISNULL(締日,0) 締日 FROM 品番取引先マスタ HT"
'''       gSL_Select = gSL_Select & "     LEFT OUTER JOIN 取引先マスタ TR"
'''       gSL_Select = gSL_Select & "         ON HT.契約先 = TR.取引先CD"
'''       gSL_Select = gSL_Select & "         AND TR.取引先区分 = 0 "
'''       gSL_Select = gSL_Select & "     WHERE 締日 = " & wwShime
'''       gSL_Select = gSL_Select & "       AND HT.契約先 <> '' "
'''       gSL_Select = gSL_Select & "       AND 品目 = 0 "
'''       gSL_Select = gSL_Select & "     GROUP BY HT.契約先,締日"
'''       If TRMRead(gSL_Select, , 1) Then
'''          Do Until Not TRM_RDSTS
'''             gSL_Select = "SELECT ISNULL(COUNT(*),0) 件数 FROM 売上実績テーブル "
'''             gSL_Select = gSL_Select & " WHERE 実績年月 = '" & iH1_YM & "'"
'''             gSL_Select = gSL_Select & "   AND 契約先 = '" & TRM.契約先 & "'"
'''             If IRNRead(gSL_Select, , 2) Then
'''               If IRN.件数 = 0 Then
'''                   If TRM.締日 = 0 Then
'''                      mShime = 99
'''                   Else
...
'+---------------------------------------------------+
'/////////////////////////////////////////////////////
Private Function JIssekiProc() As Boolean
    JIssekiProc = False
...
'''    Call CisDB.DBTran(TransBegin)
    With CisDB
        ...
        .ParaNo = 2:    .ParaValue = iH1_YM & Format(mShime, "00")  ' 終了対象年月日
        .ParaNo = 3:    .ParaValue = mTorcd                         ' 契約先
        .ParaNo = 4:    .ParaValue = mShime                         ' 締日                       ' 契約先
        ' 処理区分
        If H1Op_Proc(0).Value = True Then
           .ParaNo = 5:    .ParaValue = ""                          ' 新規
        Else
           .ParaNo = 5:    .ParaValue = "1"                         ' 再処理
        End If
        .ParaNo = 6:    .ParaValue = gTanto                         ' 処理担当者
        ...
        If .ParaValue <> 0 Then
            CisFun.MB_Lines = 5
            CisFun.MB_MSG(2) = "    売上実績処理　異常      "
            CisFun.MB_Button = Error
            CisFun.MBOX
            End
        End If
    
        .ParaNo = 8: C1lb_Input(1) = Format(CisFun.Val2(C1lb_Input(1)) + CisFun.Val2(.ParaValue), "#,###")
        .ParaNo = 9: C1lb_Output(2) = Format(CisFun.Val2(C1lb_Output(2)) + CisFun.Val2(.ParaValue), "#,###")
        .ParaNo = 10: C1lb_Output(1) = Format(CisFun.Val2(C1lb_Output(1)) + CisFun.Val2(.ParaValue), "#,###")
    End With
...
'''    Call CisDB.DBTran(TransBegin)
    With CisDB
        ...
        If H1Op_Proc(0).Value = True Then
           .ParaNo = 3:    .ParaValue = ""                          ' 新規
        Else
           .ParaNo = 3:    .ParaValue = "1"                         ' 再処理
        End If
        .ParaNo = 4:    .ParaValue = mShime                         ' 締日                       ' 契約先
        ' 処理区分
        ...
        If .ParaValue <> 0 Then
            CisFun.MB_Lines = 5
            CisFun.MB_MSG(2) = "    売上実績【型費】処理　異常      "
            CisFun.MB_MSG(4) = "    Error CD = " & .ParaValue
            CisFun.MB_Button = Error
            CisFun.MBOX
            End
        End If
    
        .ParaNo = 8: C1lb_Output(3) = Format(CisFun.Val2(C1lb_Output(3)) + CisFun.Val2(.ParaValue), "#,###")
    End With
'''    Call CisDB.DBTran(TransCommit)
