...
Begin VB.Form CXM2500 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "支払先スクラップ単価メンテ"
   ClientHeight    =   11190
   ClientLeft      =   1785
   ClientTop       =   420
   ClientWidth     =   15360
   ...
   Begin VB.PictureBox B2_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CXM2500.frx":0000
         Height          =   2370
         Left            =   90
         OleObjectBlob   =   "CXM2500.frx":0015
         TabIndex        =   15
         Top             =   1050
         Width           =   5760
      End
...
Private Sub PB_Back_Click()
    Call CisFrm.MousePT(11)
    Call iH1_Y_LostFocus
    gSL_Select = " SELECT 対象年, 対象期, 支払先"
    gSL_Select = gSL_Select & " FROM 支払先スクラップ単価マスタ"
    gSL_Select = gSL_Select & " WHERE 対象年 + CAST(対象期 AS VARCHAR) + 支払先 <"
    gSL_Select = gSL_Select & " '" & iH1_Y & "' + '" & wKi & "' + '" & RTrim(iH1_Torcd) & "'"
    gSL_Select = gSL_Select & " GROUP BY 対象年, 対象期, 支払先"
    gSL_Select = gSL_Select & " ORDER BY 対象年 + CAST(対象期 AS VARCHAR) + 支払先 Desc "
    If Not SSMRead(gSL_Select, 1) Then
        With CisFun
            .MB_Lines = 4
            .MB_MSG(2) = "     先頭レコードです            "
            .MB_Title = "【後退】"
    ...
    iH1_Y = SSM.対象年
    H1Op_Ki(SSM.対象期).Value = True
    Call DateDSP(CInt(SSM.対象期))
...
Private Sub PB_Next_Click()
    ...
    gSL_Select = gSL_Select & " '" & iH1_Y & "' + '" & wKi & "' + '" & RTrim(iH1_Torcd) & "'"
    gSL_Select = gSL_Select & " GROUP BY 対象年, 対象期, 支払先"
    gSL_Select = gSL_Select & " ORDER BY 対象年 + CAST(対象期 AS VARCHAR) + 支払先"
...
Private Sub iH1_Y_LostFocus()
    If Len(Trim(iH1_Y)) = 1 Then iH1_Y = "200" & Trim(iH1_Y)
...
Private Function DBInput(InputKey As String) As Boolean
    ...
    gSL_Select = gSL_Select & "        ISNULL(SSM.単価, 0) 単価, ISNULL(SSM.履歴NO, 0) 履歴NO" & vbCrLf
    gSL_Select = gSL_Select & "   FROM 名称マスタ SY1" & vbCrLf
    gSL_Select = gSL_Select & "        LEFT OUTER JOIN 支払先スクラップ単価マスタ SSM" & vbCrLf
    gSL_Select = gSL_Select & "             ON SSM.材質 = SY1.値" & vbCrLf
    gSL_Select = gSL_Select & "            AND SSM.支払先 = '" & RTrim(iH1_Torcd) & "'" & vbCrLf
'    If H1Op_Ki(0).Value Then
'        gSL_Select = gSL_Select & "            AND SSM.適用開始日付 = '" & SY_KamiSMD & "'" & vbCrLf
'        gSL_Select = gSL_Select & "            AND SSM.適用終了日付 = '" & SY_KamiEMD & "'" & vbCrLf
'    Else
'        gSL_Select = gSL_Select & "            AND SSM.適用開始日付 = '" & SY_SimoSMD & "'" & vbCrLf
'        gSL_Select = gSL_Select & "            AND SSM.適用終了日付 = '" & SY_SimoEMD & "'" & vbCrLf
'    End If
    gSL_Select = gSL_Select & "            AND SSM.履歴NO = (SELECT MAX(履歴NO)" & vbCrLf
    gSL_Select = gSL_Select & "                                FROM 支払先スクラップ単価マスタ SSM1" & vbCrLf
    gSL_Select = gSL_Select & "                               WHERE SSM1.支払先 = SSM.支払先" & vbCrLf
'    gSL_Select = gSL_Select & "                                 AND SSM1.適用開始日付 = SSM.適用開始日付" & vbCrLf
'    gSL_Select = gSL_Select & "                                 AND SSM1.適用終了日付 = SSM.適用終了日付" & vbCrLf
    gSL_Select = gSL_Select & "                                 AND SSM1.材質 = SSM.材質)" & vbCrLf
...
'Call Test_LogOutput(gSL_Select, 1)
    ...
    If Not SSMRead(gSL_Select, 0) Then
        Call SSMClose(0)
    Else
        Call GridSet
        DBInput = True
...
' ( 削除処理 )
        .SQL = ""
        .SQL = .SQL & " DELETE FROM 支払先スクラップ単価マスタ"
        .SQL = .SQL & "  WHERE 支払先 = '" & RTrim(iH1_Torcd) & "'"
        If H1Op_Ki(0).Value Then
...
Private Sub GridSet()
    ...
    If Not SSMRead(gSL_Select) Then Exit Sub
' 抽出処理 初期ｾｯﾄ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    End With
    ID = 0
    Do Until Not SSM_RDSTS
        'Grid Row,Col
        ID = ID + 1
        With vsGrid1
            .TextMatrix(ID, GridCol(0)) = ID
            .TextMatrix(ID, GridCol(1)) = RTrim(SSM.材質名)
            .TextMatrix(ID, GridCol(2)) = Format(SSM.単価, "#,##0.00;;#")
            .TextMatrix(ID, GridCol(4)) = RTrim(SSM.材質)
            .TextMatrix(ID, GridCol(5)) = SSM.履歴NO
        End With
        Call SSMReadNext(0)
    Loop
...
Private Sub DateDSP(Index As Integer)
    ...
    If Len(Trim(iH1_Y)) < 4 And RTrim(iH1_Y) <> "" Then
       If Len(Trim(iH1_Y)) = 1 Then iH1_Y = "200" & iH1_Y
       With CisFun
            .Date_RsFormat = fYMD
...
Private Sub RirekiDisp()
    ...
    gSL_Select = gSL_Select & "   FROM 支払先スクラップ単価マスタ"
    gSL_Select = gSL_Select & "  WHERE 支払先 = '" & RTrim(iH1_Torcd) & "'"
    If H1Op_Ki(0).Value Then
