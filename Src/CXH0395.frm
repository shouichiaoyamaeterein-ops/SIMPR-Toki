...
Begin VB.Form CXH0395 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "自社伝票（チケット）　再発行"
   ClientHeight    =   10980
   ClientLeft      =   1815
   ClientTop       =   1995
   ClientWidth     =   15315
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日 　-便|仕入先 |   仕  入  先  名   |受入|納 番 |頁|発注区|手配区|SK|品　番  　　            |"
            Rows            =   21
            Cols            =   17
            BackColor       =   12648447
...
'**       変  更  日    :   2006/02/09  By CIS  印刷連打対応
'**       変  更  日    :
'**
'**********************************************************'
...
'------------------------------< 2006/02/09 ADD  END  >
    ...
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=17 Else 1=117
            Call CXH0390LPrint(wPrintKB)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 17"
        ...
        gSL_Select = gSL_Select & "   and hch.発行形態 = 1"
    Else
'   ### 再発行既存 ###
...
'   >> 納入日
    ...
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(hch.納入日, 1, 6) = '" & iH1_SYmd & "'"
    Else
        YmdSta = "": BinSta = ""
        YmdEnd = "": BinEnd = ""
        '> 日付＆便　編集
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) = "" Then
            If Trim(BinSta) = "" Then
                gSL_Select = gSL_Select & "   and hch.納入日 = '" & YmdSta & "'"
            Else
        ...
        If Trim(YmdSta) = "" And Trim(YmdEnd) <> "" Then
            If Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and hch.納入日 <= '" & YmdEnd & "'"
            Else
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) <> "" Then
            If Trim(BinSta) = "" And Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and hch.納入日 between '" & YmdSta & "'" _
                                                             & " and '" & YmdEnd & "'"
            Else
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(13))) = "X" Then
                ...
                CHT.帳票区分 = 117
                CHT.管理文字 = RTrim(.TextMatrix(ID, CisVsGrid1.FixedGet(5)))
                CHT.管理NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(11)))
                CHT.補助番号 = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(6)))
                Call CHTInsert
            End If
        Next ID
    End With
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 117"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 7, 2, "仕入先")
        Call .FixedSet(中中, 左中, あり, 20, 3, "仕　入　先　名")
        Call .FixedSet(中中, 中中, あり, 4, 4, "受入")
        Call .FixedSet(中中, 中中, あり, 6, 5, "納 番")
        Call .FixedSet(中中, 右中, あり, 2, 6, "頁")
        Call .FixedSet(中中, 中中, あり, 6, 7, "発注区")
        Call .FixedSet(中中, 中中, あり, 6, 8, "手配区")
        Call .FixedSet(中中, 中中, あり, 2, 9, "SK")
        Call .FixedSet(中中, 左中, あり, 25, 10, "品　　　番")
        Call .FixedSet(中中, 左中, あり, 0, 11, "納品書管理NO")
        Call .FixedSet(中中, 左中, あり, 0, 12, "カード")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
    ...
    With vsGrid1
        ...
        If Trim(HCH.納入日) <> "" Then
            If RTrim(HCH.納入便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(HCH.納入日), "0000/00/00")
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(HCH.納入日), "0000/00/00") & "-" & RTrim(HCH.納入便)
            End If
        End If
        '仕入先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = RTrim(HCH.仕入先)
        '仕入先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = RTrim(HCH.仕入先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(HCH.受入)
        '納番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(HCH.納入番号)
        '頁
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = Format(HCH.頁NO, "##")
        '発注
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(HCH.発注区分名)
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = RTrim(HCH.手配区分名)
        'SK
        ...
        End Select
        '品番
        If CisFun.Mid2(HCH.納入番号, 5, 1) <> "0" Then
            .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = RTrim(HCH.表示品番(1))
        End If
        '納品書管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = HCH.納品書管理NO
        'カード
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = HCH.カード
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
