...
'**       ÉtÉHÅ[ÉÄID    :  CXHZ0535L
'**       èàóùäTóv      :
'**
'**       çÏ  ê¨  ì˙    :  2008/09/04
'**       ïœ  çX  ì˙    :  2008/11/25 By CIS  ÉçÉSÇÃéwíËïœçX
'**       ïœ  çX  ì˙    :  2009/02/03 By CIS  îwî‘çÜÅAÉoÉOèCê≥
'**       ïœ  çX  ì˙    :  2009/05/13 By CIS  ñáêî/èdó Å@ïœçX
'**
'**********************************************************'
'#î‰ärëﬁîóp#
...
Private Function PrintProc()
    ...
    Dim W_JisekiKan As Currency             ' 2009/05/13
    Dim W_Jyuryo As Long                    ' 2009/05/13
...
'    Call CisPrtCXHZ0535.ClrLstFld("B1")
'
'*-----------------------------------------*
'*  í†ï[èoóÕÉeÅ[ÉuÉãì«çûÅAàÛéöÉfÅ[É^çÏê¨
'*-----------------------------------------*
    B1_MS(0) = 0: B1_MS(10) = 0
    B1_JR(0) = 0: B1_JR(10) = 0
    Cnt_NB = 1
    
    Do Until Not CHT_RDSTS
    
        With CHT
            gSL_Select = "Select * From çﬁóøé¿ê—ä«óùÉeÅ[ÉuÉã " & vbCrLf
...
'Call Test_LogOutput(gSL_Select, 1)
            ...
            Call ItemsClearZJK
            Call ZJKRead(gSL_Select, 1, 1)
            With ZJK
                gSL_Select = "Select çﬁéø, çﬁóøñº "
                gSL_Select = gSL_Select & " From  çﬁóøÉ}ÉXÉ^ "
                gSL_Select = gSL_Select & " Where çﬁéø   = '" & .çﬁéø & "' "
                Call ItemsClearZRM
                ...
                For Cnt_W1 = 0 To 1
                    ...
                    B1_ZS(Cnt_W2) = ZRM.çﬁéø
                    B1_SA(Cnt_W2) = Format(.î¬å˙, "###0.00") & "x"
                    B1_SH(Cnt_W2) = Format(.ïù, "###0.00") & "x"
                    B1_SN(Cnt_W2) = Format(.í∑Ç≥, "###0.00")
                    
                    '===================================== 2009/02/04 Start
                    'B1_SB(Cnt_W2) = Format(.îwî‘çÜ, "#,###")
                    B1_SB(Cnt_W2) = RTrim(.îwî‘çÜ)
                    '===================================== 2009/02/04 End
                    B1_MS(Cnt_W2) = Format(.é¿ê—ñáêî, "#,###")
                    B1_JR(Cnt_W2) = Format(.é¿ê—êî, "#,###")
                    
                    '======================================= 2009/05/13 START
                    
                    W_JisekiKan = 1
                    W_Jyuryo = 0
                    If ZJK.ì¸óÕéÌóﬁ = "A" Then
                    'é©ìÆçÏê¨√ﬁ∞¿(íºëóéxãã)
                       gSL_Select = "SELECT çﬁóøä«óùî‘çÜ,édì¸êÊ,éÛì¸ FROM çﬁóøì¸â◊ÉeÅ[ÉuÉã "
                       gSL_Select = gSL_Select & " WHERE éxããé¿ê—ä«óùNO = " & .é¿ê—ä«óùNO
                       If ZYTRead(gSL_Select, 1, 1) Then
                          gSL_Select = "SELECT é¿ê—ä∑éZíl FROM çﬁóøéÊà¯êÊÉ}ÉXÉ^"
                          gSL_Select = gSL_Select & " WHERE çﬁóøä«óùî‘çÜ = " & ZYT.çﬁóøä«óùî‘çÜ
                          gSL_Select = gSL_Select & " AND   éÊà¯êÊ = '" & RTrim(ZYT.édì¸êÊ) & "'"
                          gSL_Select = gSL_Select & " AND   éÛì¸ = '" & RTrim(ZYT.éÛì¸) & "'"
                          If ZTMRead(gSL_Select, 1, 1) Then
                             W_JisekiKan = ZTM.é¿ê—ä∑éZíl
                          End If
                       End If
                    Else
                        gSL_Select = "SELECT é¿ê—ä∑éZíl FROM çﬁóøéÊà¯êÊÉ}ÉXÉ^"
                        gSL_Select = gSL_Select & " WHERE çﬁéø = '" & ZRM.çﬁéø & "'"
                        gSL_Select = gSL_Select & " AND   î¬å˙ = " & .î¬å˙
                        gSL_Select = gSL_Select & " AND   ïù = " & .ïù
                        gSL_Select = gSL_Select & " AND   í∑Ç≥ = " & .í∑Ç≥
                        gSL_Select = gSL_Select & " AND   éÊà¯êÊ = '" & RTrim(.é¿ê—êÊ) & "'"
                        gSL_Select = gSL_Select & " AND   éÛì¸ = '" & RTrim(.éÛì¸) & "'"
                        If ZTMRead(gSL_Select, 1, 1) Then
                           W_JisekiKan = ZTM.é¿ê—ä∑éZíl
                        End If
                    End If
                    If W_JisekiKan = 0 Then W_JisekiKan = 1
                    
                    W_Jyuryo = .é¿ê—êî * W_JisekiKan        'èdó ä∑éZ
                    ...
                    Else
                       B1_MS(Cnt_W2) = Format(.é¿ê—ñáêî, "#,###")
                       B1_MS(Cnt_W1 * 10) = B1_MS(Cnt_W1 * 10) + .é¿ê—ñáêî
                    End If
                    
                    B1_JR(Cnt_W2) = Format(W_Jyuryo, "#,###")
                    B1_JR(Cnt_W1 * 10) = B1_JR(Cnt_W1 * 10) + W_Jyuryo
                    
                    'B1_JR(Cnt_W1 * 10) = B1_JR(Cnt_W1 * 10) + .é¿ê—êî
                    'B1_MS(Cnt_W1 * 10) = B1_MS(Cnt_W1 * 10) + .é¿ê—ñáêî
...
'Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£ 2008/11/25 UPDATE START
                    'B1_Image1(Cnt_W1).Picture = LoadPicture("../Exe/BMP/logo.jpg")
                    B1_Image1(Cnt_W1).Picture = LoadPicture(gIniExe & "BMP\logo.jpg")
'Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£Å•Å£ 2008/11/25 UPDATE END
                Next Cnt_W1
            End With
            Cnt_NB = Cnt_NB + 1
            Call CHTReadNext
            ...
            If Cnt_NB > 3 Or .åüçıèÓïÒ <> H1_NbB(0) Or CHT_RDSTS = False Then
                ...
                B1_MS(0) = Format(B1_MS(0), "#,###")        ' çáåvÉtÉHÅ[É}ÉbÉgê›íË
                B1_JR(0) = Format(B1_JR(0), "#,###")
                B1_MS(10) = Format(B1_MS(10), "#,###")
                B1_JR(10) = Format(B1_JR(10), "#,###")
...
'                End If
                ...
                With CisPrtCXHZ0535
                    .cDataCnt = .cDataCnt + 1
                    .dpCountDisp Format(.cDataCnt, "#,###") & " / " & Format(CXHZ0535_PrtCnt, "#,###")
                    .cLine = 0
                    .cPage = .cPage + 1
