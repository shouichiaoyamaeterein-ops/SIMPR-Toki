...
Begin VB.Form CXJ0270 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D B1_C_Back 
         ...
         cAlingnment     =   6
         cPositionY      =   30
         Begin CisText_V60.CisText iB1_MM 
            Height          =   375
            Left            =   120
            TabIndex        =   15
            Top             =   420
            ...
            cILength        =   2
         End
         Begin CisText_V60.CisText iB1_DD 
            Height          =   375
            Left            =   630
            TabIndex        =   16
            Top             =   420
      ...
      Begin Cis3D_v60.CIS3D UC_3D1 
         ...
         cAlingnment     =   6
         cPositionY      =   30
         Begin CisText_V60.CisText iB1_CD 
            Height          =   375
            Left            =   120
            TabIndex        =   19
            Top             =   420
...
'**********************************************************'
'**
'**       フォーム名    :  受注チケット入力（トヨタ紡織） By ㈱キノックス
'**       フォームID    :  CXJ0270
'**       処理概要      :
'**
'**       作  成  日    :  2005/03/01  By CIS
'**       変  更  日    :  2006/03/23  By CIS 納品書を読むように修正
'**
'**********************************************************'
...
Private Sub RefHinbn(Index As Integer)
    ...
    If RV_Rtn Then
        ...
        Else
           iB12_Hinbn(Index) = Mid(RV_Hinbn, 1, 5) & "-"
        End If
...
Private Function ChekData(ReadType As Integer, OutType As Integer, ChekFlg As Integer) As Boolean
    ...
    With CisDB
        ...
        .ParaNo = 1: .ParaIO = Input用      ' 処理区分 0:ﾁｪｯｸ&出力,1:ﾁｪｯｸ
        .ParaNo = 2: .ParaIO = Input用      ' 出力区分 1:出力
        .ParaNo = 3: .ParaIO = Input用      ' 受注区分
        .ParaNo = 4: .ParaIO = Input用      ' 納入日
        .ParaNo = 5: .ParaIO = Input用      ' 納入便
        .ParaNo = 6: .ParaIO = Input用      ' ヘッド内容
        .ParaNo = 7: .ParaIO = InOut用      ' 伝票の明細部(<5明細分>)
        .ParaNo = 8: .ParaIO = OutPut用     ' 受入
        .ParaNo = 9: .ParaIO = OutPut用     ' 納入先
        .ParaNo = 10: .ParaIO = OutPut用     ' チェックエラーNO
        .ParaNo = 11: .ParaIO = OutPut用     ' チェックエラー行NO
        
        .ParaNo = 1: .ParaValue = ReadType           ' 0:ﾁｪｯｸ&出力,1:ﾁｪｯｸ
        .ParaNo = 2: .ParaValue = OutType            ' 1:出力
        .ParaNo = 3: .ParaValue = w_JKbn             ' 受注区分
        .ParaNo = 4: .ParaValue = iH1_Ymd
        .ParaNo = 5: .ParaValue = iH1_Bin
        .ParaNo = 6: .ParaValue = wcc_Head
        .ParaNo = 7: .ParaValue = wcc_Body
...
'        End If
        Select Case wErNo
            ...
            Case 9
                    .MB_MSG(1) = "【  " & StrConv(wErGyo, vbWide) & " 行目  】"
                    .MB_MSG(3) = "   チェックデジットエラー：入力内容を確認して下さい（ＢＯＤＹ部）    "
                    Set mFocusObj = iB12_CD(wErGyo - 1)
            Case 16
                    .MB_MSG(2) = "       チケット入力済みです      "
...
' エラー表示後再読み込み
    ...
    End If
'   * 画面再表示 *
    iB1_Ku = Mid(wcc_Head, 1, 1)
    iB1_Sharp = Mid(wcc_Head, 2, 1)
    iB1_SRTorcd = Mid(wcc_Head, 3, 5)
    iB1_Hacyu = Mid(wcc_Head, 8, 5)
    iB1_Uke = Mid(wcc_Head, 13, 2)
    iB1_Kibn = Mid(wcc_Head, 15, 3)
    iB1_MM = Mid(wcc_Head, 18, 2)
    iB1_DD = Mid(wcc_Head, 20, 2)
    iB1_Bin = Mid(wcc_Head, 22, 2)
    iB1_Gyo = Mid(wcc_Head, 24, 1)
...
'
    For wNum1 = 0 To 4
        wNum2 = wNum1 * 44
        If RTrim(Mid(wStr1, wNum2 + 1, 11)) = "" Then Exit For              '受付№,納入番号が空白なら終了
'
        iB12_UkeNo(wNum1) = RTrim(Mid(wStr1, wNum2 + 1, 5))                 '受付№
        iB12_Bunno(wNum1) = RTrim(Mid(wStr1, wNum2 + 6, 1))                 '分納
        iB12_Noubn(wNum1) = RTrim(Mid(wStr1, wNum2 + 7, 5))                 '納入番号
        iB12_Seban(wNum1) = RTrim(Mid(wStr1, wNum2 + 12, 3))                '背番号
        iB12_Hinbn(wNum1) = RTrim(Mid(wStr1, wNum2 + 15, 14))               '品番
        iB12_Syuyo(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 29, 5), 0, "#####")
        iB12_Hako(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 34, 3), 0, "###")
        iB12_CD(wNum1) = RTrim(Mid(wStr1, wNum2 + 43, 2))                   'CD
'
        If CisFun.Val2(Mid(wStr1, wNum2 + 37, 6)) <> 0 Then
            iB12_Su(wNum1) = CisFun.RSetFld(RTrim(Mid(wStr1, wNum2 + 37, 6)), 0, "######")
        End If
    Next wNum1
            ...
            For wNum2 = wNum1 To 4
                If RTrim(iB12_UkeNo(wNum2)) <> "" Or RTrim(iB12_Bunno(wNum2)) <> "" Or RTrim(iB12_Noubn(wNum2)) <> "" Or RTrim(iB12_Hinbn(wNum2)) <> "" Or _
                   ...
                   CisFun.Val2(iB12_Su(wNum2)) <> 0 Then
                    ...
                    iB12_UkeNo(wNum1) = RTrim(iB12_UkeNo(wNum2))
...
'   * ヘッド部作成 *
    If HB = "H" Then
        ...
        End If
        If RTrim(iB1_MM) = "" Then
           wcc_Head = wcc_Head & Space(2)
        Else
           wcc_Head = wcc_Head & Format(RTrim(iB1_MM), "!@@")
        End If
        If RTrim(iB1_DD) = "" Then
           wcc_Head = wcc_Head & Space(2)
        Else
           wcc_Head = wcc_Head & Format(RTrim(iB1_DD), "!@@")
        End If
        ...
        End If
        If RTrim(iB1_CD) = "" Then
           wcc_Head = wcc_Head & Space(2)
        Else
           wcc_Head = wcc_Head & Format(RTrim(iB1_CD), "!@@")
        End If
    End If
    If HB = "B" Then
'   * ボディ部作成 *
    ...
    '       * 品番 *
            If RTrim(iB12_Hinbn(wNum1)) = "" Then
                wcc_Body = wcc_Body & Space(14)
            Else
                wcc_Body = wcc_Body & Format(RTrim(iB12_Hinbn(wNum1)), "!@@@@@@@@@@@@@@")
            End If
    '       * 収容数 *
    ...
    '       * CD *
            If RTrim(iB12_CD(wNum1)) = "" Then
               wcc_Body = wcc_Body & Space(2)
            Else
                wcc_Body = wcc_Body & Format(RTrim(iB12_CD(wNum1)), "!@@")
            End If
        Next wNum1
    End If
End Sub
...
'    If RTrim(iH1_Bin) = "" Then
       iH1_Bin = Mid(RData, 96, 2)
'    End If

'区
    iB1_Ku = Mid(RData, 1, 1)
'♯
    iB1_Sharp = Mid(RData, 2, 1)
'仕入先
    iB1_SRTorcd = Mid(RData, 48, 4) & Mid(RData, 60, 1)
'発注者
    iB1_Hacyu = Mid(RData, 16, 4) & Mid(RData, 28, 1)
'受入
    iB1_Uke = Mid(RData, 32, 2)
'機番
    iB1_Kibn = Mid(RData, 36, 3)
'月日便
    iB1_MM = Mid(RData, 44, 2)
    iB1_DD = Mid(RData, 46, 2)
    iB1_Bin = iH1_Bin
'行
    iB1_Gyo = ""
'**BODY**
    For wI = 0 To 4
        If RTrim(Mid(RData, 68 + (wI * 74), 14)) <> "" Then
    '受付NO
           iB12_UkeNo(wI) = Mid(RData, 130 + (wI * 74), 5)
    '納入番号
           iB12_Bunno(wI) = Mid(RData, 82 + (wI * 74), 1)
           iB12_Noubn(wI) = Mid(RData, 83 + (wI * 74), 5)
    '背番号
           iB12_Seban(wI) = Mid(RData, 127 + (wI * 74), 3)
    '品番
           iB12_Hinbn(wI) = Mid(RData, 68 + (wI * 74), 14)
    '収容数
           iB12_Syuyo(wI) = Mid(RData, 112 + (wI * 74), 5)
    '箱数
           iB12_Hako(wI) = Mid(RData, 117 + (wI * 74), 3)
    '納入数
           iB12_Su(wI) = Mid(RData, 90 + (wI * 74), 6)
        End If
    Next wI
