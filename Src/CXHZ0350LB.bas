...
'    If Not CXHZ0350LPrint Then Exit Sub
'
'*   作成日  :2004/02/23   By CIS
'*   変更日  :
'*---------------------------------------------------------------------------------
Public CisPrtCXHZ0350        As New CISFormPrint
Public CXHZ0350_PrtCnt       As Long


Public Function CXHZ0350LPrint(PrintKB As Byte, Optional PrintOut As Integer = 0) As Boolean
    CXHZ0350LPrint = False
...
' 出力データ存在確認
    ...
    With CisDB
        ...
        .ReadStatus = True
        If .DBRead(1, 0) Then
            CXHZ0350_PrtCnt = CisDB.Recordset(0)
        End If
    End With
    With CisPrtCXHZ0350
         Set .iPrtForm = CXHZ0350L
         .dpMSG = "[材料]現品票　印刷中"
         If PrintOut = 0 Then
           .iPreview = pv_Print
         Else
