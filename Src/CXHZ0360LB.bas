...
'    If Not CXHZ0360LPrint Then Exit Sub
'
'*   作成日  :2004/02/24   By CIS
'*   変更日  :
'*---------------------------------------------------------------------------------
Public CisPrtCXHZ0360        As New CISFormPrint
Public CXHZ0360_PrtCnt       As Long


Public Function CXHZ0360LPrint(PrintKB As Byte, Optional PrintOut As Integer = 0) As Boolean
    CXHZ0360LPrint = False
...
' 出力データ存在確認
    ...
    End If
    '枚数退避
    CXHZ0360_PrtCnt = CisDB.RecordCount
    
    With CisPrtCXHZ0360
         Set .iPrtForm = CXHZ0360L
         .dpMSG = "[材料]端数ラベル　印刷中"
         If PrintOut = 0 Then
           .iPreview = pv_Print
         Else
