Attribute VB_Name = "CXH0361LB"
Option Explicit
'*--------------------------------------------------------------------------------
'*
'*   端数ラベル 発行
'*
'*   モジュールID  : CXH0361LB.BAS
'*
'*                  CXH0361LPrint(Integer)
'*
'*   引数 : Integer  =  0  :新規発行
'*                      1  :再発行
...
'*
'*   使用例:
'    If Not CXH0361LPrint(1) Then Exit Sub
'
...
'*
'*---------------------------------------------------------------------------------
Dim CXH0361CoRpt        As New CoReports85
Public CXH0361_PrtCnt       As Long
...
Global CXH0361PrtOut    As Integer
Global CXH0361QR    As String
Global CXH0361_Printer      As String

Public mQRMeker361      As QRmaker  ' QR

Public Function CXH0361LPrint(Optional RePrint As Byte, Optional PrintOut As Integer, Optional PrtMsg As Boolean = False)
    CXH0361LPrint = False
    App.Title = "端数ラベル"
...
' 出力データ存在確認
    ...
    End If
   '枚数獲得
    CXH0361_PrtCnt = CisDB.RecordCount
    pref_TCnt = CXH0361_PrtCnt
    
'+++++++++++++++++++++++++++++++'
'+  印刷処理
'+++++++++++++++++++++++++++++++'
    Set pref_Form = CXH0361
    pref_Msg = "【 端数ラベル 印刷 】"
...
'+                    印  刷  処  理                        ＋
'+----------------------------------------------------------+
Public Function CoPrintH0361() As Boolean
    CoPrintH0361 = False
'''''    Set CorForm = pref_Form
    
    pref_NCnt = 0                ' 現在発行ページ初期化
    On Error GoTo CoPrintH0361_Err
...
'        .SetPaperWidth = 20000
'        .SetPaperLength = 10000
        .SetPrinter = CXH0361_Printer
'---------------------------------------------------------------------< 2006.02.14 ADD  END  >
        
        Select Case CXH0361PrtOut
...
' 明細を読み込んでｾｯﾄする(ﾚｺｰﾄﾞ件数分)
    Do Until Not HCT_RDSTS
        With HCT
            ' 項目転送
            If Not CoH0361Move Then GoTo CoPrintH0361_ED
            ' 印刷開始
            CXH0361CoRpt.CoPrintOut
...
CoPrintH0361_ED:
    Call HCTClose(1)
' CoReport 終了処理
    If pref_TCnt <> 0 Then CXH0361CoRpt.CoOutEnd
...
'*                                                                  *
'*------------------------------------------------------------------*
Private Function CoH0361Move() As Boolean
    Dim wBar            As String
...
'自社名
        ...
        If Not CXH0361CoRpt.CorData("自社名", gStr, "S", 20) Then GoTo CoH0361Move_Err
        gStr = CisFun.Left2(DP_JisyaKjNm, 20)
        If Not CXH0361CoRpt.CorData("自社工場", gStr, "", 20) Then GoTo CoH0361Move_Err
...
'仕入先名
        ...
        End If
        If Not CXH0361CoRpt.CorData("仕入先", gStr, "", 8) Then GoTo CoH0361Move_Err
        gStr = CisFun.Left2(.仕入先名, 20)
        If Not CXH0361CoRpt.CorData("仕入先名", gStr, "", 20) Then GoTo CoH0361Move_Err
        gStr = CisFun.Left2(.仕入先工場名, 20)
        If Not CXH0361CoRpt.CorData("仕入先工場", gStr, "", 20) Then GoTo CoH0361Move_Err
'納入先名
        If RTrim(.納入先) <> "" Then
            If Not CXH0361CoRpt.CorData("納入先T", "納入先", "", 6) Then GoTo CoH0361Move_Err
            gStr = RTrim(.納入先CD)
            If RTrim(.納入先工場CD) <> "" Then
                gStr = gStr & "-" & RTrim(.納入先工場CD)
            End If
            If Not CXH0361CoRpt.CorData("納入先", gStr, "", 8) Then GoTo CoH0361Move_Err
            gStr = CisFun.Left2(.納入先名, 20)
            If Not CXH0361CoRpt.CorData("納入先名", gStr, "", 20) Then GoTo CoH0361Move_Err
            gStr = CisFun.Left2(.納入先工場名, 20)
            If Not CXH0361CoRpt.CorData("納入先工場", gStr, "", 20) Then GoTo CoH0361Move_Err
        End If
'納入日・便
        gStr = Format(Mid(.納入日, 1, 4), "0000")
        If Not CXH0361CoRpt.CorData("納入年", gStr, "", 4) Then GoTo CoH0361Move_Err
        gStr = Format(Mid(.納入日, 5, 2), "#")
        If Not CXH0361CoRpt.CorData("納入月", gStr, "", 2) Then GoTo CoH0361Move_Err
        gStr = Format(Mid(.納入日, 7, 2), "#")
        If Not CXH0361CoRpt.CorData("納入日", gStr, "", 2) Then GoTo CoH0361Move_Err
        If Trim(.納入便) <> "" Then
           If Not CXH0361CoRpt.CorData("納入便", .納入便, "", 2) Then GoTo CoH0361Move_Err
           If Not CXH0361CoRpt.CorData("納入便T", "便", "", 2) Then GoTo CoH0361Move_Err
        End If
'置場
        If Not CXH0361CoRpt.CorData("置場", .置場, "", 10) Then GoTo CoH0361Move_Err
'手配区分
        If Not CXH0361CoRpt.CorData("手配区分", .手配区分名, "", 4) Then GoTo CoH0361Move_Err
'背番号
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE START
        'If Not CXH0361CoRpt.CorData("背番号", .背番号, "", 5) Then GoTo CoH0361Move_Err
        If Not CXH0361CoRpt.CorData("背番号", .背番号, "", 8) Then GoTo CoH0361Move_Err
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
'数量
        gStr = Format(.端数, "#,###")
        If Not CXH0361CoRpt.CorData("数量", gStr, "", 6) Then GoTo CoH0361Move_Err
'収容器
        If Not CXH0361CoRpt.CorData("収容器", .収容器, "", 10) Then GoTo CoH0361Move_Err
'収容数
        gStr = Format(.収容数, "#,###")
        If Not CXH0361CoRpt.CorData("収容数", gStr, "", 7) Then GoTo CoH0361Move_Err
'車型
        If Not CXH0361CoRpt.CorData("車型", .車型, "", 5) Then GoTo CoH0361Move_Err
'品番
        If CisFun.Len2(RTrim(.表示品番)) <= 22 Then
            If Not CXH0361CoRpt.CorData("品番2", .表示品番, "", 22) Then GoTo CoH0361Move_Err
        Else
            If Not CXH0361CoRpt.CorData("品番", .表示品番, "", 30) Then GoTo CoH0361Move_Err
        End If
'品名
        If Not CXH0361CoRpt.CorData("品名", .品名, "", 40) Then GoTo CoH0361Move_Err
        
'受入
        If Not CXH0361CoRpt.CorData("受入", .受入, "", 2) Then GoTo CoH0361Move_Err
        
'   *------------------*
'   * 『TAIL』SET  *
'   *------------------*
'発行日
        If Not CXH0361CoRpt.CorData("発行日", DP_PrtDate, "", 10) Then GoTo CoH0361Move_Err

'   *---------------------*
'   * 『QR』    SET       *
'   *---------------------*
        Call Denpyo_QREdit
        If Not CXH0361CoRpt.CorQRData("QR", CXH0361QR, "", 150) Then Exit Function
    End With
...
Private Function Denpyo_QREdit() As Boolean
    ...
    Mid(QREdit1, 13, 1) = "H"                           'QR識別(R:臨時かんばん、S:現品票、H:端数ラベル)
...
'    Mid(QREdit1, 101, 3) = "001"                        '現枚数
'    Mid(QREdit1, 104, 3) = "001"                        '総枚数
...
'    Mid(QREdit1, 139, 1) = Format(HCT.発注区分, "0")    '発注区分
    ...
    Mid(QREdit1, 104, 3) = "001"                        '現枚数
    Mid(QREdit1, 107, 3) = "001"                        '総枚数
    CisFun.DateE_AfVal = DP_PrtDate
    CisFun.DateEditUn
