...
Begin VB.Form CXH0531 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CXH0531.frx":0000
         Height          =   5550
         Left            =   70
         OleObjectBlob   =   "CXH0531.frx":0015
         TabIndex        =   11
         Top             =   70
         Width           =   7100
      End
...
'    End If
''   >> 日付(to)
'    If RTrim(iH3_Ymd) <> "" Then
'        If CisFun.Val2(iH3_Ymd.cDay) = 0 Then
'        Else
'            If Not iH3_Ymd.cChkResult Then
'                CisFun.MB_MSG(2) = "    日付を正しく入力して下さい            "
'                CisFun.MBOX
'                iH3_Ymd.SetFocus
'                Exit Function
...
''   >> 範囲 from > to は不可
'    If CisFun.Val2(iH1_Ymd.cDay) = 0 Then
'        work1 = Val(RTrim(iH1_Ymd) & "01")
'    Else
'        work1 = Val(RTrim(iH1_Ymd))
'    End If
'    If CisFun.Val2(iH3_Ymd.cDay) = 0 Then
'        work2 = Val(RTrim(iH3_Ymd) & "31")
'    Else
'        work2 = Val(RTrim(iH3_Ymd))
'    End If
'    If work1 > work2 Then
'        CisFun.MB_MSG(2) = "    日付を正しく入力して下さい            "
'        CisFun.MBOX
...
'    End If
'==== 2008.07.01 ↑↑上記を下記に変更↓↓
'    If RTrim(iH1_Ymd) <> "" And iH3_Ymd = "" Then   'If RTrim(iH1_Ymd) <> "" Then   ' fromのみ入力の場合
'        If CisFun.Val2(iH1_Ymd.cDay) = 0 Then
'            Sl_Where = Sl_Where & "   and substring(jkt.実績日,1,6) = '" & iH1_Ymd & "'"
'        Else
'            Sl_Where = Sl_Where & "   and jkt.実績日 = '" & iH1_Ymd & "'"
'        End If
'    ElseIf RTrim(iH3_Ymd) <> "" And iH1_Ymd = "" Then                               ' toのみ入力の場合
'        If CisFun.Val2(iH3_Ymd.cDay) = 0 Then
'            Sl_Where = Sl_Where & "   and substring(jkt.実績日,1,6) = '" & iH3_Ymd & "'"
'        Else
'            Sl_Where = Sl_Where & "   and jkt.実績日 = '" & iH3_Ymd & "'"
'        End If
'    ElseIf RTrim(iH1_Ymd) <> "" And RTrim(iH3_Ymd) <> "" Then                       ' from～toで入力の場合
'        If CisFun.Val2(iH1_Ymd.cDay) = 0 Then
'            Sl_Where = Sl_Where & "   and substring(jkt.実績日,1,6) >= '" & iH1_Ymd & "'"
'        Else
'            Sl_Where = Sl_Where & "   and jkt.実績日 >= '" & iH1_Ymd & "'"
...
'----------------------------------------------------------------------------------------------------------------
    ...
    If Trim(iH1_Ymd) <> "" And CisFun.Val2(iH1_Ymd.cDay) = 0 Then
        Sl_Where = Sl_Where & "   and substring(jkt.実績日,1,6) = '" & iH1_Ymd & "'"
        If iH2_Bin <> "" Then
            Sl_Where = Sl_Where & " and jkt.実績便 = '" & Format(iH2_Bin, "00") & "'"
        End If
    End If
    ' 日付開始編集
    ...
    If Trim(DateSt) <> "" And Trim(DateEd) = "" And CisFun.Val2(iH1_Ymd.cDay) <> 0 Then
        If Trim(BinSt) = "" Then
            Sl_Where = Sl_Where & " and jkt.実績日 = '" & DateSt & "'"
        Else
            Sl_Where = Sl_Where & " and jkt.実績日+実績便 = '" & DateSt & BinSt & "'"
        End If
    End If
    ' 日付終了のみ
    If Trim(DateSt) = "" And Trim(DateEd) <> "" Then
        If Trim(BinEd) = "" Then
            Sl_Where = Sl_Where & " and jkt.実績日 <='" & DateEd & "'"
        Else
            Sl_Where = Sl_Where & " and jkt.実績日+jkt.実績便 <= '" & DateEd & BinEd & "'"
        End If
    End If
    ' 日付開始・終了
    If Trim(DateSt) <> "" And Trim(DateEd) <> "" Then
        If Trim(BinSt) = "" And Trim(BinEd) = "" Then
            Sl_Where = Sl_Where & " and jkt.実績日 >= '" & DateSt & "' and jkt.実績日 <= '" & DateEd & "'"
        Else
            If Trim(BinSt) = "" Then BinSt = "00"
            If Trim(BinEd) = "" Then BinEd = "99"
            Sl_Where = Sl_Where & " and (jkt.実績日+jkt.実績便 >= '" & DateSt & BinSt & "' and jkt.実績日+jkt.実績便 <= '" & DateEd & BinEd & "')"
        End If
    End If
Call Test_LogOutput(Sl_Where)
'----------------------------------------------------------------------------------------------------------------
...
'        Sl_Where = Sl_Where & ")"
'    End If
    Sl_Where = Sl_Where & " and  jkt.実績区分 = 3"      ' Sl_Where = Sl_Where & gStr & "3": gStr = ","
...
Call Test_LogOutput(gSL_Select)
...
'+      選択内容ｾｯﾄ & 復帰ｽﾃｰﾀｽｾｯﾄ      +
'+-------------------------------------+
Private Sub Test_LogOutput(str As String)
Exit Sub   '=========================================== 実行しないときはこの行を生かすこと =========================
    Dim dt01 As String: dt01 = CStr(DateTime.Now)
    Dim dt02 As String
    ...
    Open "SQL_" & dt02 & ".log" _
          For Append As #fileNo                 'ファイルを追加モードで開く
    Print #fileNo, "--------------------" & _
                   Chr(13) & Chr(10) & dt01 & _
                   Chr(13) & Chr(10) & str & _
                   Chr(13) & Chr(10)            'ファイルへ書き込む"
