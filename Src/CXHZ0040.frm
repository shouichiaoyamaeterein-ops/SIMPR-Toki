   ...
   Begin Cis3D_v60.CIS3D B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D18 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   $"CXHZ0040.frx":030A
            Rows            =   21
            Cols            =   20
            BackColor       =   12648447
...
' 材料発注管理オプション
    ...
    iH1_NYm = Format(DateAdd("m", NonyuMSu, CDate(Left(gn_HNYM, 4) & "/" & Mid(gn_HNYM, 5, 2) & "/01")), "yyyymm")
...
Private Sub PB_Dsp_Click()
    ...
    RV_EYmd = RTrim(iH1_NYm)
    RV_Torcd = RTrim(iH1_Torcd)
    
    CXHZ0045.Show vbModal
    Unload CXHZ0045
    Set CXHZ0045 = Nothing
...
' 発注内示テーブル SELECT文 ｾｯﾄ
    gSL_Select = "SELECT NH.手配先,NH.受入,NH.材質,NH.納期,NH.数量,NH.材料管理番号,"
    gSL_Select = gSL_Select & "     HT.SK区分,HT.背番号,HN.表示寸法,"
    ...
    With CisFun
        ...
        If RTrim(iH1_Zaist) <> "" Then
            .Where_Create AlphaMe, "NH.材質", RTrim(iH1_Zaist) & "*"
        End If
        gSL_Select = gSL_Select & .Where_Phrase
    End With
    
    gSL_Select = gSL_Select & " Order By NH.手配先,NH.受入,NH.材質,NH.納期"
...
'材料発注画面入力ワーク削除
    gSL_Select = "DELETE FROM 材料発注画面入力ワーク"
...
'
        .ParaNo = 1: .ParaValue = RTrim(iH1_NYm)   '納入年月
...
'発注完了済リスト印刷
    If PrtCntD <> 0 And OpUchiPrt = 0 Then                                               '2005/11/07追加
        lb_Message = "【 材料発注打切完了済リスト 印刷中 】"
        DoEvents
        RV_SYmd = RTrim(iH1_NJYm)
        If Not CXHZ0040LPrint Then Exit Function
        DoEvents
    End If
...
' 発注処理
    ...
    If PrtCntD <> 0 Then
        With CisFun
            .MB_Lines = 6
            .MB_MSG(1) = "     打切完了済の材料が　" & PrtCntD & " 件存在しました。      "
            .MB_MSG(3) = "     打切完了済の材料は発注されません！      "
            .MB_MSG(5) = "     材料発注打切完了済リストで確認して下さい      "
            .MB_Title = "材料発注打切済確認"
            .MB_Button = OK
            .MBOX
        End With
...
HCProc_STErr:
    Select Case CisDB.ParaValue
        ...
        Case 9: gStr = "    【　材料かんばん読取エラーテーブル　出力エラー！】"
        Case 10: gStr = "    【　材料かんばん読取エラーテーブル(今回発注数)更新エラー！】"
        Case 11: gStr = "    【　材料かんばん読取エラーテーブル(発注済数)更新エラー！】"
        Case 12: gStr = "    【　材料発注画面入力ワーク(処理)　更新エラー！】"
...
Private Function ZHGInsert_Set() As Boolean
    ZHGInsert_Set = False
    ...
    With ZHG
        ...
        .材料管理番号 = RTrim(vsGrid1.TextMatrix(gLong, CisVsGrid.FixedGet(9)))
        .発注数 = CisFun.Val2(vsGrid1.TextMatrix(gLong, CisVsGrid.FixedGet(7)))
        .作成種別 = "N"
        .処理区分 = 0
    End With
    Call ZHGInsert
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 16, 2, "仕入先名")
        Call .FixedSet(中中, 中中, あり, 4, 3, "受入")
        Call .FixedSet(中中, 左中, あり, 25, 4, "材　質")
        Call .FixedSet(中中, 左中, あり, 24, 5, "寸　法")
        Call .FixedSet(中中, 中中, あり, 11, 6, "納 入 日")
        Call .FixedSet(中中, 右中, あり, 13, 7, "発注数")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE START
        'Call .FixedSet(中中, 中中, あり, 6, 8, "背番号")
        Call .FixedSet(中中, 中中, あり, 8, 8, "背番号")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, あり, 0, 9, "X材料管理番号")
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not ZNJ_RDSTS
        With vsGrid1
            ...
            '仕入先・名
            .TextMatrix(ID, GridCol(1)) = RTrim(ZNJ.手配先)
            .TextMatrix(ID, GridCol(2)) = CisFun.Left2(ZNJ.手配先名, 18)
            '受入
            .TextMatrix(ID, GridCol(3)) = RTrim(ZNJ.受入)
            '品番
            .TextMatrix(ID, GridCol(4)) = RTrim(ZNJ.材質)
            ' 寸法
            .TextMatrix(ID, GridCol(5)) = RTrim(ZNJ.表示寸法)
            ' 納入日
            If Trim(ZNJ.納期) <> "" Then
               With CisFun
                    .DateE_BefVal = ZNJ.納期
            ...
            End If
            ' 発注数
            .TextMatrix(ID, GridCol(7)) = Format(ZNJ.数量, "#,##0.000")
            ' 背番号
            .TextMatrix(ID, GridCol(8)) = RTrim(ZNJ.背番号)
            '材料管理番号
            .TextMatrix(ID, GridCol(9)) = RTrim(ZNJ.材料管理番号)
            '納期
            .TextMatrix(ID, GridCol(10)) = RTrim(ZNJ.納期)
