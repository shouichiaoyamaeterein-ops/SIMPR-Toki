...
Begin VB.Form COK0920 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D B_JYmMsg 
         ...
         End
      End
      Begin Cis3D_v60.CIS3D B2Msg_Sikyu 
         Height          =   885
         Left            =   930
         Top             =   930
         Width           =   3645
         ...
         cAlingnment     =   6
         cPositionY      =   30
         Begin Cis3D_v60.CIS3D B1lb_Sikyu 
            Height          =   360
            Left            =   1095
            Top             =   360
            Width           =   2415
            ...
            cPositionX      =   30
         End
         Begin CisText_V60.CisText iB1_Sikyu 
            Height          =   360
            Left            =   105
            TabIndex        =   14
            Top             =   360
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/12/09  By CIS SIMPR-A
...
Private Sub PB_Back_Click()
    ...
    gSL_Select = gSL_Select & " FROM 支給実績テーブル "
...
'
    ...
    If PB_Look.Tag = "iB1_Siire" Then Call Look_Siire(0): Exit Sub
    If PB_Look.Tag = "iB1_Sikyu" Then Call Look_Siire(1): Exit Sub
...
'+       出庫者検索            +
'+----------------------------+
Private Sub Look_Siire(LTorkb As Byte)
    ...
    If RV_Rtn Then
        ...
        Else
            iB1_Sikyu = RTrim(RV_TorcdK)
            Call iB1_Sikyu_LostFocus
        End If
    End If
End Sub
'+----------------------------+
...
Private Sub Look_Kubun(CKb As Integer)
    ...
    Select Case CKb
        ...
        Case 1: RVI_Kubun = "買掛識別子"
                Rv_Select = " AND 英数字1 = 'S' AND 数字1 = 1"
    End Select
...
Private Sub Look_Tanka()
    ...
    CKK0190.Show vbModal
    Unload CKK0190
    Set CKK0190 = Nothing

    If RV_Rtn Then
        iB1_Sikyu = RTrim(RV_Torcd)
        Call iB1_Sikyu_LostFocus
        iB1_Hinbn = RTrim(RV_Hinbn)
...
'+  iB1_Sikyu
'+-----------------------------+
Private Sub iB1_Sikyu_GotFocus()
    PB_Look.Tag = ActiveControl.Name
    PB_Look.Visible = True
End Sub
Private Sub iB1_Sikyu_LostFocus()
    If ActiveControl.Name <> "PB_Look" Then
        PB_Look.Tag = ""
        PB_Look.Visible = False
    End If
    B1lb_Sikyu = ""
    If TorNmGet(iB1_Sikyu, 8) Then
        B1lb_Sikyu = TRM.略称
    End If
End Sub
'+-----------------------------+
...
Private Sub iB1_Shiki_LostFocus()
    ...
    If MeisyoGet("買掛識別子", iB1_Shiki) Then
        If SYM.数字1 = 1 And SYM.英数字1 = "S" Then
...
'+  B1Chk_Umu
'+-----------------------------+
Private Sub B1Chk_Umu_Click()
    If B1Chk_Umu.Value = 0 Then
        B1Chk_Umu.Caption = "有償"
    Else
        B1Chk_Umu.Caption = "無償"
    End If
End Sub
'+-----------------------------+
'+  B1Chk_Aka
...
'      * PF-Key 表示設定 *
    ...
    Next gObj
    If SyoriKB = "ADD" Then
'        Call OpH2_INKbn_LostFocus(1)
'        OpH2_INKbn(0).Value = True
'        Call OpH2_INKbn_Click(0)
    Else
        If SyoriKB <> "" Then
...
' 支給年月
    If Trim(iH1_KYm) = "" Then
        CisFun.MB_MSG(2) = "    支給年月を入力して下さい            "
        CisFun.MBOX
       iH1_KYm.SetFocus
       Exit Function
    End If
    If Not iH1_KYm.cChkResult Then
        CisFun.MB_MSG(2) = "    正しい支給年月を入力して下さい            "
        CisFun.MBOX
       iH1_KYm.SetFocus
       Exit Function
    End If
...
'
    SaveKanriNo = 0
    gSL_Select = "SELECT COUNT(*) 件数 FROM 支給実績テーブル"
    ...
    gSL_Select = gSL_Select & "   AND 識別子 <> '31'"                       '赤訂正以外
    If SJSRead(gSL_Select, 1) Then
        If SJS.件数 = 0 Then
            CisFun.MB_MSG(2) = "    　対象データが存在しません            "
            CisFun.MBOX
        ...
        If SJS.件数 > 1 Then
            With CisFun
                ...
                .MB_Title = ""
                .MB_Button = OK
                .MB_MSG(2) = "    　この納入番号・品番の支給実績が複数存在します。            "
...
'    End If
    ...
    Else
        ...
        If Not MeisyoGet("買掛識別子", iB1_Shiki) Then
            ...
            Exit Function
        Else
            If SYM.数字1 <> 1 Or SYM.英数字1 <> "S" Then
...
'----------------------------------------------------------------------------< 2005.07.15 MNT START >
    ...
    End If
    ' 金額
    If B1Chk_Umu.Value = 0 And CisFun.Val2(iB1_Kingk) = 0 Then
...
Private Function Body2Chk() As Boolean
    ...
    End With
    ' 選択
    If CisFun.Val2(VsGrid1.TextMatrix(VsGrid1.Row, CisVsGrid1.FixedGet(17))) = 0 Then Exit Function
    
    If CisFun.Val2(VsGrid1.TextMatrix(VsGrid1.Row, CisVsGrid1.FixedGet(23))) <> 0 And _
        CisFun.Val2(VsGrid1.TextMatrix(VsGrid1.Row, CisVsGrid1.FixedGet(22))) = "31" And _
        SyoriKB = "MNT" Then
...
Private Function DBInput(InputKey As String) As Boolean
    ...
    gSL_Select = "SELECT KJ.*,ISNULL(TR1.略称,'')出庫者名,ISNULL(SY.値名称,'')訂正理由名, "
    gSL_Select = gSL_Select & " ISNULL(HN.表示品番,'')表示品番,ISNULL(TR2.略称,'')支給先名,ISNULL(SY2.値名称,'')識別子名"
    ...
    gSL_Select = gSL_Select & "   AND TR1.取引先CD = KJ.出庫者 "
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 取引先マスタ TR2 "
    gSL_Select = gSL_Select & "   ON TR2.取引先区分 = 1 "
    gSL_Select = gSL_Select & "   AND TR2.取引先CD = KJ.支給先 "
    ...
    If Not SJSRead(gSL_Select, 0) Then
...
Private Function DBInput2() As Boolean
    ...
    gSL_Select = "SELECT * FROM 支給実績テーブル"
...
' ( 削除処理 )
        .SQL = "DELETE FROM 支給実績テーブル "
        .SQL = .SQL & " WHERE 検収管理NO = " & CisFun.Val2(VsGrid1.TextMatrix(VsGrid1.Row, CisVsGrid1.FixedGet(17))) & ""
        .DBExec
...
'*-----------------------------------------------'
    If SyoriKB = "ADD" Then
        ...
        End With
    Else
        SaveKanriNo = CisFun.Val2(VsGrid1.TextMatrix(VsGrid1.Row, CisVsGrid1.FixedGet(17)))
    End If
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, あり, 4, 5, "有無")
        Call .FixedSet(中中, 右中, あり, 7, 6, "数量")
        Call .FixedSet(中中, 中中, あり, 4, 7, "仮区")
        Call .FixedSet(中中, 右中, あり, 10, 8, "単 価")
        Call .FixedSet(中中, 右中, あり, 11, 9, "金 額")
        Call .FixedSet(中中, 左中, あり, 10, 10, "訂正/識別")
        Call .FixedSet(中中, 左中, あり, 28, 11, "出 庫 者")
        Call .FixedSet(中中, 左中, あり, 28, 12, "支 給 先")
        '*-*-*-*-* 2008/04/11 Update Start
        'Call .FixedSet(中中, 中中, あり, 6, 13, "背番号")
        Call .FixedSet(中中, 中中, あり, 8, 13, "背番号")
        '*-*-*-*-* 2008/04/11 Update End
        Call .FixedSet(中中, 中中, あり, 4, 14, "受入")
        '*-*-*-*-* 2008/04/11 Update Start
        'Call .FixedSet(中中, 中中, あり, 4, 15, "車型")
        Call .FixedSet(中中, 中中, あり, 5, 15, "車型")
        '*-*-*-*-* 2008/04/11 Update End
        Call .FixedSet(中中, 中中, あり, 6, 16, "入力")
        
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 中中, あり, 0, 17, "検収管理NO")
        Call .FixedSet(中中, 中中, あり, 0, 18, "X検収日")
        Call .FixedSet(中中, 中中, あり, 0, 19, "X仕入先")
        Call .FixedSet(中中, 中中, あり, 0, 20, "X仮単価")
        Call .FixedSet(中中, 左中, あり, 0, 21, "X訂正理由")
        Call .FixedSet(中中, 左中, あり, 0, 22, "X識別子")
        Call .FixedSet(中中, 左中, あり, 0, 23, "X訂正管理NO")
        Call .FixedSet(中中, 左中, あり, 0, 24, "X品番")
        Call .FixedSet(中中, 左中, あり, 0, 25, "X支給先")
        Call .FixedSet(中中, 左中, あり, 0, 26, "Select")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not SJS_RDSTS
        With VsGrid1
            ...
            End If
            '納入番号
            .TextMatrix(ID, GridCol(2)) = RTrim(SJS.納入番号)
            '品番
            If RTrim(SJS.表示品番) <> "" Then
                .TextMatrix(ID, GridCol(3)) = RTrim(SJS.表示品番)
            Else
                .TextMatrix(ID, GridCol(3)) = RTrim(SJS.品番)
            End If
            '赤黒
            If SJS.赤黒区分 <> 0 Then
            ...
            If SJS.数量 <> 0 Then
                If SJS.数量 < 0 Then
                    .TextMatrix(ID, GridCol(6)) = Format(SJS.数量 * -1, "#,###")
                Else
                    .TextMatrix(ID, GridCol(6)) = Format(SJS.数量, "#,###")
                End If
            End If
            '仮区分
            If RTrim(SJS.仮単価区分) = "" Then
                .TextMatrix(ID, GridCol(7)) = "正式"
            Else
                .TextMatrix(ID, GridCol(7)) = "仮"
            End If
            ' 単価
            If SJS.単価 <> 0 Then
                .TextMatrix(ID, GridCol(8)) = Format(SJS.単価, "#,##0.00")
            End If
            ' 金額
            If SJS.金額 <> 0 Then
                If SJS.金額 < 0 Then
                    .TextMatrix(ID, GridCol(9)) = Format(SJS.金額 * -1, "#,###")
                Else
                    .TextMatrix(ID, GridCol(9)) = Format(SJS.金額, "#,###")
                End If
            End If
            If SJS.識別子 = "30" Or SJS.識別子 = "31" Then
                '訂正理由
                .TextMatrix(ID, GridCol(10)) = RTrim(SJS.訂正理由名)
            Else
                '識別子
                .TextMatrix(ID, GridCol(10)) = RTrim(SJS.識別子名)
            End If
            '出庫者・名
            If RTrim(SJS.出庫者) <> "" Then
                .TextMatrix(ID, GridCol(11)) = RTrim(SJS.出庫者) & ":" & RTrim(SJS.出庫者名)
            End If
            '支給先・名
            If RTrim(SJS.支給先) <> "" Then
                .TextMatrix(ID, GridCol(12)) = RTrim(SJS.支給先) & ":" & RTrim(SJS.支給先名)
            End If
            '背番号
            .TextMatrix(ID, GridCol(13)) = RTrim(SJS.背番号)
            '受入
            .TextMatrix(ID, GridCol(14)) = RTrim(SJS.受入)
            '車型
            .TextMatrix(ID, GridCol(15)) = RTrim(SJS.車型)
            '入力種別
            If SJS.訂正管理NO <> 0 Then
                .TextMatrix(ID, GridCol(16)) = "伝票"
            Else
                .TextMatrix(ID, GridCol(16)) = "入力"
            End If
            '検収管理NO
            .TextMatrix(ID, GridCol(17)) = SJS.検収管理NO
            '検収日
            .TextMatrix(ID, GridCol(18)) = RTrim(SJS.検収日)
            '出庫者
            .TextMatrix(ID, GridCol(19)) = RTrim(SJS.出庫者)
            '仮単価区分
            .TextMatrix(ID, GridCol(20)) = RTrim(SJS.仮単価区分)
            '訂正理由
            .TextMatrix(ID, GridCol(21)) = RTrim(SJS.訂正理由)
            '識別子
            .TextMatrix(ID, GridCol(22)) = RTrim(SJS.識別子)
            '訂正管理NO
            .TextMatrix(ID, GridCol(23)) = RTrim(SJS.訂正管理NO)
            '品番
            .TextMatrix(ID, GridCol(24)) = RTrim(SJS.品番)
            '支給先
            .TextMatrix(ID, GridCol(25)) = RTrim(SJS.支給先)
...
Private Sub GridToText()
    ...
    With VsGrid1
        If Trim(.TextMatrix(.Row, CisVsGrid1.FixedGet(17))) = "" Then Exit Sub
        Select Case RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(22)))
            Case "30", "31": OpH2_INKbn(1).Value = True: Call OpH2_INKbn_GotFocus(1)
            Case Else: OpH2_INKbn(2).Value = True: Call OpH2_INKbn_GotFocus(2)
        End Select
        
        iB1_KYmd = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(18)))
        iB1_NouNo = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(2)))
        iB1_Hinbn = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(24)))
        iB1_Seban = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(13)))
        iB1_Syakei = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(15)))
        iB1_Siire = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(19)))
        Call iB1_Siire_LostFocus
        iB1_Ukeir = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(14)))
        iB1_Sikyu = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(25)))
        Call iB1_Sikyu_LostFocus
        iB1_TKbn = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(21)))
        Call iB1_TKbn_LostFocus
        iB1_Shiki = RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(22)))
        Call iB1_Shiki_LostFocus
        If RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(5))) = "有償" Then
            B1Chk_Umu.Value = 0
        Else
            B1Chk_Umu.Value = 1
        End If
        ...
        Else
            B1Chk_Aka.Value = 1
        End If
        iB1_Suryo = CisFun.Val2(.TextMatrix(.Row, CisVsGrid1.FixedGet(6)))
        
        If RTrim(.TextMatrix(.Row, CisVsGrid1.FixedGet(20))) = "" Then
            B1Chk_Kari.Value = 0
        Else
            B1Chk_Kari.Value = 1
        End If
        iB1_Tanka = CisFun.RSetFld(.TextMatrix(.Row, CisVsGrid1.FixedGet(8)), 12, "#,##0.00")
        iB1_Kingk = CisFun.Val2(.TextMatrix(.Row, CisVsGrid1.FixedGet(9)))
    End With
...
Private Sub vsGrid1_DblClick()
    ...
    With VsGrid1
        If ProcHB = "B2" Then
            If Trim(.TextMatrix(.Row, CisVsGrid1.FixedGet(17))) <> "" Then
               Call ReturnPress
            End If
        End If
    End With
...
Private Sub vsGrid1_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeySpace And Trim(VsGrid1.TextMatrix(VsGrid1.Row, CisVsGrid1.FixedGet(17))) <> "" Then
       Call ReturnPress
    End If
End Sub
'+---------------------------------------+
