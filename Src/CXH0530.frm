...
'**       処理概要      :
'**
'**       作  成  日    :   2008.07.01  By CIS SIMPR-A
'**                             (CXH0500から作成)
'**                           * 実績区分=3の判定ifを削除
'**                             実績区分=1,2に関する処理を削除
'**       変　更　日　　:   2008.08.22  By CIS - 帳票出力関係の処理を追加
'**
'**     ---------------------------------------------------
...
'   #    初期入力設定   #
'   #------------------#
    SyoriKB = "ADD"         ' 2008.07.03 add : DispChangeで必要なので追加
    ProcHB = "H2"           ' 2008.07.03 upd : 初期モードは新規入力 'ProcHB = "H1"
...
'   #-------------------#
    ...
    '*------------------------- 2008.07.01 del start : 実績区分の表示がないので削除
'   背景色の設定（出力区分）
    ...
    '*------------------------- 2008.07.01 add start : 実績区分の表示・選択（H1Op_JKbn）がないので
    '<><><> 支給 <><><>                                                     ' 実績区分=支給で固定
    wTorKB = 8                      '【発注】手配先/契約先検索
    Back_Ymd.Caption = "納 入 日"
    Back_Torcd1.Caption = "支 給 先"
    Back_Suryo.Caption = "実績数"   ' "支給数"
    Back_Suryo.Visible = False      ' 2008.07.03 add
...
Private Sub PB_END_Click()
    Call Prt_Nou            ' 納品書発行 2008.08.20 add
'   #### ﾃﾞｰﾀﾍﾞｰｽ切断 ####
...
Private Sub PB_ADD_Click()
    If SyoriKB <> "ADD" Then
        ...
        Call SyoriName(SyoriKB)
'        H1Op_JKbn(2).SetFocus      ' 2008.07.01 del
        ProcHB = "H2"               ' 2008.07.03 add
        Call DispChange(ProcHB)     ' 2008.07.03 add
        Dummy.Enabled = False
    End If
End Sub
'+----------------------------+
...
Private Sub PB_MNT_Click()
    If SyoriKB <> "MNT" Then
        Dummy.Enabled = True: Dummy.SetFocus: DoEvents
        SyoriKB = "MNT"
        'Call HeadBodyClear("B")     ' 2008.07.03 add
        'Call HeadBodyClear("H")     ' 2008.07.03 add
        Call SyoriName(SyoriKB)
        ProcHB = "H1"               ' 2008.07.03 add
        Call DispChange(ProcHB)     ' 2008.07.03 add
        iH1_NouNo.SetFocus
        Dummy.Enabled = False
    End If
...
'    If H1Op_JKbn(3) Then               ' 2008.07.02 del : （全体）実績区分=3固定、他の実績区分の処理を削除
        RV_Call = "C"
        RV_TorKb = wTorKB
    '
...
'        If TextName = "iH2_Torcd1" Then
            ...
            Call iH2_Torcd1_GotFocus    ' 2008.07.01 add 注：このFocusは
            iH2_Torcd1.SetFocus         ' 2008.07.01 add         この方法でよいか？
'        Else
...
'        RVI_SK = 1
'    End If
''    If RTrim(OpH1_HKbn(1).Tag) = "1" Then ' 2008.07.03 del
''        RVI_SK = 0                        ' 2008.07.03 del
''    End If                                ' 2008.07.03 del
...
End Sub
''+----------------------------+            ' 2008.07.03 del : 従来の品番検索　差し替えのため削除
''+       品番検索            +
''+----------------------------+
'Private Sub Look_Hinbn()
'    RV_Left = 0
'    RV_Top = 0
'    RV_Call = ""
''    If H1Op_JKbn(1) Then RVI_Hinmoku = 9   ' 2008.07.01 del
''    If H1Op_JKbn(2) Then RVI_Hinmoku = 9   ' 2008.07.01 del
''    If H1Op_JKbn(3) Then RVI_Hinmoku = 1   ' 2008.07.01 del
'    RVI_Hinmoku = 1
'
...
'    PB_END.Visible = H1Mode
    ...
    PB_Look.Visible = False
    PB_ADD.Visible = H1Mode 'False: If H1Mode Or (H2Mode And SyoriKB <> "ADD") Then PB_ADD.Visible = True
...
'    End If
    ...
    With vsGrid
        .Redraw = False
        For gInt = 1 To .Rows - 1   ' 2007.07.01 行数変更 (10→3 : グリッドの行数 - 1 とする）
            'No
...
'
        ...
        '*------------------------------------------------ 2008.07.03 add start
        '   << 品番取引先マスタ存在チェック >>
            HTM.品番 = RTrim(.TextMatrix(wRowNo1, 1))
        ...
        '   << 品番取引先マスタ　手配区分チェック >>
            If HTM.手配区分 <> "4" Then
                CisFun.MB_MSG(2) = "    【" & wRowNo1 & "行目】　品番の手配区分が支給ではありません  （品番取引先マスタ）            "
                CisFun.MBOX
                Set ErrObj = iB1_Hinbn
                GoTo Body1Chk_Err
            End If
...
'            Select Case .実績区分                      ' 実績区分の選択がないので削除
...
'            iH2_Torcd2 = RTrim(.実績元)                ' 実績元がないので削除
            iH2_Ukeir = RTrim(.受入)
            If Get_Torcd(.実績先, 0) Then H2lb_Tornm1 = RTrim(TRM.略称)
'            If Get_Torcd(.実績元, 0) Then H2lb_Tornm2 = RTrim(TRM.略称)    ' 実績元がないので削除
        End With
        DBInput = True
    End If
...
'---<< 2004.12.30 追加更新時の削除はなし  >>---------------------------Start------------
    If SyoriKB = "MNT" Then
        Call DBDelete_JKT
        Call DBDelete_CHT(0)        ' 2008.08.22 add
    End If
...
' ( 追加処理 )
    ...
    For gIndex = 1 To vsGrid.Rows - 1
        If RTrim(vsGrid.TextMatrix(gIndex, 1)) <> "" Then
            If DBInsert_JKT(gIndex) Then    ' Call DBInsert_JKT(gIndex)     2008.08.22 upd
                If SyoriKB = "ADD" And _
                  Clear_CHT = 0 Then        ' 自端末起動後最初の登録ならば  2008.08.22 add
                    Call DBDelete_CHT(1)    ' 帳票出力テーブルの            2008.08.22 add
                    Clear_CHT = 1           ' 該当データを削除する          2008.08.22 add
                End If                      '                               2008.08.22 add
                Call DBInsert_CHT(gIndex)   ' 帳票出力テーブル登録          2008.08.22 add
            End If                          '                               2008.08.22 add
        End If
    Next gIndex
'   件数表示
...
Private Function DBSet_JKT()
    ...
    Call ItemsClearJKT
    With JKT
'        If H1Op_JKbn(1) Then .実績区分 = 1 ' 2008.07.02
'        If H1Op_JKbn(2) Then .実績区分 = 2 ' 2008.07.02
'        If H1Op_JKbn(3) Then .実績区分 = 3 ' 2008.07.02
        .実績区分 = 3                       ' 2008.07.02
        .実績先 = RTrim(iH2_Torcd1)
        .受入 = RTrim(iH2_Ukeir)
'        .実績元 = RTrim(iH2_Torcd2)        ' 2008.07.02
        .実績日 = RTrim(iH2_Ymd)
        .実績便 = RTrim(iH2_Bin)
        .作成者 = gTanto
    End With
...
Private Function DBInsert_CHT(ID As Long) As Boolean
    ...
    gSL_Select = gSL_Select & " Where 品番   = '" & JKT.品番 & "' "
    gSL_Select = gSL_Select & "   And 取引先 = '" & JKT.実績先 & "' "
    gSL_Select = gSL_Select & "   And 受入   = '" & JKT.受入 & "' "
    If Not HTMRead(gSL_Select, 1, 1) Then GoTo DBInsert_CHT_Err
    
    If HTM.納品書発行区分 = 0 Then
       Call ItemsClearCHT
       With CHT
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        ...
        Call .FixedSet(中中, 右中, あり, 9, 6, "実績数")    ' 2008.07.03 update : Call .FixedSet(中中, 右中, あり, 9, 6, "実績数")
    ...
    If KeyCode = vbKeyDown Then
        If gInt + 1 > vsGrid.Rows - 1 Then
            If RTrim(vsGrid.TextMatrix(gInt, 1)) <> "" Then
                If vsGrid.Row < vsGrid.Rows - 1 Then            ' 2008.07.03 add : 行Maxを超えないようにする
                    vsGrid.Rows = gInt + 1 + 1
                    vsGrid.TextMatrix(gInt + 1, 0) = gInt + 1
                Else                                            ' 2008.07.03 add : 行Maxを超えないようにする
                    GoTo Grid_RowChange_End                     ' 2008.07.03 add : 行Maxを超えないようにする
                End If
            Else
...
Private Sub TextToVsGrid(RowNo As Integer)
    ...
    With vsGrid
        ...
        If RTrim(iB1_Hinbn) <> "" Then
            .TextMatrix(RowNo, 7) = B1Chk_UMkbn.Caption ' 2008.07.02 update : If H1Op_JKbn(3) Then .TextMatrix(RowNo, 7) = B1Chk_UMkbn.Caption
        End If
...
Private Sub VSGrid_DblClick()
    ...
    iB1_Suryo = ""
    B1lb_Hasu = ""
    B1Chk_UMkbn.Value = 1   ' 2008.07.02 rep 有償 : B1Chk_UMkbn.Value = 0
    Call TextToVsGrid(Val(Input_Area.Tag))
End Sub
