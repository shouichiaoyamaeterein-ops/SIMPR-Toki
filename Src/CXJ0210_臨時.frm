...
Private Function Head3Chk() As Boolean
    ...
    With CisFun
        .Where_CreateKB = NewDocument
         ...
         .Where_Create Numeric, "出荷実績区分", 1, 等しい, Create

         ' Toppsﾃﾞｰﾀ非対象
         .Where_Create AlphaMe, "jyt.作成種別", "T", 等しくない                 '2009/05/18
            
         '
         .Where_Create AlphaMe, "ISNULL(JI.出荷指示書NO,0)", 0, 等しい
...
'    mSL_Select = mSL_Select & ", ISNULL(TR2.略称,'') 出荷管理部署名"
    ...
    mSL_Select = mSL_Select & "  Left Outer Join 実績管理テーブル JI"
    mSL_Select = mSL_Select & "    ON  JI.実績日 > '" & CisFun.Mid2(iH1_SYmd, 1, 6) & "'"
    mSL_Select = mSL_Select & "    And JI.出荷指示書NO = jyt.出荷指示書NO"
    mSL_Select = mSL_Select & "    And JI.出荷指示書行NO = jyt.出荷指示書行NO"
