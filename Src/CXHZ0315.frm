   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |伝種|納 入 日 　-便|仕入先 |   仕  入  先  名   |受入|納 番 |頁|発注区|手配区|SK|材　質  　　            |"
            Rows            =   21
            Cols            =   17
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=25 Else 1=125
            Call CXHZ0310LPrint(wPrintKB)
...
'   ### 再発行既存 ###
    ...
    gSL_Select = gSL_Select & "    on zrm.材料管理番号 = zch.材料管理番号1"
    If H1Op_SKbn(1) Then
'   ### 再発行最新 ###
        gSL_Select = gSL_Select & " where cht.端末番号 = substring(host_name(),1,20)"
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 25"
        ...
        gSL_Select = gSL_Select & "   and zch.発行形態 = 1"
    Else
'   ### 再発行既存 ###
...
'   >> 納入日
    ...
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(zch.納入日, 1, 6) = '" & iH1_SYmd & "'"
    Else
        YmdSta = "": BinSta = ""
        YmdEnd = "": BinEnd = ""
        '> 日付＆便　編集
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) = "" Then
            If Trim(BinSta) = "" Then
                gSL_Select = gSL_Select & "   and zch.納入日 = '" & YmdSta & "'"
            Else
        ...
        If Trim(YmdSta) = "" And Trim(YmdEnd) <> "" Then
            If Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and zch.納入日 <= '" & YmdEnd & "'"
            Else
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) <> "" Then
            If Trim(BinSta) = "" And Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and zch.納入日 between '" & YmdSta & "'" _
                                                             & " and '" & YmdEnd & "'"
            Else
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(14))) = "X" Then
                ...
                CHT.検索情報 = RTrim(.TextMatrix(ID, CisVsGrid1.FixedGet(13)))
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 125"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
Private Sub ZchInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '伝種
        Select Case ZCH.カード
            Case "6": .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = "受領"
            Case "7": .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = "納品"
            Case "8": .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = "支給"
        End Select
        '納入日＆便
        If Trim(ZCH.納入日) <> "" Then
            If RTrim(ZCH.納入便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = Format(RTrim(ZCH.納入日), "0000/00/00")
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = Format(RTrim(ZCH.納入日), "0000/00/00") & "-" & RTrim(ZCH.納入便)
            End If
        End If
        '仕入先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = RTrim(ZCH.仕入先)
        '仕入先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(ZCH.仕入先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(ZCH.受入)
        '納番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(ZCH.納入番号)
        '頁
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = Format(ZCH.頁NO, "##")
        '発注
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = RTrim(ZCH.発注区分名)
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = RTrim(ZCH.手配区分名)
        'SK
        ...
        End Select
        '材質
        If CisFun.Mid2(ZCH.納入番号, 5, 1) <> "0" Then
            .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = RTrim(ZCH.材質)
        End If
        '納品書管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = ZCH.納品書管理NO
        'カード
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(13)) = ZCH.カード
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
