...
Begin VB.Form CKH0120 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "かんばん増減一括メンテナンス"
   ClientHeight    =   10980
   ClientLeft      =   720
   ClientTop       =   1440
   ClientWidth     =   15315
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |仕入先              |受|品番                          |背番号|収容数|99月内示数|99月内示数|設定期間    |設定|便当||"
            Rows            =   26
            Cols            =   21
            BackColor       =   12648384
...
'**       変  更  日    :  2005/03/08  By CIS - グリッドデータ無し行をクリック、Up/Downキー押下 → 行変更カラーに変色　対応
'**       変  更  日    :  2008/04/10  By CIS - 背番号８桁対応
'**
'**********************************************************'
...
Private Function Body1Chk() As Boolean
    ...
    With vsGrid1
        ...
        '   設定枚数
            If CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(10))) = 0 Then
                With CisFun
                    .MB_Lines = 4
...
'
    ...
    gSL_Select = gSL_Select & ", isnull(kkm.回転枚数,0) 回転枚数,isnull(krk.有効枚数,0) 有効枚数"
    ...
    gSL_Select = gSL_Select & " left outer join かんばん回転枚数テーブル kkm    on kkm.年月 = '" & iH1_Ym & "' and kkm.品番 = kzt.品番   and kkm.仕入先 = kzt.取引先   and kkm.受入 = kzt.受入"
    ...
    gSL_Select = gSL_Select & "     on krk.品番 = kzt.品番   and krk.取引先 = kzt.取引先   and krk.受入 = kzt.受入"
...
' ( 更新処理 )
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(0))) = "" Then Exit For
            If CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(10))) <> CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(14))) Or _
               CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(11))) <> CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(15))) Then
...
Public Function DBUpdate_KZT(SetRow As Long, Optional DBNo As Integer) As Boolean
    ...
    Call ItemsClearKZT
    With KZT
        .取引先 = RTrim(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(12)))
        .受入 = RTrim(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(2)))
        .品番 = RTrim(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(17)))
        .開始日 = RTrim(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(13)))
        .設定枚数 = CisFun.Val2(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(10)))
        .便当枚数 = CisFun.Val2(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(11)))
        .更新者 = gTanto
        .累積枚数 = CisFun.Val2(vsGrid1.TextMatrix(SetRow, CisVsGrid1.FixedGet(16)))
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/10 UPDATE START
        ...
        Call .FixedSet(中中, 左中, あり, 18, 1, "仕 入 先")
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/10 UPDATE END
        ...
        Call .FixedSet(中中, 右中, あり, 8, 7, "回転枚数")
        Call .FixedSet(中中, 右中, あり, 8, 8, "有効枚数")
        Call .FixedSet(中中, 中中, あり, 12, 9, "設定期間")
        Call .FixedSet(中中, 右中, あり, 4, 10, "設定")
        Call .FixedSet(中中, 右中, あり, 4, 11, "便当")
        Call .FixedSet(中中, 左中, あり, 0, 12, "X仕入先")
        Call .FixedSet(中中, 左中, あり, 0, 13, "X開始日")
        Call .FixedSet(中中, 右中, あり, 0, 14, "X設定枚数")
        Call .FixedSet(中中, 右中, あり, 0, 15, "X便当枚数")
        Call .FixedSet(中中, 右中, あり, 0, 16, "X累積枚数")
        Call .FixedSet(中中, 左中, あり, 0, 17, "X品番")
        Call .FixedSet(中中, 左中, なし, 0, 18, "NotDisp")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
'       Call .EditSet(ﾃｷｽﾄｵﾌﾞｼﾞｪｸﾄ,ｶﾗﾑNO,非表示ｶﾗﾑ,編集ﾌｫｰﾏｯﾄ,日付編集)
        Call .EditSet(iB1_SetSu, 10)
        Call .EditSet(iB1_BinSu, 11, 18)
        .InitDisp
    End With
End Sub
'***********************************************'
...
Private Sub KztInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '回転枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = Format(KZT.回転枚数, "#,###")
        '有効枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = Format(KZT.有効枚数, "#,###")
        '設定機関
        If Trim(KZT.終了日) = "" Then
            .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = CisFun.Mid2(Format(KZT.開始日, "0000/00/00"), 6, 5) & Space(7)
        Else
            .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = CisFun.Mid2(Format(KZT.開始日, "0000/00/00"), 6, 5) _
                                                 & "～" & CisFun.Mid2(Format(KZT.終了日, "0000/00/00"), 6, 5)
        End If
        '設定枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = Format(KZT.設定枚数, "###")
        '便当枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = Format(KZT.便当枚数, "###")
        '退避
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = KZT.取引先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(13)) = KZT.開始日
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(14)) = KZT.設定枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(15)) = KZT.便当枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(16)) = KZT.累積枚数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(17)) = KZT.品番
        '表示制御
...
Private Sub ChgColorGrid1(SetRow As Long)
    With vsGrid1
        ...
        NewRow = .Row
        .Row = SetRow
        If CisFun.Val2(iB1_SetSu) <> CisFun.Val2(.TextMatrix(SetRow, CisVsGrid1.FixedGet(14))) Or _
           CisFun.Val2(iB1_BinSu) <> CisFun.Val2(.TextMatrix(SetRow, CisVsGrid1.FixedGet(15))) Then
            For gInt = 1 To 11
                .Col = gInt
