...
Begin VB.Form CCT0410 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CCT0410.frx":0000
         Height          =   8580
         Left            =   150
         OleObjectBlob   =   "CCT0410.frx":0015
         TabIndex        =   6
         Top             =   90
         Width           =   14460
      End
...
'**       処理概要      :
'**
'**       作  成  日    :   2003/12/15  By CIS
'**       変  更  日    :   2005/03/03  By CIS - 重複起動ﾁｪｯｸで起動済の時、『実行時エラー'91'』を表示する対応（Timerﾌﾟﾛﾊﾟﾃｨの Interval=0 に修正）
...
' ADOｺﾝﾄﾛｰﾙ　接続情報　セット
    ...
    gSL_Select = "SELECT Max(開始日時) 開始日時 From TEXT変換履歴テーブル "
    If THRRead(gSL_Select, 1) Then
        If Format(THR.開始日時, "yyyymmdd") > "20030101" Then
           iH1_SDate = Format(THR.開始日時, "yyyymmdd")
        End If
    End If
    If iH1_SDate = "" Then
...
'       日付
        With CisFun
            ...
            Else
               ...
               End If
            End If
            .Where_Create AlphaMe, "TH.FILECD", iH1_Gyomu
            mSL_SelectW = mSL_SelectW & .Where_Phrase
        End With
        If Not H1Op_DspKB(0) Then
            If H1Op_DspKB(1) Then
                If mSL_SelectW = "" Then
                    mSL_SelectW = " WHERE TH.処理コード <> 0 "
                Else
                    mSL_SelectW = mSL_SelectW & " AND TH.処理コード <> 0 "
                End If
            Else
                If mSL_SelectW = "" Then
                    mSL_SelectW = " WHERE TH.処理コード = 0 "
...
'
    ...
    gSL_Select = " SELECT TH.*,TK.F名称 "
    gSL_Select = gSL_Select & " FROM TEXT変換履歴テーブル TH "
    gSL_Select = gSL_Select & " LEFT OUTER JOIN TEXT管理テーブル TK "
    gSL_Select = gSL_Select & "   ON TK.FILECD = TH.FILECD "
...
Private Sub ErrDisp()
    ...
    gStr = Trim(Adodc1.Recordset("受信ｴﾗｰ"))
    If gStr = "" Then Exit Sub
    If InStr(1, gStr, "Err") <> 0 Then Exit Sub
    Call ItemsClearTER

    wErrCode = gStr
    TER.ERRCODE = wErrCode
    gSL_Select = "Select * From TEXTERRMSG"
    gSL_Select = gSL_Select & " where ERRCODE = '" & TER.ERRCODE & "'"
    If Not TERRead(gSL_Select, 1) Then
       With CisFun
            .MB_Lines = 3
...
'
    ...
    With CisFun
         .MB_Lines = 6
         .MB_MSG(1) = "   【 " & Trim(Adodc1.Recordset("F名称")) & " 】   "
