   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日　-便|仕入先|仕 入 先 名 |納 番 |頁|手配|背番号|品　番　　               |収容数 |箱数 |発注数 |端数  |"
            Rows            =   21
            Cols            =   16
            BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/02/05  By CIS
'**       変  更  日    :   2004/05/28  By CIS 背番号カナ入力
'**       変  更  日    :   2007/09/20  By CIS TEC B-848 印刷モジュール追加
'**                                           （切替）gIniName [Option] - 端数ラベル = 0:ﾚｰｻﾞｰ、1:TEC
...
'   出力区分の背景色
    ...
    Dim mPrintType              As String       ' 出力先（ﾚｰｻﾞｰ・TEC）
'+----------------------------------------------------------+
'+      フォーム    《    Ｌｏａｄイベント    》              +
'+----------------------------------------------------------+
...
Private Sub Form_Activate()
    ...
    Call GetCryOption("G", "Option", "端数ラベル", mPrintType)
    If RTrim(mPrintType) = "" Then
        mPrintType = "0"
        Call GetCryOption("P", "Option", "端数ラベル", mPrintType)
    End If
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=11      Else 1=111
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            If mPrintType = "0" Then
                Call CXH0360LPrint(wPrintKB, wPrintOut)
            Else
                Call CXH0361LPrint(wPrintKB, wPrintOut)
            End If
...
'   ### 再発行既存 ###
    ...
    gSL_Select = gSL_Select & "    on hnm.品番 = hct.品番"
    gSL_Select = gSL_Select & "  left outer join 名称マスタ sym"
    gSL_Select = gSL_Select & "    on sym.区分名称 = '手配区分'"
    gSL_Select = gSL_Select & "   and sym.区分タイプ = ''"
    gSL_Select = gSL_Select & "   and sym.値 = hct.手配区分"
    If H1Op_SKbn(1) Then
'   ### 再発行最新 ###
        gSL_Select = gSL_Select & " where cht.端末番号 = substring(host_name(),1,20)"
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 11"
        gSL_Select = gSL_Select & "   and hct.発注管理NO = cht.管理NO"
        gSL_Select = gSL_Select & "   and hct.端数区分 = 1"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where hct.端数区分 = 1"
    End If
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
...
'   >> 品番
    If RTrim(iH1_Hinbn) <> "" Then
        ...
        Else
            gSL_Select = gSL_Select & "   and hct.品番 like '" & RTrim(Replace(iH1_Hinbn, "*", "%")) & "'"
        End If
    End If
'   >> 背番号
    If RTrim(iH1_Seban) <> "" Then
        If InStr(1, iH1_Seban, "*") = 0 Then
            gSL_Select = gSL_Select & "   and hct.背番号 = '" & RTrim(iH1_Seban) & "'"
        Else
            gSL_Select = gSL_Select & "   and hct.背番号 like '" & RTrim(Replace(iH1_Seban, "*", "%")) & "'"
        End If
    End If
'   >> 納入日
    ...
    gSL_Select = gSL_Select & ", hct.納入番号"
    gSL_Select = gSL_Select & ", hct.納品書頁"
    gSL_Select = gSL_Select & ", hct.納品書行"
'
End Sub
'**************************************
'*                                    *
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 111"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 13, 1, "納 入 日  -便")
        Call .FixedSet(中中, 左中, あり, 6, 2, "仕入先")
        Call .FixedSet(中中, 左中, あり, 10, 3, "仕入先名")
        Call .FixedSet(中中, 左中, あり, 6, 4, "納 番")
        Call .FixedSet(中中, 右中, あり, 2, 5, "頁")
        Call .FixedSet(中中, 左中, あり, 4, 6, "手配")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE START
        'Call .FixedSet(中中, 左中, あり, 6, 7, "背番号")
        Call .FixedSet(中中, 左中, あり, 8, 7, "背番号")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        Call .FixedSet(中中, 左中, あり, 25, 8, "品　　　番")
        Call .FixedSet(中中, 右中, あり, 7, 9, "収容数")
        Call .FixedSet(中中, 右中, あり, 5, 10, "箱数")
        Call .FixedSet(中中, 右中, あり, 7, 11, "発注数")
        Call .FixedSet(中中, 右中, あり, 6, 12, "端数")
        Call .FixedSet(中中, 左中, あり, 0, 13, "発注管理NO")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
Private Sub HctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '納番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(HCT.納入番号)
        '頁
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = Format(HCT.納品書頁, "#,###")
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(HCT.手配区分名)
        '背番号
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(HCT.背番号)
        '品番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = RTrim(HCT.表示品番)
        '収容数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = Format(HCT.収容数, "#,###")
        '箱数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = Format(HCT.枚数, "#,###")
        '発注数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = Format(HCT.発注数, "#,###")
        '端数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = Format(HCT.端数, "#,###")
        '発注管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(13)) = HCT.発注管理NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
