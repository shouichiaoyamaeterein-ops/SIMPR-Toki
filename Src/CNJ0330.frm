...
'< 前回テーブル確認 >
    ...
    gSL_Select = gSL_Select & " From 発注内示前回テーブル"
    ...
    gSL_Select = gSL_Select & " End"
    If HNJRead(gSL_Select, 1) Then gInt = HNJ.件数
...
Private Function Head1Chk() As Boolean
    ...
    lb_msg.Caption = "【 生産予定データ 抽出中 】"
    DoEvents

'[ 印刷データ読込 ]

'   ( ３ヶ月　月量データ ) < 納期:｢00000000｣/当月数量、 ｢00000001｣/翌月数量、 ｢00000002｣/翌々月数量 >
    gSL_Select = "SELECT  HJ.品番, HJ.手配先, HJ.受入, HJ.直送先, '0000000' +" & vbCrLf
    ...
    gSL_Select = gSL_Select & " ISNULL( MAX( HN.品名 ),     '' ) 品名," & vbCrLf
    gSL_Select = gSL_Select & " ISNULL( MAX( HN.品番編集 ), '' ) 品番編集," & vbCrLf
    ...
    If opOutKB = 1 Then
        ...
        gSL_Select = gSL_Select & "                     FROM 発注内示前回テーブル HZ" & vbCrLf
        gSL_Select = gSL_Select & "                     FULL OUTER JOIN 発注内示テーブル HN" & vbCrLf
        gSL_Select = gSL_Select & "                     ON    HN.内示年月 = HZ.内示年月" & vbCrLf
        gSL_Select = gSL_Select & "                       AND HN.計画年月 = HZ.計画年月" & vbCrLf
        gSL_Select = gSL_Select & "                       AND HN.品番 = HZ.品番" & vbCrLf
        ...
        gSL_Select = gSL_Select & "                     WHERE (HN.品番 Is Null Or HZ.品番 Is Null)" & vbCrLf
        gSL_Select = gSL_Select & "                       AND ISNULL( HZ.内示年月, '' )  = CASE WHEN HZ.品番 IS NULL THEN '' ELSE '" & RTrim$(iH1_YM) & "' END" & vbCrLf
        gSL_Select = gSL_Select & "                       AND ISNULL( HZ.計画年月, '' ) >= CASE WHEN HZ.品番 IS NULL THEN '' ELSE '" & RTrim$(iH1_YM) & "' END" & vbCrLf
        gSL_Select = gSL_Select & "                       AND ISNULL( HN.内示年月, '' )  = CASE WHEN HN.品番 IS NULL THEN '' ELSE '" & RTrim$(iH1_YM) & "' END" & vbCrLf
        gSL_Select = gSL_Select & "                       AND ISNULL( HN.計画年月, '' ) >= CASE WHEN HN.品番 IS NULL THEN '' ELSE '" & RTrim$(iH1_YM) & "' END" & vbCrLf
        gSL_Select = gSL_Select & "            ) BD" & vbCrLf
        gSL_Select = gSL_Select & " ON  BD.内示年月 = HJ.内示年月" & vbCrLf
        gSL_Select = gSL_Select & " And BD.品番 = HJ.品番" & vbCrLf
    ...
    gSL_Select = gSL_Select & " ON  HN.品番 = HJ.品番" & vbCrLf
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 品番取引先マスタ HT" & vbCrLf
    gSL_Select = gSL_Select & " ON  HT.品番 = HJ.品番" & vbCrLf
    gSL_Select = gSL_Select & " AND HT.取引先 = HJ.手配先" & vbCrLf
    gSL_Select = gSL_Select & " AND HT.受入 = HJ.受入" & vbCrLf
    gSL_Select = gSL_Select & " WHERE HJ.内示年月 = '" & RTrim$(iH1_YM) & "'" & vbCrLf
    gSL_Select = gSL_Select & " AND   HJ.計画年月 >= HJ.内示年月" & vbCrLf
    
    If RTrim$(iH1_Torcd) <> "" Then
        gSL_Select = gSL_Select & " AND   TR.取引先CD = '" & RTrim$(iH1_Torcd) & "'" & vbCrLf
    End If
    
    gSL_Select = gSL_Select & " AND   取引先区分 = 1 AND 契約先区分 = 0 AND 社内区分 = 1" & vbCrLf
    gSL_Select = gSL_Select & " GROUP BY HJ.品番, HJ.手配先, HJ.受入, HJ.直送先, HJ.計画年月, HJ.内示年月" & vbCrLf
    
'   ( 日程データ )
    gSL_Select = gSL_Select & " UNION" & vbCrLf
    
    gSL_Select = gSL_Select & " SELECT  HJ.品番, HJ.手配先, HJ.受入, HJ.直送先, HJ.納期, HJ.数量," & vbCrLf
    gSL_Select = gSL_Select & " '' 手配先名, '' 品名, '' 品番編集, '' 背番号, '' サイクル, 0 収容数" & vbCrLf
    gSL_Select = gSL_Select & " FROM 発注内示テーブル HJ" & vbCrLf
...
'   ( 読み込み順 )
    gSL_Select = gSL_Select & " ORDER BY HJ.手配先, HJ.品番, HJ.受入, 納期" & vbCrLf
...
Private Function PrintStart() As Boolean
    PrintStart = False
    If Op_Out(0).Value Or Op_Out(1).Value Then
        lb_msg.Caption = "【 生産予定データ 印刷中 】"
        DoEvents
        With CisPrt
             Set .iPrtForm = CNJ0330L
             .dpMSG = "生産予定表　印刷中"
