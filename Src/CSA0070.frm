...
'**       変  更  日    :   2019/10/15  By CIS  上記処理の不具合対応 及び 背番号と得意先の表示方法を変更
'**       変  更  日    :   2019/10/28  By CIS  在庫管理区分=4(日数)の際の在庫数をチェックして適正数以下ならば背景色を変える
'**       変  更  日    :
'**
'**********************************************************'
...
'                                                    6:取引先,品番単位[受注][構成/指定品番以下] 7:取引先,品番単位[発注][構成/指定品番以下]
    ...
    Dim wDayDetail(2, 5)    As Currency ' (i, J)  i=年月、j=内容    ' 2019/10/28 ADD
'                                                   年月　0:前月, 1:当月, 2:翌月
'                                                   内容　0:年月,     1:数量（１ヶ月の合計）, 2:１日当たりの数量（小数以下切捨）
'                                                         3:稼動日数, 4:在庫日数（補正値）  , 5:在庫日数（実値）
    Dim wDayDtlYMS          As String   ' 前月年月                  ' 2019/10/28 ADD
    Dim wDayDtlYME          As String   ' 翌月年月                  ' 2019/10/28 ADD
...
'   # 初 期 内 容 取 得 #
'   #------------------#
    gConnectCount = 6                       ' 2019/10/28 memo: 0,1,2のみ使用なので、在庫管理区分=4(日数)データ取得時に4を使用します
...
' 2019/10/29 DEL：記述位置を変更    ↓↓↓↓
'            '===========================================================================================' 2019/10/28 ADD START
'        '   >> 年月のｾｯﾄ
'            With CisFun
'                .Date_RsFormat = fYM
'                .Date_Add iH1_YM.Text, M, -1
'                wDayDtlYMS = Mid(.Date_Result, 1, 6)    ' 前月
'                .Date_Add iH1_YM.Text, M, 1
'                wDayDtlYME = Mid(.Date_Result, 1, 6)    ' 翌月
'            End With
'        '   >> 日別情報のクリア
'            Dim iCtI, iCtJ As Integer: iCtI = 0: iCtJ = 0
'            For iCtI = 0 To 2
'                For iCtJ = 0 To 5
'                    wDayDetail(iCtI, iCtJ) = 0
'                Next iCtJ
'            Next iCtI
'
'            gSL_Select = ""
'            gSL_Select = gSL_Select & vbCrLf & "SELECT NJ.品番      区分タイプ,"
'            gSL_Select = gSL_Select & vbCrLf & "       NJ.手配先    区分名称,"
'            gSL_Select = gSL_Select & vbCrLf & "       NJ.内示年月  値,"
'            gSL_Select = gSL_Select & vbCrLf & "       SUM(NJ.数量) 件数,"
'            gSL_Select = gSL_Select & vbCrLf & "       CASE WHEN ISNULL(稼動日数, 0) = 0 "
'            gSL_Select = gSL_Select & vbCrLf & "            THEN 1"
'            gSL_Select = gSL_Select & vbCrLf & "            ELSE FLOOR(SUM(NJ.数量) / 稼動日数)"
'            gSL_Select = gSL_Select & vbCrLf & "       END  区分桁数,"
'            gSL_Select = gSL_Select & vbCrLf & "       ISNULL(稼動日数, 0) 数字1,"
'            gSL_Select = gSL_Select & vbCrLf & "       CASE WHEN ISNULL(HM.在庫日数, 0) = 0"
'            gSL_Select = gSL_Select & vbCrLf & "            THEN 1"
'            gSL_Select = gSL_Select & vbCrLf & "            ELSE HM.在庫日数"
'            gSL_Select = gSL_Select & vbCrLf & "       END  数字2,"
'            gSL_Select = gSL_Select & vbCrLf & "       ISNULL(HM.在庫日数, 0) 数字3"
'            gSL_Select = gSL_Select & vbCrLf & "  FROM 発注内示テーブル NJ"
'            gSL_Select = gSL_Select & vbCrLf & "  LEFT OUTER JOIN カレンダマスタ CA"
'            gSL_Select = gSL_Select & vbCrLf & "    ON CA.年+CA.月 = NJ.内示年月"
'            gSL_Select = gSL_Select & vbCrLf & "   AND CA.取引先 = ''"
'            gSL_Select = gSL_Select & vbCrLf & "   AND CA.取引先区分 = 1"
'            gSL_Select = gSL_Select & vbCrLf & "  LEFT OUTER JOIN 品番マスタ HM"
'            gSL_Select = gSL_Select & vbCrLf & "    ON HM.品番         = NJ.品番"
'            gSL_Select = gSL_Select & vbCrLf & "   AND HM.在庫管理区分 = 4"
'            gSL_Select = gSL_Select & vbCrLf & " WHERE NJ.内示年月 BETWEEN '" & wDayDtlYMS + "' AND '" + wDayDtlYME & "'"
'            gSL_Select = gSL_Select & vbCrLf & "   AND SUBSTRING(納期, 1, 6) = NJ.内示年月"
'            gSL_Select = gSL_Select & vbCrLf & "   AND NJ.手配先   = '" + NSG.仕入先 + "'"
'            gSL_Select = gSL_Select & vbCrLf & "   AND NJ.品番     = '" + NSG.品番 + "'"
'            gSL_Select = gSL_Select & vbCrLf & " GROUP BY NJ.品番, NJ.内示年月, NJ.手配先"
'            gSL_Select = gSL_Select & vbCrLf & "         ,CA.稼動日数, HM.在庫日数"
'            gSL_Select = gSL_Select & vbCrLf & " ORDER BY 内示年月"
'            Call Str_Monitor(gSL_Select)
'            If SYMRead(gSL_Select, , 4) Then        ' 一時的に名称マスタのモジュールを使用して月別情報を取得する
'                Do While SYM_RDSTS
'                    Select Case SYM.値
'                        Case wDayDtlYMS
'                            iCtI = 0
...
'                            iCtI = 1
'                    End Select
'                    wDayDetail(iCtI, 0) = CisFun.Val2(SYM.値)       ' 年月
'                    wDayDetail(iCtI, 1) = SYM.件数                  ' 数量（１ヶ月の合計）
'                    wDayDetail(iCtI, 2) = SYM.区分桁数              ' １日当たりの数量（小数以下切捨）
'                    wDayDetail(iCtI, 3) = SYM.数字1                 ' 稼動日数
'                    wDayDetail(iCtI, 4) = SYM.数字2                 ' 在庫日数（補正値）
'                    wDayDetail(iCtI, 5) = SYM.数字3                 ' 在庫日数（実値）
'                    Call SYMReadNext(4)
'                Loop
'            End If
'            '===========================================================================================' 2019/10/28 ADD END
...
'                .TextMatrix(wIndex + 3, 2) = "入庫"
                ...
                If NSG.部品材料区分 = "B" Then
'                    If wSyoriDspKB >= 2 Then                       ' 2019/10/15 DEL
...
'                              & CisFun.Mid2(RTrim(NSG.機械名称), 1, 20) & "   " & CisFun.Mid2(RTrim(NSG.機械名), 1, 20)
                      ...
                      ''End If
                        ...
                        '===============================================================================================================' 2019/09/19 ADD END
'                    If wSyoriDspKB >= 2 Then                        ' 2019/10/15 ADD
'                    Else
...
'                    .Row = wInt
                    ...
                    If wb_ZaiKB Then
                        ...
                        '===================================================================' 2019/08/05 ADD END
                        '===================================================================' 2019/10/28 ADD START
                        ' 在庫管理区分=4(日数) かつ (１日当たりの在庫数×日数)＞在庫数　の場合は色変え
                        ElseIf wZK_KanriKbn = 4 Then
                                ' wDayDetail(iCtI, 0) = CisFun.Val2(SYM.値) ' 年月
                                ' wDayDetail(iCtI, 1) = SYM.件数            ' 数量（１ヶ月の合計）
                                ' wDayDetail(iCtI, 2) = SYM.区分桁数        ' １日当たりの数量（小数以下切捨）
                                ' wDayDetail(iCtI, 3) = SYM.数字1           ' 稼動日数
                                ' wDayDetail(iCtI, 4) = SYM.数字2           ' 在庫日数（補正値）
                                ' wDayDetail(iCtI, 5) = SYM.数字3           ' 在庫日数（実値）
                            Dim wTargetMonth As String
                            wTargetMonth = Mid(.TextMatrix(0, wl_ColNo), 1, 2)      ' 月のみ取得
                            Dim iCtI, iCtJ As Integer
                            For iCtI = 0 To 2
                                If Mid(CStr(wDayDetail(iCtI, 0)), 5, 2) = wTargetMonth Then
                                    Exit For
                                End If
                            Next iCtI
                            Dim cTekisei As Currency
                            If iCtI <= 2 Then
                                cTekisei = wDayDetail(iCtI, 2) * wDayDetail(iCtI, 5)
                                If cTekisei <> 0 And cTekisei > wl_Zaiko Then
                                    wInt = .Row
                                    .Col = wl_ColNo: .Row = (wY * 5) + 5
                                    .CellBackColor = &HFFFF00
                                    .Row = wInt
...
'        Case "B": gSL_Select = gSL_Select & "   and 品番 = '" & RTrim(cHinbn(wY)) & "'"
    ...
    Call Str_Monitor(gSL_Select)
    '*****  特定品番手の確認試験用  ****************↓↓↓↓    ' 2019/10/28
    'If RTrim(cHinbn(wY)) = "951110082042" Then
    '    Dim wHinBanHinBan As String
    '    wHinBanHinBan = RTrim(cHinbn(wY))
    'End If
    '*****  特定品番手の確認試験用  ****************↑↑↑↑    ' 2019/10/28
...
'   >> 日別情報のクリア
    ...
    gSL_Select = gSL_Select & vbCrLf & "   AND HM.在庫管理区分 = 4"
    gSL_Select = gSL_Select & vbCrLf & " WHERE NJ.内示年月 BETWEEN '" & wDayDtlYMS + "' AND '" + wDayDtlYME & "'"
  ''gSL_Select = gSL_Select & vbCrLf & " WHERE NJ.内示年月 BETWEEN '" & CisFun.Mid2(wYmdS, 1, 6) + "' AND '" + CisFun.Mid2(wYmdE, 1, 6) & "'"
    gSL_Select = gSL_Select & vbCrLf & "   AND SUBSTRING(納期, 1, 6) = NJ.内示年月"
    If cTorcd(wY) <> "" Then
        gSL_Select = gSL_Select & vbCrLf & "   AND NJ.手配先   = '" + RTrim(cTorcd(wY)) + "'"       ' NSG.仕入先
    Else
        gSL_Select = gSL_Select & vbCrLf & "   AND NJ.手配先   = '" + Trim(iH1_Torcd.Text) + "'"    ' NSG.仕入先
    End If
    gSL_Select = gSL_Select & vbCrLf & "   AND NJ.品番     = '" + RTrim(cHinbn(wY)) + "'"           ' NSG.品番
    gSL_Select = gSL_Select & vbCrLf & " GROUP BY NJ.品番, NJ.内示年月, NJ.手配先"
    gSL_Select = gSL_Select & vbCrLf & "         ,CA.稼動日数, HM.在庫日数"
    gSL_Select = gSL_Select & vbCrLf & " ORDER BY 内示年月"
    Call Str_Monitor(gSL_Select)
