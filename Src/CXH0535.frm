   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1413783674
            FormatString    =   "№  |納 入 日 　-便|支給先 |   支 給 先 名                  |受入|納入番号|実績管理NO|Select"
            Rows            =   21
            Cols            =   9
            BackColor       =   12648447
...
'   *-------------------*
    With CisVsGrid1
        ...
        .FixedHeight = 320
        .PatanMax = 1
        .SelectCol = 7 '11 '9 2004/05/10 MNT    ' 2008.08.25 upd : 11 → 7
...
'   mOutProc = ""               ' 2008.08.25 del
'   Call PB_Chg_Click           ' 2008.08.22 del
'   #-------------------#
'   # 画面中央位置表示   #
'   #-------------------#
...
Private Function Key_Acc(Key_Code As Integer, IVENT As String, Shift As Integer)
 ...
 '         Case vbKeyF6:        If PB_Chg.Visible Then Call PB_Chg_Click        '【切替】   ' 2008.08.22 del
...
'   納入日＆便
    ...
    If Trim(iH1_SYmd) <> "" And Trim(iH1_EYmd) <> "" Then
       ...
       End If
    End If
    If iH1_SBin <> "" And iH1_EBin <> "" And _
       iH1_SBin > iH1_EBin Then
        With CisFun
            .MB_Lines = 4
            .MB_MSG(2) = "    便を正しく入力して下さい（開始＞終了）            "
            .MB_Title = ""
            .MB_Button = Error
            .MBOX
        End With
...
Private Sub SqlSelectSet(Optional InputKey As String = "")
    ...
    Dim wk_Whr      As String   ' Where句作成用ワーク
'
   'SELECT 部
    gSL_Select = ""
    gSL_Select = gSL_Select & "select "
    gSL_Select = gSL_Select & "  JT.実績管理NO "
    gSL_Select = gSL_Select & ", JT.実績区分 "
    gSL_Select = gSL_Select & ", JT.実績日 "
    gSL_Select = gSL_Select & ", JT.実績便 "
    gSL_Select = gSL_Select & ", JT.実績先 "
    gSL_Select = gSL_Select & ", TM.略称 as 実績先名 "
    gSL_Select = gSL_Select & ", JT.受入 "
    gSL_Select = gSL_Select & ", JT.納入番号 "
    If H1Op_SKbn(1) Then    ' 再発行の場合は帳票出力テーブルの帳票区分=201のものが対象
        gSL_Select = gSL_Select & ", CT.帳票区分 "
    End If
'
   'FROM 部
    gSL_Select = gSL_Select & " from  実績管理テーブル JT "
    gSL_Select = gSL_Select & " left outer join 取引先マスタ TM "
    gSL_Select = gSL_Select & "   on TM.取引先CD   = JT.実績先 "
    gSL_Select = gSL_Select & "  and TM.取引先区分 = 1 "
    If H1Op_SKbn(1) Then    ' 再発行の場合は帳票出力テーブルの帳票区分=201のものが対象
        gSL_Select = gSL_Select & " left outer join 帳票出力テーブル CT "
        gSL_Select = gSL_Select & "   on CT.帳票区分 = 201 "
'       gSL_Select = gSL_Select & "  and CT.検索情報 = JT.納入番号 "
'       gSL_Select = gSL_Select & "  and CT.管理NO   = JT.実績管理NO "
    End If
   ...
   'WHERE 部
    ...
    If iH1_SYmd <> "" Then
        If CisFun.Val2(iH1_SYmd.cDay) = 0 Then
           iH1_EYmd = ""
           iH1_EBin = ""
           wk_Whr = " JT.実績日 like '" & iH1_SYmd & "%' "
        Else
           If iH1_EYmd = "" Then
              wk_Whr = " JT.実績日 = '" & iH1_SYmd & "' "
           Else
              wk_Whr = " JT.実績日 >= '" & iH1_SYmd & "' and " & _
                       " JT.実績日 <= '" & iH1_EYmd & "' "
           End If
        End If
    ElseIf iH1_EYmd <> "" Then
       wk_Whr = " JT.実績日 <= '" & iH1_EYmd & "' "
    End If
    '------- 納入便
    If iH1_SBin <> "" And iH1_EBin = "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.実績便 = '" & iH1_SBin & "' "
    ElseIf iH1_SBin <> "" And iH1_EBin <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.実績便 >= '" & iH1_SBin & "' and " & _
                         " JT.実績便 <= '" & iH1_EBin & "'"
    ElseIf iH1_EBin <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.実績便 <= '" & iH1_EBin & "'"
    End If
    '------- 納入番号
    If iH1_SNouNo <> "" And iH1_ENouNo = "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.納入番号 = '" & iH1_SNouNo & "' "
    ElseIf iH1_SNouNo <> "" And iH1_ENouNo <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.納入番号 >= '" & iH1_SNouNo & "' and " & _
                         " JT.納入番号 <= '" & iH1_ENouNo & "'"
    ElseIf iH1_ENouNo <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.納入番号 <= '" & iH1_ENouNo & "'"
    End If
    '------- 支給先
    If iH1_Torcd <> "" Then
    If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " JT.実績先 = '" & iH1_Torcd & "' "
    End If
    '------- 再発行の場合は帳票出力テーブルの帳票区分=201のものが対象
    If H1Op_SKbn(1) Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " CT.管理NO = JT.実績管理NO and CT.検索情報 = JT.納入番号 "
    End If
    If wk_Whr <> "" Then gSL_Select = gSL_Select & " where " & wk_Whr
'
   'ORDER 部
    gSL_Select = gSL_Select & " order by JT.納入番号, JT.実績管理NO, " & _
                                       " JT.実績日, JT.実績便, " & _
                                       " JT.実績先, JT.受入 "
Call Test_LogOutput(gSL_Select, 1) '------- DEBUG用 -----------------------------------------------
...
' ( 作成処理 )
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(7))) = "X" Then             ' 2008.08.25 11 → 7
                Call ItemsClearCHT
                CHT.端末番号 = "' + substring(host_name(),1,20) + '"
                CHT.帳票区分 = 211                                                  ' 2008.08.25 add
                CHT.検索情報 = Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(5)))        ' 2008.08.25 add
                CHT.管理NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(6)))   ' 2008.08.25 8 → 6
                Call CHTInsert
            End If
        Next ID
    End With
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 IN (211)"    ' 2008.08.25 upe : 帳票区分 IN (108,115)"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 8, 2, "支給先")
        Call .FixedSet(中中, 左中, あり, 26, 3, "支　給　先　名")
        Call .FixedSet(中中, 中中, あり, 4, 4, "受入")
        Call .FixedSet(中中, 中中, あり, 10, 5, "納入番号")
        Call .FixedSet(中中, 左中, あり, 10, 6, "実績管理NO")
        '　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, なし, 2, 7, "Select")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
...
Private Function GridSet1() As Boolean
    ...
    If JKTRead(gSL_Select, , 1) Then    ' 2008.08.22 upd : If HCTRead(gSL_Select, , 1) Then
...
'       ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
        ...
        End With
        ID = 0
        Do Until Not JKT_RDSTS          ' 2008.08.22 upd : HCT_RDSTS
            ID = ID + 1

            Call HctInfoToGrid1(ID)

            Call JKTReadNext(1)         ' 2008.08.22 upd : Call HCTReadNext(1)
        Loop
        With vsGrid1
        ...
        GridSet1 = True
    End If
    Call JKTClose                       ' 2008.08.22 upd : Call HCTClose
End Function
'+----------------------------------------------+
'+      実績管理テーブルよりグリッドに転送
'+----------------------------------------------+
Private Sub HctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '納入日＆便
        If Trim(JKT.実績日) <> "" Then
            If RTrim(JKT.実績便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(JKT.実績日), "0000/00/00") & "   "
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(JKT.実績日), "0000/00/00") & "-" & RTrim(JKT.実績便)
            End If
        End If
        '支給先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = RTrim(JKT.実績先)
        '支給先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = RTrim(JKT.実績先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(JKT.受入)
        '納入番号
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(JKT.納入番号)
        '実績管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(JKT.実績管理NO)
        'Select
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = ""
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
