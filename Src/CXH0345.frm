   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日　-便|仕入先|仕入先名    |納 番 |頁|手配|背番号|品　番　　               |収容数 |箱数 |発注数 ||"
            Rows            =   21
            Cols            =   15
            BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/02/04  By CIS
'**       変  更  日    :   2004/05/28  By CIS 背番号カナ入力
'**       変  更  日    :   2005/01/17  By CIS 既存の表示条件に発注区分=3を追加
'**       変  更  日    :   2007/09/20  By CIS TEC B-848 印刷モジュール追加
'**                                           （切替）gIniName [Option] - 臨時かんばん = 0:ﾚｰｻﾞｰ、1:TEC
...
Private Sub Form_Activate()
    ...
    Call GetCryOption("G", "Option", "臨時かんばん", mPrintType)
    If RTrim(mPrintType) = "" Then
        mPrintType = "0"
        Call GetCryOption("P", "Option", "臨時かんばん", mPrintType)
    End If
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=9       Else 1=109
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            If mPrintType = "0" Then
                Call CXH0340LPrint(wPrintKB, wPrintOut)
            Else
                Call CXH0341LPrint(wPrintKB, wPrintOut)
            End If
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 9"
        gSL_Select = gSL_Select & "   and hct.発注管理NO = cht.管理NO"
        gSL_Select = gSL_Select & "   and hct.SK区分 = 0"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where hct.SK区分 = 0"
        '--2005/01/17 追加 --------------------(START)----------------
        gSL_Select = gSL_Select & "   and hct.発注区分 = 3"
        '--2005/01/17 追加 --------------------(end)----------------
    End If
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 109"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 11, 3, "仕入先名")
...
Private Sub HctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '発注管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = HCT.発注管理NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
