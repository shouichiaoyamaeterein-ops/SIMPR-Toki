...
'    If Not CXM0150LPrint Then Exit Sub
'
'*   作成日  :2009/03/13   By CIS
'*   変更日  :2011/04/04   By CIS  支給は表示しない
'*   変更日  :2012/07/03   By CIS  号補区分表示無
'*---------------------------------------------------------------------------------
Public CisPrtCXM0150        As New CISFormPrint
Public CisSimplePrint       As New CisSimplePrint
Public CXM0150_PrtCnt       As Long
Global CXM0150_PrtOutKb     As Byte
Global L_wsHKbn             As Byte                     '2009/11/10

Global mCSVDir         As String       '2017/11/08


'Public Function CXM0150LPrint(Optional PrintOut As Integer = 0) As Boolean         '2009/11/10
Public Function CXM0150LPrint(Optional PrintOut As Integer = 0, Optional GyakKB As Byte = 0, Optional HZKB As Byte = 0, Optional TSKB As Byte = 0) As Boolean
    CXM0150LPrint = False
...
'======================= 2011/04/04 End
   ...
   If Not KOLRead(gSL_Select, 0, 1) Then
    ...
    CXM0150_PrtCnt = CisDB.RecordCount
    With CisSimplePrint
        .MaxCount = CisDB.RecordCount
        .ProgressInit
    End With
    
    If CXM0150_PrtOutKb <> 2 Then
        With CisPrtCXM0150
             Set .iPrtForm = CXM0150L
             .dpMSG = "構成マスタ照会一覧　印刷中"
             If PrintOut = 0 Then
               .iPreview = pv_Print
             Else
...
'+         CSVファイル出力  処理                        ＋
'+----------------------------------------------------------+
Public Function CXM0150_CsvOut() As Boolean
    CXM0150_CsvOut = False
    Dim cFNo           As Integer
...
'    pref_NCnt = 0                ' 現在発行ページ初期化
    On Error GoTo CXM0150_CsvOut_Err
'------------------------'
'    出力ﾃｷｽﾄ OPEN
'------------------------'
