...
Begin VB.Form CKN1210 
   ...
   Begin Cis3D_v60.CIS3D B2_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D35 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№ |仕入先 |仕入先名　　　　　　|受入|品　番　　　　　　　　　|背番号|収容器   |収容数  |発行枚数||"
            Rows            =   11
            Cols            =   11
            BackColor       =   12648447
   ...
   Begin Cis3D_v60.CIS3D B1_Area3 
      ...
      Begin Cis3D_v60.CIS3D Img_BK 
         ...
         End
      End
      Begin Cis3D_v60.CIS3D ImgMD_BK 
         Height          =   690
         Left            =   120
         Top             =   105
         Width           =   2100
...
'**********************************************************'
    ...
    Dim mImage          As String       ' 2008/06/12 ADD
...
'    s_Clip = (Mid$(Command, 1, 1) = "C")
'    If s_Clip Then Clipboard.Clear
    
    B1_Image.Move 0, 0, Img_BK.Width, Img_BK.Height     ' 2007/02/26 ADD
...
Private Sub PB_Back_Click()
    Call CisFrm.MousePT(11)
    gSL_Select = "Select HN.品名,HN.車型,HN.社内背番,RTRIM(SY.値名称) 手配区分名,"
    gSL_Select = gSL_Select & " HT.* From 品番取引先マスタ HT"
...
'
        gSL_Select = "Select HT.背番号,HT.取引先,HT.受入,HT.品番,HN.表示品番,HN.社内背番,"
        gSL_Select = gSL_Select & " Case When HT.最終発行連番 > KN.発行NO"
        gSL_Select = gSL_Select & " Then  HT.最終発行連番 Else KN.発行NO End 最終発行連番,"
        gSL_Select = gSL_Select & " Rtrim(TR.略称) 取引先名,HT.手配区分"
        gSL_Select = gSL_Select & " From 品番取引先マスタ HT"
        ...
        gSL_Select = gSL_Select & " ON  KN.取引先 = HT.取引先"
        gSL_Select = gSL_Select & " AND KN.受入 = HT.受入"
        gSL_Select = gSL_Select & " AND KN.品番 = HT.品番"
        gSL_Select = gSL_Select & " Left Outer Join 品番マスタ HN"
        gSL_Select = gSL_Select & "   On HN.品番 = HT.品番"
        gSL_Select = gSL_Select & " Left Outer Join 取引先マスタ TR"
        gSL_Select = gSL_Select & "   On  TR.取引先区分 = 1"
...
Private Sub BodySet(SetType As String)
    ...
    gSL_Select = gSL_Select & " Where 取引先 = '" & RTrim(HTM.取引先) & "'"
    gSL_Select = gSL_Select & " And   受入 = '" & RTrim(HTM.受入) & "'"
    gSL_Select = gSL_Select & " And   品番 = '" & RTrim(HTM.品番) & "'"
    If KRTRead(gSL_Select, 1) Then
        If KRT.発行NO > CisFun.Val2(B1lb_EndSu) Then
           B1lb_EndSu = Format(KRT.発行NO, "000")
           Back_SSu(0).Visible = True
        End If
    End If
    '------------------------------------------------------------------- 2005/03/01 Insert End
    
    gSL_Select = "Select Count(*) 件数 From かんばん流動管理テーブル "
    gSL_Select = gSL_Select & " Where 取引先 = '" & RTrim(HTM.取引先) & "'"
    gSL_Select = gSL_Select & " And   受入 = '" & RTrim(HTM.受入) & "'"
    gSL_Select = gSL_Select & " And   品番 = '" & RTrim(HTM.品番) & "'"
    gSL_Select = gSL_Select & " And   収容区分 = 1"
    If KRTRead(gSL_Select, 1) Then
        B1lb_HZSu(0) = Format(KRT.件数, "###")
    End If
    ...
    If Back_HSu(1).Visible Then
        ...
        If KRTRead(gSL_Select, 1) Then
            B1lb_HZSu(1) = Format(KRT.件数, "###")
        End If
    End If
    
    
'@@@@@     @@@@@     @@@@@     @@@@@     @@@@@     @@@@@     @@@@@     @@@@@ 2008/06/12 ADD
    gSL_Select = "SELECT HN.IMAGE,"
    gSL_Select = gSL_Select & " ISNULL( SY1.数字1, ISNULL(SY3.数字1, 14737632 ) ) 背景色コード,"
    gSL_Select = gSL_Select & " ISNULL( SY2.数字1, ISNULL(SY4.数字1, 0 ) ) 枠色コード"
    gSL_Select = gSL_Select & "   FROM 品番取引先マスタ HT"
    gSL_Select = gSL_Select & "       LEFT OUTER JOIN 品番マスタ HN"
    gSL_Select = gSL_Select & "            ON HN.品番 = HT.品番"
    gSL_Select = gSL_Select & "       LEFT OUTER JOIN 色設定テーブル IR"
    gSL_Select = gSL_Select & "            ON  IR.受入 = HT.受入"
    gSL_Select = gSL_Select & "       LEFT OUTER JOIN 名称マスタ SY1"
    gSL_Select = gSL_Select & "            ON  SY1.区分名称 = '色設定'"
    gSL_Select = gSL_Select & "           AND SY1.値 = IR.背景色"
    gSL_Select = gSL_Select & "       LEFT OUTER JOIN 名称マスタ SY2"
    gSL_Select = gSL_Select & "            ON  SY2.区分名称 = '色設定'"
    gSL_Select = gSL_Select & "           AND SY2.値 = IR.枠色"
    gSL_Select = gSL_Select & "       LEFT OUTER JOIN 名称マスタ SY3"
    gSL_Select = gSL_Select & "            ON  SY3.区分名称 = '色設定'"
    gSL_Select = gSL_Select & "           AND SY3.値 = '00'"
    gSL_Select = gSL_Select & "       LEFT OUTER JOIN 名称マスタ SY4"
    gSL_Select = gSL_Select & "            ON  SY4.区分名称 = '色設定'"
    gSL_Select = gSL_Select & "           AND SY4.値 = '16'"
    gSL_Select = gSL_Select & "  WHERE HT.品番 = '" & RTrim(iH1_Hinbn) & "'"
    gSL_Select = gSL_Select & "    AND HT.取引先 = '" & RTrim(iH1_Torcd) & "'"
    gSL_Select = gSL_Select & "    AND HT.受入 = '" & RTrim(iH1_Ukeir) & "'"
    gSL_Select = gSL_Select & "    AND HT.品目 = 1"
    ...
    If HNMRead(gSL_Select, 1) Then
        ImgMD_BK.BackColor = HNM.背景色コード
        B1_Area3.BackColor = HNM.枠色コード
        If RTrim$(HNM.IMAGE) <> "" Then
            B1_Image.Visible = True
            On Error Resume Next
            B1_Image.Picture = LoadPicture(mImage & HNM.IMAGE)
            On Error GoTo 0
        Else
            B1_Image.Visible = False
        End If
    Else
        B1_Image.Visible = False
    End If
...
'
    ...
    ' 最終発行連番を求める
        wHMai = 0
        gSL_Select = "Select Max(発行NO) as 発行NO From かんばん流動管理テーブル"
        gSL_Select = gSL_Select & " Where 品番 = '" & RTrim(.TextMatrix(.Row, 9)) & "'"
    ...
    ' 最終発行連番を求める
        ...
        gSL_Select = gSL_Select & " And   品番 = '" & RTrim(.TextMatrix(.Row, 9)) & "'"
        gSL_Select = gSL_Select & " And   取引先 = '" & RTrim(.TextMatrix(.Row, 1)) & "'"
        gSL_Select = gSL_Select & " And   受入 = '" & RTrim(.TextMatrix(.Row, 3)) & "'"
    ...
    If KHTRead(gSL_Select, 1) Then
       ...
       With CisDB
            ...
            .SQL = .SQL & " Where 品番 = '" & RTrim(vsGrid1.TextMatrix(vsGrid1.Row, 9)) & "'"
            .SQL = .SQL & "   And 取引先 = '" & RTrim(vsGrid1.TextMatrix(vsGrid1.Row, 1)) & "'"
            .SQL = .SQL & "   And 受入 = '" & RTrim(vsGrid1.TextMatrix(vsGrid1.Row, 3)) & "'"
            If Not .DBExec Then GoTo DBDelete_Err
       End With
...
'   品番取引先情報
    ...
    If Trim(HTM.納入先) <> "" Then
        gSL_Select = "Select * From 取引先マスタ "
        gSL_Select = gSL_Select & " Where 取引先区分 = 1"
        gSL_Select = gSL_Select & "   AND 取引先CD = '" & HTM.納入先 & "'"
    ...
    gSL_Select = "Select * From 取引先マスタ "
    gSL_Select = gSL_Select & " Where 取引先区分 = 1"
    gSL_Select = gSL_Select & "   AND 取引先CD = '" & HTM.取引先 & "'"
    If Not TRMRead(gSL_Select, 1) Then
       Call ItemsClearTRM
    End If
    wTorNm = TRM.略称
    ...
    gSL_Select = gSL_Select & " Where 取引先区分 = 1"
    gSL_Select = gSL_Select & "   AND 取引先CD = '" & HTM.取引先 & "'"
    gSL_Select = gSL_Select & "   AND 受入 = '" & HTM.受入 & "'"
    If Not TUMRead(gSL_Select, 1) Then
       Call ItemsClearTUM
    End If
    ' 納入時間情報
    ...
    If HCMRead(gSL_Select) Then
       ...
       Do Until Not HCM_RDSTS
           If Trim(HCM.入荷時間) <> "" And HCM.入荷時間 <> "0000" Then
              wNTime(gCnt1) = CisFun.RSetFld(Mid(HCM.入荷時間, 1, 2), 2, "##") & ":" & Mid(HCM.入荷時間, 3, 2)
              gCnt1 = gCnt1 + 1
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/06/12 UPDATE END
                    ...
                    .車型 = HNM.車型
                    .自社CD = CKN1230_Jisya
                    .自社名1 = CisFun.Mid2(CKN1230_JisyaNM, 1, 20)
                    .自社名2 = CisFun.Mid2(CKN1230_JisyaKJNM, 1, 14)
                    .実数区分 = HNM.実数区分
                    .備考 = HTM.備考
...
' 抽出処理 初期ｾｯﾄ
    ...
    gSL_Select = gSL_Select & "    Where KT.かんばん種類 = 0"
