...
Begin VB.Form CXHZ0500 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料　入出庫・支給実績入力"
   ClientHeight    =   9945
   ClientLeft      =   120
   ClientTop       =   1545
   ClientWidth     =   15345
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/02/26  By CIS SIMPR-A
'**       変  更  日    :   2004/05/18  By CIS - 入力処理時、グリッド移動による不具合修正
'**       変  更  日    :   2004/05/28  By CIS 背番号カナ入力・背番号半角ﾁｪｯｸ
'**       変  更  日    :   2004/06/23  By CIS - 1.機能追加（入出庫入力データ検索）
'**       　  　  　                             2.バグ対応（修正時、追加行の実績管理NOがｾｯﾄされない！）
'**       　  　  　                             3.材料取引先マスタの索引方法を修正
...
Private Sub Look_JKT()
    ...
    CXHZ0501.Show vbModal
    Unload CXHZ0501
    Set CXHZ0501 = Nothing
    ...
    Select Case KeyCode
        Case 48 To 57, 96 To 105
            If CisFun.Val2(B1lb_Syuyo) <> 0 Then
                wSuryo = CisFun.Val2(B1lb_Syuyo) * CisFun.Val2(iB1_Maisu & "9")
                If iB1_Suryo.MaxLength < CisFun.Len2(Format(wSuryo, "###0.000")) Then
                    SendKeys "{TAB}"
                End If
            End If
    End Select
...
'*--------------------------------------------------<< 2004/06/23 >> Update Start
        ...
        If RTrim(iB1_Zaist) <> "" Then
            If H1Op_JKbn(3) Then .TextMatrix(RowNo, 11) = B1Chk_UMkbn.Caption
        End If
