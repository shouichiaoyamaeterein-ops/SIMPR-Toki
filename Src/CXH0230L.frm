...
'**********************************************************'
'**
'**       ÉtÉHÅ[ÉÄñº    :  ì¸â◊ÉvÉãÅ[ÉtÉäÉXÉgÅ@î≠çs
'**       ÉtÉHÅ[ÉÄID    :  CXH0230L
'**       èàóùäTóv      :
'**
'**       çÏ  ê¨  ì˙    :  2004/02/11
...
'*---------------------------------*
    ...
    Call CisPrtCXH0230.ClrLstFld("B1")
    
'*---------------------------------*
'*  î≠íçÉeÅ[ÉuÉãì«çûÇ›
'*---------------------------------*
    Do Until Not NGW_RDSTS
        If Key_Torcd <> NGW.édì¸êÊ Or Key_KTorcd <> NGW.å_ñÒêÊ Or _
            CisPrtCXH0230.cLine >= CisPrtCXH0230.iLineMax Then
            If RTrim(Key_Torcd) <> "" Then
                B1_K_LINE1.Visible = True
                Call CisPrtCXH0230.PrintFld("B1*")
                CisPrtCXH0230.cLine = CisPrtCXH0230.cLine + 1
                Call CisPrtCXH0230.ClrLstFld("B1")
            End If
            Call HeadWrite(0)
        End If
...
'                GTMaisu = GTMaisu + .ñáêî
    ...
    'BODYïîàÛç¸
        ...
        With CisPrtCXH0230
            .cDataCnt = .cDataCnt + 1
            .dpCountDisp Format(.cDataCnt, "#,###") & " / " & Format(CXH0230_PrtCnt, "#,###")
        End With
        
    'KEYçÄñ⁄ëﬁî
        Key_KTorcd = NGW.å_ñÒêÊ
        Key_Torcd = NGW.édì¸êÊ
        Key_NYmd = RTrim(NGW.î[ì¸ì˙) & RTrim(NGW.î[ì¸ï÷)
        Key_Nouno = NGW.î[ì¸î‘çÜ
...
'ç≈èIï≈Å@ñáêîåvàÛç¸
    ...
    ' àÛç¸èIóπ
    Call CisPrtCXH0230.PrintEndDoc
...
Private Function HeadWrite(KB As Byte)
    ...
    'ï≈
        If RTrim(Key_Torcd) <> RTrim(NGW.édì¸êÊ) Then
            SiirePage = 0
        End If
        SiirePage = SiirePage + 1
        H1_Page = Format(SiirePage, "##0")
    'édì¸êÊÅEédì¸êÊñº
        H1_Siire = RTrim(NGW.édì¸êÊCD)
        If RTrim(NGW.édì¸êÊçHèÍCD) <> "" Then
            H1_Siire = H1_Siire & " - " & RTrim(NGW.édì¸êÊçHèÍCD)
        End If
        H1_SiireNm = RTrim(NGW.édì¸êÊñº)
        H1_SiireKj = RTrim(NGW.édì¸êÊçHèÍñº)
    'édì¸êÊÅEédì¸êÊñº
        H1_KTorcd = RTrim(NGW.å_ñÒêÊ)
    'î≠çsì˙ÅEé©é–ñº
