...
Begin VB.Form CNJZ2320 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料発注内示表(A4)　発行"
   ClientHeight    =   11205
   ClientLeft      =   2340
   ClientTop       =   2295
   ClientWidth     =   15360
...
'   ( ３ヶ月　月量データ ) < 納期:｢00000000｣/当月数量、 ｢00000001｣/翌月数量、 ｢00000002｣/翌々月数量 >
    ...
    gSL_Select = gSL_Select & " ISNULL( MAX( HN.表示寸法 ), '' ) 表示寸法," & vbCrLf
...
'    gSL_Select = gSL_Select & " ISNULL( MAX( HT.発注品番 ), '' ) 発注品番," & vbCrLf            '2010/02/08 Insert
'    gSL_Select = gSL_Select & " ISNULL( MAX( HT.受給品番 ), '' ) 受給品番" & vbCrLf             '2010/02/08 Insert
...
Private Function CsvPut() As Boolean
    ...
    '当月日付編集
    gStr = RTrim$(iH1_YM)
    wYM = Mid(gStr, 1, 4) & "年 " & CisFun.RSetFld(Mid(gStr, 5, 2), 2, "##") & "月"
    With CisFun
        '翌月日付編集
        .Date_RsFormat = fYM
        .Date_Add gStr & "01", M, 1, LastDate
        gStr = .Date_Result
        wYMy = Mid(gStr, 1, 4) & "年 " & CisFun.RSetFld(Mid(gStr, 5, 2), 2, "##") & "月"

        '翌々月日付編集
        .Date_RsFormat = fYM
        .Date_Add gStr & "01", M, 1, LastDate
        gStr = .Date_Result
        wYMyy = Mid(gStr, 1, 4) & "年 " & CisFun.RSetFld(Mid(gStr, 5, 2), 2, "##") & "月"
    End With
    
    '見出し編集
    MD1 = """内示年月"",""手配先"",""手配先名"""
'--------------------------------------------------------------------------------------------------2010/02/08 Update Start
    MD2 = """№"",""材料寸法"",""単位"",""収容数"",""" & wYM & """,""" & wYMy & """,""" & wYMyy
'    MD2 = """№"",""材料名"",""単位"",""収容数"",""" & wYM & """,""" & wYMy & """,""" & wYMyy & """,""発注品番"",""受給品番"""
'--------------------------------------------------------------------------------------------------2010/02/08 Update End
    '明細編集
    With ZNJ
        OutCnt = 0:     PgBk_Torcd = "":    OutStr = ""
        Erase w_MSu:    NoCnt = 0

        Call GetKadou       'カレンダマスタの稼働日数取得
...
'                PrtK_Zainm = RTrim$(.材料名)
                PrtK_Zainm = RTrim$(.材質) & " " & RTrim$(.表示寸法)
            ...
            If RTrim$(OutStr) = "" Then
                ...
                OutStr = """" & NoCnt & ""","
'                OutStr = OutStr & """" & RTrim(.材料名) & ""","
                OutStr = OutStr & """" & RTrim$(.材質) & " " & RTrim$(.表示寸法) & ""","
                OutStr = OutStr & """" & RTrim(.手配単位名) & ""","
                OutStr = OutStr & """" & .収容数 & ""","
            End If
            ...
            If Not ZNJ_RDSTS Or _
               ...
               PrtK_Zainm <> RTrim$(.材質) & " " & RTrim$(.表示寸法) Then
'               PrtK_Zainm <> RTrim$(.材料名) Then

                '月量
                OutStr = OutStr & w_MSu(0) & ","
'                If RTrim$(CLM.稼動日数) > 0 Then
'                    gLong = CisFun.HasuProc((w_MSu(0) / RTrim$(CLM.稼動日数)), 切上げ, 0)
'                Else
'                    gLong = 0
'                End If
'                OutStr = OutStr & """" & gLong & ""","
                OutStr = OutStr & """" & w_MSu(1) & ""","
                OutStr = OutStr & """" & w_MSu(2) & ""","
'--------------------------------------------------------------2010/02/08 DELETE START
'                If .寸法区分 = 0 Then
'                    gStr = CisFun.RSetFld(.板厚, 7, "###0.00") & "×"
'                    gStr = gStr & CisFun.RSetFld(.幅, 7, "###0.00") & "×"
'                    If .長さ = 0 Then
'                        gStr = gStr & CisFun.RSetFld("Ｃ", 4, "")
'                    Else
'                        gStr = gStr & CisFun.RSetFld(.長さ, 7, "###0.00")
'                    End If
'                    OutStr = OutStr & """" & RTrim(gStr) & """"
'                End If
'--------------------------------------------------------------2010/02/08 DELETE END
'--------------------------------------------------------------2010/02/08 INSERT START
'                OutStr = OutStr & """" & RTrim(.発注品番) & ""","
'                OutStr = OutStr & """" & RTrim(.受給品番) & """"
'--------------------------------------------------------------2010/02/08 INSERT END
                '出力
                Print #OutFNo, OutStr
