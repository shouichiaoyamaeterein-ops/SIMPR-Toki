...
Begin VB.Form CKNZ1220 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin vsFlexLib.vsFlexArray vsGrid1 
         ...
         Appearance      =   1
         ConvInfo        =   1418783674
         FormatString    =   "№ |仕入先 |仕入先名　　　　　　|受入|材  質　　　　　　　　　|背番号|収容器   |収容数  |発行連番|編集前品番|Select|"
         Rows            =   21
         Cols            =   12
         BackColor       =   12648447
...
Private Function Head1Chk() As Boolean
    ...
    gSL_Select = "select KT.取引先,KT.受入,KT.背番号,KT.収容器,KT.材質,KT.表示品番,"
    gSL_Select = gSL_Select & "KT.収容数,KT.発行NO,KT.発行済,KT.材料管理番号,"
    ...
    With CisFun
         ...
         ' 材質
         .Where_Create AlphaMe, "KT.材質", iH1_Zai
         ' 背番号
         .Where_Create AlphaMe, "KT.背番号", iH1_Seban
         ' 板厚
         If .Val2(iH1_Ita) <> 0 Then
            .Where_Create Numeric, "板厚", .Val2(iH1_Ita)
         End If
         ' 幅
         If .Val2(iH1_Haba) <> 0 Then
            .Where_Create Numeric, "幅", .Val2(iH1_Haba)
         End If
         ' 長さ
         If .Val2(iH1_Nagasa) <> 0 Then
            .Where_Create Numeric, "長さ", .Val2(iH1_Nagasa)
         End If
    
         gSL_Select = gSL_Select & .Where_Phrase
    End With
    
    gSL_Select = gSL_Select & " Order By KT.発行種類,KT.取引先,KT.受入,KT.材料管理番号,KT.背番号,KT.収容区分,KT.発行NO"
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
                    .SQL = .SQL & " And   材料管理番号 = " & CisFun.Val2(vsGrid1.TextMatrix(ID, CisVsGrid.FixedGet(9)))
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 40, 4, "材 質      板厚×幅×長さ")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE START
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not KHT_RDSTS
        With vsGrid1
            ...
            .TextMatrix(ID, 3) = KHT.受入 & "     " & KHT.取引先 & KHT.材料管理番号
            .TextMatrix(ID, 4) = KHT.材質 & " " & KHT.表示品番
            .TextMatrix(ID, 5) = KHT.背番号 & "    " & KHT.取引先 & KHT.材料管理番号
