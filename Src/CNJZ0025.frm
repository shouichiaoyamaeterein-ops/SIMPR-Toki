...
'                   < 材料発注内示テーブルより >
                    ...
                    gSL_Select = gSL_Select & " FROM 材料発注内示テーブル HJ "
                    gSL_Select = gSL_Select & " LEFT OUTER JOIN 材料取引先マスタ HT "
                    gSL_Select = gSL_Select & "   ON HT.材料管理番号 = HJ.材料管理番号"
                    ...
                    gSL_Select = gSL_Select & "   AND HJ.材質 = '" & RV_Zaist & "'"
                    gSL_Select = gSL_Select & "   AND HJ.板厚 =  " & RV_Itatu
                    gSL_Select = gSL_Select & "   AND HJ.幅   =  " & RV_Width
                    gSL_Select = gSL_Select & "   AND HJ.長さ =  " & RV_Long
                    gSL_Select = gSL_Select & " GROUP BY HJ.手配先, HJ.受入, HT.発注比率, TR.略称"
                    gSL_Select = gSL_Select & " ORDER BY HJ.手配先, HJ.受入"
                    If Not ZTMRead(gSL_Select) Then GoTo BodySet_Err
        Case Else
'                   < 材料取引先マスタより >
                    gSL_Select = "Select HT.取引先, HT.受入, HT.発注比率, ISNULL( TR.略称, '' ) 手配先名"
                    gSL_Select = gSL_Select & " From 材料取引先マスタ HT"
                    gSL_Select = gSL_Select & " Left Outer Join 取引先マスタ TR"
                    gSL_Select = gSL_Select & " On  TR.取引先CD = HT.取引先"
                    gSL_Select = gSL_Select & " And TR.取引先区分 = 1 "
                    gSL_Select = gSL_Select & " Where 材質 = '" & RV_Zaist & "'"
                    gSL_Select = gSL_Select & " And   板厚 =  " & RV_Itatu
                    gSL_Select = gSL_Select & " And   幅   =  " & RV_Width
                    gSL_Select = gSL_Select & " And   長さ =  " & RV_Long
                    If Not ZTMRead(gSL_Select) Then GoTo BodySet_Err
    End Select
    ...
    Do Until Not ZTM_RDSTS
        ...
        With VsGrid1
            ...
            .TextMatrix(gInt, SCol(1)) = RTrim(ZTM.取引先)
            .TextMatrix(gInt, SCol(2)) = RTrim(ZTM.手配先名)
            .TextMatrix(gInt, SCol(3)) = RTrim(ZTM.受入)
            .TextMatrix(gInt, SCol(4)) = RTrim(ZTM.発注比率)
