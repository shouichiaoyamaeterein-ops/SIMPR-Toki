...
Begin VB.Form CXHZ0325 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料・自社伝票　ファイル再出力"
   ClientHeight    =   10980
   ClientLeft      =   -720
   ClientTop       =   1440
   ClientWidth     =   15315
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |作成ファイル名 　        |伝種|納 入 日 　-便|仕入先 |   仕  入  先  名   |受入|納 番 |頁|発注区|手配区|SK|"
            Rows            =   21
            Cols            =   17
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        ...
        '--( TEXT出力先　更新 )-- [クライアント情報]
            ...
            wPrintKB = 1                ' 帳票区分：0=26 Else 1=126
            Call CXHZ0320TFile(wPrintKB)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & ", 材料発注帳票発行テーブル zch"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " from"
        gSL_Select = gSL_Select & "  材料発注帳票発行テーブル zch"
    ...
    gSL_Select = gSL_Select & "    on trm.取引先CD = zch.仕入先"
    ...
    gSL_Select = gSL_Select & "   and sy1.値 = zch.発注区分"
    gSL_Select = gSL_Select & "  left outer join 名称マスタ sy2"
    gSL_Select = gSL_Select & "    on sy2.区分名称 = '手配区分'"
    gSL_Select = gSL_Select & "   and sy2.区分タイプ = ''"
    gSL_Select = gSL_Select & "   and sy2.値 = zch.手配区分"
    If H1Op_SKbn(1) Then
'   ### 再発行最新 ###
        gSL_Select = gSL_Select & " where cht.端末番号 = substring(host_name(),1,20)"
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 26"
        gSL_Select = gSL_Select & "   and zch.納品書管理NO = cht.管理NO"
        gSL_Select = gSL_Select & "   and zch.納入番号 = cht.管理文字"
        gSL_Select = gSL_Select & "   and zch.頁NO = cht.補助番号"
        gSL_Select = gSL_Select & "   and zch.カード = cht.検索情報"
        gSL_Select = gSL_Select & "   and zch.発行種類 = 1"
        gSL_Select = gSL_Select & "   and zch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and zch.TEXTNAME <> ''"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where zch.発行種類 = 1"
        gSL_Select = gSL_Select & "   and zch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and zch.TEXTNAME <> ''"
    End If
'   >> 伝票種類
    If H1Chk_Card(6) = 1 Or _
       H1Chk_Card(7) = 1 Or _
       H1Chk_Card(8) = 1 Then
        gSL_Select = gSL_Select & "   and zch.カード in"
...
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
        gSL_Select = gSL_Select & "   and zch.納入番号 like '" & RTrim(Replace(iH1_SNouNo, "*", "%")) & "'"
    Else
        If Trim(iH1_SNouNo) <> "" Then
            If Trim(iH1_ENouNo) = "" Then
                '> 開始のみ
                If CisFun.Val2(iH1_SPage) = 0 Then
                    gSL_Select = gSL_Select & "   and zch.納入番号 = '" & RTrim(iH1_SNouNo) & "'"
                Else
                    gSL_Select = gSL_Select & "   and"
                    gSL_Select = gSL_Select & "   zch.納入番号 + space(5 - len(zch.納入番号))"
                    gSL_Select = gSL_Select & " + right('0' + cast(cast(zch.頁NO as decimal(2)) as varchar(2)), 2)"
                    gSL_Select = gSL_Select & " = "
                    gSL_Select = gSL_Select & "'"
        ...
        If Trim(iH1_ENouNo) <> "" Then
            If CisFun.Val2(iH1_EPage) = 0 Then
                gSL_Select = gSL_Select & "   and zch.納入番号 <= '" & RTrim(iH1_ENouNo) & "'"
            Else
                gSL_Select = gSL_Select & "   and"
                gSL_Select = gSL_Select & "   zch.納入番号 + space(5 - len(zch.納入番号))"
                gSL_Select = gSL_Select & " + right('0' + cast(cast(zch.頁NO as decimal(2)) as varchar(2)), 2)"
...
'   >> 仕入先
    If RTrim(iH1_Torcd) <> "" Then
        gSL_Select = gSL_Select & "   and zch.仕入先 = '" & RTrim(iH1_Torcd) & "'"
    End If
'   >> 納入日／作成日
    If Trim(iH1_SYmd) <> "" And CisFun.Val2(iH1_SYmd.cDay) = 0 Then
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(zch." & wYmdName & ", 1, 6) = '" & iH1_SYmd & "'"
    Else
        YmdSta = "": BinSta = ""
        YmdEnd = "": BinEnd = ""
        '> 日付＆便　編集
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) = "" Then
            If Trim(BinSta) = "" Then
                gSL_Select = gSL_Select & "   and zch." & wYmdName & " = '" & YmdSta & "'"
            Else
                gSL_Select = gSL_Select & "   and zch.納入日 + zch.納入便 = '" & YmdSta & BinSta & "'"
            End If
        End If
        '> 終了日のみ
        If Trim(YmdSta) = "" And Trim(YmdEnd) <> "" Then
            If Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and zch." & wYmdName & " <= '" & YmdEnd & "'"
            Else
                gSL_Select = gSL_Select & "   and zch.納入日 + zch.納入便 <= '" & YmdEnd & BinEnd & "'"
            End If
        End If
        '> 開始日＆終了日
        If Trim(YmdSta) <> "" And Trim(YmdEnd) <> "" Then
            If Trim(BinSta) = "" And Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and zch." & wYmdName & " between '" & YmdSta & "'" _
                                                                       & " and '" & YmdEnd & "'"
            Else
                If Trim(BinSta) = "" Then BinSta = "00"
                If Trim(BinEnd) = "" Then BinEnd = "99"
                gSL_Select = gSL_Select & "   and zch.納入日 + zch.納入便 between '" & YmdSta & BinSta & "'" _
                                                                         & " and '" & YmdEnd & BinEnd & "'"
            End If
        End If
    End If
'   >> 発注区分
    If RTrim(H1Cmb_HKbn) <> "" Then
        ...
        gSL_Select = gSL_Select & "   and zch.発注区分 = " & CisCombo1.Code & ""
    End If
'   >> 手配区分
    If RTrim(H1Cmb_TKbn) <> "" Then
        Call CisCombo2.CodeGet
        gSL_Select = gSL_Select & "   and zch.手配区分 = " & CisCombo2.Code & ""
    End If
    gSL_Select = gSL_Select & " order by"
    gSL_Select = gSL_Select & "  zch.TEXTNAME"
    gSL_Select = gSL_Select & ", zch.カード"
    gSL_Select = gSL_Select & ", zch.納入日"
    gSL_Select = gSL_Select & ", zch.納入便"
    gSL_Select = gSL_Select & ", zch.仕入先"
    gSL_Select = gSL_Select & ", zch.受入"
    gSL_Select = gSL_Select & ", zch.納入番号"
    gSL_Select = gSL_Select & ", zch.頁NO"
'
End Sub
'**************************************
'*                                    *
...
' ( 作成処理 )
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(15))) = "X" Then
                Call ItemsClearCHT
                CHT.端末番号 = "' + substring(host_name(),1,20) + '"
                CHT.帳票区分 = 126
                CHT.管理文字 = RTrim(.TextMatrix(ID, CisVsGrid1.FixedGet(7)))
                CHT.検索情報 = RTrim(.TextMatrix(ID, CisVsGrid1.FixedGet(14)))
                CHT.管理NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(13)))
                CHT.補助番号 = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(8)))
                Call CHTInsert
            End If
        Next ID
    End With
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 126"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, あり, 4, 2, "伝種")
        Call .FixedSet(中中, 左中, あり, 13, 3, "納 入 日  -便")
        Call .FixedSet(中中, 左中, あり, 7, 4, "仕入先")
        Call .FixedSet(中中, 左中, あり, 20, 5, "仕　入　先　名")
        Call .FixedSet(中中, 中中, あり, 4, 6, "受入")
        Call .FixedSet(中中, 中中, あり, 6, 7, "納 番")
        Call .FixedSet(中中, 右中, あり, 2, 8, "頁")
        Call .FixedSet(中中, 中中, あり, 6, 9, "発注区")
        Call .FixedSet(中中, 中中, あり, 6, 10, "手配区")
        Call .FixedSet(中中, 中中, あり, 2, 11, "SK")
        Call .FixedSet(中中, 左中, あり, 25, 12, "Dummy1")
        Call .FixedSet(中中, 左中, あり, 0, 13, "納品書管理NO")
        Call .FixedSet(中中, 左中, あり, 0, 14, "カード")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
End Function
'+---------------------------------------------------+
'+      材料発注帳票発行テーブルよりグリッドに転送
'+---------------------------------------------------+
Private Sub ZchInfoToGrid1(SetRow As Long)
    With vsGrid1
        'No
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(0)) = SetRow
        '作成ﾌｧｲﾙ名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = ZCH.TEXTNAME
        '伝種
        Select Case ZCH.カード
            Case "6": .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = "受領"
            Case "7": .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = "納品"
            Case "8": .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = "支給"
        End Select
        '納入日＆便
        If Trim(ZCH.納入日) <> "" Then
            If RTrim(ZCH.納入便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = Format(RTrim(ZCH.納入日), "0000/00/00")
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = Format(RTrim(ZCH.納入日), "0000/00/00") & "-" & RTrim(ZCH.納入便)
            End If
        End If
        '仕入先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(ZCH.仕入先)
        '仕入先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(ZCH.仕入先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(ZCH.受入)
        '納番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(ZCH.納入番号)
        '頁
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = Format(ZCH.頁NO, "##")
        '発注
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = RTrim(ZCH.発注区分名)
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = RTrim(ZCH.手配区分名)
        'SK
        Select Case ZCH.SK区分
            Case 0: .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = "K"
            Case 1: .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = "S"
        End Select
        '納品書管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(13)) = ZCH.納品書管理NO
        'カード
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(14)) = ZCH.カード
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
