...
Begin VB.Form COK0430 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "受給検収・発注入荷対比照会"
   ClientHeight    =   11070
   ClientLeft      =   1650
   ClientTop       =   1560
   ClientWidth     =   15375
   ...
   Begin VB.PictureBox B1_Area2 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGridH 
         Bindings        =   "COK0430.frx":0000
         Height          =   8295
         Left            =   45
         OleObjectBlob   =   "COK0430.frx":0015
         TabIndex        =   8
         Top             =   60
         Width           =   6825
      End
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "COK0430.frx":150BD
         Height          =   8310
         Left            =   90
         OleObjectBlob   =   "COK0430.frx":150D2
         TabIndex        =   7
         Top             =   60
         Width           =   7980
      End
...
'+---------------------+
    ...
    ' 手配区分
    Set CisComboHKbn = New CisCombo
    With CisComboHKbn
        Set .Connect = CisDB
        Set .CombObj = H1Cmb_HKbn
        .Control = "名称マスタ"
        .ComboName = "発注区分"
        .Ess = True
        .IniPath = gIniExe
...
Private Sub H1Cmb_HKbn_GotFocus()
    H1Cmb_HKbn.BackColor = iH1_SDate.cGotColor
End Sub
Private Sub H1Cmb_HKbn_LostFocus()
    H1Cmb_HKbn.BackColor = iH1_SDate.cLostColor
End Sub
'****************************
'*                          *
'*      HEAD/BODY 処理      *
...
'       表示条件 セット
       ...
       With CisFun
            ...
            If Trim(iH1_SDate) <> "" And CisFun.Val2(iH1_SDate.cDay) = 0 Then
               .Where_Create AlphaMe, "Substring(納入日,1,6)", iH1_SDate
            End If
            ' 日付開始編集
            If Trim(iH1_SDate) <> "" Then
               DateSt = iH1_SDate
            ...
            ' 手配区分
            Call CisComboHKbn.CodeGet
            If Trim(CisComboHKbn.Code) <> "" Then
               .Where_Create AlphaMe, "発注区分", CisComboHKbn.Code
            End If
            
            mSL_SelectW = .Where_Phrase
            CisDBGridH.Select_Where = Replace(mSL_SelectW, "納入日", "検収日")
       
            If RTrim(mSL_SelectW) = "" Then
                mSL_SelectW = " WHERE "
            Else
                mSL_SelectW = mSL_SelectW & " AND "
            End If
            mSL_SelectW = mSL_SelectW & " 手配区分 = '3'"
            
            gSL_Select = "Select Sum(Case When 分納区分 = 0 Then 発注数 Else 0 End )"
            gSL_Select = gSL_Select & " From 発注入荷状況問合ビュー "
            ...
            gSL_Select = "Select Sum(検収数) From 受給検収問合ビュー"
...
' ﾚｲｱｳﾄ表示内容よりﾃﾞｰﾀ連結名を求める
    ...
    gSL_Select = ""
    gSL_Select = gSL_Select & " Select * From   発注入荷状況問合ビュー "
...
' 抽出文　作成
    ...
    mSl_Select2 = mSl_Select2 & CisDBGridH.Select_Where
    mSl_Select2 = mSl_Select2 & CisDBGridH.Select_OrderBy
' グリッド内表示
...
''         Call .GridInitSet(, , "表示納入番号", "納入番号", 10)
...
Private Sub TotalDsp(JCol As Long, SelGrid As String)
        ...
        With DBGridH
            Select Case JCol
              Case -1
                   ...
                   wWhere = wWhereH
              Case 1, 2, 4, 6, 8, 9
                   ' 納入番号(1),品番(2),仕入先(4),手配区分(8),納入先(9),SK区分(10),置場(11),契約先(13)
        ...
        With DBGrid1
            Select Case JCol
              Case -1
                   ...
                   wWhereH = wWhere
              Case 1, 2, 4, 6, 11, 13
                   ' 納入番号(1),仕入先(2),品番(3),手配区分(15),納入先(17),SK区分(19),置場(20),契約先(22)
                   wMsg = .Columns(JCol).Caption
                   wValue = .Columns(JCol).Text
              ...
              Case 0            ' 納入日(4)
                   wMsg = .Columns(JCol).Caption
                   wValue = .Columns(JCol).Text
                   wWhere = "SUBSTRING(" & .Columns(JCol).DataField & ",1,10) = '" & CisFun.Mid2(RTrim(.Columns(JCol).Value), 1, 10) & "'"
                   wWhereH = "編集検収日 = '" & Mid(RTrim(.Columns(JCol).Value), 1, 10) & "'"
               Case Else
                    Exit Sub
            End Select
        End With
    ...
    If wWhereH <> "" Then
        ...
        Else
            gSL_Select = gSL_Select & CisDBGridH.Select_Where
        End If
        gSL_Select = Replace(gSL_Select, "入荷日", "検収日")
