...
Begin VB.Form CKN1260 
   ...
   Begin Cis3D_v60.CIS3D B1_Area2 
      ...
      cFont3DColor2   =   8421504
      cBoderStyle     =   1
      Begin Cis3D_v60.CIS3D ChgDispBody 
         Height          =   735
         Left            =   45
         Top             =   45
         Width           =   1305
   ...
   Begin VB.PictureBox H1_Area1 
      ...
      Top             =   1725
      Width           =   9000
      Begin Cis3D_v60.CIS3D ChgDispHead 
         Height          =   780
         Left            =   60
         Top             =   60
         Width           =   1185
   ...
   Begin Cis3D_v60.CIS3D B2_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D35 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№ |納入先 |納入先名　　　　　　|受入|品　番　　　　　　　　　|背番号|収容器   |収容数  |発行枚数||"
            Rows            =   11
            Cols            =   11
            BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/08/18  By CIS SIMPR-A
'**       変  更  日    :   2005/09/05  By CIS 下部グリッド納入先名修正(GridSet)
...
Private Sub PB_Back_Click()
    Call CisFrm.MousePT(11)
    gSL_Select = "Select HN.品名,HN.車型,HN.社内背番,"
    ...
    If PB_CHG.Tag = "" Then
        gSL_Select = gSL_Select & " Where HN.社内背番 + HT.取引先 + HT.受入 + HT.品番 < '"
    Else
    ...
    gSL_Select = gSL_Select & " And 品目 = 0 and SK区分 = 0 and HT.背番号 <> '' "
    If PB_CHG.Tag = "" Then
       gSL_Select = gSL_Select & " ORDER BY HN.社内背番 + HT.取引先 + HT.受入 + HT.品番 Desc "
    Else
...
Private Sub PB_Next_Click()
    ...
    gSL_Select = gSL_Select & " And 品目 = 0 and SK区分 = 0 and HT.背番号 <> '' "
    If PB_CHG.Tag = "" Then
       gSL_Select = gSL_Select & " ORDER BY HN.社内背番 + HT.取引先 + HT.受入 + HT.品番 "
    Else
...
Private Sub iH1_Seban_LostFocus()
    ...
    If Trim(iH1_Seban) <> "" Then
        If PB_CHG.Tag = "" Then
            gSL_Select = "Select HT.取引先,HT.受入,HT.品番 From 品番取引先マスタ HT "
            gSL_Select = gSL_Select & " Left Outer Join 品番マスタ HN "
            gSL_Select = gSL_Select & "    ON  HN.品番 = HT.品番 "
            gSL_Select = gSL_Select & " Where HN.社内背番 = '" & iH1_Seban & "'"
            gSL_Select = gSL_Select & "   And HT.品目 = 0"
            gSL_Select = gSL_Select & "   And HT.SK区分 = 0"
            gSL_Select = gSL_Select & "   And HT.背番号 <> '' "
        Else
...
Private Function Head1Chk() As Boolean
    ...
    If PB_CHG.Tag = "" Then
        ...
        gSL_Select = gSL_Select & " And   HN.社内背番  = '" & RTrim(iH1_Seban) & "'"
        gSL_Select = gSL_Select & " And   HT.背番号 <> ''"
    Else
...
Private Sub BodySet(SetType As String)
    If HTM.品目 <> 0 Or HTM.SK区分 <> 0 Then Exit Sub
...
'
    ...
    ' 最終発行連番を求める
        ...
        gSL_Select = gSL_Select & " Where かんばん種類 = 3"
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 7, 1, "納入先")
        Call .FixedSet(中中, 左中, あり, 20, 2, "納入先名")
