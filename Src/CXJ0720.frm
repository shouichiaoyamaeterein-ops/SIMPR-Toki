...
Begin VB.Form CXJ0720 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "受注テキスト変換状況問合せ"
   ClientHeight    =   10845
   ClientLeft      =   1680
   ClientTop       =   1695
   ClientWidth     =   15270
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CXJ0720.frx":0000
         Height          =   8580
         Left            =   150
         OleObjectBlob   =   "CXJ0720.frx":0015
         TabIndex        =   6
         Top             =   90
         Width           =   14460
      End
...
' ADOｺﾝﾄﾛｰﾙ　接続情報　セット
    ...
    gSL_Select = "SELECT Max(処理日付) 処理日付 From テキスト変換履歴テーブル "
    If TRIRead(gSL_Select, 1) Then
        If TRI.処理日付 > "20000101" Then
           iH1_SDate = TRI.処理日付
        End If
    End If
    If iH1_SDate = "" Then
...
'       日付
    ...
    gSL_Select = gSL_Select & " TH.重複 + TH.ERR収容数 + TH.ERR背番号 + TH.ERR受注 ERR件数,TH.処理名称 "
    gSL_Select = gSL_Select & " FROM テキスト変換履歴テーブル TH "
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 名称マスタ SY "
    gSL_Select = gSL_Select & "   ON  SY.区分名称 = 'テキスト変換エラー内容' "
    gSL_Select = gSL_Select & "   And SY.区分タイプ = '' "
    gSL_Select = gSL_Select & "   And SY.値 = TH.ERR "
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 名称マスタ SY2 "
    gSL_Select = gSL_Select & "   ON  SY2.区分名称 = 'テキスト変換処理内容' "
...
Private Sub ReProc()
    ...
    If Text_Proc = 0 Then
       If mEXE_Flg = 0 Then
          wProcID = "PXJ0710.exe"           '愛知機械
       Else
          If mEXE_Flg = 1 Then
             wProcID = "PXJ0730.exe"           'ﾃｸﾉｴｲﾄ
          Else
             wProcID = "PXJ0750.exe"           'ﾖﾛｽﾞ愛知
          End If
       End If
    Else
       '出荷再変換
    ...
    With CisFun
        ...
        .ShellAplPara = "/R" & Format(Text_Rireki, "00000000")
        .ShellTask Freeze
    End With
...
Private Sub ReProcSave()
    ...
    If Not IsNull(Adodc1.Recordset("処理名称")) Then
       If Adodc1.Recordset("処理名称") = "愛知機械受注データ変換" Then
          mEXE_Flg = 0
       Else
          If Adodc1.Recordset("処理名称") = "ﾃｸﾉｴｲﾄ受注データ変換" Then
             mEXE_Flg = 1
          Else
             mEXE_Flg = 2
          End If
    ...
    If Not IsNull(Adodc1.Recordset("受注出荷区分")) Then
       Text_Proc = Adodc1.Recordset("受注出荷区分")
       If Text_Proc > 1 Then Text_Proc = Text_Proc - 2
       Text_ProcYMD = Adodc1.Recordset("処理日付")
       Text_ProcCNT = Adodc1.Recordset("処理回数")
    ...
    If Not IsNull(Adodc1.Recordset("履歴管理番号")) Then
       If Adodc1.Recordset("履歴管理番号") <> "" Then
          Text_Rireki = Adodc1.Recordset("履歴管理番号")
       End If
    End If
...
' テキスト変換ｴﾗｰ内容
    ...
    With SYMCreate
        ...
        If Not SYMInitCreate(True) Then
            ...
            .値 = "10": .値名称 = "テキスト変換端末接続ｴﾗｰ": Call SYMInitCreate
            .値 = "11": .値名称 = "テキスト変換退避先取得ｴﾗｰ": Call SYMInitCreate
            .値 = "12": .値名称 = "退避先作成ｴﾗｰ(日付)": Call SYMInitCreate
            .値 = "13": .値名称 = "退避先作成ｴﾗｰ(回数)": Call SYMInitCreate
            .値 = "20": .値名称 = "テキスト変換TEXT移動ｴﾗｰ": Call SYMInitCreate
            .値 = "21": .値名称 = "テキスト変換ﾌｧｲﾙ移動ｴﾗｰ": Call SYMInitCreate
            .値 = "31": .値名称 = "テキスト変換ﾊﾞｯｸｱｯﾌﾟ削除ｴﾗｰ": Call SYMInitCreate
            .値 = "32": .値名称 = "テキスト変換変換履歴削除ｴﾗｰ": Call SYMInitCreate
            .値 = "33": .値名称 = "テキスト変換変換削除ｴﾗｰ": Call SYMInitCreate
            .値 = "41": .値名称 = "テキスト変換ﾃｷｽﾄｲﾝﾎﾟｰﾄｴﾗｰ": Call SYMInitCreate
            .値 = "42": .値名称 = "テキスト変換変換削除ｴﾗｰ": Call SYMInitCreate
            .値 = "43": .値名称 = "テキスト変換ｲﾝﾎﾟｰﾄ削除ｴﾗｰ": Call SYMInitCreate
            .値 = "44": .値名称 = "テキスト変換ｲﾝﾎﾟｰﾄ処理ｴﾗｰ": Call SYMInitCreate
            .値 = "45": .値名称 = "テキスト変換TEXT変換ｴﾗｰ": Call SYMInitCreate
            .値 = "46": .値名称 = "ｲﾝﾎﾟｰﾄ対象ﾌｧｲﾙなし": Call SYMInitCreate
            .値 = "51": .値名称 = "テキスト変換更新ｴﾗｰ": Call SYMInitCreate
            .値 = "52": .値名称 = "テキスト変換受注累積ｴﾗｰ": Call SYMInitCreate
            .値 = "53": .値名称 = "テキスト変換分納更新ｴﾗｰ": Call SYMInitCreate
