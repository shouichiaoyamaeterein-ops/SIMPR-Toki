...
'**       変  更  日    :   2007/10/03  By CIS 実績更新時(中)に、終了が効いて止まってしまう
'**       変  更  日    :
'**
'**********************************************************'
        ...
        For gLong = 1 To .Rows - 1
            If Trim(.TextMatrix(gLong, 1)) = "完納" Then
               With CisDB
                    ...
                    .SQL = "UPDATE 発注帳票発行テーブル SET "
                    .SQL = .SQL & " 納入日 = 入荷日,"
                    .SQL = .SQL & " 納入便 = 入荷便"
                    .SQL = .SQL & "   From 発注帳票発行テーブル NY"
                    .SQL = .SQL & "          Left Outer Join 発注テーブル HC"
                    .SQL = .SQL & "            On HC.納品書管理NO = NY.納品書管理NO"
                    .SQL = .SQL & "            And HC.納入番号 = NY.納入番号"
                    .SQL = .SQL & "            And HC.納品書頁 = NY.頁NO"
                    .SQL = .SQL & "         Left Outer Join 入荷テーブル NC"
...
'------------------------------------------------------------------ 2005/08/31 Insert End
            ...
            iH2_NYmd = typQRAfter.納入日
            iH2_Nouno = typQRAfter.納入番号
            iH2_NPage = typQRAfter.ページNO
            ProcHB = "H2"
