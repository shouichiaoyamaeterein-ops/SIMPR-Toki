...
'    If Not CXHZ0240LPrint Then Exit Sub
'
'*   作成日  :2008/12/16   By CIS
'*   変更日  :
'*---------------------------------------------------------------------------------
Public CisPrtCXHZ0240        As New CISFormPrint
Public CisSimplePrint       As New CisSimplePrint
Public CXHZ0240_PrtCnt       As Long
Global CXHZ0240_PrtOutKb     As Byte

Public Function CXHZ0240LPrint(HinKB As Byte, Optional PrintOut As Integer = 0) As Boolean
    CXHZ0240LPrint = False

    CXHZ0240_PrtOutKb = PrintOut
' 出力データ存在確認
    If HinKB = 0 Then
       gSL_Select = "SELECT NW.*,HN.表示寸法,HN.材種コード,HT.受給品番 品番,"
    Else
       gSL_Select = "SELECT NW.*,HN.表示寸法,HN.材種コード,HT.発注品番 品番,"
    End If
    ...
    CXHZ0240_PrtCnt = CisDB.RecordCount
    With CisSimplePrint
        .MaxCount = CisDB.RecordCount
        .ProgressInit
    End With
    
    If CXHZ0240_PrtOutKb <> 2 Then
        With CisPrtCXHZ0240
             Set .iPrtForm = CXHZ0240L
...
'+         CSVファイル出力  処理                        ＋
'+----------------------------------------------------------+
Public Function CXHZ0240_CsvOut() As Boolean
    CXHZ0240_CsvOut = False
    Dim cFNo           As Integer
...
'    pref_NCnt = 0                ' 現在発行ページ初期化
    On Error GoTo CXHZ0240_CsvOut_Err
'------------------------'
'    出力ﾃｷｽﾄ OPEN
'------------------------'
