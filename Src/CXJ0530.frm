   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |伝票種類|納 入 日 　-便|納入先 |   納  入  先  名   |受入|納入番号 |頁|受注区分|SK|"
            Rows            =   21
            Cols            =   17
            BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/03/29  By CIS
'**       変  更  日    :   2006/02/02  By CIS - INIファイル-クライアント対応（TEXT出力先）
'**       変  更  日    :
'**
'**********************************************************'
...
'-----------------------------------------------< 2006/02/02 ADD START >
            ...
            wPrintKB = 1                ' 帳票区分：0=54 Else 1=154
            Call CXJ0150TFile(wPrintKB)
...
'
    Select Case H1Chk_Ymd
        Case 1:    wYmdName = "受注日"
        Case Else: wYmdName = "納入日"
    End Select
'
    gSL_Select = ""
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 54"
...
'       <><><> 受領書 <><><>
        ...
        gSL_Select = gSL_Select & "   and ht6.受領書形態 = '2'"
        gSL_Select = gSL_Select & " group by"
        gSL_Select = gSL_Select & "  jy6.納入日"
        gSL_Select = gSL_Select & ", jy6.納入便"
        gSL_Select = gSL_Select & ", jy6.納入先"
...
'       <><><> 納品書 <><><>
        ...
        gSL_Select = gSL_Select & "   and ht7.納品書形態 = '2'"
        gSL_Select = gSL_Select & " group by"
        gSL_Select = gSL_Select & "  jy7.納入日"
        gSL_Select = gSL_Select & ", jy7.納入便"
        gSL_Select = gSL_Select & ", jy7.納入先"
...
'       <><><> 支給書 <><><>
        ...
        gSL_Select = gSL_Select & "   and ht8.納品書形態 = '2'"
...
'   >> 納入日／作成日
    ...
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(jyt." & wYmdName & ", 1, 6) = '" & iH1_SYmd & "'"
    Else
        YmdSta = "": BinSta = ""
        YmdEnd = "": BinEnd = ""
        '> 日付＆便　編集
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) = "" Then
            If Trim(BinSta) = "" Then
                gSL_Select = gSL_Select & "   and jyt." & wYmdName & " = '" & YmdSta & "'"
            Else
        ...
        If Trim(YmdSta) = "" And Trim(YmdEnd) <> "" Then
            If Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and jyt." & wYmdName & " <= '" & YmdEnd & "'"
            Else
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) <> "" Then
            If Trim(BinSta) = "" And Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and jyt." & wYmdName & " between '" & YmdSta & "'" _
                                                                       & " and '" & YmdEnd & "'"
            Else
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 154"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 0, 10, "X納品書管理NO")
        Call .FixedSet(中中, 左中, あり, 0, 11, "Xカード")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
