...
Begin VB.Form CNJ0040 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin vsFlexLib.vsFlexArray vsGrid1 
         ...
         EndProperty
         ConvInfo        =   1418783674
         FormatString    =   "№  |品番                  |手配先                |受入|今回計画数|前回計画数|計画差   |率  |今回内示数|前回内示数|内示差   |"
         Rows            =   28
         Cols            =   13
         BackColor       =   12648447
         ForeColorFixed  =   128
...
'   < 生産計画対象年月 >
    Call GetNJProc_YM(2)
    If RTrim$(gn_SKYM) = "" Then
        With CisFun
            .MB_Lines = 4
            .MB_MSG(2) = "    生産計画対象年月を獲得出来ません"
            .MB_Button = Error
            .MBOX
            Call PB_END_Click
            Exit Sub
...
'   ( Select 文 作成 )
    ...
    Call SKKRead(gSL_Select, 1)

    With vsGrid1
        
        .TextMatrix(.Row, 4) = Format(SKK.今回計画数, "#,###")
        .TextMatrix(.Row, 5) = Format(SKK.前回計画数, "#,###")
        .TextMatrix(.Row, 6) = Format(SKK.計画差, "#,###")
        
        If SKK.増減率 > 999 Then
            .TextMatrix(.Row, 7) = "999"
        Else
            .TextMatrix(.Row, 7) = Format(SKK.増減率, "#")
        End If
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 右中, あり, 10, 8, "今回内示数")
        Call .FixedSet(中中, 右中, あり, 10, 9, "前回内示数")
        Call .FixedSet(中中, 右中, あり, 9, 10, "内示差")
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not SKK_RDSTS
        ...
        With vsGrid1
            ...
            If RTrim$(SKK.表示品番) <> "" Then
                .TextMatrix(ID, 1) = RTrim$(SKK.表示品番)
            Else
                .TextMatrix(ID, 1) = RTrim$(SKK.品番)
            End If
            
            If RTrim$(SKK.手配先) = "" Then
                .TextMatrix(ID, 2) = ""
            Else
                .TextMatrix(ID, 2) = RTrim$(SKK.手配先) & Space(7 - Len(RTrim$(SKK.手配先))) & ":" & RTrim$(SKK.手配先名)
            End If
            .TextMatrix(ID, 3) = RTrim$(SKK.受入)
            .TextMatrix(ID, 4) = Format(SKK.今回計画数, "#,###")
            .TextMatrix(ID, 5) = Format(SKK.前回計画数, "#,###")
            .TextMatrix(ID, 6) = Format(SKK.計画差, "#,###")
            
            If SKK.増減率 > 999 Then
                .TextMatrix(ID, 7) = "999"
            Else
                .TextMatrix(ID, 7) = Format(SKK.増減率, "#")
            End If
        
            .TextMatrix(ID, 8) = Format(SKK.今回内示数, "#,###")
            .TextMatrix(ID, 9) = Format(SKK.前回内示数, "#,###")
            .TextMatrix(ID, 10) = Format(SKK.内示差, "#,###")
            .TextMatrix(ID, 11) = RTrim$(SKK.品番)
        End With
...
'       < 受入 >
    ...
    gSL_Select = gSL_Select & " ISNULL( SH.数量, 0 ) 今回内示数," & vbCrLf
    gSL_Select = gSL_Select & " ISNULL( HZ.数量, 0 ) 前回内示数," & vbCrLf
    gSL_Select = gSL_Select & " ISNULL( SH.数量, 0 ) - ISNULL( HZ.数量, 0 ) 内示差" & vbCrLf

'[ 読込基 ( 生産計画テーブル ) ]
    gSL_Select = gSL_Select & " FROM ( SELECT 内示年月, 品番, 手配先, 受入," & vbCrLf
    gSL_Select = gSL_Select & "                          SUBSTRING( 納期, 1, 6 ) 対象年月, SUM(数量) 数量" & vbCrLf
    gSL_Select = gSL_Select & "                          FROM 生産計画テーブル" & vbCrLf
    gSL_Select = gSL_Select & "                          WHERE 内示年月 = '" & gn_SKYM & "'" & vbCrLf
...
'[ 生産計画前回テーブル ]
    ...
    gSL_Select = gSL_Select & "                          SUBSTRING( 納期, 1, 6 ) 対象年月, SUM(数量) 数量" & vbCrLf
    gSL_Select = gSL_Select & "                          FROM 生産計画前回テーブル" & vbCrLf
    gSL_Select = gSL_Select & "                          WHERE 内示年月 = '" & gn_SKYM & "'" & vbCrLf
    gSL_Select = gSL_Select & "                          AND   SUBSTRING( 納期, 1, 6 ) = '" & iH1_YM & "'" & vbCrLf
    gSL_Select = gSL_Select & sw_Hinbn & sw_Torcd & sw_Ukeir
    gSL_Select = gSL_Select & "                          GROUP BY 内示年月, 品番, 手配先, 受入, SUBSTRING( 納期, 1, 6 )" & vbCrLf
...
'[ 生産計画必要数テーブル ]
    gSL_Select = gSL_Select & " LEFT OUTER JOIN ( SELECT 内示年月, 品番, 手配先, 受入," & vbCrLf
    gSL_Select = gSL_Select & "                          SUBSTRING( 納期, 1, 6 ) 対象年月, SUM(数量) 数量" & vbCrLf
    gSL_Select = gSL_Select & "                          FROM 生産計画必要数テーブル" & vbCrLf
    gSL_Select = gSL_Select & "                          WHERE 内示年月 = '" & gn_SKYM & "'" & vbCrLf
    gSL_Select = gSL_Select & "                          AND   SUBSTRING( 納期, 1, 6 ) = '" & iH1_YM & "'" & vbCrLf
    gSL_Select = gSL_Select & sw_Hinbn & sw_Torcd & sw_Ukeir
    gSL_Select = gSL_Select & "                          GROUP BY 内示年月, 品番, 手配先, 受入, SUBSTRING( 納期, 1, 6 )" & vbCrLf
...
'[ 生産計画必要数前回テーブル ]
    gSL_Select = gSL_Select & " LEFT OUTER JOIN ( SELECT 内示年月, 品番, 手配先, 受入," & vbCrLf
    gSL_Select = gSL_Select & "                          SUBSTRING( 納期, 1, 6 ) 対象年月, SUM(数量) 数量" & vbCrLf
    gSL_Select = gSL_Select & "                          FROM 生産計画必要数前回テーブル" & vbCrLf
    gSL_Select = gSL_Select & "                          WHERE 内示年月 = '" & gn_SKYM & "'" & vbCrLf
    gSL_Select = gSL_Select & "                          AND   SUBSTRING( 納期, 1, 6 ) = '" & iH1_YM & "'" & vbCrLf
    gSL_Select = gSL_Select & sw_Hinbn & sw_Torcd & sw_Ukeir
    gSL_Select = gSL_Select & "                          GROUP BY 内示年月, 品番, 手配先, 受入, SUBSTRING( 納期, 1, 6 )" & vbCrLf
