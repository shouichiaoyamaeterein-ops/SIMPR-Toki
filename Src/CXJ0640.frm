...
Begin VB.Form CXJ0640 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "受注納品書入力 [千代田/秋田]"
   ClientHeight    =   10980
   ClientLeft      =   2070
   ClientTop       =   1650
   ClientWidth     =   15330
...
'**       処理概要      :
'**
'**       作  成  日    :   2009/09/07  By CIS SIMPR-A
'**       変  更  日    :   2009/12/22  By CIS 分納伝票対応
'**                         2010/08/26  By CIS 端数発生時の枚数計算修正
'**********************************************************'
...
'   #------------------#
    ...
    B2_Area1.Left = -1200   ' 2009.09.04 upd : 1000→1200
...
Private Sub iB12_Hinbn_GotFocus(Index As Integer)
    ...
    PB_Look.Visible = True
    If ProcHB = "H1" Then       ' 2009.09.07 add
        PB_Look.Visible = False ' 2009.09.07 add
        PB_Look.Tag = ""        ' 2009.09.07 add
    End If                      ' 2009.09.07 add
End Sub
'*---------------------------------------------------*
...
Private Sub iB12_Hako_LostFocus(Index As Integer)
    Dim dNumL As Double                                                     ' 2009.09.10 add
...
'   * 納入数計算 *
    ...
    dNumL = CisFun.Val2(iB12_Hako(Index)) * CisFun.Val2(iB12_Syuyo(Index))  ' 2009.09.10 upd : wNumL-->dNumL
    iB12_Su(Index) = Format(Format(dNumL, "#"), String(10, "@"))            ' 2009.09.10 upd : 7-->10 : wNumL-->dNumL
End Sub
'*---------------------------------------------------*
...
'   * 省略時 １ *
    If CisFun.Val2(iB12_Syuyo(Index)) = 0 Then
       ...
       Else
          iB12_Syuyo(Index) = Format(Format(CisFun.Val2(iB12_Su(Index)), "#"), String(6, "@"))  ' 2009.09.10 upd : String(5 --> String(6
       End If
    End If
'   * 納入数計算 *
    If CisFun.Val2(iB12_Su(Index)) <> 0 Then Exit Sub
...
'
   ...
   'PB_ADD.Visible = H1Mode     ' 2009.09.07 del
   'PB_RED.Visible = H1Mode     ' 2009.09.07 del
'-------------------------------' 2009.09.07 add start
    If SyoriKb = "ADD" Then
        PB_ADD.Visible = False
...
'   * 受注区分 *
    If ProcHB = "H1" Then
        ...
        ProcHB = "B1"
        Call DispChange(ProcHB)
        iB1_NTorcd.SetFocus         ' 2009.09.04 add
        GoTo ReturnPress_Ed
    End If
...
'       * 品番 *
        If RTrim(iB12_Hinbn(wNum1)) = "" Then
            wcc_Body = wcc_Body & Space(25)
        Else
            wcc_Body = wcc_Body & Format(RTrim(iB12_Hinbn(wNum1)), "!" & String(25, "@"))
            BodySetFlg = True
            ' 明細行数
...
'       * 背番号*
        If RTrim(iB12_Seban(wNum1)) = "" Then
            wcc_Body = wcc_Body & Space(8)
        Else
            wcc_Body = wcc_Body & Format(RTrim(iB12_Seban(wNum1)), "!@@@@@@@@")
            BodySetFlg = True
        End If
'       * 収容数 *
        wcc_Body = wcc_Body & Format(CisFun.Val2(iB12_Syuyo(wNum1)), "000000")

'       * 箱数 *
        wcc_Body = wcc_Body & Format(CisFun.Val2(iB12_Hako(wNum1)), "00000")

'       * 納入数 *
        wcc_Body = wcc_Body & Format(CisFun.Val2(iB12_Su(wNum1)), "0000000000")     ' 2009.09.10 upd : 10桁にした
    
        If RTrim(iB12_Hinbn(wNum1)) = "" And RTrim(iB12_Seban(wNum1)) = "" Then
           If CisFun.Val2(iB12_Syuyo(wNum1)) <> 0 Or CisFun.Val2(iB12_Hako(wNum1)) <> 0 Or CisFun.Val2(iB12_Su(wNum1)) <> 0 Then
                With CisFun
                     .MB_Lines = 5
                     .MB_MSG(2) = "  品番/背番号を入力して下さい      "
                     .MB_Title = ""
                     .MB_Button = Error
                     .MBOX
                End With
                iB12_Hinbn(wNum1).SetFocus
                Exit Function
           End If
        End If

    Next wNum1
'Call MsgBox("-->" & wcc_Body & "<--")   ' 2009.090.07：確認用
'
    ...
    End If
'   * ヘッド部作成 *
    If Trim(iB1_NTorcd) = "" Then           ' 納入先：5桁（取引先コード）*ストアドのほうに合わせて6桁で作成
       wcc_Head = Space(6)
    Else
       wcc_Head = Format(RTrim(iB1_NTorcd), "!@@@@@@")
    End If
    If Trim(iB1_NUkeir) = "" Then           ' 納事業所：2桁（受入）
       wcc_Head = wcc_Head & Space(2)
    Else
        wcc_Head = wcc_Head & Format(iB1_NUkeir, "!@@")
    End If
    If Trim(iB1_NSYmd) = "" Then            ' 納入指示日：8桁
       wcc_Head = wcc_Head & Space(8)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_NSYmd), "!@@@@@@@@")
    End If
    If Trim(iB1_NBin) = "" Then             ' 納入指示便：2桁
       wcc_Head = wcc_Head & Space(2)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_NBin), "00")
    End If
    If Trim(iB1_Kibn) = "" Then             ' 機番：3桁
       wcc_Head = wcc_Head & Space(3)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_Kibn), "!@@@")
    End If
    If Trim(iB1_Sharp) = "" Then            ' カード丼：1桁
       wcc_Head = wcc_Head & Space(1)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_Sharp), "@")
    End If
    If Trim(iB1_STorcd) = "" Then           ' 支給元：5桁
        wcc_Head = wcc_Head & Space(5)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_STorcd), "!@@@@@")
    End If
    If Trim(iB1_JTorcd) = "" Then           ' 受注者：5桁
       wcc_Head = wcc_Head & Space(5)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_JTorcd), "!@@@@@")
    End If
    If Trim(iB1_JKojyo) = "" Then           ' 受事業所：2桁
       wcc_Head = wcc_Head & Space(2)
    Else
       wcc_Head = wcc_Head & Format(RTrim(iB1_JKojyo), "@@")
    End If
    If Trim(iB1_Noubn) = "" Then            ' 納入番号：5桁
       wcc_Head = wcc_Head & Space(5)
    Else
   ...
   'wcc_Head = wcc_Head & Format(CisFun.Val2(iB1_PageE.Tag), "0")       ' 発注区分   ' 2009.09.04 del
'Call MsgBox("-->" & wcc_Head & "<--")   ' 2009.090.07：確認用
...
Private Function ChekData(ReadType As Integer) As Boolean
    ...
    gStr = wcc_Body
    wcc_Body = Mid(gStr, 1, 270)        ' 2009.09.10 upd : 185→270 54*5=270    ' 37 * 5
    wcc_Body2 = Mid(gStr, 271)          ' 2009.09.07 upd : 186→271
'Call MsgBox("-->" & wcc_Body & "<--")  ' 2009.09.07：確認用
'Call MsgBox("-->" & wcc_Body2 & "<--") ' 2009.09.07：確認用
'Call Test_LogOutput(wcc_Body)          ' 2009.09.10：確認用
'Call Test_LogOutput(wcc_Body2)         ' 2009.09.10：確認用

    sv_Uke2 = ""
    sv_NOTor = ""

    With CisDB
        .SQL = "受注SIM納品書チェック"      ' 2009.09.07 upd : 受注JAMAチェック
        .StoadoCount = 11
...
'.ParaNo = 1: .ParaValue = 0    ' 確認試験用
        ...
        ' 明細1-5
        .ParaNo = 6: wStr1 = Mid(.ParaValue, 1, 270)    ' 2009.09.07 upd : Mid(.ParaValue, 1, 185)
        ' 明細6-10
...
'   * 画面再表示 *
    Call HeadBodyClear("B")
'
'-------------------------------------------------------' 2009.09.07 upd start : 各データの位置を変更
    
    iB1_NTorcd = Mid(wcc_Head, 1, 5)
    iB1_NUkeir = Mid(wcc_Head, 7, 2)
    iB1_NSYmd = Mid(wcc_Head, 9, 8)
    iB1_NBin = Mid(wcc_Head, 17, 2)
    iB1_Kibn = Mid(wcc_Head, 19, 3)
    iB1_Sharp = Mid(wcc_Head, 22, 1)
    iB1_STorcd = Mid(wcc_Head, 23, 5)
    iB1_JTorcd = Mid(wcc_Head, 28, 5)
    iB1_JKojyo = Mid(wcc_Head, 33, 2)
    iB1_Noubn = Mid(wcc_Head, 35, 5)
    iB1_PageS = CisFun.RSetFld(Mid(wcc_Head, 40, 2), 0, iB1_PageS.cFormat)
    iB1_PageE = CisFun.RSetFld(Mid(wcc_Head, 42, 2), 0, iB1_PageE.cFormat)
    iB1_PageS.Tag = CisFun.Val2(Mid(wcc_Head, 44, 2))    ' 行数
    
    For wNum1 = 0 To 9
        wNum2 = wNum1 * 54                              ' 2009.09.04 upd : 37-->54
'Call MsgBox(Mid(wStr1, wNum2 + 1, 54))  ' 確認用
        '''If RTrim(Mid(wStr1, wNum2 + 1, 33)) = "" Then Exit For

        iB12_Hinbn(wNum1) = RTrim(Mid(wStr1, wNum2 + 1, 25))
        iB12_Seban(wNum1) = RTrim(Mid(wStr1, wNum2 + 26, 8))
        iB12_Syuyo(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 34, 6), 0, iB12_Syuyo(wNum1).cFormat)
        iB12_Hako(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 40, 5), 0, iB12_Hako(wNum1).cFormat)
        iB12_Su(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 45, 10), 0, iB12_Su(wNum1).cFormat)
...
'            If Not DBSet_JNW(wIndex) Then GoTo DBPut_Err
        '   >> 出力
'            Call JNWInsert
            If Right(RTrim(iB1_Noubn), 1) = "0" Then
               If Not DBSet_JNW(wIndex) Then GoTo DBPut_Err
               Call JNWInsert
            Else
               If Not DBSet_JSB(wIndex) Then GoTo DBPut_Err
               Call JSBInsert
            End If
            '======================== 2009/12/22 End
        End If
...
'    End With
    With JNW
        ...
        .出荷予定便 = .納入便
        .受入 = RTrim(sv_Uke2)
        .機番 = RTrim(Mid(iB1_Kibn, 1, 3))  ' 2009.09.10 upd : 2桁-->3桁
        .納入番号 = RTrim(iB1_Noubn)
        .納品書頁 = CisFun.Val2(iB1_PageS)
        .納品書行 = Index + 1
'        .品番 = RTrim(iB12_Hinbn(Index))
        gSL_Select = "SELECT 品番 FROM 品番マスタ "
        gSL_Select = gSL_Select & " WHERE REPLACE(品番,'-','') = '" & Replace(RTrim(iB12_Hinbn(Index)), "-", "") & "'"
        ...
        If .収容数 <> 0 Then
           ...
           End If
        End If
'        If .端数 <> 0 Then .枚数 = .枚数 + 1           '2010/08/26
        Select Case .受注区分
            Case 2, 4, 5, 6, 7, 8
                .エフ発行枚数 = .枚数
                .エフ発行端数 = .端数
...
'+      セット - 受注SIM分納伝票ワーク
'+------------------------------------+
Private Function DBSet_JSB(Index As Integer)
    DBSet_JSB = False
...
OcrProc_Err:
    Call CisFun.ErrorBox("SIM QR読取")      ' 2009.09.14 upd : JAMA-->SIM
    End
End Function
'+------------------------------+
'+  読取処理     *
...
Private Sub PDQComm1_OnComm()
    ...
    If Mid(QT10QSR_ReadData, 1, 7) <> "SIMPR-A" Then
        With CisFun
             .MB_Lines = 5
             .MB_MSG(2) = "  対象外伝票です      "
             .MB_Title = ""
        ...
        Exit Sub
    End If
    If Mid(QT10QSR_ReadData, 8, 4) <> "4202" And Mid(QT10QSR_ReadData, 8, 4) <> "4204" Then
        With CisFun
             .MB_Lines = 5
             .MB_MSG(2) = "  対象外伝票です      "
             .MB_Title = ""
...
'           wcc_Head = Space(5)
'        Else
'           wcc_Head = Format(RTrim(.納入先CD), "!@@@@@")
'        End If
'        If Trim(.納入先工場CD) = "" Then                                 ' 2009.09.04 del start
'           'wcc_Head = wcc_Head & Space(2)
'           wcc_Head = wcc_Head & Space(1)
'        Else
'           'wcc_Head = wcc_Head & Format(RTrim(.納入先工場CD), "!@@")
'           wcc_Head = wcc_Head & Format(RTrim(.納入先工場CD), "!@")
'        End If                                                         ' 2009.09.04 del end
'        'wcc_Head = wcc_Head & Space(1)                                  ' 2009.09.09 add
        
        If Trim(.納入先CD) = "" Then
           wcc_Head = Space(6)
        Else
           wcc_Head = Format(RTrim(.納入先CD) & Trim(.納入先工場CD), "!@@@@@@")
        End If
        'wcc_Head = wcc_Head & Space(1)                                  ' 2009.09.09 add
        ...
        End If
        If Trim(.契約先CD) = "" Then                                    ' 2009.09.04 upd : If Trim(.支給元体系) = "" Then
            wcc_Head = wcc_Head & Space(5)
        Else
            wcc_Head = wcc_Head & Format(RTrim(.契約先CD), "!@@@@@")    ' 2009.09.04 upd : Format(RTrim(.支給元体系), "!@@@@")
        End If
        If Trim(.仕入先CD) = "" Then
            wcc_Head = wcc_Head & Space(5)
        Else
            wcc_Head = wcc_Head & Format(RTrim(.仕入先CD), "!@@@@@")
        End If
        If Trim(.仕入先工場CD) = "" Then                                ' 2009.09.04 upd : If Trim(.仕入先工区) = "" Then
            wcc_Head = wcc_Head & Space(2)
        Else
            wcc_Head = wcc_Head & Format(RTrim(.仕入先工場CD), "@@")
        End If
        If Trim(.納入番号) = "" Then
            wcc_Head = wcc_Head & Space(5)
        Else
        ...
        End If
'        wcc_Head = wcc_Head & Format(CisFun.Val2(.物理現頁数), "00")    ' 2009.09.04 upd : Format(CisFun.Val2(.ページNO), "00")
'        wcc_Head = wcc_Head & Format(CisFun.Val2(.物理総頁数), "00")    ' 2009.09.04 upd : Format(CisFun.Val2(.総ページ数), "00")
'        wcc_Head = wcc_Head & Format(CisFun.Val2(.総頁数), "00")        ' 2009.09.04 upd : 明細行 Format(CisFun.Val2(.明細行数), "00")

        wcc_Head = wcc_Head & Format(CisFun.Val2(.頁NO), "00")     ' 2009.09.04 upd : Format(CisFun.Val2(.ページNO), "00")
        wcc_Head = wcc_Head & Format(CisFun.Val2(.総頁数), "00")     ' 2009.09.04 upd : Format(CisFun.Val2(.総ページ数), "00")
        wcc_Head = wcc_Head & Format(CisFun.Val2(.総頁数), "00")        ' 2009.09.04 upd : 明細行 Format(CisFun.Val2(.明細行数), "00")

'        wcc_Head = wcc_Head & Format(CisFun.Val2(.発注区分), "0")      ' 2009.09.04 del : 発注区分
'
    ''   * ボディ部作成 *
        wcc_Body = "": wcc_Body2 = ""
        For wNum1 = 1 To 10
    '       * 品番 *
            If RTrim(.品番(wNum1)) = "" Then
                wcc_Body = wcc_Body & Space(25)
            Else
                wcc_Body = wcc_Body & Format(Mid(.品番(wNum1), 1, 25), "!" & String(25, "@"))
            End If
    ...
    '       * 背番号*
            If RTrim(.背番号(wNum1)) = "" Then
                wcc_Body = wcc_Body & Space(8)
            Else
                wcc_Body = wcc_Body & Format(Mid(.背番号(wNum1), 1, 8), "!@@@@@@@@")
            End If
    '       * 収容数 *
            wcc_Body = wcc_Body & Format(CisFun.Val2(.収容数(wNum1)), "000000")

    '       * 箱数 *
            wcc_Body = wcc_Body & Format(CisFun.Val2(.箱数(wNum1)), "00000")

    '       * 納入数 *
            wcc_Body = wcc_Body & Format(CisFun.Val2(.納入数(wNum1)), "0000000000")

        Next wNum1
    End With
'wNum1 = Len(wcc_Body): Call MsgBox(Str(wNum1) + vbCrLf + wcc_Body)  ' (確認用)
'
    If ChekData(0) Then
       H1lb_OCRCnt = Format(CisFun.Val2(H1lb_OCRCnt) + 1, "#,###")
       DoEvents
...
'================================= 2009/12/22 Start
    gSL_Select = "SELECT * FROM 受注SIM伝票分納ワーク "
    gSL_Select = gSL_Select & " WHERE 端末番号 = HOST_NAME()"
    If JSBRead(gSL_Select, 1) Then
    '分納伝票リスト出力先確認
        With CisSimplePrint
             .PrintTitle = "分納伝票リスト"
             .SaveKey = gRegKey
             .Preservation = "CXJ0640"
    ...
    '分納伝票読取リスト
        If Not CXJ0645LPrint(PrtOut) Then Exit Sub
...
'分納データ削除
    gSL_Select = "DELETE FROM 受注SIM伝票分納ワーク"
    gSL_Select = gSL_Select & " WHERE 端末番号 = HOST_NAME()"
    With CisDB
        .DBTran (TransBegin) ' ﾄﾗﾝｻﾞｸｼｮﾝ開始
        .SQL = gSL_Select
