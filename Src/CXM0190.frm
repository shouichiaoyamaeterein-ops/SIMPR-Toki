   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D6 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "履歴№|単  価      |仮単|適用開始日  |適用終了日  |有無償|||||"
            Rows            =   11
            Cols            =   24
            BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2009/09/28  By CIS SIMPR-A
'**       変  更  日    :
'**
'**********************************************************'
    ...
    Dim Hinku_top(3)   As Integer       ' 品番表示位置      ' 2009.09.28 add
...
'   #    初期入力設定   #
'   #------------------#
    ProcHB = "H0"                               ' 2009.09.28 upd : "H1"-->"H0"
    Call DispChange(ProcHB)
'-----------------------------------------------' 2009.09.28 add start
...
'   #  処理区分初期表示 #
'   #------------------#
'    SyoriKB = "ADD"                            ' 2009.09.28 del
'    Call SyoriName(SyoriKB)                    ' 2009.09.28 del
'   #-------------------#
'   # 画面中央位置表示   #
'   #-------------------#
...
''''
    ...
    If H1Op_Hinku(3).Value = True Then
      'iH1_Hinbn.Visible = False
       H1_ZaiBox.Top = 540 '930 : 930:試験用 / 540:正しい位置
    ...
    If iH1_Hinbn.Tag = "1" Then     ' 2009.09.29 add
       If H1Op_Hinku(3).Value = True Then
          Call iH1_Zaist.SetFocus
       Else
          Call iH1_Hinbn.SetFocus   ' 2009.09.29 add
       End If
    ElseIf iH1_Torcd.Tag = "1" Then ' 2009.09.29 add
        Call iH1_Torcd.SetFocus     ' 2009.09.29 add
    End If                          ' 2009.09.29 add
End Sub
'+-------------------------+
'+  H0Op_Tanku             +
'+-------------------------+
...
Private Sub PB_ADD_Click()
    If SyoriKB <> "ADD" Then
        ...
        Call SyoriName(SyoriKB)
    End If
    If ProcHB = "H1" Then iH1_Hinbn.SetFocus    ' 2009.09.28 upd : if文を追加（以前は iH1_Hinbn.SetFocus のみ）
End Sub
'+----------------------------+
'+    修  正  キ ー(F2)       +
'+----------------------------+
...
Private Sub PB_Back_Click()
    Dim flg As Boolean                      ' 2009.09.29 add
    Dim wsl As String                       ' 2009.09.30 add

    Call CisFrm.MousePT(11)

'---------------------------------------------------------------------------' 2009.09.29 add start
    gStr = RTrim(iH1_Torcd) & Space(7 - CisFun.Len2(RTrim(iH1_Torcd)))
    If H1Op_Hinku(3).Value = True Then
...
'---------------------------------------------------------------------------' 2009.09.29 add end
    ...
    If H0Op_Tanku(0).Value = True Then      ' 2009.09.29 add
        gSL_Select = " SELECT *"
        gSL_Select = gSL_Select & " FROM 支給単価マスタ "
        gSL_Select = gSL_Select & " WHERE 品番 + 取引先 < '" & RTrim(iH1_Hinbn) & iH1_Torcd & "'"
        gSL_Select = gSL_Select & " ORDER BY 品番 + 取引先 Desc "
        If Not STMRead(gSL_Select, 1) Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "     先頭レコードです            "
                .MB_Title = "【後退】"
        ...
        gSL_Select = "Select * FROM 支給単価マスタ "
        gSL_Select = gSL_Select & " WHERE 品番 = '" & RTrim(iH1_Hinbn) & "'"
        gSL_Select = gSL_Select & "   And 取引先 = '" & RTrim(iH1_Torcd) & "'"
'-------------------------------------------------------------------------------'2009.09.29 add start
    Else    ' 単価区分が材料の場合
       gSL_Select = ""
       gSL_Select = gSL_Select & " SELECT SM.*, HT.発注品番, HT.受給品番, HT.自社品番,"     ' , ISNULL(HT.背番号, '') 背番号を外した
       gSL_Select = gSL_Select & "        ZM.材質, ZM.板厚, ZM.幅, ZM.長さ"
       gSL_Select = gSL_Select & "   FROM 材料支給単価マスタ SM"
       gSL_Select = gSL_Select & "        LEFT OUTER JOIN 材料マスタ ZM"
       gSL_Select = gSL_Select & "             ON ZM.材料管理番号 = SM.材料管理番号"
       gSL_Select = gSL_Select & "        LEFT OUTER JOIN 材料取引先マスタ HT"
       gSL_Select = gSL_Select & "             ON HT.材料管理番号 = SM.材料管理番号"
       gSL_Select = gSL_Select & "            AND HT.取引先 = SM.取引先"
''''   gSL_Select = gSL_Select & "            AND HT.品目 = 1"
'
       If H1Op_Hinku(0).Value Then          ' 発注品番
          gSL_Select = gSL_Select & " WHERE SM.取引先 + SPACE(7 - LEN(SM.取引先)) + "
          gSL_Select = gSL_Select & "       HT.発注品番 + SPACE(25 - LEN(HT.発注品番)) < '" & gStr & "'"
          gSL_Select = gSL_Select & " ORDER BY SM.取引先 + SPACE(7 - LEN(SM.取引先)) + "
...
'
       Else                                 ' 材質/寸法
          ...
          gSL_Select = gSL_Select & " ORDER BY SM.取引先 + SPACE( 7 - LEN(SM.取引先)) + " & wsl
       End If
       gSL_Select = gSL_Select & "       DESC"
'''''''Call Test_LogOutput(gSL_Select, 1)       ' SQL確認用
       
       If Not ZSMRead(gSL_Select, 1) Then
          With CisFun
              .MB_Lines = 4
              .MB_MSG(2) = "     先頭レコードです            "
              .MB_Title = "【後退】"
              ...
              .MBOX
          End With
          iH1_Hinbn.SetFocus                    ' 2009.09.29 add
          Call CisFrm.MousePT(1)
          Exit Sub
       End If
       
       H1lb_Hinbn(0).Caption = ZSM.発注品番
       H1lb_Hinbn(1).Caption = ZSM.受給品番
       H1lb_Hinbn(2).Caption = ZSM.自社品番
       H1lb_Hinbn(3).Caption = RTrim(ZSM.材質) & " " & _
                               Format(ZSM.板厚, "#,##0.00") & "X" & _
                               Format(ZSM.幅, "#,##0.00") & "X" & _
                               Format(ZSM.長さ, "#,##0.00")
       H1_ZaiRyo.Caption = RTrim(ZSM.材質) & Space(25 - Len(RTrim(ZSM.材質))) & "/" & _
                           Format(ZSM.板厚, "0000.00") & "x" & _
                           Format(ZSM.幅, "0000.00") & "x" & _
                           Format(ZSM.長さ, "0000.00")
       ...
       Call H1_ZaiBox_Set
       iH1_Torcd = ZSM.取引先
      'H1lb_Zairyo = RTrim(ZSM.材質) & " " & Format(ZSM.板厚, "#,##0.00") & "X" & Format(ZSM.幅, "#,##0.00") & "X" & Format(ZSM.長さ, "#,##0.00")
       H1lb_ZaiKan = RTrim(ZSM.材料管理番号)
       Call iH1_Torcd_LostFocus
       Call HinbnSet
    End If
'-------------------------------------------------------------------------------'2009.09.29 add end
    ...
    flg = DBInput("Back")               ' 2009.09.29 upd : Call DBInput("Back")
    Dummy.Enabled = True: Dummy.SetFocus
    iH1_Hinbn.SetFocus
    If flg = True Then                  ' 2009.09.29 add
       iH1_Torcd.SetFocus               ' 2009.09.29 add
    End If                              ' 2009.09.29 add
    Dummy.Enabled = False
    Call CisFrm.MousePT(1)
End Sub
...
'---------------------------------------------------------------------------' 2009.09.29 add end
    ...
    If H0Op_Tanku(0).Value = True Then      ' 2009.09.28 add
        gSL_Select = " SELECT *"
        gSL_Select = gSL_Select & " FROM 支給単価マスタ "
        gSL_Select = gSL_Select & " WHERE 品番 + 取引先 > '" & RTrim(iH1_Hinbn) & iH1_Torcd & "'"
...
''''''Call Test_LogOutput(gSL_Select, 1)       ' SQL確認用
       ...
       If Not ZSMRead(gSL_Select, 1) Then
          With CisFun
              .MB_Lines = 4
              .MB_MSG(2) = "     最終レコードです            "  ' 2009.09.29 upd : メッセージが間違っていたので修正
              .MB_Title = "【前進】"                            ' 2009.09.29 upd : メッセージが間違っていたので修正
              .MB_Button = OK
              .MBOX
          End With
          iH1_Hinbn.SetFocus                    ' 2009.09.29 add
...
'-------------------------------------------------------------------------------'2009.09.28 add end
    ...
    flg = DBInput("Next")               ' 2009.09.29 upd : Call DBInput("Next")
    Dummy.Enabled = True: Dummy.SetFocus
    iH1_Hinbn.SetFocus
    If flg = True Then                  ' 2009.09.29 add
       iH1_Torcd.SetFocus               ' 2009.09.29 add
...
'+     品番セット(品番のみ)     +
'+------------------------------+
Private Sub HinbnSet(Optional KB As Integer = 1)
    If H1lb_ZaiKan = "" And RTrim(iH1_Hinbn) = "" And KB = 0 Then Exit Sub
    If H1Op_Hinku(0).Value Then
       ...
       If RTrim(iH1_Hinbn) <> "" Then
          gSL_Select = gSL_Select & "  WHERE ZT.発注品番 = '" & RTrim(iH1_Hinbn) & "'"
       Else
          gSL_Select = gSL_Select & "  WHERE ZT.材料管理番号 = " & CisFun.Val2(RTrim(H1lb_ZaiKan))
       End If
''''       If ZRMRead(gSL_Select) Then
''''          If CisDB.RecordCount = 1 Then
''''             H1lb_HinBn = RTrim(ZRM.受給品番)
''''             H1lb_Zairyo = RTrim(ZRM.材質) & " " & _
''''                           Format(ZRM.板厚, "#,##0.00") & "X" & _
''''                           Format(ZRM.幅, "#,##0.00") & "X" & _
''''                           Format(ZRM.長さ, "#,##0.00")
''''             H1lb_ZaiKan = RTrim(ZRM.材料管理番号)
''''          End If
''''       Else
''''          H1lb_HinBn = ""
''''          H1lb_Zairyo = ""
...
''''       End If
    Else
       ...
       gSL_Select = gSL_Select & " ON  ZT.材料管理番号 = ZM.材料管理番号 "
       If RTrim(iH1_Hinbn) <> "" Then
          gSL_Select = gSL_Select & "  WHERE ZT.受給品番 = '" & RTrim(iH1_Hinbn) & "'"
       Else
          gSL_Select = gSL_Select & "  WHERE ZT.材料管理番号 = " & CisFun.Val2(RTrim(H1lb_ZaiKan))
       End If
''''       If ZRMRead(gSL_Select) Then
''''          If CisDB.RecordCount = 1 Then
''''             H1lb_HinBn = RTrim(ZRM.発注品番)
''''             H1lb_Zairyo = RTrim(ZRM.材質) & " " & _
''''                           Format(ZRM.板厚, "#,##0.00") & "X" & _
''''                           Format(ZRM.幅, "#,##0.00") & "X" & _
''''                           Format(ZRM.長さ, "#,##0.00")
...
Private Sub iH1_Hinbn_LostFocus()
    ...
    If H0Op_Tanku(1).Value Then
       Call HinbnSet(0) '-*-*-*-*-*-*-*-*-*-* 2008/02/29 ADD
    End If

    Call Chk_TnkMst     ' 2009.09.30 add (仮)
    iH1_Hinbn.Tag = "1" ' 2009.09.29 add
    iH1_Torcd.Tag = ""  ' 2009.09.29 add
End Sub
Private Sub Chk_TnkMst()                    ' 2009.09.30 add start (仮)
Exit Sub
    If H0Op_Tanku(1).Value = False Then Exit Sub
    For cnt = 0 To 3
        If Trim(H1lb_Hinbn(cnt).Caption) <> "" Then
           cnt = 99
           Exit For
        End If
    Next
    If Trim(iH1_Torcd.Text) <> "" And (Trim(iH1_Hinbn.Text) <> "" Or cnt = 99) Then
       Call DBInput("INV")
    End If
End Sub                                     ' 2009.09.30 add end   (仮)
...
Private Sub iH1_Torcd_LostFocus()
    ...
    Call Chk_TnkMst     ' 2009.09.30 add (仮)
    iH1_Hinbn.Tag = ""  ' 2009.09.29 add
    iH1_Torcd.Tag = "1" ' 2009.09.29 add
End Sub
'+-----------------------------+
'+  B1Ch_Kari
'+-----------------------------+
...
Private Function DispChange(Pro As String)
    Dim H0Mode      As Boolean          ' 2009.09.28 add
    Dim H0Color     As Long             ' 2009.09.28 add
    Dim H1Mode      As Boolean
    Dim H1Color     As Long
    Dim B1Mode      As Boolean
    Dim B1Color     As Long
    H0Mode = False                      ' 2009.09.28 add
    H0Color = gPLostSel                 ' 2009.09.28 add
    H1Mode = False
    H1Color = gPLostSel
    B1Mode = False
    B1Color = gPLostSel
...
'
    Select Case Pro
       Case "H0"                        ' 2009.09.28 add
          H0Mode = True                 ' 2009.09.28 add
          H0Color = gPGotSel            ' 2009.09.28 add
       Case "H1"
          H1Mode = True
          H1Color = gPGotSel
       Case "B1"
    ...
    H0_Area1.Enabled = H0Mode           ' 2009.09.28 add
    H0_Area1.BackColor = H0Color        ' 2009.09.28 add
    ...
    PB_CAN.Visible = Not H0Mode         ' 2009.09.28 upd : H1Mode --> H0Mode
    K_Sykbnm.Visible = Not H0Mode       ' 2009.09.28 add
    PB_END.Visible = H0Mode             ' 2009.09.28 add
...
' << 削除処理 >>
        ...
        Else
            ProcHB = "B1"
            Call DispChange(ProcHB)
            iB1_Tanka.SetFocus                  ' 2009.09.29 add
        End If
        GoTo ReturnPress_Ed
    End If
    If ProcHB = "B1" Then
...
Private Function Head1Chk() As Boolean
    ...
    If H0Op_Tanku(0).Value Then         ' 2009.09.29 add
...
'-----------------------------------------------------------------------' 2009.10.01 add start
    ...
    End If
    '============== 支給先登録 =================
    If Not TorNmGet(iH1_Torcd, 0) Then
        With CisFun
            .MB_Lines = 4
            .MB_MSG(2) = "    支給先未登録            "
            .MB_Title = ""
...
''''    If H0Op_Tanku(0).Value Then
       ...
       If RTrim(iH1_Hinbn) = "" Then
          ...
          '============== 品番取引先登録 =================
          gSL_Select = ""
          gSL_Select = gSL_Select & " SELECT 品番" & vbCrLf     ' , 背番号 削除
...
'          gSL_Select = gSL_Select & "    AND 背番号 = '" & RTrim(iH1_Seban) & "'" & vbCrLf
          gSL_Select = gSL_Select & "  GROUP BY 品番"           ' , 背番号 削除
          If STMRead(gSL_Select) Then
             If CisDB.RecordCount = 1 Then
                iH1_Hinbn = RTrim(STM.品番)
'                iH1_Seban = RTrim(STM.背番号)
             Else
...
'       gSL_Select = gSL_Select & "  WHERE 品番 = '" & RTrim(iH1_Hinbn) & "'"
'       gSL_Select = gSL_Select & "    AND 取引先 = '" & RTrim(iH1_Torcd) & "'"
'       'gSL_Select = gSL_Select & "    AND (品目 = 0 OR (品目 = 1 AND 手配区分 = 1))"
...
'---------------------------------------------------------------------------------------------------' 2009.10.29 add start
        ...
        With CisFun
              ...
              .MB_MSG(2) = "    手配区分が違います          "
              .MB_MSG(4) = "     (手配区分：なし)           "
              If HTM.手配区分 = "0" Then .MB_MSG(4) = "     (手配区分：購入)           "
              If HTM.手配区分 = "1" Then .MB_MSG(4) = "     (手配区分：直送)           "
              If HTM.手配区分 = "2" Then .MB_MSG(4) = "     (手配区分：社内)           "
              If HTM.手配区分 = "3" Then .MB_MSG(4) = "     (手配区分：受給)           "
              .MB_Title = ""
              .MB_Button = Error
              .MBOX
          End With
...
'-----------------------------------------------------------' 2009.09.29 upd start
    Else
'        '============== 品番確認 =================
'        If Trim(iH1_Hinbn.Text) = "" And _
'           Trim(H1lb_HinBn(0).Caption) = "" And Trim(H1lb_HinBn(1).Caption) = "" And _
'           Trim(H1lb_HinBn(2).Caption) = "" And Trim(H1lb_HinBn(3).Caption) = "" _
'        Then
'            With CisFun
'                .MB_Lines = 4
'                .MB_MSG(2) = "    品番を入力して下さい            "
...
'        End If
       ...
       If RTrim(iH1_Hinbn) = "" And RTrim(H1lb_ZaiKan) = "" Then
          '=================== 材料取引先登録 =======================
          gSL_Select = ""
          gSL_Select = gSL_Select & " SELECT ZT.受給品番, ZT.発注品番, ZT.自社品番,"
          gSL_Select = gSL_Select & "        ZM.材質, ZM.材料管理番号, ZT.板厚, ZT.幅, ZT.長さ" & vbCrLf
          ...
          If RTrim(iH1_Hinbn.Text) <> "" Then
          For cnt = 0 To 3
            If H1Op_Hinku(cnt).Value = True Then Exit For
          Next
          If cnt > 3 Then cnt = 0
            Select Case cnt
              Case 0
                  gSL_Select = gSL_Select & "    AND ZT.発注品番 = '" & RTrim(iH1_Hinbn) & "'" & vbCrLf
              Case 1
                  gSL_Select = gSL_Select & "    AND ZT.受給品番 = '" & RTrim(iH1_Hinbn) & "'" & vbCrLf
              Case 2
                  gSL_Select = gSL_Select & "    AND ZT.自社品番 = '" & RTrim(iH1_Hinbn) & "'" & vbCrLf
              Case 3
                  gSL_Select = gSL_Select & "    AND ZT.材質 LIKE  '" & RTrim(iH1_Hinbn) & "%'" & vbCrLf
            End Select
          End If
          
          If ZTMRead(gSL_Select) Then
             If CisDB.RecordCount = 1 Then
                H1lb_Hinbn(0).Caption = ZTM.発注品番
                H1lb_Hinbn(1).Caption = ZTM.受給品番
                H1lb_Hinbn(2).Caption = ZTM.自社品番
                H1lb_Hinbn(3).Caption = RTrim(ZTM.材質) & " " & _
             ...
             Else
                ...
                If RV_Rtn Then
                    ' RV_HHinbn : 発注品番 / RV_JHinbn : 受給品番 / RV_Zainm : 自社品番 / Rv_KanriNo : 材料管理番号
                    ' RV_Zaist   : 材質    / RV_Itatu  : 板厚     / RV_Width : 幅       / RV_Long : 長さ
                    H1lb_Hinbn(0).Caption = RV_HHinbn
                    H1lb_Hinbn(1).Caption = RV_JHinbn
                    H1lb_Hinbn(2).Caption = RV_Zainm
                    H1lb_Hinbn(3).Caption = RV_Zaist & " " & _
                                            Format(RV_Itatu, "#,##0.00") & "X" & _
                                            Format(RV_Width, "#,##0.00") & "X" & _
                                            Format(RV_Long, "#,##0.00")
                    H1_ZaiRyo.Caption = RTrim(RV_Zaist) & Space(25 - Len(RTrim(RV_Zaist))) & "/" & _
...
''========================= 2009/03/03 Start                ' 2009.09.30 del start
'       '======== 材料取引先マスタ登録確認 ========
'       gSL_Select = ""
'       gSL_Select = gSL_Select & " SELECT * FROM 材料取引先マスタ ZT"
'       gSL_Select = gSL_Select & "          LEFT OUTER JOIN 材料マスタ ZM"
'       gSL_Select = gSL_Select & "               ON ZM.材料管理番号 = ZT.材料管理番号"
''       If H1Op_Hinku(0).Value Then
''          gSL_Select = gSL_Select & "  WHERE ZM.発注品番 = '" & RTrim(iH1_Hinbn) & "'"
''       Else
''          gSL_Select = gSL_Select & "  WHERE ZM.受給品番 = '" & RTrim(iH1_Hinbn) & "'"
''       End If
'       gSL_Select = gSL_Select & "  WHERE ZT.材料管理番号 = " & CisFun.Val2(H1lb_ZaiKan)
...
'           gSL_Select = gSL_Select & "               ON ZM.材料管理番号 = ZT.材料管理番号"
'           gSL_Select = gSL_Select & "  WHERE ZT.材料管理番号 = " & CisFun.Val2(H1lb_ZaiKan)
'           gSL_Select = gSL_Select & "    AND ZT.メーカー = '" & RTrim(iH1_Torcd) & "'"
'           gSL_Select = gSL_Select & "    AND ZT.品目 = 1 "
'           gSL_Select = gSL_Select & "    AND ZT.手配区分 = '1'"
'           If ZTMRead(gSL_Select) Then
'              iH1_Seban = ZTM.背番号
'           Else
'              iH1_Seban = ""
'           End If
'       End If
''========================= 2009/03/03 End                  ' 2009.09.30 del end
       ...
       'gSL_Select = gSL_Select & "    AND ZT.契約先 = '" & RTrim(iH1_KTorcd) & "'" & vbCrLf
        ...
        If ZTM.手配区分 <> 4 Then
           With CisFun
              ...
              .MB_MSG(2) = "    手配区分が違います          "
              .MB_MSG(4) = "     (手配区分：なし)           "
              If ZTM.手配区分 = 1 Then .MB_MSG(4) = "     (手配区分：購入)           "
              If ZTM.手配区分 = 2 Then .MB_MSG(4) = "     (手配区分：直送)           "
              If ZTM.手配区分 = 3 Then .MB_MSG(4) = "     (手配区分：社内)           "
              If ZTM.手配区分 = 4 Then .MB_MSG(4) = "     (手配区分：受給)           "
              .MB_Title = ""
              .MB_Button = Error
              .MBOX
           End With
...
'-----------------------------------------------------------------------------------------------' 2009.09.29 add start
    Else
       ...
       gSL_Select = gSL_Select & "    END 適用終了日付,"
       gSL_Select = gSL_Select & "    部品単価,加工単価,その他単価"
       gSL_Select = gSL_Select & "    From 材料支給単価マスタ"
       gSL_Select = gSL_Select & " WHERE 材料管理番号 = " & CisFun.Val2(H1lb_ZaiKan)
       gSL_Select = gSL_Select & "   And 取引先 = '" & RTrim(iH1_Torcd) & "'"
       gSL_Select = gSL_Select & " Order By 履歴NO Desc "
    End If
...
'-----------------------------------------------------------------------------------------------' 2009.09.29 add end
    ...
    If Not STMRead(gSL_Select, 0) Then
        Call STMClose(0)
...
' ( 削除処理 )
        If H0Op_Tanku(0).Value = True Then                              ' 2009.09.30 add
           .SQL = "DELETE FROM 支給単価マスタ "
           .SQL = .SQL & " WHERE 品番 = '" & RTrim(iH1_Hinbn) & "'"
           .SQL = .SQL & "   And 取引先 = '" & RTrim(iH1_Torcd) & "'"
'-----------------------------------------------------------------------' 2009.09.30 add start
        Else
           .SQL = "DELETE FROM 材料支給単価マスタ "
...
' ( 削除処理 )
    ...
    If H0Op_Tanku(0).Value Then         '************************************* 2008/02/29 ADD
    gSL_Select = "Select max(履歴NO) 履歴NO From 支給単価マスタ "
    gSL_Select = gSL_Select & " WHERE 品番 = '" & RTrim(iH1_Hinbn) & "'"
    gSL_Select = gSL_Select & "   And 取引先 = '" & RTrim(iH1_Torcd) & "'"
    Call STMRead(gSL_Select, 1)
    gLong = STM.履歴NO + 1
'-----------------------------'
'   支給単価マスタ出力
'-----------------------------'
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, あり, 7, 5, "有無償")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
Private Sub GridSet()
    ...
    If Not STMRead(gSL_Select) Then Exit Sub
' 抽出処理 初期ｾｯﾄ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not STM_RDSTS
        ...
        ID = ID + 1
        With vsGrid1
            .TextMatrix(ID, GridCol(0)) = STM.履歴NO
            .TextMatrix(ID, GridCol(1)) = Format(STM.単価, "#,##0.00")
            .TextMatrix(ID, GridCol(2)) = RTrim(STM.仮単価区分)
            .TextMatrix(ID, GridCol(3)) = RTrim(STM.適用開始日付)
            .TextMatrix(ID, GridCol(4)) = RTrim(STM.適用終了日付)
            If STM.有無償区分 = 0 Then
