...
'
   'SELECT 部
    ...
    gSL_Select = gSL_Select & "  ZJ.実績管理NO "
    gSL_Select = gSL_Select & ", ZJ.実績区分 "
    gSL_Select = gSL_Select & ", ZJ.実績日 "
    gSL_Select = gSL_Select & ", ZJ.実績便 "
    gSL_Select = gSL_Select & ", ZJ.実績先 "
    gSL_Select = gSL_Select & ", TM.略称 as 実績先名 "
    gSL_Select = gSL_Select & ", ZJ.受入 "
    gSL_Select = gSL_Select & ", ZJ.納入番号 "
    If H1Op_SKbn(1) Then    ' 再発行の場合は帳票出力テーブルの帳票区分=302のものが対象
        gSL_Select = gSL_Select & ", CT.帳票区分 "
    End If
'
   'FROM 部
    gSL_Select = gSL_Select & " from  材料実績管理テーブル ZJ "
    gSL_Select = gSL_Select & " left outer join 取引先マスタ TM "
    gSL_Select = gSL_Select & "   on TM.取引先CD   = ZJ.実績先 "
    gSL_Select = gSL_Select & "  and TM.取引先区分 = 1 "
    If H1Op_SKbn(1) Then    ' 再発行の場合は帳票出力テーブルの帳票区分=302のものが対象
        gSL_Select = gSL_Select & " left outer join 帳票出力テーブル CT "
        gSL_Select = gSL_Select & "   on CT.帳票区分 = 302 "
'       gSL_Select = gSL_Select & "  and CT.検索情報 = ZJ.納入番号 "
'       gSL_Select = gSL_Select & "  and CT.管理NO   = ZJ.実績管理NO "
    End If
'
   'WHERE 部
    wk_Whr = ""
    ...
    If iH1_SYmd <> "" Then
        If CisFun.Val2(iH1_SYmd.cDay) = 0 Then
           iH1_EYmd = ""
           iH1_EBin = ""
           wk_Whr = " ZJ.実績日 like '" & iH1_SYmd & "%' "
        Else
           If iH1_EYmd = "" Then
              wk_Whr = " ZJ.実績日 = '" & iH1_SYmd & "' "
           Else
              wk_Whr = " ZJ.実績日 >= '" & iH1_SYmd & "' and " & _
                       " ZJ.実績日 <= '" & iH1_EYmd & "' "
           End If
        End If
    ElseIf iH1_EYmd <> "" Then
       wk_Whr = " ZJ.実績日 <= '" & iH1_EYmd & "' "
    End If
    '------- 納入便
    If iH1_SBin <> "" And iH1_EBin = "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.実績便 = '" & iH1_SBin & "' "
    ElseIf iH1_SBin <> "" And iH1_EBin <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.実績便 >= '" & iH1_SBin & "' and " & _
                         " ZJ.実績便 <= '" & iH1_EBin & "'"
    ElseIf iH1_EBin <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.実績便 <= '" & iH1_EBin & "'"
    End If
    '------- 納入番号
    If iH1_SNouNo <> "" And iH1_ENouNo = "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.納入番号 = '" & iH1_SNouNo & "' "
    ElseIf iH1_SNouNo <> "" And iH1_ENouNo <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.納入番号 >= '" & iH1_SNouNo & "' and " & _
                         " ZJ.納入番号 <= '" & iH1_ENouNo & "'"
    ElseIf iH1_ENouNo <> "" Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.納入番号 <= '" & iH1_ENouNo & "'"
    End If
    '------- 支給先
    If iH1_Torcd <> "" Then
    If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " ZJ.実績先 = '" & iH1_Torcd & "' "
    End If
    '------- 再発行の場合は帳票出力テーブルの帳票区分=302のものが対象
    If H1Op_SKbn(1) Then
       If wk_Whr <> "" Then wk_Whr = wk_Whr & " and "
       wk_Whr = wk_Whr & " CT.管理NO = ZJ.実績管理NO and CT.検索情報 = ZJ.納入番号 "
    End If
    If wk_Whr <> "" Then gSL_Select = gSL_Select & " where " & wk_Whr
'
   'ORDER 部
    gSL_Select = gSL_Select & " order by ZJ.納入番号, ZJ.実績管理NO, " & _
                                       " ZJ.実績日, ZJ.実績便, " & _
                                       " ZJ.実績先, ZJ.受入 "
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 IN (312)"    ' 2008.08.25 upe : 帳票区分 IN (108,115)"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
Private Function GridSet1() As Boolean
    ...
    If ZJKRead(gSL_Select, , 1) Then    ' 2008.08.22 upd : If HCTRead(gSL_Select, , 1) Then
...
'       ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
        ...
        End With
        ID = 0
        Do Until Not ZJK_RDSTS          ' 2008.08.22 upd : HCT_RDSTS
            ID = ID + 1

            Call HctInfoToGrid1(ID)

            Call ZJKReadNext(1)         ' 2008.08.22 upd : Call HCTReadNext(1)
        Loop
        With vsGrid1
        ...
        GridSet1 = True
    End If
    Call ZJKClose                       ' 2008.08.22 upd : Call HCTClose
End Function
'+----------------------------------------------+
'+      実績管理テーブルよりグリッドに転送
'+----------------------------------------------+
Private Sub HctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '納入日＆便
        If Trim(ZJK.実績日) <> "" Then
            If RTrim(ZJK.実績便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(ZJK.実績日), "0000/00/00") & "   "
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(ZJK.実績日), "0000/00/00") & "-" & RTrim(ZJK.実績便)
            End If
        End If
        '支給先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = RTrim(ZJK.実績先)
        '支給先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = RTrim(ZJK.実績先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(ZJK.受入)
        '納入番号
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(ZJK.納入番号)
        '実績管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(ZJK.実績管理NO)
        'Select
