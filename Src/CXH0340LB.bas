...
'    If Not CXH0340LPrint Then Exit Sub
'
'*   作成日  :2004/01/30   By CIS
'*   変更日  :2007/10/30   By CIS   帳票発行後にFax処理が正常に動作しない点を修正
'*---------------------------------------------------------------------------------
Public CisPrtCXH0340        As New CISFormPrint
Public CXH0340_PrtCnt       As Long


Public Function CXH0340LPrint(PrintKB As Byte, Optional PrintOut As Integer = 0) As Boolean
    CXH0340LPrint = False
...
' 出力データ存在確認
   ...
   '枚数獲得
    ...
    With CisDB
        ...
        .ReadStatus = True
        If .DBRead(1, 0) Then
            CXH0340_PrtCnt = CisDB.Recordset(0)
        End If
    End With
    
    With CisPrtCXH0340
         Set .iPrtForm = CXH0340L
         .dpMSG = "臨時かんばん　印刷中"
         If PrintOut = 0 Then
           .iPreview = pv_Print
         Else
