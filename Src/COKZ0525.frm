...
Begin VB.Form COKZ0525 
   BorderStyle     =   1  '固定(実線)
   Caption         =   "仕入先別買掛実績表 印刷"
   ClientHeight    =   2625
   ClientLeft      =   6270
   ClientTop       =   5850
   ClientWidth     =   5265
...
'**********************************************************'
'**
'**       フォーム名    :  仕入先別買掛実績表　印刷指示
'**       フォームID    :  COKZ0525
'**       処理概要      :
'**
'**       作  成  日    :  2008/05/22  By CIS
'**       変  更  日    :
'**
'**********************************************************'
...
'
    ...
    mSavePrtName = Printer.DeviceName
    Op_Out(0).SetFocus
'
    FormAct = Not FormAct
End Sub
...
Private Sub PB_ENT_Click()
    ...
    If mSavePrtName <> Printer.DeviceName Then
       Call CisFrm.PrintSet(mSavePrtName)
    End If
...
Private Sub ReturnPress()
    ...
    COKZ0520_PrtCnt = CisDB.RecordCount
    COKZ0520_Kara = False
    Set CisPrt = New CISFormPrint
    
'   出力先
    lb_msg.Caption = "【 仕入先別買掛実績表 印刷中 】"
    DoEvents
    With CisPrt
         Set .iPrtForm = COKZ0520L
         .dpMSG = "仕入先別買掛実績表　印刷中"
