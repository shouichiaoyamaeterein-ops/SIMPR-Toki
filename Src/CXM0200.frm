...
'**       処理概要      :
'**
'**       作  成  日    :   2004/11/29  By CIS SIMPR-A
'**       変  更  日    :
'**
'**
'**
...
Private Function DBInput(InputKey As String) As Boolean
    ...
    gSL_Select = gSL_Select & " ORDER BY 適用開始年月"
    SaveSelect = gSL_Select
    If Not KTKRead(gSL_Select, 0) Then
...
'  ( 確認メッセージ )
    ...
    gSL_Select = "SELECT COUNT(*) 件数 FROM 買掛型費テーブル "
    gSL_Select = gSL_Select & " WHERE 取引先 = '" & RTrim(iH1_Torcd) & "'"
    gSL_Select = gSL_Select & "   AND 品番 = '" & RTrim(iH1_Hinbn) & "'"
...
'
    SaveRenban = 0
    If SyoriKB = "ADD" Then
        gSL_Select = "SELECT MAX(連番)連番 FROM 買掛型費テーブル"
        gSL_Select = gSL_Select & " WHERE 取引先 = '" & RTrim(iH1_Torcd) & "'"
        gSL_Select = gSL_Select & "   AND 品番 = '" & RTrim(iH1_Hinbn) & "'"
        If KTKRead(gSL_Select, 1) Then
            SaveRenban = KTK.連番
...
'
    ...
    '*=================*
    ' 買掛型費テーブル　削除
        gSL_Select = " DELETE FROM 買掛型費テーブル"
        gSL_Select = gSL_Select & " WHERE 取引先 = '" & RTrim(iH1_Torcd) & "'"
        gSL_Select = gSL_Select & "   AND 品番 = '" & RTrim(iH1_Hinbn) & "'"
...
DBPut_Err:
    ...
    End
End Function
Public Function KTKInsert_Proc() As Boolean
    KTKInsert_Proc = False
    ...
    With KTK
        ...
        For gCnt1 = StartYyMm To CisFun.Val2(iB1_TEYm)
            ...
            gSL_Select = " SELECT 対象開始年月 FROM 買掛型費テーブル"
            gSL_Select = gSL_Select & " WHERE 取引先 = '" & RTrim(iH1_Torcd) & "'"
            gSL_Select = gSL_Select & "   AND 品番 = '" & RTrim(iH1_Hinbn) & "'"
            gSL_Select = gSL_Select & "   AND 連番 = " & SaveRenban & ""
    ...
    KTKInsert_Proc = True
End Function
Public Function KTKInsertMnt_Proc() As Boolean
    KTKInsertMnt_Proc = False
    ...
    For gCnt1 = CisFun.Val2(iB1_TSYm) To CisFun.Val2(iB1_TEYm)
        ...
        gCnt2 = gCnt2 + 1
        gSL_Select = " SELECT 予想年月,実績年月 FROM 買掛型費テーブル"
        gSL_Select = gSL_Select & " WHERE 取引先 = '" & RTrim(iH1_Torcd) & "'"
        gSL_Select = gSL_Select & "   AND 品番 = '" & RTrim(iH1_Hinbn) & "'"
        gSL_Select = gSL_Select & "   AND 連番 = " & SaveRenban & ""
        gSL_Select = gSL_Select & "   AND 対象開始年月 = '" & Format(gCnt1, "000000") & "'"
        If Not KTKRead(gSL_Select, 1) Then
            ...
            End If
        Else
            wJiskiYm = RTrim(KTK.実績年月)
            wYosouYm = RTrim(KTK.予想年月)
            If RTrim(iB1_KYm) <> "" Or Format(gCnt1, "000000") >= RTrim(iB1_KYm) Then
                If RTrim(KTK.実績年月) = "" Then
    ...
    KTKInsertMnt_Proc = True
End Function
Public Function KTKUpdate_Proc() As Boolean
    KTKUpdate_Proc = False
    With KTK
        gSL_Select = "Update 買掛型費テーブル  Set "
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not KTK_RDSTS
        With VsGrid1
            ID = ID + 1
            '№
            .TextMatrix(ID, GridCol(0)) = Format(ID, "##")
            ' 適用開始年月
            If Trim(KTK.適用開始年月) <> "" Then
               .TextMatrix(ID, GridCol(1)) = Left(KTK.適用開始年月, 4) & "/" & Mid(KTK.適用開始年月, 5, 2)
            End If
            ' 適用終了年月
            If Trim(KTK.適用終了年月) <> "" Then
               .TextMatrix(ID, GridCol(2)) = Left(KTK.適用終了年月, 4) & "/" & Mid(KTK.適用終了年月, 5, 2)
            End If
            '月発生金額
            If KTK.月発生金額 <> 0 Then
                .TextMatrix(ID, GridCol(3)) = Format(KTK.月発生金額, "#,###")
            End If
            '回数
            If KTK.回数 <> 0 Then
                .TextMatrix(ID, GridCol(4)) = Format(KTK.回数, "##")
            End If
            '最終月調整金額
            If KTK.調整金額 <> 0 Then
                .TextMatrix(ID, GridCol(5)) = Format(KTK.調整金額, "#,###")
            End If
            '月発生金額
            If KTK.投資金額 <> 0 Then
                .TextMatrix(ID, GridCol(6)) = Format(KTK.投資金額, "#,###")
            End If
            ' 切替年月
            If Trim(KTK.切替年月) <> "" Then
               .TextMatrix(ID, GridCol(7)) = Left(KTK.切替年月, 4) & "/" & Mid(KTK.切替年月, 5, 2)
            End If
            
            '連番
            .TextMatrix(ID, GridCol(8)) = KTK.連番
            '適用開始年月
            .TextMatrix(ID, GridCol(9)) = RTrim(KTK.適用開始年月)
            '適用終了年月
            .TextMatrix(ID, GridCol(10)) = RTrim(KTK.適用終了年月)
            '切替年月
            .TextMatrix(ID, GridCol(11)) = RTrim(KTK.切替年月)
