   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日  -便  |仕入先  |   仕  入  先  名         |受入|発注区分|手配区分|SK区分    |明細書NO|出力|明細書発行|SELECT|"
            Rows            =   21
            Cols            =   13
            BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/02/16  By CIS
'**       変  更  日    :   2004/05/10  By CIS FAX送信機能追加
'**
'**********************************************************'
...
'   出力区分の背景色
    ...
    Dim mOutProc                As String

    Dim mHikae                  As String
...
Private Sub PB_Chg_Click()
    ...
    Else
       ...
       Chg_Msg = "ﾌﾟﾘﾝﾀｰ" & mHikae
       mOutProc = ""
    End If
End Sub
Private Sub Chg_Msg_Click()
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=8       Else 1=108
            ...
            Call CXH0330LPrint(wPrintKB, wPrintOut)

            Set CXH0330_Frm = CisFrm
'            Set CXH0330_FaxObj = MfRemote1
            CXH0330_PrtOut = "3"
            Call CXH0330LPrintFax(wPrintKB, wPrintOut)
            
            CXH0330_PrtOut = "4"
            Call CXH0330LPrintMail(wPrintKB, wPrintOut)
...
'
    ...
    gSL_Select = gSL_Select & ", hct.仕入先"
    gSL_Select = gSL_Select & ", hct.受入"
    gSL_Select = gSL_Select & ", hct.明細書NO"
    gSL_Select = gSL_Select & ", hct.SK区分"
    ...
    gSL_Select = gSL_Select & ", isnull(HT.明細書発行, '') 明細書発行"  ' 2004/05/10 Add
    If H1Op_SKbn(1) Then
'   ### 再発行最新 ###
...
'   ### 再発行既存 ###
    ...
    gSL_Select = gSL_Select & "    on trm.取引先CD = hct.仕入先"
    ...
    gSL_Select = gSL_Select & "   and sy1.値 = hct.発注区分"
    gSL_Select = gSL_Select & "  left outer join 名称マスタ sy2"
    gSL_Select = gSL_Select & "    on sy2.区分名称 = '手配区分'"
    gSL_Select = gSL_Select & "   and sy2.区分タイプ = ''"
    gSL_Select = gSL_Select & "   and sy2.値 = hct.手配区分"
    ' 2004/05/10 Add
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 品番取引先マスタ HT"
    gSL_Select = gSL_Select & "   ON HT.品番 = hct.品番"
    gSL_Select = gSL_Select & "   AND HT.取引先 = hct.仕入先"
    gSL_Select = gSL_Select & "   AND HT.受入 = hct.受入"
    gSL_Select = gSL_Select & "   AND HT.品目 = 1"
    ' 2004/05/10 Add end
    If H1Op_SKbn(1) Then
'   ### 再発行最新 ###
        gSL_Select = gSL_Select & " where cht.端末番号 = substring(host_name(),1,20)"
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 8"
        gSL_Select = gSL_Select & "   and hct.明細書NO = cht.管理NO"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where hct.明細書NO <> 0"
    End If
    
    gSL_Select = gSL_Select & " And HT.明細書発行 <> '2'"

'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
        gSL_Select = gSL_Select & "   and hct.納入番号 like '" & RTrim(Replace(iH1_SNouNo, "*", "%")) & "'"
    Else
        If Trim(iH1_SNouNo) <> "" Then
            If Trim(iH1_ENouNo) = "" Then
                '> 開始のみ
                If CisFun.Val2(iH1_SPage) = 0 Then
                    gSL_Select = gSL_Select & "   and hct.納入番号 = '" & RTrim(iH1_SNouNo) & "'"
                Else
                    gSL_Select = gSL_Select & "   and"
                    gSL_Select = gSL_Select & "   hct.納入番号 + space(5 - len(hct.納入番号))"
                    gSL_Select = gSL_Select & " + right('0' + cast(cast(hct.納品書頁 as decimal(2)) as varchar(2)), 2)"
        ...
        If Trim(iH1_ENouNo) <> "" Then
            If CisFun.Val2(iH1_EPage) = 0 Then
                gSL_Select = gSL_Select & "   and hct.納入番号 <= '" & RTrim(iH1_ENouNo) & "'"
            Else
                gSL_Select = gSL_Select & "   and"
                gSL_Select = gSL_Select & "   hct.納入番号 + space(5 - len(hct.納入番号))"
                gSL_Select = gSL_Select & " + right('0' + cast(cast(hct.納品書頁 as decimal(2)) as varchar(2)), 2)"
...
'   >> 仕入先
    If RTrim(iH1_Torcd) <> "" Then
        gSL_Select = gSL_Select & "   and hct.仕入先 = '" & RTrim(iH1_Torcd) & "'"
    End If
'   >> 納入日
    If Trim(iH1_SYmd) <> "" And CisFun.Val2(iH1_SYmd.cDay) = 0 Then
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(hct.納入日, 1, 6) = '" & iH1_SYmd & "'"
    Else
        YmdSta = "": BinSta = ""
        YmdEnd = "": BinEnd = ""
        '> 日付＆便　編集
        ...
        If Trim(YmdSta) <> "" And Trim(YmdEnd) = "" Then
            If Trim(BinSta) = "" Then
                gSL_Select = gSL_Select & "   and hct.納入日 = '" & YmdSta & "'"
            Else
                gSL_Select = gSL_Select & "   and hct.納入日 + hct.納入便 = '" & YmdSta & BinSta & "'"
            End If
        End If
        '> 終了日のみ
        If Trim(YmdSta) = "" And Trim(YmdEnd) <> "" Then
            If Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and hct.納入日 <= '" & YmdEnd & "'"
            Else
                gSL_Select = gSL_Select & "   and hct.納入日 + hct.納入便 <= '" & YmdEnd & BinEnd & "'"
            End If
        End If
        '> 開始日＆終了日
        If Trim(YmdSta) <> "" And Trim(YmdEnd) <> "" Then
            If Trim(BinSta) = "" And Trim(BinEnd) = "" Then
                gSL_Select = gSL_Select & "   and hct.納入日 between '" & YmdSta & "'" _
                                                             & " and '" & YmdEnd & "'"
            Else
                If Trim(BinSta) = "" Then BinSta = "00"
                If Trim(BinEnd) = "" Then BinEnd = "99"
                gSL_Select = gSL_Select & "   and hct.納入日 + hct.納入便 between '" & YmdSta & BinSta & "'" _
                                                                         & " and '" & YmdEnd & BinEnd & "'"
            End If
        End If
    End If
'   >> 発注区分
    If RTrim(H1Cmb_HKbn) <> "" Then
        ...
        gSL_Select = gSL_Select & "   and hct.発注区分 = " & CisCombo1.Code & ""
    End If
'   >> 手配区分
    If RTrim(H1Cmb_TKbn) <> "" Then
        Call CisCombo2.CodeGet
        gSL_Select = gSL_Select & "   and hct.手配区分 = " & CisCombo2.Code & ""
    End If
    ...
    gSL_Select = gSL_Select & ", isnull(HT.明細書発行, '')" ' 2004/05/10 Add
    
    If Trim(iH1_SNouNo) = "" And Trim(iH1_SPage) = "" And _
        Trim(iH1_ENouNo) = "" And Trim(iH1_EPage) = "" And _
        Trim(H1Cmb_HKbn.Text) = "" And Trim(H1Cmb_TKbn) = "" And mKaraFlg Then
    ' 空明細書
        ...
        gSL_Select = gSL_Select & ", hct.受入"
        gSL_Select = gSL_Select & ", hct.明細書NO"
        gSL_Select = gSL_Select & ", 2 SK区分"
        gSL_Select = gSL_Select & ", isnull(trm.略称, '') 仕入先名"
        gSL_Select = gSL_Select & ", '' 発注区分名"
        gSL_Select = gSL_Select & ", '' 手配区分名"
        gSL_Select = gSL_Select & ", isnull(trm.明細書発行, '') 明細書発行"
        If H1Op_SKbn(1) Then
    '   ### 再発行最新 ###
            gSL_Select = gSL_Select & " from"
            gSL_Select = gSL_Select & "  帳票出力テーブル cht"
            gSL_Select = gSL_Select & ", 空明細書テーブル hct"
        Else
    '   ### 再発行既存 ###
            gSL_Select = gSL_Select & " from"
            gSL_Select = gSL_Select & "  空明細書テーブル hct"
    ...
    '   ### 再発行最新 ###
            gSL_Select = gSL_Select & " where cht.端末番号 = substring(host_name(),1,20)"
            gSL_Select = gSL_Select & "   and cht.帳票区分 = 15"
            gSL_Select = gSL_Select & "   and hct.明細書NO = cht.管理NO"
        Else
    '   ### 再発行既存 ###
            gSL_Select = gSL_Select & " where hct.明細書NO <> 0"
...
' ( 作成処理 )
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(11))) = "X" Then
                Call ItemsClearCHT
                CHT.端末番号 = "' + substring(host_name(),1,20) + '"
                If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(5))) = "" Then
                   CHT.帳票区分 = 115
                Else
                   CHT.帳票区分 = 108
                End If
                If mOutProc = "" Then   ' 控え
                   CHT.管理文字 = "1"
                Else
                   CHT.管理文字 = Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(10)))
                End If
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 IN (108,115)"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, なし, 5, 9, "出力")    '2004/05/10 Add
        Call .FixedSet(中中, 左中, なし, 0, 10, "明細書発行")    '2004/05/10 Add
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        'Call .FixedSet(中中, 左中, なし, 0, 9, "Select")   2004/05/10 Del
        Call .FixedSet(中中, 左中, なし, 0, 11, "Select")    '2004/05/10 Add
'  表示ﾊﾟﾀｰﾝｾｯﾄ
'       Call .PatanSet(ﾊﾟﾀｰﾝNO,表示パターン)
        Call .PatanSet(0, "0,1,2,3,4,5,6,7,9")
        .InitDisp
    End With
End Sub
'***********************************************'
...
'+      発注テーブルよりグリッドに転送
'+----------------------------------------------+
Private Sub HctInfoToGrid1(SetRow As Long)
    With vsGrid1
        'No
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(0)) = SetRow
        '納入日＆便
        If Trim(HCT.納入日) <> "" Then
            If RTrim(HCT.納入便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(HCT.納入日), "0000/00/00") & "   "
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = Format(RTrim(HCT.納入日), "0000/00/00") & "-" & RTrim(HCT.納入便)
            End If
        End If
        '仕入先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(2)) = RTrim(HCT.仕入先)
        '仕入先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = RTrim(HCT.仕入先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(HCT.受入)
        '発注
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(HCT.発注区分名)
        '手配
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(HCT.手配区分名)
        'SK区分
        Select Case HCT.SK区分
            Case 0: .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = "かんばん"
            Case 1: .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = "指示"
        End Select
        '明細書NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = HCT.明細書NO
        '出力
        Select Case HCT.明細書発行
            Case "1": gStr = ""
            Case "3": gStr = "FAX"
            Case "4": gStr = "Mail"
            Case Else
                gStr = "？？"
        End Select
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = gStr
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = HCT.明細書発行
