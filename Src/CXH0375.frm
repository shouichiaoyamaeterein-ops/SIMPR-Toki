...
Begin VB.Form CXH0375 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "JAMA　ファイル再出力"
   ClientHeight    =   10980
   ClientLeft      =   90
   ClientTop       =   1530
   ClientWidth     =   15315
   ...
   Begin VB.PictureBox H1_Area2 
      ...
      Begin Cis3D_v60.CIS3D CIS3D6 
         ...
         cAlingnment     =   6
         Begin Cis3D_v60.CIS3D PB_JamaFDir 
            Height          =   315
            Left            =   4875
            Top             =   255
            Width           =   345
            ...
            cButton         =   -1  'True
         End
         Begin Cis3D_v60.CIS3D H1lb_JamaFDir 
            Height          =   315
            Left            =   75
            Top             =   240
            Width           =   4785
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |作成ファイル名 　 　　       |伝種|納 入 日 　-便|仕入先 |   仕  入  先  名   |受入|納 番 |頁|発注区|"
            Rows            =   21
            Cols            =   15
            BackColor       =   12648447
...
'-----------------------------------------------< 2006/02/02 ADD START >
        ...
        .INI_Default = gIniExe
        .GetIni
        H1lb_JamaFDir = RTrim(.INI_String)
'-----------------------------------------------< 2006/02/02 ADD START >
...
'-----------------------------------------------< 2006/02/02 ADD START >
            ...
            wPrintKB = 1                ' 帳票区分：0=7 Else 1=107
            Call CXH0370TFile(wPrintKB)
...
'+  PB_JamaFDir
'+-----------------------------+
Private Sub PB_JamaFDir_Click()
    With CisFun
        .Folder_Title = "JAMA伝票ファイル出力先参照"
        .Folder_DefDir = H1lb_JamaFDir
        .FolderSel

        If RTrim$(.Folder_Result) <> "" Then H1lb_JamaFDir = RTrim$(.Folder_Result)
    End With
End Sub
'+-----------------------------+
'+  H1Op_SKbn
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 7"
        ...
        gSL_Select = gSL_Select & "   and hch.発行種類 = 2"
        gSL_Select = gSL_Select & "   and hch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and hch.TEXTNAME <> ''"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where hch.発行種類 = 2"
        gSL_Select = gSL_Select & "   and hch.発行形態 = 2"
        gSL_Select = gSL_Select & "   and hch.TEXTNAME <> ''"
    End If
'   >> 伝票種類
    If H1Chk_Card(6) = 1 Or _
       H1Chk_Card(7) = 1 Or _
       H1Chk_Card(8) = 1 Then
        gSL_Select = gSL_Select & "   and hch.カード in"
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 107"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 30, 1, "作成ファイル名")
...
Private Sub HchInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '作成ﾌｧｲﾙ名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(1)) = RTrim(HCH.TEXTNAME)
        '伝種
        Select Case HCH.カード
        ...
        If Trim(HCH.納入日) <> "" Then
            If RTrim(HCH.納入便) = "" Then
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = Format(RTrim(HCH.納入日), "0000/00/00")
            Else
                .TextMatrix(SetRow, CisVsGrid1.FixedGet(3)) = Format(RTrim(HCH.納入日), "0000/00/00") & "-" & RTrim(HCH.納入便)
            End If
        End If
        '仕入先
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(4)) = RTrim(HCH.仕入先)
        '仕入先名
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(5)) = RTrim(HCH.仕入先名)
        '受入
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(6)) = RTrim(HCH.受入)
        '納番
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(HCH.納入番号)
        '頁
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = Format(HCH.頁NO, "##")
        '発注
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = RTrim(HCH.発注区分名)
        '納品書管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = HCH.納品書管理NO
        'カード
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = HCH.カード
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
