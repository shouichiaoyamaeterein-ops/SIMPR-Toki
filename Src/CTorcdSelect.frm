...
Begin VB.Form CTorcdSelect 
   ...
   Begin VB.PictureBox Picture 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CTorcdSelect.frx":0000
         Height          =   1860
         Left            =   30
         OleObjectBlob   =   "CTorcdSelect.frx":0015
         TabIndex        =   4
         Top             =   30
         Width           =   6750
      End
   ...
   End
End
Attribute VB_Name = "CTorcdSelect"
...
Private Sub Form_Activate()
    ...
    mSL_Select = ""
    mSL_Select = mSL_Select & " SELECT ZT.材料管理番号, ZT.取引先,"
    ...
    mSL_Select = mSL_Select & "        CASE WHEN ISNULL(ZT.取引先, '') = '' THEN ''"
    mSL_Select = mSL_Select & "             ELSE ZT.取引先 + ':' + TR.略称 END 表示取引先"
    ...
    mSL_Select = mSL_Select & "             ON TR.取引先CD = ZT.取引先"
    mSL_Select = mSL_Select & "  WHERE (ZT.発注品番 = '" & RV_JHinbn & "'"
    mSL_Select = mSL_Select & "     OR  ZT.受給品番 = '" & RV_JHinbn & "')"
'    mSL_Select = mSL_Select & "    AND (ZT.品目 = 0 OR (ZT.品目 = 1 AND ZT.手配区分 = 1))"
    mSL_Select = mSL_Select & "  ORDER BY ZT.材質, ZT.取引先"
