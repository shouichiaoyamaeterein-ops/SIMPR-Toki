...
Begin VB.Form CZK0030 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "棚卸プレシート発行(材料)"
   ClientHeight    =   11190
   ClientLeft      =   2175
   ClientTop       =   2505
   ClientWidth     =   15390
...
'   CSVﾌｧｲﾙ出力先
    mCsvName = Mid(CisFun.WinRoot, 1, 3) & "棚卸プレシート（材料）.csv"
    mCsvKey = "CZK0030"
...
'   対象年月
    ...
    If Not Trim(iH1_SYMD) = "" Then
        If Trim(iH1_SYMD.cYear) = "" Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    最終出庫日（開始） - 年を入力して下さい            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
            ...
            Exit Function
        End If
        If Trim(iH1_SYMD.cMonth) = "" Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    最終出庫日（開始） - 月を入力して下さい            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
            ...
            Exit Function
        End If
        If Trim(iH1_SYMD.cDay) = "" Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    最終出庫日（開始） - 日を入力して下さい            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
    ...
    If Not Trim(iH1_EYMD) = "" Then
        If Trim(iH1_EYMD.cYear) = "" Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    最終出庫日（終了） - 年を入力して下さい            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
            ...
            Exit Function
        End If
        If Trim(iH1_EYMD.cMonth) = "" Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    最終出庫日（終了） - 月を入力して下さい            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
            ...
            Exit Function
        End If
        If Trim(iH1_EYMD.cDay) = "" Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    最終出庫日（終了） - 日を入力して下さい            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
    ...
    With CisFun
        .MB_Lines = 4
        .MB_MSG(1) = "   　棚卸プレシート発行（材料）を行います。            "
        .MB_MSG(3) = "   　よろしいですか？            "
        .MB_Title = "棚卸プレシート発行（材料）"
        .MB_Button = Yes_No
...
Private Function PrintStart() As Boolean
    PrintStart = False
    If Op_Out(0).Value Or Op_Out(1).Value Then
        lb_msg.Caption = "【 棚卸プレシート（材料） 印刷中 】"
        DoEvents
        With CisPrtCZK0030
            .cDataMaxCnt = CisDB.RecordCount
            Set .iPrtForm = CZK0030L
            .dpMSG = "棚卸プレシート（材料）　印刷中"
...
'   出力ﾌｧｲﾙ存在確認
    ...
    lb_msg.Caption = "【 棚卸プレシート（材料）データ 出力中 】"
...
Private Sub CsvEdit_Body1(CsvStr As String, CsvCnt As Long)
    ...
    With ZTP
        ...
        CsvStr = CsvStr & "," & RTrim(.材質) & " " & RTrim(.表示寸法)
...
Private Function Create_Pure() As Boolean
    ...
    lb_msg.Caption = "【 棚卸プレシート（材料）データ 抽出中 】"
    DoEvents
    
'''    Call CisDB.DBTran(TransBegin)
    With CisDB
        .SQL = "材料棚卸プレシート発行データ作成"
        .StoadoCount = 6
        ...
        If .ParaValue <> 0 Then
            CisFun.MB_Lines = 5
            CisFun.MB_MSG(2) = "    棚卸プレシート発行（材料）データ作成　異常！      "
...
'''    Call CisDB.DBTran(TransCommit)
    ...
    '   < 印刷データ読み込み >
    gSL_Select = ""
    gSL_Select = gSL_Select & "Select ZTP.*,ISNULL(MS.値名称,'') 在庫場所名1,ISNULL(MT.値名称,'') 在庫場所名2" & Chr(13)
    gSL_Select = gSL_Select & "      ,ISNULL(ZRM.材質,'') 材質,ISNULL(ZRM.表示寸法,'') 表示寸法" & Chr(13)
    gSL_Select = gSL_Select & "      ,ISNULL(MU.値名称,'') 手配単位,ISNULL(ZRM.材料名,'') 材料名" & Chr(13)
    gSL_Select = gSL_Select & "  FROM 材料棚卸プレシート発行テーブル ZTP" & Chr(13)
    gSL_Select = gSL_Select & "  LEFT OUTER JOIN 名称マスタ MS" & Chr(13)
    gSL_Select = gSL_Select & "    ON MS.区分名称 = '管理場所'" & Chr(13)
    gSL_Select = gSL_Select & "   AND MS.値 = ZTP.在庫場所1" & Chr(13)
    gSL_Select = gSL_Select & "  LEFT OUTER JOIN 名称マスタ MT" & Chr(13)
    gSL_Select = gSL_Select & "    ON MT.区分名称 = '管理場所ユーザー'" & Chr(13)
    gSL_Select = gSL_Select & "   AND MT.値 = ZTP.在庫場所2" & Chr(13)
    gSL_Select = gSL_Select & "  LEFT OUTER JOIN 材料マスタ ZRM" & Chr(13)
    gSL_Select = gSL_Select & "    ON ZRM.材料管理番号 = ZTP.材料管理番号" & Chr(13)
    gSL_Select = gSL_Select & "  LEFT OUTER JOIN 名称マスタ MU" & Chr(13)
    gSL_Select = gSL_Select & "    ON MU.区分名称 = '単位区分'" & Chr(13)
    gSL_Select = gSL_Select & "   AND MU.値 = ZRM.手配単位" & Chr(13)
    gSL_Select = gSL_Select & " WHERE ZTP.発行年月 = '" & RTrim(iH1_TYM) & "'" & Chr(13)
    gSL_Select = gSL_Select & "   AND ZTP.在庫場所1 LIKE '" & RTrim(wStr) & "'" & Chr(13)
    gSL_Select = gSL_Select & " ORDER BY ZTP.伝票NO,ZTP.伝票行NO" & Chr(13)

    
    If Not ZTPRead(gSL_Select, , 1) Then
        With CisFun
            .MB_Lines = 4
