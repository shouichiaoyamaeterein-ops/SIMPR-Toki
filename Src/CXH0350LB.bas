...
'    If Not CXH0350LPrint Then Exit Sub
'
'*   作成日  :2004/01/30   By CIS
'*   変更日  :2007/10/30   By CIS   帳票発行後にFax処理が正常に動作しない点を修正
'*---------------------------------------------------------------------------------
Public CisPrtCXH0350        As New CISFormPrint
Public CXH0350_PrtCnt       As Long


Public Function CXH0350LPrint(PrintKB As Byte, Optional PrintOut As Integer = 0) As Boolean
    CXH0350LPrint = False
...
' 出力データ存在確認
    ...
    With CisDB
        ...
        .ReadStatus = True
        If .DBRead(1, 0) Then
            CXH0350_PrtCnt = CisDB.Recordset(0)
        End If
    End With
    With CisPrtCXH0350
         Set .iPrtForm = CXH0350L
         .dpMSG = "現品票　印刷中"
         If PrintOut = 0 Then
           .iPreview = pv_Print
         Else
