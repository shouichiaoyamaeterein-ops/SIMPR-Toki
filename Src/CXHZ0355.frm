   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日 　|仕入先 |仕  入  先  名 |納 番 |頁|手配|材  質　　               |収容数     |枚数|発注数    |"
            Rows            =   21
            Cols            =   14
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=30      Else 1=130
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            Call CXHZ0350LPrint(wPrintKB, wPrintOut)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 30"
        gSL_Select = gSL_Select & "   and zct.発注管理NO = cht.管理NO"
        gSL_Select = gSL_Select & "   and zct.SK区分 = 1"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where zct.SK区分 = 1"
    End If
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 130"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 16, 3, "仕 入 先 名")
        Call .FixedSet(中中, 左中, あり, 6, 4, "納 番")
        Call .FixedSet(中中, 右中, あり, 2, 5, "頁")
        Call .FixedSet(中中, 左中, あり, 4, 6, "手配")
        Call .FixedSet(中中, 左中, あり, 25, 7, "材　　　質")
        Call .FixedSet(中中, 右中, あり, 11, 8, "収容数")
        Call .FixedSet(中中, 右中, あり, 4, 9, "枚数")
        Call .FixedSet(中中, 右中, あり, 11, 10, "発注数")
...
Private Sub ZctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '材質
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(7)) = RTrim(ZCT.材質)
        '収容数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(8)) = Format(ZCT.収容数, "#,##0.000")
        '箱数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(9)) = Format(ZCT.枚数, "#,###")
        '発注数
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(10)) = Format(ZCT.発注数, "#,##0.000")
        '発注管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(11)) = ZCT.発注管理NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
