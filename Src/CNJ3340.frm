...
'   ( ３ヶ月　月量データ ) < 納期:｢00000000｣/当月数量、 ｢00000001｣/翌月数量、 ｢00000002｣/翌々月数量 >
    ...
    gSL_Select = "SELECT  HJ.品番, HN.品名, HJ.手配先, '0000000' +" & vbCrLf
    ...
    Else
        '前回分(=2)
        gSL_Select = gSL_Select & " FROM 発注内示前回テーブル HJ" & vbCrLf
    End If
    ...
    With CisFun
        .Date_RsFormat = fYM
        .Date_Add iH1_YM, M, 2
        gSL_Select = gSL_Select & " AND   HJ.計画年月 <= '" & Mid$(.Date_Result, 1, 6) & "'" & vbCrLf
    End With
    ...
    gSL_Select = gSL_Select & " GROUP BY HJ.品番, HN.品名, HJ.手配先, 背番号, HJ.計画年月, HJ.内示年月" & vbCrLf

'   ( 読み込み順 )
    gSL_Select = gSL_Select & " ORDER BY HJ.手配先, HJ.品番, 背番号" & vbCrLf
...
Private Function CsvPut() As Boolean
    ...
    '見出し編集
    MD1 = """内示年月"",""仕入先"",""仕入先名"""
    MD2 = """№"",""背番号"",""品番"",""品名"",""収容数"",""" & wYM & """,""日当数"",""" & wYMy & """,""" & wYMyy & """"
    ...
    With HNJ
        ...
        Do Until Not HNJ_RDSTS
            ...
            If Not HNJ_RDSTS Or _
               ...
               PrtK_Seban <> RTrim$(.背番号) Or _
               PrtK_Hnban <> RTrim$(.品番) Then
