...
'**********************************************************'
    ...
    Dim Last_No         As Integer      ' 現在登録されている最後の登録№
    Dim Next_No         As Integer      ' 次に登録するときの登録№
'+----------------------------------------------------------+
'+      フォーム    《    Ｌｏａｄイベント    》              +
'+----------------------------------------------------------+
...
''----  試験用  --------------------------------' Start
'Dim left_sa As Integer: left_sa = 2400
'B1_Area1.Left = B1_Area1.Left - left_sa
'B1_Area1.Width = B1_Area1.Width + left_sa
'UC_3D4(1).Width = UC_3D4(1).Width + 2940
'BLB_DD(1).Width = BLB_DD(1).Width + 2880
'vsGrid.Width = vsGrid.Width + 2820
'CIS3D11.Left = CIS3D11.Left - left_sa
'CIS3D12.Left = CIS3D12.Left - left_sa
'DebugPicture.Visible = True
''----  試験用  --------------------------------' End
'   #-------------------#
'   # 画面中央位置表示   #
'   #-------------------#
...
Private Sub PB_ADD_Click()
    ...
    If SyoriKB <> "ADD" Then
       SyoriKB = "ADD"
       PB_CAN.Visible = False       ' 2009.10.27 upd : True
    Else
       SyoriKB = ""
...
Private Sub PB_DEL_Click()
    ...
    If SyoriKB <> "DEL" Then        ' 2009.10.22 upd : = "" → <> "DEL" に変更
        SyoriKB = "DEL"
        PB_CAN.Visible = False      ' 2009.10.27 upd : true
'vsGrid.HighLight = flexHighlightAlways
    Else
        SyoriKB = ""
        PB_CAN.Visible = False
...
Private Sub PB_Back_Click()
    ...
    gSL_Select = gSL_Select & "   AND SUBSTRING(M1.値名称 + '       ', 1, 7) + SUBSTRING(M1.英数字1 + '   ', 1, 3) < "
...
'
    ...
    PB_CAN.Visible = B1Mode         ' 2009.10.27 upd : Not H1Mode
...
'          SyoriKB = ""
'          Call SyoriName(SyoriKB)
           vsGrid.Row = 1: Call vsGrid.SetFocus
        Else
           ProcHB = "B1"
           Call DispChange(ProcHB)
           vsGrid.Row = 1
...
'   gSL_Select = gSL_Select & "              AND T2.社内区分   = 1"             ' 社内区分　：社内  ' 2009.10.26 del
    ...
    If DBInput("INV") Then
        If SyoriKB = "" Or SyoriKB = "ADD" Then
            If SyoriKB = "ADD" Then
               With CisFun
                 .MB_Lines = 4
                 .MB_MSG(2) = "    工程かんばん発行パターン登録済      "
                 .MB_Title = ""
                 .MB_Button = OK
                 .MBOX
               End With
...
Private Function Body1Chk() As Boolean
    ...
    iB1_BusyoCD = Trim(iB1_BusyoCD)
    iB1_BasyoCD = Trim(iB1_BasyoCD)
            ...
            If Not BodyInput(gCnt1) Then
                For gCnt2 = gCnt1 + 1 To .Rows - 1
                    If BodyInput(gCnt2) Then
                        For gCnt3 = 0 To .Cols - 1      ' 2009.10.27 upd : １カラム目からだったものを０からに変更
...
''''            End If
            If RTrim(.TextMatrix(gCnt1, 2)) = "" And _
               RTrim(.TextMatrix(gCnt1, 1)) <> "" _
            Then
...
''''            End If
        '<< 重複チェック >>
            For gCnt2 = gCnt1 + 1 To .Rows - 1
                If RTrim(.TextMatrix(gCnt1, 1)) <> "" And _
                   RTrim(.TextMatrix(gCnt1, 1)) = RTrim(.TextMatrix(gCnt2, 1)) And _
                   RTrim(.TextMatrix(gCnt1, 3)) = RTrim(.TextMatrix(gCnt2, 3)) _
                Then
...
' ( 削除処理 )
        ...
        iT1_Delsu = Format(CisFun.Val2(iT1_Delsu) + CisFun.Val2(Kensu1), "####0")      ' 2009.10.27 add
        Kensu1 = 0                                          ' 2009.10.27 add
...
Private Function DBPut(Optional pKB As Byte = 0)
    ...
    For wCnt = 1 To vsGrid.Rows - 1
        If vsGrid.TextMatrix(wCnt, 0) <> "" Then
           Kensu2 = Kensu2 + 1
        End If
    Next
    ...
    If SyoriKB = "MNT" Then
        ...
        With vsGrid
            For wCnt = 1 To .Rows - 1
                If Trim(.TextMatrix(wCnt, 0)) = "" Then Exit For
'               If Trim(.TextMatrix(wCnt, 7)) <> "" Then
                If CisFun.Val2(Trim(.TextMatrix(wCnt, 0))) <= Last_No Then
                   If wSel = "" Then
                      wSel = "DELETE FROM 名称マスタ"
                      wSel = wSel & " WHERE 区分名称 = '工程かんばん発行パターン' AND "
                      wSel = wSel & "     ( 値 = '" & Trim(.TextMatrix(wCnt, 0)) & "'"
    ...
    Else    ' SyoriKB = "ADD" のときはこちら
        With CisFun
            If Trim(vsGrid.TextMatrix(1, 0)) = "" And _
               Trim(vsGrid.TextMatrix(1, 1)) = "" And _
               Trim(vsGrid.TextMatrix(1, 3)) = "" And _
               SyoriKB = "ADD" _
            Then
                .MB_Lines = 4
                .MB_MSG(2) = "    登録するデータがありません                "
                .MB_Title = "新規登録処理"
                .MB_MSG(3) = ""
                .MB_Button = OK
                .MBOX
...
'    End If
    ...
    With vsGrid
        For wCnt = 1 To .Rows - 1
             If Trim(.TextMatrix(wCnt, 0)) = "" Then Exit For
             If Trim(.TextMatrix(wCnt, 1)) <> "" Then
               Call ItemsClearSYM
               With SYM
                    .区分名称 = "工程かんばん発行パターン"
                   '.区分タイプ = ""
    ...
    If SyoriKB = "ADD" Then
       iT1_Addsu = Format(CisFun.Val2(iT1_Addsu) + CisFun.Val2(Kensu2), "####0")
    Else
       iT1_Mntsu = Format(CisFun.Val2(iT1_Mntsu) + CisFun.Val2(Kensu2), "####0")
    End If
    Kensu2 = 0
...
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 中中, あり, 0, 5, "契約取引先")
        Call .FixedSet(中中, 中中, あり, 0, 6, "契約取引工場")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, なし, 0, 7, "Select")
        Call .FixedSet(中中, 中中, あり, 0, 8, "Number")
        Call .FixedSet(中中, 中中, あり, 0, 9, "Emp")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
'       Call .PatanSet(ﾊﾟﾀｰﾝNO,表示パターン)
'       Call .PatanSet(0, "0,1,2,3,4")          ' 2009.10.22 del : これを実行すると何故かカラムの順序が入れ替わるため
        Call .PatanSet(1, "0,1,2,3,4")           '               　 パターンNOを１に変更
        .InitDisp
    End With
End Sub
'******************************************'
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not SYM_RDSTS
        ...
        With vsGrid
            ...
            .TextMatrix(ID, 8) = .TextMatrix(ID, 0)     ' 発行№保持
        End With
...
'グリッドからテキストに表示
    ...
    If Trim(vsGrid.TextMatrix(vsGrid.Row, 8)) = "" Then
       vsGrid.TextMatrix(vsGrid.Row, 0) = Format(Next_No, "000")
    Else
       vsGrid.TextMatrix(vsGrid.Row, 0) = vsGrid.TextMatrix(vsGrid.Row, 8)
    End If
Grid_RowChange_End:
'---------------------------------------------------------------------------' 2009.10.27 add start
    If Trim(vsGrid.TextMatrix(vsGrid.Row, 8)) = "" Then
       vsGrid.TextMatrix(vsGrid.Row, 0) = Format(Next_No, "000")
    Else
       vsGrid.TextMatrix(vsGrid.Row, 0) = vsGrid.TextMatrix(vsGrid.Row, 8)
...
'       >> 返却場所１
        ...
        If Trim(iB1_BusyoCD) = "" And Trim(iB1_BasyoCD) = "" Then      ' 登録№処理
           If SyoriKB = "ADD" Then
              .TextMatrix(tRow, 0) = ""
              .TextMatrix(tRow, 8) = ""
            Else
              If CisFun.Val2(.TextMatrix(tRow, 0)) > Last_No Then
                 .TextMatrix(tRow, 0) = ""
                 .TextMatrix(tRow, 8) = ""
              End If
            End If
        Else
           If Trim(iB1_BusyoCD) <> "" And Trim(.TextMatrix(tRow, 8)) = "" Then
              .TextMatrix(tRow, 8) = .TextMatrix(tRow, 0)
              Next_No = Next_No + 1
           End If
        End If
    End With
...
Private Sub VsGridToText(tRow As Integer)
    With vsGrid
        ...
        .TextMatrix(tRow, 0) = .TextMatrix(tRow, 8)     ' 2009.10.27 add
        If Trim(.TextMatrix(tRow, 0)) = "" Then
           .TextMatrix(tRow, 0) = Format(Next_No, "000")
        End If
    End With
End Sub
