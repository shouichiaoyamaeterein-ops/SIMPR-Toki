...
Begin VB.Form CTP3110 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "eかんばん受注データ変換"
   ClientHeight    =   11235
   ClientLeft      =   1335
   ClientTop       =   2385
   ClientWidth     =   15360
...
'------------------   新eかんばん受注累積処理
    If eKanban_ProcKB <> 0 Then lb_msg.Caption = "新eかんばんﾃﾞｰﾀ累積中": DoEvents
    
    If Not Jyucyu_Proc Then GoTo eKanbanProc_Er
'------------------   新eかんばん受注分納更新処理
' 新eかんばん受注データに分納分はなし                   2008/07/18DEL
    If eKanban_ProcKB <> 0 Then lb_msg.Caption = "新eかんばんﾃﾞｰﾀ分納更新中": DoEvents
...
''------------------- 出荷指示書発行
'    If eKanban_ProcKB <> 0 Then lb_msg.Caption = "出荷指示書発行": DoEvents
'    ' ログ出力
'    gStr = "【 出荷指示書発行 開始 】"
'    Call eKanban_LogPut(gStr, "", 1)
'
...
'============================================= 2009/07/06
''------------------- 箱種一覧発行
'    If eKanban_ProcKB <> 0 Then lb_msg.Caption = "箱種一覧発行": DoEvents
'    ' ログ出力
'    gStr = "【 箱種一覧発行 開始 】"
'    Call eKanban_LogPut(gStr, "", 1)
'
'    EHR.ERR = "62"      '箱種一覧発行ｴﾗｰ
'    If Not Middle_EHRUpDate Then Exit Function
'
'    If eKanban_ProcKB = 0 And eKanban_ProcDisp = 0 Then
'        pref_DispForm = 1
...
Public Function Jyucyu_Proc() As Boolean
    ...
    ' ログ出力
    gStr = "【 新eかんばん受注累積（出荷指示書NO採番) 開始 】"
    Call eKanban_LogPut(gStr, "", 1)
    
    EHR.ERR = "52"      '新eかんばん受注累積ｴﾗｰ
    If Not Middle_EHRUpDate Then Exit Function
    With CisDB
        .SQL = "出荷指示書NO採番累積"
        .StoadoCount = 5
...
Jyucyu_Proc_ErDisp:
    ...
    ' ログ出力
    gStr = "【 新eかんばん受注累積（出荷指示書NO採番) 異常終了 】"
    Call eKanban_LogPut(gStr, "", 1)
...
Public Function Bun_Proc() As Boolean
    ...
    ' ログ出力
    gStr = "【 新eかんばん受注分納更新 開始 】"
    Call eKanban_LogPut(gStr, "", 1)
    
    EHR.ERR = "53"      '新eかんばん分納更新ｴﾗｰ
    If Not Middle_EHRUpDate Then Exit Function
    With CisDB
        .SQL = "eかんばん受注分納更新"
        .StoadoCount = 3
...
Private Function Convert_ListProc() As Boolean
    ...
    If Not eKanban_CvtList(eKanban_Proc) Then
       ...
       Exit Function
    End If
    If eKanban_ProcKB <> 0 Then lb_msg.Caption = "受注変換リスト発行": DoEvents
    
    ' ログ出力
    gStr = "【 新eかんばん受注変換リスト出力 開始 】"
    Call eKanban_LogPut(gStr, "", 1)
...
' * 変換エラーリスト
    ...
    If EHTRead(gSL_Select) Then
        ...
        lb_msg = "【 新eかんばん受注 】 変換リスト 印刷中"
        DoEvents
        With eKanban_Prt
             Set .iPrtForm = CTP3110L
             .dpMSG = "新eかんばん受注変換リスト　印刷中"
             .iPreview = pv_Print
             If eKanban_ProcKB = 0 And eKanban_ProcDisp = 0 Then
                .dpMSGScreen = 画面なし
             Else
...
Private Function ZenProc_Get()
    ...
    Call ItemsClearSYM
    gSL_Select = "Select * From 名称マスタ "
    gSL_Select = gSL_Select & " Where 区分名称   = '新eかんばん管理'"
    gSL_Select = gSL_Select & " And   区分タイプ = ''"
