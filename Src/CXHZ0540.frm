...
Begin VB.Form CXHZ0540 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料・支給実績入力-品番入力"
   ClientHeight    =   9945
   ClientLeft      =   1635
   ClientTop       =   1725
   ClientWidth     =   14790
   ...
   Begin VB.PictureBox H2_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D4 
         ...
         cAlingnment     =   6
         cPositionY      =   30
         Begin VB.OptionButton OpH2_HinKB 
            BackColor       =   &H00C0C0C0&
            Caption         =   "受給品番"
            BeginProperty Font 
               Name            =   "ＭＳ ゴシック"
      ...
      Begin Cis3D_v60.CIS3D UC_3D4 
         ...
         Begin Cis3D_v60.CIS3D BLB_DD 
            ...
            Begin vsFlexLib.vsFlexArray vsGrid 
               ...
               Appearance      =   1
               ConvInfo        =   1418783674
               FormatString    =   "№ |品 番 / 材質-寸法                             |背番号  |収容数    |枚数 |端　数    |支給数      |単位|有無償   |"
               Rows            =   4
               Cols            =   19
               BackColor       =   12648384
               ...
               Begin Cis3D_v60.CIS3D Input_Area 
                  ...
                  cFont3DColor2   =   8421504
                  cBoderStyle     =   1
                  Begin Cis3D_v60.CIS3D B1lb_Nagasa 
                     Height          =   210
                     Left            =   9840
                     Top             =   240
                     Visible         =   0   'False
                     ...
                     cPositionX      =   30
                  End
                  Begin Cis3D_v60.CIS3D B1lb_Haba 
                     Height          =   210
                     Left            =   10425
                     Top             =   195
                     Visible         =   0   'False
                     ...
                     cPositionX      =   30
                  End
                  Begin Cis3D_v60.CIS3D B1lb_Ita 
                     Height          =   210
                     Left            =   10425
                     Top             =   0
                     Visible         =   0   'False
...
'        iB1_Width = RV_Width
'        iB1_Long = RV_Long
        Call iB1_Hinbn_LostFocus    ' 2008.09.04 add
    End If
End Sub
...
Private Sub iB1_Hinbn_LostFocus()
    ...
    vsGrid.TextMatrix(CisFun.Val2(Input_Area.Tag), 1) = iB1_Hinbn & vbCr & B1lb_ZSunpo
    vsGrid.TextMatrix(CisFun.Val2(Input_Area.Tag), 14) = iB1_Hinbn
...
' マスタ入力 SELECT文 ｾｯﾄ
    ...
    If OpH2_HinKB(0) Then
       gSL_Select = gSL_Select & " zt.受給品番 品番,zm.表示寸法,"
    Else
       gSL_Select = gSL_Select & " zt.発注品番 品番,zm.表示寸法,"
    End If
    ...
    gSL_Select = gSL_Select & "  on  zjk.材質 = zm.材質 "
    gSL_Select = gSL_Select & "  and zjk.板厚 = zm.板厚 "
    gSL_Select = gSL_Select & "  and zjk.幅 = zm.幅 "
    gSL_Select = gSL_Select & "  and zjk.長さ = zm.長さ "
    gSL_Select = gSL_Select & " left outer join 材料取引先マスタ zt"
    gSL_Select = gSL_Select & "  on  zt.材料管理番号 = zm.材料管理番号 "
    gSL_Select = gSL_Select & "  and zjk.実績先 = zt.取引先 "
    gSL_Select = gSL_Select & "  and zjk.受入 = zt.受入 "
        ...
        For wRowNo1 = 1 To .Rows - 1
            If RTrim(.TextMatrix(wRowNo1, 1)) = "" Then
                For wRowNo2 = wRowNo1 + 1 To .Rows - 1
                    If RTrim(.TextMatrix(wRowNo2, 14)) <> "" Then
                        For wColNo1 = 1 To 13
...
'
        For wRowNo1 = 1 To .Rows - 1
            If RTrim(.TextMatrix(wRowNo1, 14)) = "" Then Exit For

        '   << 重複チェック >>
            For wRowNo2 = wRowNo1 + 1 To .Rows - 1
                If RTrim(.TextMatrix(wRowNo1, 14)) = RTrim(.TextMatrix(wRowNo2, 14)) Then
        ...
        '   << 材料マスタ存在チェック >>
            ...
            gSL_Select = gSL_Select & " where 材質 = '" & RTrim(.TextMatrix(wRowNo1, 12)) & "'"
            gSL_Select = gSL_Select & "   and 板厚 = " & CisFun.Val2(.TextMatrix(wRowNo1, 15)) & ""
            gSL_Select = gSL_Select & "   and 幅 = " & CisFun.Val2(.TextMatrix(wRowNo1, 16)) & ""
            gSL_Select = gSL_Select & "   and 長さ = " & CisFun.Val2(.TextMatrix(wRowNo1, 17)) & ""
'Call Test_LogOutput(gSL_Select, 1)  '<<<<<< Debug
            If Not ZRMRead(gSL_Select, 1) Then
                CisFun.MB_MSG(2) = "    【" & wRowNo1 & "行目】　材料マスタ　未登録！            "
                CisFun.MBOX
                Set ErrObj = iB1_Hinbn
                GoTo Body1Chk_Err
            End If
...
'            If Not ZTMRead("", 1) Then
            If Not ZTMRead(gSL_Select, 1) Then
                CisFun.MB_MSG(2) = "    【" & wRowNo1 & "行目】　材料取引先マスタ　未登録！            "
                CisFun.MBOX
                Set ErrObj = iB1_Hinbn
                GoTo Body1Chk_Err
            End If
...
' ( 追加処理 )
    ...
    For gIndex = 1 To vsGrid.Rows - 1
        If RTrim(vsGrid.TextMatrix(gIndex, 14)) <> "" Then
...
'       ## 修正の場合 ##
    ...
    With ZJK
        .納品書行 = ID
        .材質 = RTrim(vsGrid.TextMatrix(ID, 12))
        .板厚 = CisFun.Val2(vsGrid.TextMatrix(ID, 15))
        .幅 = CisFun.Val2(vsGrid.TextMatrix(ID, 16))
        .長さ = CisFun.Val2(vsGrid.TextMatrix(ID, 17))
        ...
        .有無償区分 = CisFun.Val2(vsGrid.TextMatrix(ID, 9))
    End With
    Call ZJKInsert
...
Private Sub Hinbn_Set()
    ...
    gSL_Select = gSL_Select & ", isnull(zrm.板厚, '') 板厚"
    gSL_Select = gSL_Select & ", isnull(zrm.幅, '') 幅"
    gSL_Select = gSL_Select & ", isnull(zrm.長さ, '') 長さ"
    gSL_Select = gSL_Select & " from"
...
'*--------------------------------------------------<< 2004/06/23 >> Update End
    ...
    If OpH2_HinKB(0) Then
       gSL_Select = gSL_Select & " where ztm.受給品番 = '" & RTrim(iB1_Hinbn) & "'"
    Else
       gSL_Select = gSL_Select & " where ztm.発注品番 = '" & RTrim(iB1_Hinbn) & "'"
    End If
'*--------------------------------------------------<< 2004/06/23 >> Insert Start
    gSL_Select = gSL_Select & " order by"
...
Private Sub Seban_Set()
    ...
    gSL_Select = gSL_Select & "  isnull(sym.値名称, '') 手配単位名"
    If OpH2_HinKB(0) Then
       gSL_Select = gSL_Select & ", isnull(ztm.受給品番, '') 品番"
    Else
       gSL_Select = gSL_Select & ", isnull(ztm.発注品番, '') 品番"
    End If
    gSL_Select = gSL_Select & ", isnull(zrm.材質, '') 材質"
    gSL_Select = gSL_Select & ", isnull(zrm.表示寸法, '') 表示寸法"
    gSL_Select = gSL_Select & ", isnull(ztm.変更日, '') 変更日"
...
'*--------------------------------------------------<< 2004/06/23 >> Update End
    ...
    gSL_Select = gSL_Select & "   and sym.値 = zrm.単位"
    gSL_Select = gSL_Select & "   and sym.区分タイプ = ''"
    gSL_Select = gSL_Select & " where ztm.背番号 = '" & RTrim(iB1_Seban) & "'"
'*--------------------------------------------------<< 2004/06/23 >> Insert Start
    gSL_Select = gSL_Select & " order by"
    gSL_Select = gSL_Select & "  isnull(ztm.受入, '')"
'*--------------------------------------------------<< 2004/06/23 >> Insert End
    If ZTMRead(gSL_Select, 1) Then
        iB1_Hinbn = RTrim(ZTM.品番)
        B1lb_Tannm = RTrim(ZTM.手配単位名)
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 46, 1, "品番 / 材質:寸法")
        ...
        Call .FixedSet(中中, 右中, あり, 12, 6, "実績数")
        Call .FixedSet(中中, 中中, あり, 4, 7, "単位")
        Call .FixedSet(中中, 中中, なし, 9, 8, "有無償")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, なし, 0, 9, "有無償区分")
        Call .FixedSet(中中, 左中, なし, 0, 10, "実績管理NO")
        Call .FixedSet(中中, 左中, なし, 0, 11, "Select")
        Call .FixedSet(中中, 左中, なし, 0, 12, "材質")
        Call .FixedSet(中中, 左中, なし, 0, 13, "表示寸法")
        Call .FixedSet(中中, 左中, なし, 0, 14, "品番")
        Call .FixedSet(中中, 右中, あり, 0, 15, "板厚")
        Call .FixedSet(中中, 右中, あり, 0, 16, "幅")
        Call .FixedSet(中中, 右中, あり, 0, 17, "長さ")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
...
'
    With vsGrid
        ...
        '材質
        .TextMatrix(SetRow, 12) = RTrim(ZJK.材質)
        '品番 / 材質寸法
        .TextMatrix(SetRow, 1) = RTrim(ZJK.品番) & vbCr & RTrim(ZJK.材質) & " " & RTrim(ZJK.表示寸法)
        '背番号
        .TextMatrix(SetRow, 2) = RTrim(ZJK.背番号)
        '収容数
        If ZJK.収容数 <> 0 Then
            .TextMatrix(SetRow, 3) = Format(ZJK.収容数, "###0.000")
        End If
        '枚数
        .TextMatrix(SetRow, 4) = Format(ZJK.実績枚数, "####")
        '端数
        If ZJK.実績端数 <> 0 Then
            .TextMatrix(SetRow, 5) = Format(ZJK.実績端数, "###0.000")
        End If
        '実績数
        If ZJK.実績数 <> 0 Then
            .TextMatrix(SetRow, 6) = Format(ZJK.実績数, "###0.000")
        End If
        '単位
        .TextMatrix(SetRow, 7) = RTrim(SYM.値名称)
        '有無償
        If ZJK.実績区分 = 3 Then
            Select Case ZJK.有無償区分
                Case 0: .TextMatrix(SetRow, 8) = "無償"
                Case 1: .TextMatrix(SetRow, 8) = "有償"
            End Select
        End If
        .TextMatrix(SetRow, 9) = ZJK.有無償区分
        .TextMatrix(SetRow, 10) = ZJK.実績管理NO
    
        .TextMatrix(SetRow, 12) = ZJK.材質
        .TextMatrix(SetRow, 13) = ZJK.表示寸法
        .TextMatrix(SetRow, 14) = ZJK.品番
        '板厚
        If ZJK.板厚 <> 0 Then
            .TextMatrix(SetRow, 15) = Format(ZJK.板厚, "###0.00")
        End If
        '幅
        If ZJK.幅 <> 0 Then
            .TextMatrix(SetRow, 16) = Format(ZJK.幅, "###0.00")
        End If
        '長さ
        If ZJK.長さ <> 0 Then
            .TextMatrix(SetRow, 17) = Format(ZJK.長さ, "###0.00")
        End If
    End With
End Sub
   ...
   '↓
    If KeyCode = vbKeyDown Then
        If gInt + 1 > vsGrid.Rows - 1 Then
            If RTrim(vsGrid.TextMatrix(gInt, 14)) <> "" Then
...
Private Sub TextToVsGrid(RowNo As Integer)
    ...
    With vsGrid
        '品番/材質寸法
        .TextMatrix(RowNo, 1) = RTrim(iB1_Hinbn) & vbCr & RTrim(B1lb_ZSunpo)
        
        '背番号
        .TextMatrix(RowNo, 2) = RTrim(iB1_Seban)
        '収容数
        .TextMatrix(RowNo, 3) = B1lb_Syuyo
        '枚数
        If CisFun.Val2(iB1_Maisu) <> 0 Then
        ...
        End If
        '端数
        .TextMatrix(RowNo, 5) = B1lb_Hasu
        '実績数
        If CisFun.Val2(iB1_Suryo) <> 0 Then
            .TextMatrix(RowNo, 6) = Format(CisFun.Val2(iB1_Suryo), "###0.000")
        Else
            .TextMatrix(RowNo, 6) = ""
        End If
        '単位
        .TextMatrix(RowNo, 7) = RTrim(B1lb_Tannm)
        '有無償
        .TextMatrix(RowNo, 8) = ""
        If RTrim(iB1_Hinbn) <> "" Then
            .TextMatrix(RowNo, 8) = B1Chk_UMkbn.Caption        ' If H1Op_JKbn(3) Then
        End If
        .TextMatrix(RowNo, 9) = B1Chk_UMkbn.Value
    
        .TextMatrix(RowNo, 12) = RTrim(B1lb_Zaisitu)
        .TextMatrix(RowNo, 13) = RTrim(B1lb_Sunpo)
        .TextMatrix(RowNo, 14) = RTrim(iB1_Hinbn)
    
        .TextMatrix(RowNo, 15) = RTrim(B1lb_Ita)
        .TextMatrix(RowNo, 16) = RTrim(B1lb_Haba)
        .TextMatrix(RowNo, 17) = RTrim(B1lb_Nagasa)
...
Private Sub VsGridToText(RowNo As Integer)
    With vsGrid
        ...
        '品番
        iB1_Hinbn = RTrim(.TextMatrix(RowNo, 14))
        '材質/寸法
        B1lb_ZSunpo = RTrim(.TextMatrix(RowNo, 12)) & " " & RTrim(.TextMatrix(RowNo, 13))
        '背番号
        iB1_Seban = RTrim(.TextMatrix(RowNo, 2))
        '収容数
        B1lb_Syuyo = .TextMatrix(RowNo, 3)
        '枚数
        If CisFun.Val2(.TextMatrix(RowNo, 4)) <> 0 Then
            iB1_Maisu = CisFun.Val2(.TextMatrix(RowNo, 4))
        Else
            iB1_Maisu = ""
        End If
        '端数
        B1lb_Hasu = .TextMatrix(RowNo, 5)
        '支給数
        If CisFun.Val2(.TextMatrix(RowNo, 6)) <> 0 Then
            iB1_Suryo = CisFun.Val2(.TextMatrix(RowNo, 6))
        Else
            iB1_Suryo = ""
        End If
        '単位
        B1lb_Tannm = RTrim(.TextMatrix(RowNo, 7))
        '有無償
        B1Chk_UMkbn = CisFun.Val2(.TextMatrix(RowNo, 9))
    
        B1lb_Zaisitu = RTrim(.TextMatrix(RowNo, 12))
        B1lb_Sunpo = RTrim(.TextMatrix(RowNo, 13))
        
        B1lb_Ita = RTrim(.TextMatrix(RowNo, 15))
        B1lb_Haba = RTrim(.TextMatrix(RowNo, 16))
        B1lb_Nagasa = RTrim(.TextMatrix(RowNo, 17))
...
Private Sub VSGrid_DblClick()
    If RTrim(vsGrid.TextMatrix(vsGrid.Row, 14)) = "" Then
        Exit Sub
    End If
    With CisFun
        .MB_Lines = 4
...
'    iB1_Long = ""
    ...
    B1lb_Ita = "": B1lb_Haba = "": B1lb_Nagasa = ""
    Call TextToVsGrid(Val(Input_Area.Tag))
End Sub
