...
'       変更日        : 2023/08/23   CIS  東プレ:受注(Prefix 4)を追加
'       変更日        : 2025/12/17   CIS  リバース:受注(Prefix 5)を追加
'
'***********************************************************************************
' ------ 2023/08/23 MNT Start ------
'Public Text_Proc           As Byte     '0:受注[愛知機械],1:受注[テクノエイト],2:出荷[愛知機械],3:受注[ヨロズ愛知]
' ------ 2025/12/17 UPD Start ------
'Public Text_Proc           As Byte     '0:受注[愛知機械],1:受注[テクノエイト],2:出荷[愛知機械],3:受注[ヨロズ愛知],4:受注[東プレ]
Public Text_Proc           As Byte     '0:受注[愛知機械],1:受注[テクノエイト],2:出荷[愛知機械],3:受注[ヨロズ愛知],4:受注[東プレ],5:受注[リバース]
' ------ 2025/12/17 UPD End   ------
' ------ 2023/08/23 MNT End   ------
...
' ------ 2025/12/17 UPD Start ------
'Public Text_Prefix(4)      As String   ' ﾌﾟﾚﾌｨｯｸｽ (0:JY(愛知機械),1:JY(ﾃｸﾉｴｲﾄ),2:SK(愛知機械),3:JY(ﾖﾛｽﾞ愛知),4:JY(東プレ) )
'Public Text_PrefixNm(4)    As String   ' ﾌﾟﾚﾌｨｯｸｽ名 (0:受注,1:受注,2:出荷,4:受注)
'Public Text_InDir(4)       As String   ' 端末ﾃｷｽﾄ格納先
'Public Text_TextName(4)    As String   ' 端末ﾃｷｽﾄ名(ﾜｲﾙﾄﾞｶｰﾄﾞ指定可能)
'Public Text_OutDir(4)      As String   ' SIMPR-A ﾃﾞｰﾀ退避先
'Public Text_DelDay(4)      As Long     ' 保持期間(ﾃｷｽﾄﾃﾞｰﾀ･ﾛｸﾞ･履歴情報)
'Public Text_CvtList(4)     As Boolean  ' 変換ﾘｽﾄ発行ﾌﾗｸﾞ(True:発行あり,False:発行なし)
'Public Text_CvtErr(4)      As Byte     '   ｴﾗｰ内容    0:対象、1:対象外
'Public Text_CvtDbl(4)      As Byte     '   重複       0:対象、1:対象外
'Public Text_CvtKari(4)     As Byte     '   仮登録内容 0:対象、1:対象外
'Public Text_CvtEtc(4)      As Byte     '   その他     0:対象、1:対象外

'Public Text_Orderer(4)     As String   ' 受注先（愛知機械/ﾃｸﾉｴｲﾄ/ﾖﾛｽﾞ愛知/東プレ）

Public Text_Prefix(5)      As String   ' ﾌﾟﾚﾌｨｯｸｽ (0:JY(愛知機械),1:JY(ﾃｸﾉｴｲﾄ),2:SK(愛知機械),3:JY(ﾖﾛｽﾞ愛知),4:JY(東プレ),5:JY(リバース) )
Public Text_PrefixNm(5)    As String   ' ﾌﾟﾚﾌｨｯｸｽ名 (0:受注,1:受注,2:出荷,4:受注,5:受注)
Public Text_InDir(5)       As String   ' 端末ﾃｷｽﾄ格納先
Public Text_TextName(5)    As String   ' 端末ﾃｷｽﾄ名(ﾜｲﾙﾄﾞｶｰﾄﾞ指定可能)
Public Text_OutDir(5)      As String   ' SIMPR-A ﾃﾞｰﾀ退避先
Public Text_DelDay(5)      As Long     ' 保持期間(ﾃｷｽﾄﾃﾞｰﾀ･ﾛｸﾞ･履歴情報)
Public Text_CvtList(5)     As Boolean  ' 変換ﾘｽﾄ発行ﾌﾗｸﾞ(True:発行あり,False:発行なし)
Public Text_CvtErr(5)      As Byte     '   ｴﾗｰ内容    0:対象、1:対象外
Public Text_CvtDbl(5)      As Byte     '   重複       0:対象、1:対象外
Public Text_CvtKari(5)     As Byte     '   仮登録内容 0:対象、1:対象外
Public Text_CvtEtc(5)      As Byte     '   その他     0:対象、1:対象外

Public Text_Orderer(5)     As String   ' 受注先（愛知機械/ﾃｸﾉｴｲﾄ/ﾖﾛｽﾞ愛知/東プレ/リバース）
' ------ 2025/12/17 UPD End   ------
' ------ 2023/08/23 MNT End   ------
...
' ------ 2023/08/23 ADD Start ------
    Text_Prefix(4) = "JY": Text_PrefixNm(4) = "受注"
' ------ 2023/08/23 ADD End   ------
' ------ 2025/12/17 ADD Start ------
    Text_Prefix(5) = "JY": Text_PrefixNm(5) = "受注"
' ------ 2025/12/17 ADD End   ------
    ' 受注先をセット
...
' ------ 2023/08/23 ADD Start ------
           Case 4: sw_Table = "テキスト受注データ"
                   sw_Stoado = "テキスト受注TEXT変換_東プレ"
' ------ 2023/08/23 ADD End   ------
' ------ 2025/12/17 ADD Start ------
           Case 5: sw_Table = "テキスト受注データ"
                   sw_Stoado = "テキスト受注TEXT変換_リバース"
' ------ 2025/12/17 ADD End   ------
...
' ------ 2025/12/17 UPD Start ------
        'If Text_Proc = 4 Then
        If Text_Proc = 4 Or Text_Proc = 5 Then
' ------ 2025/12/17 UPD End   ------
            Dim lsRec() As String
        End If
' ------ 2023/08/23 ADD End   ------
...
' ------ 2025/12/17 UPD Start ------
'       Text_Rec = Replace(Text_Rec, """", "")
        If Text_Proc <> 5 Then
            Text_Rec = Replace(Text_Rec, """", "")
        End If
' ------ 2025/12/17 UPD End   ------
        If CisFun.Mid2(Text_Rec, 1, 6) <> "納入者" Then

' ------ 2023/08/23 ADD Start ------
        If Text_Proc = 4 Then
            '注文番号=0は対象外
            lsRec = Split(Text_Rec, ",")
'            If CLng(lsRec(6)) = 0 Then
            Debug.Print lsRec(6)
            If Replace((lsRec(6)), """", "") = "00000000" Then
                GoTo Text_Text_Proc_Skip
            End If
        End If
' ------ 2023/08/23 ADD End   ------
' ------ 2025/12/17 ADD Start ------
...
' ------ 2023/08/23 ADD Start ------
        If Text_Proc = 4 Then
            '取引先、受入を取得
            gSL_Select = "SELECT 取引先, 受入 FROM 品番取引先マスタ WHERE 品番 = '" & RTrim(lsRec(2)) & "' AND 品目 = 0"
            If Not HTMRead(gSL_Select, 1) Then
                ReDim DispMsg(4)
                DispMsg(0) = "      " & sw_Stoado & "エラー"
                DispMsg(2) = "    品番取引先マスタ未登録            "
                DispMsg(3) = "    品番 = " & RTrim(lsRec(2))
                GoTo Text_Text_Proc_ErrDisp
            Else
                Text_Rec = Text_Rec & "," & HTM.取引先 & "," & HTM.受入
                lsRec = Split(Text_Rec, ",")
            End If
        End If
' ------ 2023/08/23 ADD End   ------
' ------ 2025/12/17 ADD Start ------
        If Text_Proc = 5 Then
            ...
            Hinban = Replace(RTrim(lsRec(4)), "-", "")
            '取引先、受入を取得
            gSL_Select = "SELECT 取引先, 受入 FROM 品番取引先マスタ WHERE 品番 = '" & Hinban & "' AND 品目 = 0"
            If Not HTMRead(gSL_Select, 1) Then
                ReDim DispMsg(4)
                DispMsg(0) = "      " & sw_Stoado & "エラー"
                DispMsg(2) = "    品番取引先マスタ未登録            "
                DispMsg(3) = "    品番 = " & RTrim(lsRec(4))
                GoTo Text_Text_Proc_ErrDisp
            End If
        End If
' ------ 2025/12/17 ADD End   ------
...
'        If Text_Proc = 0 Or Text_Proc = 3 Then
        If Text_Proc = 0 Or Text_Proc = 3 Or Text_Proc = 4 Then
' ------ 2023/08/23 MNT End   ------
...
' ------ 2025/12/17 UPD Start ------
'        If Text_Proc <> 0 And Text_Proc <> 1 And Text_Proc <> 3 And Text_Proc <> 4 Then
        If Text_Proc <> 0 And Text_Proc <> 1 And Text_Proc <> 3 And Text_Proc <> 4 And Text_Proc <> 5 Then
' ------ 2025/12/17 UPD End   ------
' ------ 2023/08/23 MNT End   ------
...
'            If (Text_Proc = 0) Or (Text_Proc = 1 And gInt <> 1) Or (Text_Proc = 2) Or (Text_Proc = 3) Then
            If (Text_Proc = 0) Or (Text_Proc = 1 And gInt <> 1) Or (Text_Proc = 2) Or (Text_Proc = 3) Or (Text_Proc = 4) Then
' ------ 2023/08/23 MNT End   ------
                .SQL = gSL_Select
                If Not .DBExec Then
                   gStr = .ErrCode & ":" & .ErrMsg
...
' ------ 2023/08/23 ADD Start ------
            If Text_Proc = 4 Then
                gStr = " " & Format(Format(gInt, "#,###"), "@@@@@") & "："
                gStr = gStr & lsRec(131) & " " _
                            & lsRec(132) & " "
                gStr = gStr & lsRec(14) & Right(lsRec(4), 2) _
                            & " " & lsRec(6) & " "
                gStr = gStr & lsRec(2)
            End If
' ------ 2023/08/23 ADD End   ------
...
' ------ 2025/12/17 ADD Start ------
        If Text_Proc = 5 Then
            Dim Shun As Long
            Dim LastDay As Long
            Dim NounyuuDate As Date
            Dim CalcNounyuuDate As Long
            Dim targetDate As Long

            Shun = CLng(CisFun.Mid2(lsRec(8), 7, 1))
            If Shun = 1 Or Shun = 2 Then
                '10日分の値を設定
                LastDay = 18
            Else
                '月末最終日に合わせた値を設定　月末最終日 + CSV開始位置(8) - 1～20日までの除外する日(20)
                LastDay = Day(DateSerial(CLng(CisFun.Left2(lsRec(8), 4)), CLng(CisFun.Mid2(lsRec(8), 5, 2)) + 1, 0)) + 8 - 20
            End If

            For targetDate = 9 To LastDay
                If lsRec(targetDate) <> 0 Then
                    CalcNounyuuDate = (Shun - 1) * 10 + (targetDate - 8)
                    NounyuuDate = DateSerial(CisFun.Left2(lsRec(8), 4), CisFun.Mid2(lsRec(8), 5, 2), CalcNounyuuDate)
                    ' ------ 2026/02/18 ADD Start ------
                    ' 取込除外品番チェック（名称マスタ）
                    gSL_Select = "SELECT TOP 1 値 FROM 名称マスタ" _
                               & " WHERE 区分名称 = '取込除外品番(リバース)'" _
                               & " AND 値 = '" & Hinban & "'"
                    ...
                    ' 取込確認品番チェック（名称マスタ）
                    gSL_Select = "SELECT TOP 1 値 FROM 名称マスタ" _
                               & " WHERE 区分名称 = '取込確認品番(リバース)'" _
                               & " AND 値 = '" & Hinban & "'"
                    If SYMRead(gSL_Select, 1) Then
                        Dim lsMsg As String
                        Dim lnMsgResult As Integer
                        lsMsg = "以下のデータを取り込みますか？" & vbCrLf _
                              & "品番：" & RTrim(lsRec(4)) & vbCrLf _
                              & "納入日：" & Format(NounyuuDate, "yyyymmdd") & vbCrLf _
                              & "数量：" & lsRec(targetDate)
                        lnMsgResult = MsgBox(lsMsg, vbYesNo + vbQuestion, "取込確認")
                        If lnMsgResult = vbNo Then
                            GoTo ConfirmSkip
                        End If
                    End If
                    ' ------ 2026/02/18 ADD End ------
                    sw_TE_Text = Hinban & ",1054,01," & Format(NounyuuDate, "yyyymmdd") & "," & lsRec(targetDate) & "," & Format(Date, "yyyymmdd")
                    gSL_Select = "INSERT INTO テキスト受注データ (REC1) VALUES ('" & CisFun.Mid2(sw_TE_Text, 1) & "')"
                    ...
                    If Not .DBExec Then
                        ...
                        GoTo Text_Text_Proc_Err
                    End If
                    gStr = " " & Format(Format(gInt, "#,###"), "@@@@@") & "：" & lsRec(4) & " " & Format(NounyuuDate, "yyyymmdd") & " "
                    Call Text_LogPut(gStr, , 5)
                    End With
' ------ 2026/02/18 ADD Start (スキップラベル) ------
ConfirmSkip:
' ------ 2026/02/18 ADD End ------
                End If
            Next targetDate
...
' ------ 2023/08/23 UPD Start ------
'        If Text_Proc = 0 Or Text_Proc = 1 Or Text_Proc = 3 Then
' ------ 2025/12/17 UPD Start ------
'        If Text_Proc = 0 Or Text_Proc = 1 Or Text_Proc = 3 Or Text_Proc = 4 Then
        If Text_Proc = 0 Or Text_Proc = 1 Or Text_Proc = 3 Or Text_Proc = 4 Or Text_Proc = 5 Then
' ------ 2025/12/17 UPD End   ------
' ------ 2023/08/23 UPD End   ------
...
' ------ 2025/12/17 UPD Start ------
'              .SQL = .SQL & "   And 受注出荷区分 IN ( 1,3,4 )"
              .SQL = .SQL & "   And 受注出荷区分 IN ( 1,3,4,5 )"
' ------ 2025/12/17 UPD End   ------
' ------ 2023/08/23 UPD End   ------
           End If
        Else
...
' ------ 2023/08/23 UPD End   ------
       If Text_Proc <> 2 Then
          ...
          gSL_Select = gSL_Select & " And   TH.処理区分 = 0 "                       'IS
       Else
...
''       gSL_Select = gSL_Select & " And   TH.処理区分 = 0 "
       'gSL_Select = gSL_Select & " And   TH.処理区分 = " & Text_Proc & ""
       If Text_Proc <> 4 Then                                                       'IS
           gSL_Select = gSL_Select & " And   TH.処理区分 = " & Text_Proc & ""       'IS
       Else                                                                         'IS
          gSL_Select = gSL_Select & " And   TH.処理区分 = 0 "                       'IS
       End If
'==================================== 2010/07/23 End
...
'--------------------------------------------------------------------------------
'*
'*   CSV形式の1行文字列を配列に分割して返す
'*
'*   モジュールID    : ParseCSVLine
'*
'*   初期設定項目    : なし
'*   引数           : line    -> CSV形式の1行文字列
'*   戻値           : 各フィールドを要素とする配列（Variant型）
'*
'*   備考           : ダブルクォートで囲まれたカンマや、連続クォート（""）のエスケープに対応
'*   作成日  :2025/12/17
'*   変更日  :
'--------------------------------------------------------------------------------
Function ParseCSVLine(ByVal line As String) As Variant                                          ' CSV1行を配列に分割
    Dim result() As String                                                                      ' 結果格納用配列
    Dim currentField As String                                                                  ' 現在のフィールド
    Dim inQuotes As Boolean                                                                     ' ダブルクォート内フラグ
    Dim charIndex As Integer                                                                    ' 文字位置インデックス
    Dim currentChar As String                                                                   ' 現在の文字
    Dim fieldIndex As Integer                                                                   ' フィールド番号

    ReDim result(0)                                                                             ' 配列初期化
    currentField = ""                                                                           ' フィールド初期化
    inQuotes = False                                                                            ' クォート内フラグ初期化
    fieldIndex = 0                                                                              ' フィールド番号初期化

    For charIndex = 1 To Len(line)                                                              ' 1文字ずつループ
        currentChar = Mid(line, charIndex, 1)                                                   ' 現在の文字取得
        If currentChar = """" Then                                                              ' ダブルクォートの場合
            If inQuotes And charIndex < Len(line) And Mid(line, charIndex + 1, 1) = """" Then
                currentField = currentField & """"                                              ' 連続クォートの場合エスケープ文字と判断
                charIndex = charIndex + 1
            Else
                inQuotes = Not inQuotes                                                         ' クォート内外切替
            End If
        ElseIf currentChar = "," And Not inQuotes Then                                          ' カンマでクォート外なら区切り
            result(fieldIndex) = currentField
            currentField = ""
            fieldIndex = fieldIndex + 1
            ReDim Preserve result(fieldIndex)
        Else
            currentField = currentField & currentChar                                           ' それ以外は追加
        End If
    Next charIndex
    result(fieldIndex) = currentField                                                           ' 最後のフィールド格納
    ParseCSVLine = result                                                                       ' 結果を返す
End Function
' ------ 2025/12/17 ADD End   ------
