...
'*---------------------------------*
    ...
    Call CisPrtCXH0330.ClrLstFld("B1")
    
'*---------------------------------*
'*  発注テーブル読込み
'*---------------------------------*
    Do Until Not HCT_RDSTS
        If Key_MeiNo <> HCT.明細書NO Or CisPrtCXH0330.cLine >= CisPrtCXH0330.iLineMax Then
            If Key_MeiNo <> 0 Then
                If Key_MeiNo <> HCT.明細書NO Then
                    ...
                    If MeiPage > 1 Then
                        ...
                        End If
                        If Not CXH0330_Kara Then
                           Call CisPrtCXH0330.PrintFld("B1*")
                        End If
                        CisPrtCXH0330.cLine = CisPrtCXH0330.cLine + 1
                        Call CisPrtCXH0330.ClrLstFld("B1")
        ...
        Else
            B2_Kara = "※※※　今回発注はありません　※※※"
            Call CisPrtCXH0330.PrintFld("B2*")
            CisPrtCXH0330.cLine = CisPrtCXH0330.cLine + 1
            Call CisPrtCXH0330.ClrLstFld("B2")
        End If
        '*---------------------------------*
        '*  カウンタ　ＵＰ
        '*---------------------------------*
        With CisPrtCXH0330
            .cDataCnt = .cDataCnt + 1
            .dpCountDisp Format(.cDataCnt, "#,###") & " / " & Format(CXH0330_PrtCnt, "#,###")
        End With
        
        If Not CXH0330_Kara Then
            ' 発注行テーブル　発行日更新(新規発行のみ)
            If CXH0330_PrintKb = 8 Then
                ...
                With CisDB
                    ...
                    On Error GoTo CoPrintH0330_Err
                    .SQL = gSL_Select
                    .DBExec
...
'最終頁　枚数計印刷
    ...
    ' 印刷終了
    Call CisPrtCXH0330.PrintEndDoc
...
Private Function HeadWrite(KB As Byte)
    ...
    '発行日・自社名
        ...
        gSL_Select = gSL_Select & "   AND 英数字1 = '" & RTrim(HCT.受入) & "'"
        gSL_Select = gSL_Select & "   AND 区分タイプ = ''"
        If SYMRead(gSL_Select, 1, 0) Then
            H1_JisyaKj = RTrim(SYM.値名称)
        Else
            H1_JisyaKj = mJisyaKj
        End If
        '=================== *** 2008/11/14 End
...
'
    ...
    '    gSL_Select = gSL_Select & "   AND CH.管理文字 = '" & CXH0330_PrtOut & "'"
        If CXH0330_Hikae Then
        ...
        With CisDB
            ...
            If .DBRead(0, 0) Then
                Do Until .Recordset.EOF
                    ...
                    TotalPage = TotalPage + (gLong \ CisPrtCXH0330.iLineMax)
                    If gLong Mod CisPrtCXH0330.iLineMax <> 0 Then
