...
'**       処理概要      :
'**
'**       作  成  日    :   2006/08/22  By CIS SIMPR-A
'**       変  更  日    :   2007/02/27  By CIS SIMPR-A 赤黒の数量修正
'**       変  更  日    :   2008/01/18  By CIS ｷﾉｯｸｽよりSIMPR-Aに移行
...
' 買掛実績年月
    ...
    gSL_Select = gSL_Select & " And   値 = 'POK0110'"
...
Private Sub iB1_Maisu_LostFocus()
    If CisFun.Val2(iB1_Maisu) <> 0 Then
        vsGrid.TextMatrix(CisFun.Val2(Input_Area.Tag), 5) = iB1_Maisu
...
' マスタ入力 SELECT文 ｾｯﾄ
    gSL_Select = " SELECT * "
    gSL_Select = gSL_Select & " FROM 受給検収テーブル "
    gSL_Select = gSL_Select & " WHERE 作成種別 = 'G' "
    ...
    gSL_Select = gSL_Select & "    OR  実績年月 >= '" & Trim(wsKKYM) & "')"
    If DBInput("INV") Then
        If SyoriKb = "ADD" Then
            CisFun.MB_MSG(2) = "    この仕入先･受入･検収日の受給検収データ　登録済！           "
            CisFun.MBOX
            iH1_Torcd.SetFocus
            Exit Function
        End If
    Else
        If SyoriKb <> "ADD" Then
            CisFun.MB_MSG(2) = "    この仕入先･受入･検収日の受給検収データ　未登録！           "
            CisFun.MBOX
            iH1_Torcd.SetFocus
            Exit Function
        End If
...
'                gCnt1 = gCnt2
        ...
        '<< 重複チェック >>
            If RTrim(.TextMatrix(gCnt1, 1)) <> "" Then
                For gCnt2 = gCnt1 + 1 To .Rows - 1
                    If RTrim(.TextMatrix(gCnt1, 1)) = RTrim(.TextMatrix(gCnt2, 1)) And _
                       ...
                       RTrim(.TextMatrix(gCnt1, 17)) = RTrim(.TextMatrix(gCnt2, 17)) Then
                        CisFun.MB_MSG(1) = "    納入番号･品番・赤黒が重複しています。            "
            ...
            'OCRデータとの重複チェック
                ...
                gSL_Select = gSL_Select & "   AND 仕入先 = '" & RTrim(iH1_Torcd) & "'"
                gSL_Select = gSL_Select & "   AND 受入 = '" & RTrim(iH1_Ukeir) & "'"
                gSL_Select = gSL_Select & "   AND 納入番号 = '" & RTrim(.TextMatrix(gCnt1, 1)) & "'"
                gSL_Select = gSL_Select & "   AND 検収日 = '" & RTrim(iH1_Ymd) & "'"
                gSL_Select = gSL_Select & "   AND 作成種別 = 'A'"
                If JYKRead(gSL_Select, 1) Then
                    CisFun.MB_MSG(1) = "    仕入先･受入･検収日･納入番号･品番・赤黒が重複しています。            "
                    CisFun.MB_MSG(3) = "    " & gCnt1 & "行目 OCR読取データと重複します。           "
                    CisFun.MBOX
                    Set ErrObj = iB1_Nouno
                    GoTo Body1Chk_Err
                End If
        ...
        '<< 品番取引先マスタ存在チェック >>
            ...
            If Not HTMRead(gSL_Select, 1) Then
                ...
                GoTo Body1Chk_Err
            End If
            .TextMatrix(gCnt1, 12) = HTM.契約先
            .TextMatrix(gCnt1, 13) = HTM.代行契約先
...
'''            End If
        '<< かんばん部品の時、枚数チェック >>
'            If HTM.SK区分 = 0 And CisFun.Val2(.TextMatrix(gCnt1, 5)) = 0 Then
...
Private Function DBInput(InputKey As String) As Boolean
    ...
    If Not JYKRead(gSL_Select, 0, 1) Then
        Call JYKClose
...
'   マスタ項目移送
    For gIndex = 1 To vsGrid.Rows - 1
        ...
        wsNouno = RTrim(vsGrid.TextMatrix(gIndex, 1))
        ...
        wsKanriGyo = CisFun.Val2(vsGrid.TextMatrix(gIndex, 16))
        ...
        If wsSuryo = 0 Then
            ...
            If wsKanri <> 0 Then
                With CisDB
                    ...
                    .SQL = .SQL & "   And 受入 = '" & RTrim(iH1_Ukeir) & "'"
                    .SQL = .SQL & "   And 検収日 = '" & RTrim(iH1_Ymd) & "'"
                    .SQL = .SQL & "   And 受給管理NO = " & wsKanri
                    .SQL = .SQL & "   And 受給管理行NO = " & wsKanriGyo
                    .SQL = .SQL & "   And 作成種別 = 'G'"
                    .DBExec
                    .DBTran (TransCommit) ' ﾄﾗﾝｻﾞｸｼｮﾝ 終了
                End With
        ...
        Else
            ...
            If wsKanri = 0 Then
                With CisDB
                    ...
                    If Not .DBStored Then
                        CisFun.MB_Lines = 4
                        CisFun.MB_MSG(2) = "    受給管理NOの獲得に失敗しました            "
            ...
            AddFlg = True
            gSL_Select = "SELECT * FROM 受給検収テーブル "
            gSL_Select = gSL_Select & " WHERE 受給管理NO = " & wsKanri
            gSL_Select = gSL_Select & "   AND 受給管理行NO = " & wsKanriGyo
            If JYKRead(gSL_Select, 1) Then
                AddFlg = False
            End If
            
            If JYK.納入番号 <> wsNouno Or JYK.枚数 <> wsMaisu Or JYK.検収数 <> wsSuryo Then
                With JYK
                    .受給管理NO = wsKanri
                    .受給管理行NO = wsKanriGyo
                    .検収日 = RTrim(iH1_Ymd)
                    ...
                    .契約先 = RTrim(vsGrid.TextMatrix(gIndex, 12))
                    .代行契約先 = RTrim(vsGrid.TextMatrix(gIndex, 13))
                    .納入番号 = RTrim(vsGrid.TextMatrix(gIndex, 1))
                    .品番 = RTrim(vsGrid.TextMatrix(gIndex, 3))
                    .背番号 = RTrim(vsGrid.TextMatrix(gIndex, 2))
                    .赤黒区分 = RTrim(vsGrid.TextMatrix(gIndex, 17))
                    .枚数 = wsMaisu
                ...
                If AddFlg Then
                 '( 追加処理 )
'                    JYK.作成者 = RTrim(uINTanto)
                    Call JYKInsert
                    iT1_Addsu = Format(CisFun.Val2(iT1_Addsu) + 1, "###")
                Else
                ' ( 更新処理 )
'                    JYK.更新者 = RTrim(uINTanto)
                    Call JYKUpdate
...
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        ...
        Call .FixedSet(中中, 左中, なし, 0, 12, "契約先")
        Call .FixedSet(中中, 左中, なし, 0, 13, "代行契約先")
        Call .FixedSet(中中, 左中, なし, 0, 14, "実績年月")
        Call .FixedSet(中中, 左中, なし, 0, 15, "変更前数量")
        Call .FixedSet(中中, 左中, なし, 0, 16, "検収管理行NO")
        Call .FixedSet(中中, 左中, なし, 0, 17, "赤黒区分")
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not JYK_RDSTS
        ...
        With vsGrid
            ...
            .TextMatrix(ID, GridCol(1)) = RTrim(JYK.納入番号)
            .TextMatrix(ID, GridCol(2)) = RTrim(JYK.背番号)
            .TextMatrix(ID, GridCol(3)) = RTrim(JYK.品番)
            ...
            End If
            .TextMatrix(ID, GridCol(17)) = RTrim(JYK.赤黒区分)
            
            .TextMatrix(ID, GridCol(5)) = CisFun.RSetFld(JYK.収容数, 7, "#,###")
            .TextMatrix(ID, GridCol(6)) = CisFun.RSetFld(JYK.枚数, 7, "#,###")

            gLong = 0
            If JYK.収容数 <> 0 Then gLong = JYK.検収数 Mod JYK.収容数

            .TextMatrix(ID, GridCol(7)) = CisFun.RSetFld(gLong, 7, "#,###")
            .TextMatrix(ID, GridCol(8)) = CisFun.RSetFld(JYK.検収数, 9, "#,###")
            .TextMatrix(ID, GridCol(15)) = CisFun.RSetFld(JYK.検収数, 9, "#,###")
            
            If JYK.実績年月 <> "" And JYK.実績年月 < wsKKYM Then
                .TextMatrix(ID, GridCol(9)) = "X"
            End If
            .TextMatrix(ID, GridCol(10)) = JYK.受給管理NO
            .TextMatrix(ID, GridCol(11)) = JYK.検収日
            .TextMatrix(ID, GridCol(12)) = JYK.仕入先
            .TextMatrix(ID, GridCol(13)) = JYK.受入
            .TextMatrix(ID, GridCol(16)) = JYK.受給管理行NO
'            .TextMatrix(ID, GridCol(18)) = "X"
        End With
        Call JYKReadNext(1)
    Loop
...
Private Sub TextToVsGrid(tRow As Integer)
    With vsGrid
        ...
        If RTrim(iB1_Hinbn) <> "" Then
            .TextMatrix(tRow, 17) = B1Chk_RBKbn.Value
            If B1Chk_RBKbn.Value = 0 Then
                .TextMatrix(tRow, 4) = "黒"
            Else
                .TextMatrix(tRow, 4) = "赤"
...
Private Sub VsGridToText(tRow As Integer)
    ...
    With vsGrid
        ...
        '赤黒
        B1Chk_RBKbn.Value = CisFun.Val2(.TextMatrix(tRow, 17))
        '収容数
