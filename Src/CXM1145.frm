...
'    B1lb_Kensu = Format(Ttl_Kensu, "###,##0")
    ...
    Do Until Not HTM_RDSTS
        With vsGridS
            ...
            gSL_Select = gSL_Select & " AND   ISNULL(納入先,'') = '" & RTrim(RV_NTorcd) & "'"
            gSL_Select = gSL_Select & " AND   ISNULL(納入先工場,'') = '" & RTrim(RV_NTorkj) & "'"
            gSL_Select = gSL_Select & " AND   ISNULL(納入先受入,'') = '" & RTrim(RV_NUkeir) & "'"
            gSL_Select = gSL_Select & " AND   ISNULL(納入先名称,'') = '" & RTrim(RV_NTorNm) & "'"
            ...
            Else
                ...
                gSL_Select = gSL_Select & " OR    ISNULL(納入先,'') <> '" & RTrim(RV_NTorcd) & "'"
                gSL_Select = gSL_Select & " OR    ISNULL(納入先工場,'') <> '" & RTrim(RV_NTorkj) & "'"
                gSL_Select = gSL_Select & " OR    ISNULL(納入先受入,'') <> '" & RTrim(RV_NUkeir) & "'"
                gSL_Select = gSL_Select & " OR    ISNULL(納入先名称,'') <> '" & RTrim(RV_NTorNm) & "' )"
                If THMRead(gSL_Select, 1, 1) Then
...
'           < 削除 or 代表で更新 >
    ...
    ' ( 取引先変換マスタ削除処理 )
        ...
        .SQL = .SQL & " AND   ISNULL(納入先,'') = '" & RTrim(RV_NTorcd) & "'"
        .SQL = .SQL & " AND   ISNULL(納入先工場,'') = '" & RTrim(RV_NTorkj) & "'"
        .SQL = .SQL & " AND   ISNULL(納入先受入,'') = '" & RTrim(RV_NUkeir) & "'"
        .SQL = .SQL & " AND   ISNULL(納入先名称,'') = '" & RTrim(RV_NTorNm) & "'"
'        .SQL = .SQL & " AND   品番 <> '' "
        .DBExec
        On Error GoTo 0
    End With
    ...
    For gInt = 1 To vsGridS.Rows - 1
        If RTrim(vsGridS.TextMatrix(gInt, 7)) = "1" Then
            With THM
                ...
                .納入先 = RTrim(RV_NTorcd)
                .納入先工場 = RTrim(RV_NTorkj)
                .納入先CD = RTrim(RV_NTorcd) & RTrim(RV_NTorkj)
                .納入先受入 = RTrim(RV_NUkeir)
                .納入先名称 = RTrim(RV_NTorNm)
                .作成者 = gTanto
...
'            .代行 = RTrim(RV_NTorcd)
'            .代行工場 = RTrim(RV_NTorkj)
'            .代行受入 = RTrim(RV_NUkeir)
'            .代行名称 = RTrim(RV_NTorNm)
'            .作成者 = gTanto
...
'    '変換マスタ同一内容登録済対象
    ...
    gSL_Select = gSL_Select & " AND ISNULL(TH1.納入先,'') = '" & RTrim(RV_NTorcd) & "'" & vbCrLf
    gSL_Select = gSL_Select & " AND ISNULL(TH1.納入先工場,'') = '" & RTrim(RV_NTorkj) & "'" & vbCrLf
    gSL_Select = gSL_Select & " AND ISNULL(TH1.納入先受入,'') = '" & RTrim(RV_NUkeir) & "'" & vbCrLf
    gSL_Select = gSL_Select & " AND ISNULL(TH1.納入先名称,'') = '" & RTrim(RV_NTorNm) & "'" & vbCrLf
    gSL_Select = gSL_Select & " WHERE HT.取引先 = '" & RTrim(RV_Torcd) & "'" & vbCrLf
    gSL_Select = gSL_Select & "   AND HT.受入 = '" & RTrim(RV_Ukeir) & "'" & vbCrLf
    gSL_Select = gSL_Select & "   AND HT.契約先 = '" & RTrim(RV_TorcdK) & "'" & vbCrLf
    gSL_Select = gSL_Select & "   AND HT.代行契約先 = '" & RTrim(RV_DaikoK) & "'" & vbCrLf                  ' 2007/12/17 ADD
...
Private Sub Init_Para_Set()
    ...
    H1lb_STorcd2 = RV_STorcd2                                           '支給元2
    
    If RV_CyokuKb <> 0 Then
        H1lb_CyokuKb = "対象"
    Else
        H1lb_CyokuKb = ""
    End If
    H1lb_DTorcd = RV_NTorcd                                             '代行ｺｰﾄﾞ
    If RV_NTorkj <> "" Then
       H1lb_DTorcd = H1lb_DTorcd & "-" & RV_NTorkj
    End If
    H1lb_DUke = RV_NUkeir                                                 '代行受入
    H1lb_DTorNm = RV_NTorNm                                             '代行名称
