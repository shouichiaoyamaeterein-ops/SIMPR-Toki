...
Begin VB.Form CKN1270 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin vsFlexLib.vsFlexArray vsGrid1 
         ...
         Appearance      =   1
         ConvInfo        =   1418783674
         FormatString    =   "№ |仕入先 |仕入先名　　　　　　|受入|品　番　　　　　　　　　|背番号|社内背番号|収容器   |収容数  |発行連番|編集前品番|Select|"
         Rows            =   26
         Cols            =   13
         BackColor       =   12648447
...
'**       処理概要      :
'**
'**       作  成  日    :   2004/09/13  By CIS SIMPR-A
...
Private Function Head1Chk() As Boolean
    ...
    gSL_Select = "select KT.取引先,KT.受入,KT.表示品番,KT.背番号,KT.収容器,KT.品番,KT.材質,"
    ...
    With CisFun
         ...
         ' 背番号
         If PB_CHG.Tag = "X" Then
            .Where_Create AlphaMe, "KT.背番号", iH1_Seban
         Else
            .Where_Create AlphaMe, "KT.材質", iH1_Seban     ' 社内背番号
         End If
         
         gSL_Select = gSL_Select & .Where_Phrase
    End With
    
    gSL_Select = gSL_Select & "    Order by KT.発行種類,KT.取引先,KT.受入,KT.品番,KT.背番号,KT.材質,KT.収容区分,KT.発行NO"
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
                    .SQL = .SQL & " And   品番 = '" & RTrim(vsGrid1.TextMatrix(ID, CisVsGrid.FixedGet(10))) & "'"
                    ...
                    .SQL = .SQL & " And   発行NO = " & CisFun.Val2(vsGrid1.TextMatrix(ID, CisVsGrid.FixedGet(9)))
                    .DBExec
                End With
            End If
        Next ID
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 左中, あり, 17, 2, "納入先名")
        Call .FixedSet(中中, 左中, あり, 2, 3, "受")
        Call .FixedSet(中中, 左中, あり, 25, 4, "品　番")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE START
        'Call .FixedSet(中中, 左中, あり, 6, 5, "得背番")
        Call .FixedSet(中中, 左中, あり, 8, 5, "得背番")
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        Call .FixedSet(中中, 左中, あり, 6, 6, "社背番")
        Call .FixedSet(中中, 左中, あり, 10, 7, "収容器")
        Call .FixedSet(中中, 右中, あり, 9, 8, "収容数")
        Call .FixedSet(中中, 中中, なし, 0, 9, "発行連番")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not KHT_RDSTS
        With vsGrid1
            ...
            .TextMatrix(ID, 6) = KHT.材質 & "    " & KHT.取引先 & KHT.品番      ' 社内背番号
            .TextMatrix(ID, 7) = KHT.収容器
            .TextMatrix(ID, 8) = Format(KHT.収容数, "#,###")
            .TextMatrix(ID, 9) = Format(KHT.発行NO, "000")
