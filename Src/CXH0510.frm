...
Begin VB.Form CXH0510 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "入出庫・支給明細書　発行"
   ClientHeight    =   9225
   ClientLeft      =   90
   ClientTop       =   1500
   ClientWidth     =   14655
...
'**
'**       作  成  日    :  2004/02/19
'**       変  更  日    :  2004/03/17 - 取引先が未登録時に異常終了の修正
...
'   CSVﾌｧｲﾙ出力先
    mCsvApl = "CXH0510"
    mCsvName(1) = Mid(CisFun.WinRoot, 1, 3) & "入庫明細書.csv"
    mCsvName(2) = Mid(CisFun.WinRoot, 1, 3) & "出庫明細書.csv"
    mCsvName(3) = Mid(CisFun.WinRoot, 1, 3) & "支給明細書.csv"
    For gInt = 1 To 3
...
'   入庫先 or 出庫先 or 支給先
    If Trim(iH1_Torcd) <> "" Then
        If Not TorNmGet(iH1_Torcd, wTorKB) Then
            With CisFun
                .MB_Lines = 4
                .MB_MSG(2) = "    " & Back_Torcd.Caption & "　未登録！（取引先マスタ）            "
                .MB_Title = ""
                .MB_Button = Error
                .MBOX
            End With
    ...
    gSL_Select = gSL_Select & ", isnull(hnm.表示品番, jkt.品番) 表示品番"
    gSL_Select = gSL_Select & " from"
    gSL_Select = gSL_Select & "  実績管理テーブル jkt"
    gSL_Select = gSL_Select & "  left outer join 取引先マスタ trm"
    gSL_Select = gSL_Select & "    on trm.取引先CD = jkt.実績先"
    gSL_Select = gSL_Select & "    and trm.取引先区分 = 0"
    gSL_Select = gSL_Select & "  left outer join 取引先マスタ tr2"
    gSL_Select = gSL_Select & "    on tr2.取引先CD = jkt.実績先"
    gSL_Select = gSL_Select & "    and tr2.取引先区分 = 1"
    gSL_Select = gSL_Select & "  left outer join 品番マスタ hnm"
    gSL_Select = gSL_Select & "    on hnm.品番 = jkt.品番"
    gSL_Select = gSL_Select & " where jkt.実績区分 = " & wJkbn & ""
'   >> 実績先（入庫先 or 出庫先 or 支給先）
    If RTrim(iH1_Torcd) <> "" Then
        gSL_Select = gSL_Select & "   and jkt.実績先 = '" & RTrim(iH1_Torcd) & "'"
    End If
'   >> 実績日（入庫日 or 出庫日 or 納入日）
    If Trim(iH1_SYmd) <> "" And CisFun.Val2(iH1_SYmd.cDay) = 0 Then
    '   > 年月指定
        gSL_Select = gSL_Select & "   and substring(jkt.実績日, 1, 6) = '" & iH1_SYmd & "'"
    Else
        '> 開始日のみ
        If Trim(iH1_SYmd) <> "" And Trim(iH1_EYmd) = "" Then
            gSL_Select = gSL_Select & "   and jkt.実績日 = '" & Trim(iH1_SYmd) & "'"
        End If
        '> 終了日のみ
        If Trim(iH1_SYmd) = "" And Trim(iH1_EYmd) <> "" Then
            gSL_Select = gSL_Select & "   and jkt.実績日 <= '" & Trim(iH1_EYmd) & "'"
        End If
        '> 開始日＆終了日
        If Trim(iH1_SYmd) <> "" And Trim(iH1_EYmd) <> "" Then
            gSL_Select = gSL_Select & "   and jkt.実績日 between '" & Trim(iH1_SYmd) & "'" _
                                                         & " and '" & Trim(iH1_EYmd) & "'"
        End If
    End If
    ...
    gSL_Select = gSL_Select & ", jkt.品番"
    gSL_Select = gSL_Select & ", jkt.実績日"
    gSL_Select = gSL_Select & ", jkt.実績便"

    If Not JKTRead(gSL_Select, , 1) Then
        With CisFun
            .MB_Lines = 4
...
Private Function PrintStart() As Boolean
    PrintStart = False
    If Op_Out(0).Value Or Op_Out(1).Value Then
        lb_msg.Caption = "【 " & Op_JKbn(wJkbn).Caption & "データ 印刷中 】"
        DoEvents
        With CisPrtCXH0510
            .cDataMaxCnt = CisDB.RecordCount
            Set .iPrtForm = CXH0510L
            .dpMSG = Op_JKbn(wJkbn).Caption & "明細書　印刷中"
...
'   出力ﾌｧｲﾙ存在確認
    ...
    lb_msg.Caption = "【 " & Op_JKbn(wJkbn).Caption & "データ 出力中 】"
...
Private Sub CsvEdit_Head1(CsvStr As String)
    CsvStr = ""
    Select Case wJkbn
        Case 1: CsvStr = CsvStr & "入庫明細書"
        Case 2: CsvStr = CsvStr & "出庫明細書"
        Case 3: CsvStr = CsvStr & "支給明細書"
    End Select
End Sub
