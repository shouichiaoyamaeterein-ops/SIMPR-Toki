...
Begin VB.Form CNJZ0050 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin vsFlexLib.vsFlexArray vsGrid1 
         ...
         EndProperty
         ConvInfo        =   1418783674
         FormatString    =   "№  |品番                  |9999.99x9999.99x9999.99|手配先                 |受入|今回計画数  |前回計画数  |計画差     |率  |"
         Rows            =   28
         Cols            =   15
         BackColor       =   12648447
         ForeColorFixed  =   128
...
'**********************************************************'
    ...
    Dim sv_ZKanri               As Long
    Dim sv_Zaist                As String
    Dim sv_Itatu                As Currency
    Dim sv_Width                As Currency
    Dim sv_Long                 As Currency
...
Private Sub PB_Look_Click()
    ...
    If PB_Look.Tag = "iH1_Itatu" Then Call Look_Zaist: Exit Sub
...
Private Function Body1Chk() As Boolean
    ...
    With vsGrid1
        If RTrim$(.TextMatrix(.Row, 1)) = "" Then Exit Function         '品番
    End With
...
Private Sub AplExec()
    ...
    With vsGrid1
        sv_Zaist = RTrim$(.TextMatrix(.Row, 1))
        sv_Itatu = CisFun.Val2(.TextMatrix(.Row, 10))
        sv_Width = CisFun.Val2(.TextMatrix(.Row, 11))
        sv_Long = CisFun.Val2(.TextMatrix(.Row, 12))
        sv_Torcd = RTrim$(Mid$(.TextMatrix(.Row, 3), 1, 7))
        sv_Ukeir = RTrim$(.TextMatrix(.Row, 4))
    End With
...
'   ( Select 文 作成 )
    ...
    Call ZNJRead(gSL_Select, 1)

    With vsGrid1
        
        .TextMatrix(.Row, 5) = Format(ZNJ.今回計画数, "###0.00")
        .TextMatrix(.Row, 6) = Format(ZNJ.前回計画数, "###0.00")
        .TextMatrix(.Row, 7) = Format(ZNJ.計画差, "###0.00")
        
        If ZNJ.増減率 > 999 Then
            .TextMatrix(.Row, 8) = "999"
        Else
            .TextMatrix(.Row, 8) = Format(ZNJ.増減率, "#")
        End If
...
'  ( 確認メッセージ )
    ...
    With vsGrid1
        sv_ZKanri = CisFun.Val2(.TextMatrix(.Row, 9))
        sv_Torcd = RTrim$(Mid$(.TextMatrix(.Row, 3), 1, 7))
        sv_Ukeir = RTrim$(.TextMatrix(.Row, 4))
    End With
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, あり, 24, 2, "寸  法")
        Call .FixedSet(中中, 左中, あり, 22, 3, "手配先")
        Call .FixedSet(中中, 中中, あり, 2, 4, "受")
        Call .FixedSet(中中, 右中, あり, 13, 5, "今回計画数")
        Call .FixedSet(中中, 右中, あり, 13, 6, "前回計画数")
        Call .FixedSet(中中, 右中, あり, 13, 7, "計画差")
        Call .FixedSet(中中, 右中, あり, 4, 8, "率")
        Call .FixedSet(中中, 左中, あり, 0, 9, "材料管理番号")
        Call .FixedSet(中中, 左中, あり, 0, 10, "板厚")
        Call .FixedSet(中中, 左中, あり, 0, 11, "幅")
        Call .FixedSet(中中, 左中, あり, 0, 12, "長さ")
        ...
        With vsGrid1
            ...
            If ZNJ.寸法区分 = 0 Then
                If ZNJ.板厚 = 0 Then
                    gStr = gStr & "       x"
                Else
                    gStr = gStr & Space(7 - Len(Format(ZNJ.板厚, "###0.00"))) & Format(ZNJ.板厚, "###0.00") & "x"
                End If
                If ZNJ.幅 = 0 Then
                    gStr = gStr & "       x"
                Else
                    gStr = gStr & Space(7 - Len(Format(ZNJ.幅, "###0.00"))) & Format(ZNJ.幅, "###0.00") & "x"
                End If
                If ZNJ.長さ = 0 Then
                    gStr = gStr & "       "
                Else
                    gStr = gStr & Space(7 - Len(Format(ZNJ.長さ, "###0.00"))) & Format(ZNJ.長さ, "###0.00")
                End If
            End If
            .TextMatrix(ID, 2) = gStr
            
            If RTrim$(ZNJ.手配先) = "" Then
                .TextMatrix(ID, 3) = ""
            Else
                .TextMatrix(ID, 3) = RTrim$(ZNJ.手配先) & Space(7 - Len(RTrim$(ZNJ.手配先))) & ":" & RTrim$(ZNJ.手配先名)
            End If
            .TextMatrix(ID, 4) = RTrim$(ZNJ.受入)
            .TextMatrix(ID, 5) = Format(ZNJ.今回計画数, "####.000")
            .TextMatrix(ID, 6) = Format(ZNJ.前回計画数, "####.000")
            .TextMatrix(ID, 7) = Format(ZNJ.計画差, "####.000")
            
            If ZNJ.増減率 > 999 Then
                .TextMatrix(ID, 8) = "999"
            Else
                .TextMatrix(ID, 8) = Format(ZNJ.増減率, "#")
            End If
        
            .TextMatrix(ID, 9) = RTrim$(ZNJ.材料管理番号)
            .TextMatrix(ID, 10) = ZNJ.板厚
            .TextMatrix(ID, 11) = ZNJ.幅
            .TextMatrix(ID, 12) = ZNJ.長さ
...
Private Sub CrtSelect(pKB As Byte)
    Static sw_Zaist             As String
    Static sw_Itatu             As String
    Static sw_Width             As String
    Static sw_Long              As String
    Static sw_Torcd             As String
    Static sw_Ukeir             As String
    
    sw_Zaist = "":  sw_Torcd = "":  sw_Ukeir = ""
    sw_Itatu = "":  sw_Width = "":  sw_Long = ""
    If pKB = 0 Then
'       < 材質 >
        If RTrim$(iH1_Zaist) <> "" Then
            sw_Zaist = "                          "
            If InStr(iH1_Zaist, "*") = 0 Then
                sw_Zaist = sw_Zaist & "AND   材質 = '" & RTrim$(iH1_Zaist) & "'" & vbCrLf
            Else
                sw_Zaist = sw_Zaist & "AND   材質 Like '" & Replace(RTrim$(iH1_Zaist), "*", "%") & "'" & vbCrLf
            End If
        End If
'       < 板厚 >
        If CisFun.Val2(iH1_Itatu) <> 0 Then
            sw_Itatu = "                          "
            sw_Itatu = sw_Itatu & "AND   板厚 = " & CisFun.Val2(iH1_Itatu) & "" & vbCrLf
        End If
'       < 幅 >
        If CisFun.Val2(iH1_Width) <> 0 Then
            sw_Width = "                          "
            sw_Width = sw_Width & "AND   幅 = " & CisFun.Val2(iH1_Width) & "" & vbCrLf
        End If
'       < 長さ >
        If CisFun.Val2(iH1_Long) <> 0 Then
            sw_Long = "                          "
            sw_Long = sw_Long & "AND   長さ = " & CisFun.Val2(iH1_Long) & "" & vbCrLf
        End If
...
'       < 受入 >
    ...
    Else
'       < 材質 >
        sw_Zaist = "                          AND   材質 = '" & RTrim$(sv_Zaist) & "'" & vbCrLf
'       < 板厚 >
        sw_Itatu = "                          AND   板厚 = " & sv_Itatu & "" & vbCrLf
'       < 幅 >
        sw_Width = "                          AND   幅   = " & sv_Width & "" & vbCrLf
'       < 長さ >
        sw_Long = "                           AND   長さ = " & sv_Long & "" & vbCrLf
'       < 手配先 >
...
'       < 受入 >
    ...
    gSL_Select = "SELECT SK.内示年月, SK.材質, SK.手配先, ISNULL( TR.略称, '' ) 手配先名, SK.受入, " & vbCrLf
    gSL_Select = gSL_Select & " SK.寸法区分, SK.板厚, SK.幅, SK.長さ, "
    gSL_Select = gSL_Select & " ISNULL( ZR.表示寸法, '' ) 表示寸法," & vbCrLf
...
'[ 読込基 ( 材料内示発注テーブル ) ]
    gSL_Select = gSL_Select & " FROM ( SELECT 内示年月, 材料管理番号, 手配先, 受入," & vbCrLf
    gSL_Select = gSL_Select & "                          寸法区分,材質,板厚,幅,長さ," & vbCrLf
    gSL_Select = gSL_Select & "                          SUBSTRING( 納期, 1, 6 ) 対象年月, SUM(数量) 数量" & vbCrLf
    gSL_Select = gSL_Select & "                          FROM 材料発注内示テーブル" & vbCrLf
    gSL_Select = gSL_Select & "                          WHERE 内示年月 = '" & gn_HNYM & "'" & vbCrLf
    gSL_Select = gSL_Select & "                          AND   SUBSTRING( 納期, 1, 6 ) = '" & iH1_YM & "'" & vbCrLf
    gSL_Select = gSL_Select & sw_Zaist & sw_Itatu & sw_Width & sw_Long & sw_Torcd & sw_Ukeir
    gSL_Select = gSL_Select & "                          GROUP BY 内示年月, 材料管理番号, 手配先, 受入, " & vbCrLf
    gSL_Select = gSL_Select & "                                     寸法区分, 材質, 板厚, 幅, 長さ, SUBSTRING( 納期, 1, 6 )" & vbCrLf
    gSL_Select = gSL_Select & "      ) SK" & vbCrLf

'[ 材料内示発注前回テーブル ]
    gSL_Select = gSL_Select & " LEFT OUTER JOIN ( SELECT 内示年月, 材料管理番号, 手配先, 受入," & vbCrLf
    gSL_Select = gSL_Select & "                          材質,板厚,幅,長さ," & vbCrLf
    gSL_Select = gSL_Select & "                          SUBSTRING( 納期, 1, 6 ) 対象年月, SUM(数量) 数量" & vbCrLf
    gSL_Select = gSL_Select & "                          FROM 材料発注内示前回テーブル" & vbCrLf
    gSL_Select = gSL_Select & "                          WHERE 内示年月 = '" & gn_HNYM & "'" & vbCrLf
    gSL_Select = gSL_Select & "                          AND   SUBSTRING( 納期, 1, 6 ) = '" & iH1_YM & "'" & vbCrLf
    gSL_Select = gSL_Select & sw_Zaist & sw_Itatu & sw_Width & sw_Long & sw_Torcd & sw_Ukeir
    gSL_Select = gSL_Select & "                          GROUP BY 内示年月, 材料管理番号, 手配先, 受入, " & vbCrLf
    gSL_Select = gSL_Select & "                                     材質, 板厚, 幅, 長さ, SUBSTRING( 納期, 1, 6 )" & vbCrLf
    gSL_Select = gSL_Select & "                 ) SZ" & vbCrLf
    gSL_Select = gSL_Select & " ON  SZ.内示年月     = SK.内示年月" & vbCrLf
    gSL_Select = gSL_Select & " AND SZ.材料管理番号 = SK.材料管理番号" & vbCrLf
...
'[ 表示品番 (品番マスタ) ]
    gSL_Select = gSL_Select & " LEFT OUTER JOIN 材料マスタ ZR " & vbCrLf
    gSL_Select = gSL_Select & " ON  ZR.材料管理番号 = SK.材料管理番号" & vbCrLf
...
'[ 増減率 ]
    If pKB = 0 Then
        ...
        gSL_Select = gSL_Select & " Order By SK.材質, SK.板厚, SK.幅, SK.長さ, SK.手配先, SK.受入"
    End If
