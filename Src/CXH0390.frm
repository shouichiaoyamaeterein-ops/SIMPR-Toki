...
'**       変  更  日    :   2006/02/09  By CIS - (INI)[かんばん印刷先]初期値セットDeviceName修正
'**       変  更  日    :   2006/02/22  By CIS - (INI)[かんばん印刷先]->[自給チケット印刷先] < ｸﾗｲｱﾝﾄINI >
'**       変  更  日    :
'**
'**********************************************************'
...
'-----------------------------------------------< 2006/02/02 ADD START >
        ...
        .INI_Default = ""
        .GetIni
        ChkDir = RTrim(.INI_String)
        If Right(RTrim(ChkDir), 1) <> "\" Then
            ChkDir = RTrim(ChkDir) & "\"
        End If
        ChkDir = ChkDir & "TDZ2*.HKO"
        
        .INI_Section = "Option"
        .INI_Key = "STICKETCHECK"
        .INI_Default = ""
        .GetIni
        If RTrim(.INI_String) = "" Then
            ...
            .PutIni
        End If
        ChkKb = RTrim(.INI_String)
'-----------------------------------------------< 2006/02/02 ADD START >
        .INI_FileID = gIniExe & gIniName
'-----------------------------------------------< 2006/02/02 ADD START >
    End With
    ChkFile = RTrim$(Dir(ChkDir))
    If RTrim(ChkFile) <> "" Then
        Prt_Msg = "支給チケット印刷中"
        DoEvents
        Do While RTrim$(ChkFile) <> ""
            If ChkKb = "0" Then
                With CisFun
                    .MB_Lines = 6
                    .MB_MSG(2) = "   　  【　支給チケット　印刷中！　】      "
                    .MB_MSG(4) = "     支給チケット印刷終了まで、しばらくお待ち下さい      "
                    .MB_Title = "処理確認"
                    .MB_Button = OK
                    .MBOX
                End With
...
'-----     -----     -----     -----     -----     < 2006/02/14 MNT START >
'
'        .INI_Section = "かんばん印刷先"
'        .INI_FileID = gIniExe & gIniName
'
        .INI_Section = "自給チケット印刷先"
        .INI_FileID = gIniExe & gwsIniName
'-----     -----     -----     -----     -----     < 2006/02/14 MNT  END  >
