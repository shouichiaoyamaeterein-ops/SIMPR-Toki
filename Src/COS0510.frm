...
Begin VB.Form COS0510 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "売上差異明細リスト　発行"
   ClientHeight    =   11190
   ClientLeft      =   75
   ClientTop       =   1530
   ClientWidth     =   15360
...
'
    ...
    If Not USTRead(gSL_Select) Then
        With CisFun
            .MB_Lines = 4
...
Private Function PrintStart() As Boolean
    PrintStart = False
    If Op_Out(0).Value Or Op_Out(1).Value Then
        lb_msg.Caption = "【 売上差異明細リスト 印刷中 】"
        DoEvents
        With CisPrt
             Set .iPrtForm = COS0510L
             .dpMSG = "売上差異明細リスト　印刷中"
...
' 抽出処理 初期ｾｯﾄ
    ...
    gSL_Select = gSL_Select & " Where 区分名称 = '照合結果売上'"
