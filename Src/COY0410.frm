...
Begin VB.Form COY0410 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin vsFlexLib.vsFlexArray VSGrid1 
         ...
         Appearance      =   1
         ConvInfo        =   1418783674
         FormatString    =   "NO   |工区|契約先|車型|品番|数量1|金額1|数量2|金額2|数量3|金額3|"
         Rows            =   26
         Cols            =   13
         BackColor       =   12648447
   ...
   Begin VB.PictureBox H1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D2 
         ...
         cAlingnment     =   6
         cPositionY      =   60
         Begin VB.OptionButton OpH1_DspSyu 
            BackColor       =   &H00C0C0C0&
            Caption         =   "型費"
            BeginProperty Font 
               Name            =   "ＭＳ ゴシック"
...
'**********************************************************'
    ...
    Dim mNJCol          As Byte
    Dim mNJYm1          As String
    Dim mNJYm2          As String
    Dim mNJYm3          As String
'+----------------------------------------------------------+
'+      フォーム    《    Ｌｏａｄイベント    》              +
'+----------------------------------------------------------+
...
' 売上実績年月を求める
    ...
    If UYTRead(gSL_Select, 1) Then
       iH1_SYM = RTrim(UYT.内示年月)
    Else
       iH1_SYM = Format(Now, "YYYYMM")
    End If
    If iH1_SYM = "" Then
       iH1_SYM = Format(Now, "YYYYMM")
    End If
    Call NJYm_Edit(iH1_SYM)
    Call GridInit
...
'                      ｾﾙ桁数,列番号,見出し内容)
        ...
        Call .FixedSet(中中, 左中, あり, 4, 1, "工区")
        Call .FixedSet(中中, 左中, あり, 21, 2, "契 約 先")
        '*-*-*-*-* 2008/04/11 Update Start
        'Call .FixedSet(中中, 中中, あり, 5, 3, "車型")
        Call .FixedSet(中中, 中中, あり, 6, 3, "車型")
        '*-*-*-*-* 2008/04/11 Update End
        Call .FixedSet(中中, 左中, あり, 22, 4, "品  番")
        Call .FixedSet(中中, 右中, なし, 10, 5, mNJYm1 & "月数量")
        Call .FixedSet(中中, 右中, なし, 9, 6, mNJYm1 & "月金額")
        Call .FixedSet(中中, 右中, なし, 10, 7, mNJYm2 & "月数量")
        Call .FixedSet(中中, 右中, なし, 9, 8, mNJYm2 & "月金額")
        Call .FixedSet(中中, 右中, なし, 10, 9, mNJYm3 & "月数量")
        Call .FixedSet(中中, 右中, なし, 9, 10, mNJYm3 & "月金額")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
...
Private Function BodySet() As Boolean
    ...
    If OpH1_DspSyu(1).Value Then
        If Not NJYm_Kata Then Exit Function
    End If
    Sl_Where = "": gSL_Select = ""
' 表示条件 セット
    With CisFun
        .Where_CreateKB = NewDocument
        If OpH1_DspSyu(0).Value Then
            .Where_Create AlphaMe, "UY.内示年月", iH1_SYM
        Else
            .Where_Create AlphaMe, "UY.予想年月", iH1_SYM
        End If
        ' 工区
        .Where_Create AlphaMe, "UY.工区", iH1_Kouku
        ' 契約先
        .Where_Create AlphaMe, "UY.契約先", iH1_Torcd
        ' 車型
        .Where_Create AlphaMe, "UY.車型", iH1_Syakei
        ' 品番
        .Where_Create AlphaMe, "UY.品番", iH1_Hinbn
    End With

    If OpH1_DspSyu(0).Value Then
        gSL_Select = "SELECT 内示年月,UY.工区,UY.契約先,ISNULL(TR.略称,'') 契約先名,UY.車型,UY.品番,UY.品番編集,"
        gSL_Select = gSL_Select & " 数量1,金額1,数量2,金額2,数量3,金額3"
    ...
    Else
        gSL_Select = "SELECT 開始年月,UY.予想年月 内示年月,UY.契約先,ISNULL(TR.略称,'') 契約先名,UY.車型,UY.品番,UY.品番編集,"
        gSL_Select = gSL_Select & " 金額" & mNJCol & " 金額1,金額" & mNJCol + 1 & " 金額2,金額" & mNJCol + 2 & " 金額3"
...
' 抽出文　作成
    ...
    If OpH1_DspSyu(1).Value Then
        gSL_Select = gSL_Select & " AND (金額" & mNJCol & " <> 0 OR 金額" & mNJCol + 1 & " <> 0 OR 金額" & mNJCol + 2 & " <> 0)"
    End If
    gSL_Select = gSL_Select & " ORDER BY UY.工区,ISNULL(TR.略称,''),UY.車型,UY.品番 "
    
    If Not UYTRead(gSL_Select) Then GoTo BodySet_ED1
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not UYT_RDSTS
        With VSGrid1
            ...
            Else
                .TextMatrix(ID, GridCol(1)) = UYT.工区
            End If
            If Trim(UYT.契約先名) = "" Then
                .TextMatrix(ID, GridCol(2)) = "    "
            Else
                .TextMatrix(ID, GridCol(2)) = UYT.契約先名
            End If
            If Trim(UYT.車型) = "" Then
                .TextMatrix(ID, GridCol(3)) = "    "
            Else
                .TextMatrix(ID, GridCol(3)) = UYT.車型
            End If
            If Trim(UYT.品番) = "" Then
                .TextMatrix(ID, GridCol(4)) = "    "
            Else
                .TextMatrix(ID, GridCol(4)) = EditHinbn(UYT.品番, UYT.品番編集)
            End If
            
            If OpH1_DspSyu(0).Value Then
                .TextMatrix(ID, GridCol(5)) = CisFun.RSetFld(UYT.数量1, 9, "#,###,###")
            Else
                If UYT.金額1 <> 0 Then
                    .TextMatrix(ID, GridCol(5)) = CisFun.RSetFld(1, 9, "#,###,###")
                End If
            End If
            gLong = UYT.金額1 / 1000
            .TextMatrix(ID, GridCol(6)) = CisFun.RSetFld(gLong, 6, "###,###")
            If OpH1_DspSyu(0).Value Then
                .TextMatrix(ID, GridCol(7)) = CisFun.RSetFld(UYT.数量2, 9, "#,###,###")
            Else
                If UYT.金額2 <> 0 Then
                    .TextMatrix(ID, GridCol(7)) = CisFun.RSetFld(1, 9, "#,###,###")
                End If
            End If
            gLong = UYT.金額2 / 1000
            .TextMatrix(ID, GridCol(8)) = CisFun.RSetFld(gLong, 6, "###,###")
            If OpH1_DspSyu(0).Value Then
                .TextMatrix(ID, GridCol(9)) = CisFun.RSetFld(UYT.数量3, 9, "#,###,###")
            Else
                If UYT.金額3 <> 0 Then
                    .TextMatrix(ID, GridCol(9)) = CisFun.RSetFld(1, 9, "#,###,###")
                End If
            End If
            gLong = UYT.金額3 / 1000
            .TextMatrix(ID, GridCol(10)) = CisFun.RSetFld(gLong, 6, "###,###")
            ID = ID + 1
        End With
        Call UYTReadNext
    Loop
...
' 合計表示・非表示
        If Op_Overlap(0).Value Then
            Call DispTotal(True)
            If .MouseCol > 0 And .MouseCol < 4 Then
...
'+   見出し年月設定
'+----------------------------------------------------------+
Private Sub NJYm_Edit(wYM As String)
    mNJYm1 = Mid(wYM, 5, 2)
    mNJYm2 = Format(DateAdd("m", 1, CDate(Left(wYM, 4) & "/" & Mid(wYM, 5, 2) & "/01")), "mm")
    mNJYm3 = Format(DateAdd("m", 2, CDate(Left(wYM, 4) & "/" & Mid(wYM, 5, 2) & "/01")), "mm")
End Sub
'+----------------------------------------------------------+
'+   型費の時、対象３ヶ月を算出
'+----------------------------------------------------------+
Private Function NJYm_Kata()
    NJYm_Kata = False
    
    gSL_Select = "SELECT MAX(内示位置) 内示位置 FROM 売上予想型費テーブル"
    gSL_Select = gSL_Select & " WHERE 予想年月 = '" & RTrim(iH1_SYM) & "'"
    If UYKRead(gSL_Select, 1) Then
        mNJCol = UYK.内示位置
    End If

    If mNJCol = 0 Then Exit Function
