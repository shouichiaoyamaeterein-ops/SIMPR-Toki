   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |出荷指示書No|出荷 日 　-便|納入先 |   納　入  先  名   |受入|受注区|出荷管理部署||"
            Rows            =   21
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=50 Else 1=150
            Call CXJ0120LPrint(wPrintKB)
    ...
    If Trim(iH1_SYmd) <> "" And Trim(iH1_EYmd) <> "" Then
       ...
       If iH1_SYmd = iH1_EYmd And _
          iH1_SBin > iH1_EBin Then
           With CisFun
               .MB_Lines = 4
               .MB_MSG(2) = "    出荷日・便を正しく入力して下さい（開始＞終了）            "
               .MB_Title = ""
               .MB_Button = Error
               .MBOX
           End With
...
Private Function DBInput() As Boolean
    ...
    With CisFun
         ...
         ' 再発行最新
         If H1Op_SKbn(1).Value Then
            .Where_Create AlphaMe, "CH.端末番号", .WSID
            .Where_Create Numeric, "CH.帳票区分", 50
         End If
         ' 日付
         DateStr = "JY.納入"
         ...
         If Trim(iH1_SYmd) <> "" And CisFun.Val2(iH1_SYmd.cDay) = 0 Then
            .Where_Create AlphaMe, "Substring(" & DateStr & "日,1,6)", iH1_SYmd
            .Where_Create AlphaMe, DateStr & "便", Format(iH1_SBin, "00")
         End If
         ' 日付開始編集
         If Trim(iH1_SYmd) <> "" Then
            DateSt = iH1_SYmd
         ...
         End If
         ' 日付開始のみ
         If Trim(DateSt) <> "" And Trim(DateEd) = "" And CisFun.Val2(iH1_SYmd.cDay) <> 0 Then
            If Trim(BinSt) = "" Then
         ...
         If Trim(CisCombo1.Code) <> "" Then
            .Where_Create Numeric, "JY.受注区分", CisFun.Val2(CisCombo1.Code), , Create
         End If
    End With
    
    gSL_Select = "Select JY.出荷指示書NO,JY.納入日,JY.納入便,"
    gSL_Select = gSL_Select & "JY.納入先,JY.受入,"
    gSL_Select = gSL_Select & "ISNULL(TR.略称,'') 納入先名,"
    gSL_Select = gSL_Select & "ISNULL(SY.値名称,'') 受注区分名,"
    gSL_Select = gSL_Select & "ISNULL(TR2.略称,'') 出荷管理部署名"
    If H1Op_SKbn(1).Value Then
       gSL_Select = gSL_Select & " From 帳票出力テーブル CH"
       gSL_Select = gSL_Select & " Left Outer Join 受注テーブル JY"
       gSL_Select = gSL_Select & "   On JY.出荷指示書NO = CH.管理NO"
    Else
       gSL_Select = gSL_Select & " From 受注テーブル JY"
    End If
    gSL_Select = gSL_Select & "  Left Outer Join 取引先マスタ TR"
    gSL_Select = gSL_Select & "    ON TR.取引先区分 = 0"
    gSL_Select = gSL_Select & "    And TR.取引先CD = JY.納入先"
    gSL_Select = gSL_Select & "  Left Outer Join 名称マスタ SY"
    gSL_Select = gSL_Select & "    ON  SY.区分名称 = '受注区分'"
    gSL_Select = gSL_Select & "    And SY.値 = JY.受注区分"
    gSL_Select = gSL_Select & "    And SY.区分ﾀｲﾌﾟ = ''"
    
    gSL_Select = gSL_Select & "  Left Outer Join 品番取引先マスタ HT"
    gSL_Select = gSL_Select & "    ON  HT.品番 = JY.品番"
    gSL_Select = gSL_Select & "    And HT.取引先 = JY.納入先"
    ...
    gSL_Select = gSL_Select & " Group By JY.出荷指示書NO,JY.納入日,JY.納入便,"
    gSL_Select = gSL_Select & "          JY.納入先,JY.受入,ISNULL(TR.略称,''),ISNULL(SY.値名称,''),ISNULL(TR2.略称,'')"
    gSL_Select = gSL_Select & " Order By JY.出荷指示書NO,JY.納入日,JY.納入便,"
    gSL_Select = gSL_Select & "          JY.納入先,JY.受入"
...
' ( 作成処理 )
    ...
    With vsGrid1
        For ID = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(ID, CisVsGrid1.FixedGet(8))) = "X" Then
                Call ItemsClearCHT
                CHT.端末番号 = "' + substring(host_name(),1,20) + '"
                CHT.帳票区分 = 150
                CHT.管理NO = CisFun.Val2(.TextMatrix(ID, CisVsGrid1.FixedGet(1)))
                Call CHTInsert
            End If
        Next ID
    End With
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 150"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'                      ｾﾙ桁数,列番号,見出し内容,分類ﾀｲﾌﾟ)
        ...
        Call .FixedSet(中中, 中中, あり, 12, 1, "出荷指示書No")
        Call .FixedSet(中中, 中中, あり, 16, 2, " 出 荷 日  -便")
        Call .FixedSet(中中, 左中, あり, 7, 3, "納入先")
        Call .FixedSet(中中, 左中, あり, 30, 4, "納　入  先  名")
        Call .FixedSet(中中, 中中, あり, 0, 5, "受入")
        Call .FixedSet(中中, 中中, あり, 10, 6, "受注区分")
        Call .FixedSet(中中, 左中, あり, 14, 7, "出荷管理部署")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
    ...
    With vsGrid1
       .Redraw = False
        ...
        Do Until Not JYT_RDSTS
            ...
            .TextMatrix(ID, 1) = Format(JYT.出荷指示書NO, "0000000000")
            With CisFun
                .DateE_BefVal = JYT.納入日
                .DateEdit
                If CisFun.Val2(JYT.納入便) <> 0 Then
                   gStr = .DateE_AfVal & "-" & Format(CisFun.Val2(JYT.納入便), "00")
                Else
                   gStr = .DateE_AfVal & "    "
                End If
            End With
            .TextMatrix(ID, 2) = gStr
            .TextMatrix(ID, 3) = JYT.納入先
            .TextMatrix(ID, 4) = JYT.納入先名
            .TextMatrix(ID, 5) = JYT.受入
            .TextMatrix(ID, 6) = JYT.受注区分名
            .TextMatrix(ID, 7) = JYT.出荷管理部署名
