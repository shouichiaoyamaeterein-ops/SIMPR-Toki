...
Begin VB.Form CXHZ0450 
   ...
   Begin Cis3D_v60.CIS3D B2_Area1 
      ...
      Begin vsFlexLib.vsFlexArray vsGrid1 
         ...
         Appearance      =   1
         ConvInfo        =   1418783674
         FormatString    =   "№|材  質　　　　　　　     |仕入先  　　　　　|受入|背番号|構成|||"
         Rows            =   9
         Cols            =   11
         BackColor       =   12648384
      ...
      Begin Cis3D_v60.CIS3D B2lb_Mei 
         ...
         _ExtentY        =   582
         BackColor       =   14737632
         Caption         =   "9999.99 x 9999.99 x 9999.99"
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "ＭＳ ゴシック"
            Size            =   11.25
            Charset         =   128
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CXHZ0450.frx":0000
         Height          =   7710
         Left            =   60
         OleObjectBlob   =   "CXHZ0450.frx":0015
         TabIndex        =   20
         Top             =   60
         Width           =   14535
      End
...
'+---------------------+
    With CisDBGrid1
        ...
        .LayOutName = CisFun.WSID & "HZ0450.Lay"    ' ﾚｲｱｳﾄ情報格納先、ﾌｧｲﾙ名
        .PrintHeadLine = あり
        .PrintBodyLine = あり
        .PrintBodyGyo = 10
...
Private Sub PB_MNT_Click()
    ...
    RV_Call = "CHXZ0450"
    With ZUT
        .打切種別 = CisFun.Val2(DBGrid1.Columns(14))
        .材質 = RTrim(DBGrid1.Columns(15))
        .板厚 = CisFun.Val2(DBGrid1.Columns(16))
        .幅 = CisFun.Val2(DBGrid1.Columns(17))
        .長さ = CisFun.Val2(DBGrid1.Columns(18))
        .仕入先 = RTrim(DBGrid1.Columns(19))
    End With

    CXHZ0050.Show vbModal, Me
    Unload CXHZ0050
    Set CXHZ0050 = Nothing
...
'       表示条件 セット
        ...
        '   材質
            If RTrim(iH1_Zaist) <> "" Then
                If InStr(1, iH1_Zaist, "*") = 0 Then
                    .Select_Where = .Select_Where & "   and 材質 = '" & RTrim(iH1_Zaist) & "'"
                Else
                    .Select_Where = .Select_Where & "   and 材質 like '" & RTrim(Replace(iH1_Zaist, "*", "%")) & "'"
                End If
            End If
        '   仕入先
...
' ﾚｲｱｳﾄ表示内容よりﾃﾞｰﾀ連結名を求める
    ...
    gSL_Select = gSL_Select & "select zut.*"
    gSL_Select = gSL_Select & ", case when zut.起算日 = '' then ''"
    gSL_Select = gSL_Select & "       else substring(zut.起算日,1,4)+'/'+substring(zut.起算日,5,2)+'/'+substring(zut.起算日,7,2)+'-'+zut.起算便"
    gSL_Select = gSL_Select & "   end 編集起算日"
    gSL_Select = gSL_Select & ", case when zut.完了日 = '' then ''"
    gSL_Select = gSL_Select & "       else substring(zut.完了日,1,4)+'/'+substring(zut.完了日,5,2)+'/'+substring(zut.完了日,7,2)+'-'+zut.完了便"
    gSL_Select = gSL_Select & "   end 編集完了日"
    gSL_Select = gSL_Select & ", case when zut.打切日 = '' then ''"
    gSL_Select = gSL_Select & "       else substring(zut.打切日,1,4)+'/'+substring(zut.打切日,5,2)+'/'+substring(zut.打切日,7,2)"
    gSL_Select = gSL_Select & "   end 編集打切日"
    gSL_Select = gSL_Select & ", case when zut.警告日 = '' then ''"
    gSL_Select = gSL_Select & "       else substring(zut.警告日,1,4)+'/'+substring(zut.警告日,5,2)+'/'+substring(zut.警告日,7,2)"
    gSL_Select = gSL_Select & "   end 編集警告日"
    gSL_Select = gSL_Select & ", case when zut.仕入先 = '' then ''"
    gSL_Select = gSL_Select & "       else zut.仕入先+space(7-len(zut.仕入先))+':'+isnull(tr1.略称,'')"
    gSL_Select = gSL_Select & "   end 編集仕入先"
    gSL_Select = gSL_Select & ", case when zut.切替仕入先 = '' then ''"
    gSL_Select = gSL_Select & "       else zut.切替仕入先+space(7-len(zut.切替仕入先))+':'+isnull(tr2.略称,'')"
    gSL_Select = gSL_Select & "   end 編集切替仕入先"
    gSL_Select = gSL_Select & ", isnull(zrm.表示寸法,'') 表示寸法"
    gSL_Select = gSL_Select & " from"
    gSL_Select = gSL_Select & "  材料発注打切テーブル zut"
    gSL_Select = gSL_Select & "  left outer join 材料マスタ zrm"
    gSL_Select = gSL_Select & "    on zrm.材料管理番号 = zut.材料管理番号"
    gSL_Select = gSL_Select & "  left outer join 取引先マスタ tr1"
    gSL_Select = gSL_Select & "    on tr1.取引先CD = zut.仕入先"
    gSL_Select = gSL_Select & "   and tr1.取引先区分 = 1"
    gSL_Select = gSL_Select & "  left outer join 取引先マスタ tr2"
    gSL_Select = gSL_Select & "    on tr2.取引先CD = zut.切替仕入先"
...
' 抽出文　作成
    ...
    gSL_Select = gSL_Select & CisDBGrid1.Select_Where
' グリッド内表示
...
' Call .GridInitSet(ﾏｰｼﾞ処理,分類処理,ﾃﾞｰﾀ連結名,印刷見出し,印刷桁数,合計印刷)
    With CisDBGrid1
         Call .GridInitSet(True, , "材質", "材     質", 25)
         Call .GridInitSet(True, , "表示寸法", "寸     法", 25)
         Call .GridInitSet(True, , "編集仕入先", "仕入先", 25)
         Call .GridInitSet(True, , "編集起算日", "起算日 - 便", 15)
         Call .GridInitSet(, , "設定数", "設定数", 12, False)
         Call .GridInitSet(, , "警告数", "警告数", 12, False)
         Call .GridInitSet(, , "発注数", "発注数", 12, True)
         Call .GridInitSet(, , "入荷数", "入荷数", 12, True)
         Call .GridInitSet(, , "設定残数", "設定残数", 12, False)
...
Private Sub BodySet()
    ...
    wUKbn = CisFun.Val2(DBGrid1.Columns(14))
    wZKno = RTrim(DBGrid1.Columns(20))
    wTorcd = RTrim(DBGrid1.Columns(19))

    gSL_Select = ""
    gSL_Select = gSL_Select & "select"
    gSL_Select = gSL_Select & "  ztm.取引先"
    gSL_Select = gSL_Select & ", ztm.受入"
    gSL_Select = gSL_Select & ", ztm.材質"
    gSL_Select = gSL_Select & ", ztm.背番号"
    gSL_Select = gSL_Select & ", isnull(trm.略称, '') 手配先名"
    gSL_Select = gSL_Select & ", ("
    gSL_Select = gSL_Select & "select count(*)"
    gSL_Select = gSL_Select & "  from 材料品番マスタ zhm"
    gSL_Select = gSL_Select & " where zhm.材料管理番号 = ztm.材料管理番号"
    ...
    Select Case wUKbn
        Case 0: gSL_Select = gSL_Select & " where ztm.材料管理番号 = '" & wZKno & "'"
        Case 1: gSL_Select = gSL_Select & " where ztm.材料管理番号 = '" & wZKno & "'"
        Case 3: gSL_Select = gSL_Select & " where ztm.取引先 = '" & wTorcd & "'"
        Case Else
            gSL_Select = gSL_Select & " where ztm.材料管理番号 = '" & wZKno & "'"
            gSL_Select = gSL_Select & "   and ztm.取引先 = '" & wTorcd & "'"
    End Select
...
'       ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
        ...
        Do Until Not ZTM_RDSTS
            ...
            With vsGrid1
                ...
                .TextMatrix(ID, CisVsGrid1.FixedGet(1)) = RTrim(ZTM.材質)
                .TextMatrix(ID, CisVsGrid1.FixedGet(2)) = ZTM.取引先 & Space(7 - CisFun.Len2(ZTM.取引先)) & ":" & RTrim(ZTM.手配先名)
                .TextMatrix(ID, CisVsGrid1.FixedGet(3)) = RTrim(ZTM.受入)
                .TextMatrix(ID, CisVsGrid1.FixedGet(4)) = RTrim(ZTM.背番号)
                If ZTM.件数 <> 0 Then
