...
'*---------------------------------*
    Do Until Not HCM_RDSTS
        ...
        If wHJikan <> RTrim(HCM.発行時間) Then
            wHJikan = RTrim(HCM.発行時間)
            CisPrtCKN0110.cLine = CisPrtCKN0110.iLineMax
        End If
        Call HeadWrite(0)
    ...
    '   Tail部　印刷
       'With CisPrtCKN0110
       '    If Not HCM_RDSTS Or .iLineMax = .cLine Or wHJikan <> RTrim(HCM.発行時間) Then
       '        .PrintFld ("T1*")
       '    End If
       'End With
    Loop
...
Private Function HeadWrite(KB As Byte)
    ...
    '   発行日・頁
        ...
        If CisFun.Val2(HCM.発行時間) <> 0 Then
           H1_HJikan = CisFun.Mid2(HCM.発行時間, 1, 2) & ":" & CisFun.Mid2(HCM.発行時間, 3, 2)
        End If
        .PrintNewPage       '改頁処理
        .PrintFld ("H1")    'HEAD部印刷
    End With
