...
'**       処理概要      :
'**
'**       作  成  日    :   2009/09/10  By CIS SIMPR-A
...
'       * 背番号*
        If RTrim(iB12_Seban(wNum1)) = "" Then
            wcc_Body = wcc_Body & Space(6)                                          ' 2009.09.10 upd : 6桁にした
        Else
...
'       * 納入数 *
    ...
    Next wNum1
'Call MsgBox("-->" & wcc_Body & "<--")   ' 2009.09.10：確認用
'
...
Private Function ChekData(ReadType As Integer) As Boolean
    ...
    gStr = wcc_Body
    wcc_Body = Mid(gStr, 1, 260)        ' 2009.09.10 upd : 52*5=260
    wcc_Body2 = Mid(gStr, 261)          ' 2009.09.10 upd :
'Call Test_LogOutput(wcc_Head & vbCrLf & wcc_Body & vbCrLf & wcc_Body2)     ' (確認用)

    sv_Uke2 = ""
    sv_NOTor = ""

    With CisDB
        .SQL = "受注チケットチェック_テクノ"   ' 2009.09.10 upd
        .StoadoCount = 11
...
'.ParaNo = 1: .ParaValue = 0    ' 確認試験用
        ...
        ' 明細1-5
        .ParaNo = 6: wStr1 = Mid(.ParaValue, 1, 260)    ' 2009.09.10 upd
        ' 明細6-10
...
'Call MsgBox(Mid(wStr1, wNum2 + 1, 54))  ' 確認用
        If RTrim(Mid(wStr1, wNum2 + 1, 31)) = "" Then Exit For

        iB12_Hinbn(wNum1) = RTrim(Mid(wStr1, wNum2 + 1, 25))
        iB12_Seban(wNum1) = RTrim(Mid(wStr1, wNum2 + 26, 6))
        iB12_Syuyo(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 32, 6), 0, iB12_Syuyo(wNum1).cFormat)
        iB12_Hako(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 38, 5), 0, iB12_Hako(wNum1).cFormat)
        iB12_Su(wNum1) = CisFun.RSetFld(Mid(wStr1, wNum2 + 43, 10), 0, iB12_Su(wNum1).cFormat)
...
'        Exit Sub
'    End If
    If Mid(QT10QSR_ReadData, 25, 4) <> "3999" Then
        With CisFun
             .MB_Lines = 5
             .MB_MSG(2) = "  対象外伝票です      "
             .MB_Title = ""
...
'
    ...
    '       * 背番号*
            ...
            Else
                wcc_Body = wcc_Body & Format(Mid(.背番号(wNum1), 1, 6), "!@@@@@@")  ' 2009.09.10 upd : 桁数
            End If
    '       * 収容数 *
...
'Exit Sub   '=========================================== 実行しないときはこの行を生かすこと =========================
    ...
    f_No = FreeFile
    MsgBox "\\tsclient\D\SIMPR-Toki\Exe\Log\" & dt02 & ".log"
    
    'ファイル番号の取得
    Open "D:\SIMPR-Toki\Exe\Log\" & dt02 & ".log" _
          For Output As #f_No                   'ファイルを追加モードで開く
