...
Begin VB.Form CXJ0760 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "【リバース】受注データ変換"
   ClientHeight    =   11190
   ClientLeft      =   150
   ClientTop       =   1290
   ClientWidth     =   15360
...
'-------------------  文字列分割（Split）関数を生成
    With CisDB
        .SQL = ""
        .SQL = .SQL & "SELECT COUNT(*) FROM sys.objects WHERE type = 'TF' AND name = 'Split'" & vbCrLf
        If .DBRead(1) Then
           If .Recordset.Fields.Item(0).Value = 0 Then
                .SQL = ""
                .SQL = .SQL & "CREATE FUNCTION Split(" & vbCrLf
                .SQL = .SQL & "    @target_str AS VARCHAR (MAX)       --分割対象の文字列" & vbCrLf
                .SQL = .SQL & "    , @separator AS VARCHAR (1)        --分割する文字" & vbCrLf
                .SQL = .SQL & ")" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "RETURNS @out_table TABLE (" & vbCrLf
                .SQL = .SQL & "    Id INT IDENTITY (1, 1)             --テーブル挿入インデックス（自動で連番を振る）" & vbCrLf
                .SQL = .SQL & "    , Data VARCHAR (MAX)               --分割後の文字列" & vbCrLf
                .SQL = .SQL & ") AS" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "BEGIN" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "---------------------------------------------" & vbCrLf
                .SQL = .SQL & "--文字列分割（Split）関数" & vbCrLf
                .SQL = .SQL & "--  使い方：  SELECT * FROM dbo.Split( 'aaa,bbb,ccc,ddd,eee', ',')" & vbCrLf
                .SQL = .SQL & "---------------------------------------------" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "    DECLARE @start_pos INT = 1" & vbCrLf
                .SQL = .SQL & "    DECLARE @end_pos INT = CHARINDEX(@separator, @target_str)" & vbCrLf
                .SQL = .SQL & "    DECLARE @str_len INT = LEN(@target_str)" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "    ---------------------------------------------" & vbCrLf
                .SQL = .SQL & "    --指定された文字列が空の場合は処理を行わない" & vbCrLf
                .SQL = .SQL & "    ---------------------------------------------" & vbCrLf
                .SQL = .SQL & "    IF @target_str = '' OR @target_str IS NULL BEGIN RETURN END" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "    --------------------------------------------------------------" & vbCrLf
                .SQL = .SQL & "    --文字列内の最後の分割する文字まで分割文字を検索して分割する" & vbCrLf
                .SQL = .SQL & "    --------------------------------------------------------------" & vbCrLf
                .SQL = .SQL & "    WHILE @end_pos > 0" & vbCrLf
                .SQL = .SQL & "    BEGIN" & vbCrLf
                .SQL = .SQL & "        INSERT INTO @out_table(Data)" & vbCrLf
                .SQL = .SQL & "        VALUES (" & vbCrLf
                .SQL = .SQL & "            SUBSTRING(@target_str, @start_pos, @end_pos - @start_pos)" & vbCrLf
                .SQL = .SQL & "        )" & vbCrLf
                .SQL = .SQL & "        SET @start_pos = @end_pos + 1" & vbCrLf
                .SQL = .SQL & "        SET @end_pos = CHARINDEX(@separator, @target_str, @start_pos)" & vbCrLf
                .SQL = .SQL & "    END" & vbCrLf
                .SQL = .SQL & "    ---------------------------------------------" & vbCrLf
                .SQL = .SQL & "    --最後の１つが残るのでここで追加する" & vbCrLf
                .SQL = .SQL & "    ---------------------------------------------" & vbCrLf
                .SQL = .SQL & "    INSERT INTO @out_table(Data)" & vbCrLf
                .SQL = .SQL & "    VALUES (" & vbCrLf
                .SQL = .SQL & "        SUBSTRING(@target_str, @start_pos, @str_len + 1 - @start_pos)" & vbCrLf
                .SQL = .SQL & "    )" & vbCrLf
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "    ---------------------------------------------" & vbCrLf
                .SQL = .SQL & "    --処理終了" & vbCrLf
                ...
                .SQL = .SQL & "" & vbCrLf
                .SQL = .SQL & "END" & vbCrLf
                If Not .DBExec Then GoTo JTextProc_Er
           End If
        End If
    End With
...
Public Function Jyucyu_Proc() As Boolean
    ...
    gStr = "【 テキスト受注累積（出荷指示書発行) 開始 】"
    Call Text_LogPut(gStr, "G", 1)
   '   出荷指示書発行
    Call CXJ0120LPrint(0)
    ' ログ出力
    gStr = "【 テキスト受注累積（出荷指示書発行) 終了 】"
    Call Text_LogPut(gStr, "G", 1)
...
Private Function ZenProc_Get()
    ...
    Call ItemsClearSYM
    gSL_Select = "Select * From 名称マスタ "
    gSL_Select = gSL_Select & " Where 区分名称   = 'テキスト管理[リバース]'"
    gSL_Select = gSL_Select & " And   区分タイプ = ''"
