...
'#比較退避用#
    ...
    Dim BK_Zais         As String       '材質
    Dim BK_Saiz         As String       'サイズ
    Dim BK_TehaiKB      As String       '手配区分(0/購入,1/直送,2/社内,3/受給)
    Dim BK_Syuyo        As Currency     '収容数
...
Private Function PrintProc()
    ...
    Call CisPrtCKHZ0910.ClrLstFld("B1")
    
    Do Until Not ZRE_RDSTS
        With ZRE
        ...
        '*---------------------------------*
            If FirstFLG Then
                If lIx + 1 > 5 Or Not BreakFLG Then
                    B1_Line.Visible = Not BreakFLG
                    If Not BreakFLG Then
                       B1_MAISU = CisFun.RSetFld(TMai, 3, "##0")
                       TMai = 0
                    End If
                    Call CisPrtCKHZ0910.PrintFld("B1*")
                    CisPrtCKHZ0910.cLine = CisPrtCKHZ0910.cLine + 1
                    Call CisPrtCKHZ0910.ClrLstFld("B1")
                    lIx = 0: Erase mNo
                End If
            Else
        ...
        '*---------------------------------*
            '#エラー要因
            If CisPrtCKHZ0910.cLine >= CisPrtCKHZ0910.iLineMax Or _
               .エラー要因 <> BK_ErrCD Then
                Select Case .エラー要因
                    Case 1: B1_ERRCMNT = "対象外かんばん(かんばん区分)"
                    Case 2: B1_ERRCMNT = "材料取引先未登録"
...
'----------------------------------------------------------------< 2005.02.24 ADD START >
            ...
            If .取引先 <> BK_Torcd Then
                B1_TORCD = .取引先                     '.取引先
                B1_TORNM = CisFun.Mid2(.取引先名, 1, 12)   '.取引先略称
                BK_Ukeir = ""
            End If
            ...
            '#品番,(背番号)
            If .材質 <> BK_Zais Or .サイズ <> BK_Saiz Then
                B1_Zais = CisFun.Mid2(.材質, 1, 23)
                B1_Sunpou = .表示寸法
                B1_Seban = .背番号
            End If
            ...
            If Not BreakFLG Then
               B1_TEHAIKBN = .手配区分名
               B1_SYUYO = CisFun.RSetFld(.収容数, 9, "##0.000")
               If Trim(.サイクル) <> "" Then
        ...
        With CisPrtCKHZ0910
            .cDataCnt = .cDataCnt + 1
            .dpCountDisp Format(.cDataCnt, "#,###") & " / " & Format(CKHZ0910_PrtCnt, "#,###")
        End With
    ...
    ' 印刷終了
    Call CisPrtCKHZ0910.PrintEndDoc
