   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin Cis3D_v60.CIS3D CIS3D11 
         ...
         Begin vsFlexLib.vsFlexArray vsGrid1 
            ...
            Appearance      =   1
            ConvInfo        =   1418783674
            FormatString    =   "№  |納 入 日　-便|仕入先|仕入先名    |納 番 |頁|手配|背番号|材  質　　               |収容数    |箱数 |発注数    ||"
            Rows            =   21
            Cols            =   15
            BackColor       =   12648447
...
Private Sub PB_Prt_Click()
    If ProcHB = "B1" Then
        If Body1Chk Then
            ...
            wPrintKB = 1                ' 帳票区分：0=29      Else 1=129
            wPrintOut = gDemoPrt        ' 出力先　：0=Printer Else 1=Preview
            Call CXHZ0340LPrint(wPrintKB, wPrintOut)
...
'   ### 再発行最新 ###
        ...
        gSL_Select = gSL_Select & "   and cht.帳票区分 = 29"
        gSL_Select = gSL_Select & "   and zct.発注管理NO = cht.管理NO"
        gSL_Select = gSL_Select & "   and zct.SK区分 = 0"
    Else
'   ### 再発行既存 ###
        gSL_Select = gSL_Select & " where zct.SK区分 = 0"
        '--2005/01/17 追加 --------------------(START)----------------
        gSL_Select = gSL_Select & "   and zct.発注区分 = 3"
        '--2005/01/17 追加 --------------------(end)----------------
    End If
'   >> 納入番号
    If InStr(1, iH1_SNouNo, "*") > 0 Then
...
Public Function DBDelete_CHT(Optional DBNo As Integer) As Boolean
    ...
    With CHT
        ...
        gSL_Select = gSL_Select & "   and 帳票区分 = 129"
    End With
    With CisDB
        .ConnectNo = DBNo
        .SQL = gSL_Select
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        ...
        Call .FixedSet(中中, 右中, あり, 10, 9, "収容数")
        Call .FixedSet(中中, 右中, あり, 5, 10, "箱数")
        Call .FixedSet(中中, 右中, あり, 10, 11, "発注数")
...
Private Sub ZctInfoToGrid1(SetRow As Long)
    With vsGrid1
        ...
        '発注管理NO
        .TextMatrix(SetRow, CisVsGrid1.FixedGet(12)) = ZCT.発注管理NO
    End With
End Sub
'******************************************'
'*****      行選択（クリック）
