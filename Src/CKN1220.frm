...
'**       変  更  日    :   2006/08/09 発注かんばん、工程かんばんレイアウト変更　(CoReports8.5)
'** -------------------------------------------------------
'**       変  更  日    :   2007/01/25  キノックスより移行
'**                                     かんばんレイアウト変更
'**       変  更  日    :   2007/02/16  By CIS ナルコより移行
'**                                             秋田用レイアウト変更
'**       変  更  日    :   2008/09/16  By CIS 千代田Ver
'**
'**********************************************************'
...
Private Sub PB_Prt_Click()
    ...
    If kn_PrtKB = 0 Then
        CKN1230_PrtKB = 0
        CKN1230_PrtSy = 0
        Call CKN1230LPrint(1)
'''    Else
'''        CKN1240_PrtKB = 1
'''        CKN1240_PrtSy = 1
'''        Call CKN1240LPrint(1)
    End If
...
'---------------------------------------------------------------------- 2008/09/16 UPDATE START
        'Call .FixedSet(中中, 左中, あり, 6, 5, "背番号")
        Call .FixedSet(中中, 左中, あり, 8, 8, "背番号")
'---------------------------------------------------------------------- 2008/09/16 UPDATE END
...
Private Sub ColorSet(cRow As Long, Optional cColSel As Integer = 0)
    ...
    With vsGrid1
        ...
        With CisDB
            ...
            .SQL = .SQL & " Where かんばん種類 = " & kn_PrtKB
' 2008/09/16 UPDATE START -------------------------------------------------------------------------
            '.SQL = .SQL & " And   背番号 = '" & RTrim(Mid(vsGrid1.TextMatrix(cRow, 5), 1, 6)) & "'"
