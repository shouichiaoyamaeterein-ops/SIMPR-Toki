...
'**       処理概要      :
'**
'**       作  成  日    :   2004/05/07  By CIS
'**       変  更  日    :   2005/06/18  By CIS かんばん流動履歴テーブル削除追加
'**       変  更  日    :   2006/12/20  By CIS 版数表示（かんばん重複チェックテーブルに版数追加）
'**
'**********************************************************'
...
Private Sub PB_Look_Click()
    ...
    If PB_Look.Tag = "iH1_Hinbn" Then Call Look_Hinbn(1): Exit Sub
End Sub
'+-----------------------------+
'+       取引先検索             +
'+-----------------------------+
...
'        iH1_Ukeir = RV_Ukeir
        If KB = 1 Then iH1_Hinbn = RV_Hinbn
...
' かんばん重複チェックテーブル SELECT文 ｾｯﾄ
    ...
    gSL_Select = gSL_Select & "     RTRIM(TR.略称) 仕入先名,ISNULL(HN.表示品番,'') 表示品番 "
    gSL_Select = gSL_Select & "     From かんばん重複チェックテーブル KJ"
    ...
    gSL_Select = gSL_Select & " Order By KJ.取引先,KJ.受入,KJ.背番号,KJ.品番,"
    gSL_Select = gSL_Select & "            KJ.連番"
    
    If Not KJCRead(gSL_Select) Then
       Call KJCClose
       With CisFun
            .MB_Lines = 4
    ...
    With vsGrid1
        For gLong = 1 To .Rows - 1
            ...
            If Trim(.TextMatrix(gLong, CisVsGrid.FixedGet(CisVsGrid.SelectCol))) = "X" Then
                ' かんばん重複チェックテーブル 削除
                CisDB.SQL = "Delete かんばん重複チェックテーブル "
                CisDB.SQL = CisDB.SQL & " Where 処理区分 = ''"
                CisDB.SQL = CisDB.SQL & "   And 納入日 = '" & Trim(.TextMatrix(gLong, CisVsGrid.FixedGet(10))) & "'"
                ...
                CisDB.SQL = CisDB.SQL & "   And 品番 = '" & Trim(.TextMatrix(gLong, CisVsGrid.FixedGet(9))) & "'"
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        ...
        Call .FixedSet(中中, 右中, あり, 4, 12, "版数")
'　ｸﾞﾘｯﾄﾞ選択処理用　退避ｴﾘｱ
        Call .FixedSet(中中, 左中, あり, 0, 13, "Select")
'  表示ﾊﾟﾀｰﾝｾｯﾄ
'       Call .PatanSet(ﾊﾟﾀｰﾝNO,表示パターン)
        Call .PatanSet(0, "0,1,2,3,4,5,6,12,7,8")
'  編集内容セット
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    End With
    ID = 0
    Do Until Not KJC_RDSTS
        With vsGrid1
            ID = ID + 1
            '№
            .TextMatrix(ID, GridCol(0)) = Format(ID, "####")
            ' 仕入先
            .TextMatrix(ID, GridCol(1)) = KJC.取引先
            .TextMatrix(ID, GridCol(2)) = KJC.仕入先名
            ' 受入
            .TextMatrix(ID, GridCol(3)) = KJC.受入
            ' 背番号
            .TextMatrix(ID, GridCol(4)) = KJC.背番号
            ' 品番
            If Trim(KJC.表示品番) = "" Then
                gStr = KJC.品番
            Else
                gStr = KJC.表示品番
            End If
            .TextMatrix(ID, GridCol(5)) = gStr
            .TextMatrix(ID, GridCol(9)) = KJC.品番
            ' 連番
            .TextMatrix(ID, GridCol(6)) = Format(KJC.連番, "000")
            ' 読取日時
            .TextMatrix(ID, GridCol(7)) = Format(KJC.作成日, "YYYY/MM/DD hh:nn:ss")
            ' 納入日･便
            If Trim(KJC.納入日) <> "" Then
               With CisFun
                    .DateE_BefVal = KJC.納入日
                    .DateEdit
                    gStr = .DateE_AfVal
               End With
               If Trim(KJC.納入便) <> "" Then
                  gStr = gStr & "-" & Format(KJC.納入便, "00")
               Else
                  gStr = gStr & "   "
               End If
               .TextMatrix(ID, GridCol(8)) = gStr
               .TextMatrix(ID, GridCol(10)) = KJC.納入日
               .TextMatrix(ID, GridCol(11)) = Format(KJC.納入便, "00")
               
            End If
            If KJC.版数 <> 0 Then
            '------------------------------------------------------------2006/12/20 追加 (Start)
            ' 版数
                .TextMatrix(ID, GridCol(12)) = KJC.版数
            End If
            '------------------------------------------------------------2006/12/20 追加 (End)
