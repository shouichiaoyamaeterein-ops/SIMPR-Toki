...
Begin VB.Form CXHZ0230 
   BackColor       =   &H00808000&
   BorderStyle     =   1  '固定(実線)
   Caption         =   "材料・入荷画面入力 [単品]"
   ClientHeight    =   9945
   ClientLeft      =   75
   ClientTop       =   1275
   ClientWidth     =   14715
      ...
      Begin Cis3D_v60.CIS3D UC_3D4 
         ...
         Begin Cis3D_v60.CIS3D BLB_DD 
            ...
            Begin vsFlexLib.vsFlexArray vsGrid 
               ...
               Appearance      =   1
               ConvInfo        =   1418783674
               FormatString    =   "№ |背番号  |材  質　　　　　　　　　 |板厚   |幅     |長さ   |収容数    |枚数 |端   　数 |入荷数   　|単位| |"
               Rows            =   11
               Cols            =   20
               BackColor       =   12648384
...
'発注区分 = 通常の時、
    ...
    End If
' マスタ入力 SELECT文 ｾｯﾄ
    gSL_Select = " SELECT ZW.*,ISNULL(SY1.値名称,'')手配単位名 "
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2008/04/11 UPDATE END
        ...
        Call .FixedSet(中中, 右中, あり, 13, 9, "入荷数")
...
' ｸﾞﾘｯﾄﾞ行再ｾｯﾄ & 列位置取得
    ...
    Do Until Not ZNG_RDSTS
        ...
        With vsGrid
            ...
            .TextMatrix(ID, GridCol(2)) = RTrim(ZNG.材質)
            If ZNG.板厚 <> 0 Then
                .TextMatrix(ID, GridCol(3)) = Format(ZNG.板厚, "###0.00")
            End If
            If ZNG.幅 <> 0 Then
                .TextMatrix(ID, GridCol(4)) = Format(ZNG.幅, "###0.00")
            End If
            If ZNG.長さ <> 0 Then
                .TextMatrix(ID, GridCol(5)) = Format(ZNG.長さ, "###0.00")
            End If
            If ZNG.収容数 <> 0 Then
                .TextMatrix(ID, GridCol(6)) = Format(ZNG.収容数, "#####0.000")
            End If
            .TextMatrix(ID, GridCol(7)) = Format(ZNG.枚数, "####")
            If ZNG.端数 <> 0 Then
                .TextMatrix(ID, GridCol(8)) = Format(ZNG.端数, "#####0.000")
            End If
            If ZNG.発注数 <> 0 Then
                .TextMatrix(ID, GridCol(9)) = Format(ZNG.発注数, "#####0.000")
            End If
            .TextMatrix(ID, GridCol(10)) = RTrim(ZNG.手配単位名)
        End With
        Call ZNGReadNext(1)
    Loop
...
'入荷プルーフリスト
    ...
    If Not CXHZ0230LPrint(PrtOut) Then Exit Sub
