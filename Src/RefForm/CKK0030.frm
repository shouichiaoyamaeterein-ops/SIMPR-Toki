...
Begin VB.Form CKK0030 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0030.frx":0000
         Height          =   5580
         Left            =   90
         OleObjectBlob   =   "CKK0030.frx":0015
         TabIndex        =   5
         Top             =   60
         Width           =   11175
      End
...
Private Function BodySet() As Boolean
    ...
    gSL_Select = "SELECT HN.品番,HN.表示品番,HN.社内背番,' ' + HN.社内背番 表示社内背番,HN.品名,"
    gSL_Select = gSL_Select & " Case When isnull(HT1.品番,'') = '' then 0 Else 1 End 受注,"
    gSL_Select = gSL_Select & " Case When isnull(HT2.品番,'') = '' then 0 Else 1 End 発注"
    gSL_Select = gSL_Select & " From 品番マスタ HN"
    gSL_Select = gSL_Select & "  Left Outer Join"
    gSL_Select = gSL_Select & " ( Select 品番 From 品番取引先マスタ Where 品目 = 0 "
    gSL_Select = gSL_Select & " Group by 品番 ) HT1"
    gSL_Select = gSL_Select & "    ON HT1.品番 = HN.品番"
    gSL_Select = gSL_Select & "  Left Outer Join"
    gSL_Select = gSL_Select & " ( Select 品番 From 品番取引先マスタ Where 品目 = 1 "
    gSL_Select = gSL_Select & " Group by 品番 ) HT2"
    gSL_Select = gSL_Select & "    ON HT2.品番 = HN.品番"
     
    mSL_OrderA = " Order By HN.品番 "
    mSL_OrderD = " Order By HN.品番 Desc"
...
' 選択条件セット
    ...
    With CisFun
        ...
        If H1Ck_Hinmoku(0).Value = 1 Then
           .Where_Create AlphaMe, "ISNULL(HT1.品番,'')", "", 等しくない, Create
        End If
        If H1Ck_Hinmoku(1).Value = 1 Then
           .Where_Create AlphaMe, "HT2.品番", "", NotNul, Create
        End If
