...
Begin VB.Form CKK0028 
   ...
   Begin VB.PictureBox H1_Area1 
      ...
      Begin Cis3D_v60.CIS3D UC_3D 
         ...
         Begin Cis3D_v60.CIS3D UC_3D2 
            ...
            cFont3DColor2   =   8421504
            cAlingnment     =   6
            Begin CisText_V60.CisText iH1_SKana 
               Height          =   375
               Left            =   315
               TabIndex        =   0
               Top             =   330
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0028.frx":0000
         Height          =   5550
         Left            =   90
         OleObjectBlob   =   "CKK0028.frx":0015
         TabIndex        =   4
         Top             =   60
         Width           =   7305
      End
...
'**       処理概要      :   取引先区分により内容を変更
'**
'**       作  成  日    :   2003/11/28  By CIS
'**       変  更  日    :   2004/01/29  取引先区分KEY追加に伴う変更
...
'    Timer1.Enabled = True    ' 時刻表示開始
    If RV_TorKb = 1 Then
       FormCap = "【受注】納入先検索": HLB_Title = "納入先受入 検索"
       TorWhere = " TR.取引先区分 = 0  AND TR.契約先区分 = 0"
    End If
    
    If RV_TorKb = 2 Then
       FormCap = "【受注】契約先検索": HLB_Title = "[受注]契約先受入 検索"
       TorWhere = " TR.取引先区分 = 0 AND TR.契約先区分 = 1"
    End If
    If RV_TorKb = 3 Then
       FormCap = "【受注】納入先/契約先 検索": HLB_Title = "納入先/契約先受入 検索"
       TorWhere = " TR.取引先区分 = 0 "
    End If
    If RV_TorKb = 4 Then
       FormCap = "【発注】手配先検索": HLB_Title = "手配先受入 検索"
       TorWhere = " TR.取引先区分 = 1 AND TR.契約先区分 = 0 AND TR.社内区分 = 0"
    End If
    If RV_TorKb = 5 Then
       FormCap = "【発注】契約先検索": HLB_Title = "[発注]契約先受入 検索"
       TorWhere = " TR.取引先区分 = 1 AND TR.契約先区分 = 1 AND TR.社内区分 = 0"
    End If
    If RV_TorKb = 6 Then
       FormCap = "【発注】社内検索": HLB_Title = "社内受入 検索"
       TorWhere = " TR.取引先区分 = 1 AND TR.契約先区分 = 0 AND TR.社内区分 = 1"
    End If
    If RV_TorKb = 7 Then
       FormCap = "【発注】手配先/社内検索": HLB_Title = "手配先/社内受入 検索"
       TorWhere = " TR.取引先区分 = 1 AND TR.契約先区分 = 0"
    End If
    If RV_TorKb = 8 Then
       FormCap = "【発注】手配先/契約先検索": HLB_Title = "手配先/契約先受入 検索"
       TorWhere = " TR.取引先区分 = 1 AND TR.社内区分 = 0"
    End If
    If RV_TorKb = 9 Then
       FormCap = "【発注】手配先/契約先/社内検索": HLB_Title = "手配先受入 検索"
       TorWhere = " TR.取引先区分 = 1 "
    End If
    TorWhere = TorWhere & " AND TU.受入 IS NOT NULL "
    Me.Caption = FormCap
'   #------------------#
'   #    初期入力設定   #
'   #------------------#
...
' ADOｺﾝﾄﾛｰﾙ　接続情報　セット
    ...
    If RV_Torcd <> "" Then
       With CisDB
            .SQL = "Select 取引先,工場CD From 取引先マスタ"
            .SQL = .SQL & " WHERE 取引先CD = '" & RV_Torcd & "'"
            If .DBRead(1) Then
               RV_Torcd = .Recordset("取引先")
               RV_Kojyo = .Recordset("工場CD")
            End If
       End With
    End If
...
Private Function BodySet() As Boolean
    ...
    BodySet = False
    gSL_Select = "SELECT TR.取引先,TR.取引先CD,TU.受入,TR.工場CD,"
    gSL_Select = gSL_Select & "CASE WHEN RTRIm(TR.工場CD) = '' THEN TR.取引先 ELSE TR.取引先 + '-' + TR.工場CD END 表示取引先,  "
    gSL_Select = gSL_Select & "TR.略称,TR.工場名 "
    gSL_Select = gSL_Select & " FROM 取引先マスタ TR "
    gSL_Select = gSL_Select & " Left Outer Join 取引先受入マスタ TU "
    gSL_Select = gSL_Select & "    On  TU.取引先区分 = TR.取引先区分"
    gSL_Select = gSL_Select & "    AND TU.取引先CD = TR.取引先CD"
    mSL_OrderA = " Order By TR.取引先CD,TU.受入 "
    mSL_OrderD = " Order By TR.取引先CD Desc,TU.受入 Desc"
' 選択条件セット
    mSL_SelectW = ""
    With CisFun
        .Where_CreateKB = NewDocument
        .Where_Create AlphaMe, "TR.索引カナ", iH1_SKana
        .Where_Create AlphaMe, "TR.取引先", iH1_Torcd
        .Where_Create AlphaMe, "TR.工場CD", iH1_Kojyo
        .Where_Create AlphaMe, "TU.受入", iH1_Ukeir
        mSL_SelectW = .Where_Phrase
    End With
    If Trim(mSL_SelectW) <> "" Then
       If Trim(TorWhere) <> "" Then
          mSL_SelectW = mSL_SelectW & " And " & TorWhere
       End If
    Else
       If Trim(TorWhere) <> "" Then
          mSL_SelectW = " Where " & TorWhere
       End If
    End If
    If Op_Sort(0) Then
...
Private Sub RtnSet()
    With Adodc1
        RV_Torcd = .Recordset("取引先")
        RV_Kojyo = .Recordset("工場CD")
        RV_TorRName = .Recordset("略称")
        RV_Ukeir = .Recordset("受入")
        RV_TorcdK = RV_Torcd & RV_Kojyo
    End With
    Adodc1.Recordset.Close
    RV_Rtn = True
    Call PB_END_Click
