...
Begin VB.Form CKK0029 
   ...
   Begin VB.PictureBox H1_Area1 
      ...
      Begin Cis3D_v60.CIS3D UC_3D 
         ...
         Begin Cis3D_v60.CIS3D CIS3D1 
            ...
            cFont3DColor2   =   8421504
            cAlingnment     =   6
            Begin Cis3D_v60.CIS3D H1lb_JTornm 
               Height          =   360
               Left            =   1035
               Top             =   330
               Width           =   2640
               ...
               cPositionX      =   50
            End
            Begin CisText_V60.CisText iH1_JTorcd 
               Height          =   375
               Left            =   60
               TabIndex        =   11
               Top             =   330
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0029.frx":0000
         Height          =   5550
         Left            =   90
         OleObjectBlob   =   "CKK0029.frx":0015
         TabIndex        =   3
         Top             =   60
         Width           =   6870
      End
...
'**********************************************************'
'**
'**       フォーム名    :   取引先マスタ検索(品番取引先マスタ)
'**       フォームID    :   CKK0029
'**       処理概要      :   実契約先パラメータから品番取引先マスタを検索
'**
'**       作  成  日    :   2006/02/10  By CIS
...
' ADOｺﾝﾄﾛｰﾙ　接続情報　セット
    ...
    iH1_JTorcd = RV_JTorcd
    Call iH1_JTorcd_LostFocus
...
End Sub
Private Sub iH1_JTorcd_LostFocus()
    If RTrim(iH1_JTorcd) <> "" Then
       If RV_TorKb = 0 Then
          If TorNmGet(RTrim(iH1_JTorcd), 3) Then
             H1lb_JTornm = TRM_RName
          Else
             H1lb_JTornm = ""
          End If
       Else
          If TorNmGet(RTrim(iH1_JTorcd), 9) Then
             H1lb_JTornm = TRM_RName
          Else
             H1lb_JTornm = ""
          End If
...
Private Function BodySet() As Boolean
    ...
    BodySet = False
    gSL_Select = "SELECT TR.取引先,TR.工場CD,CASE WHEN RTRIm(TR.工場CD) = '' THEN TR.取引先 ELSE TR.取引先 + '-' + TR.工場CD END 表示取引先,  "
    ...
    gSL_Select = gSL_Select & "  AND " & TorWhere
    mSL_OrderA = " Order By TR.取引先 "
    mSL_OrderD = " Order By TR.取引先 Desc"
' 選択条件セット
    mSL_SelectW = ""
    With CisFun
        .Where_CreateKB = NewDocument
        .Where_Create AlphaMe, "HT.契約先", iH1_JTorcd
        mSL_SelectW = .Where_Phrase
    End With
    If Trim(mSL_SelectW) <> "" Then
       mSL_SelectW = mSL_SelectW & " And " & HinWhere
    Else
       mSL_SelectW = " Where " & HinWhere
    End If
    mSL_SelectW = mSL_SelectW & " AND  ISNULL(TR.取引先,'') <> '' "
    mSL_SelectW = mSL_SelectW & " GROUP BY TR.取引先,TR.工場CD,TR.略称,TR.工場名 "
