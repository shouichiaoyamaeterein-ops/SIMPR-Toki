   ...
   Begin VB.PictureBox H1_Area1 
      ...
      Begin Cis3D_v60.CIS3D UC_3D 
         ...
         Begin Cis3D_v60.CIS3D CIS3D7 
            ...
            cAlingnment     =   6
            cPositionY      =   30
            Begin CisText_V60.CisText iH1_Size 
               Height          =   360
               Left            =   150
               TabIndex        =   5
               Top             =   345
...
'**       変  更  日    :   2010/01/21  By CIS 検索項目追加
...
Private Function BodySet(SetType As String) As Boolean
    ...
    gSL_Select = gSL_Select & "    ISNULL(ZT.ﾒｰｶｰ受入,'') ﾒｰｶｰ受入,ISNULL(ZT.置場,'') 置場,"
    gSL_Select = gSL_Select & "    ISNULL(ZT.アドレス,'') アドレス,ISNULL(ZM.材料名,'') 材料名"

'---------------------------------------------------------------' 2009.02.27 add start
    gSL_Select = gSL_Select & "   ,ISNULL(ZT.自社品番, '') 自社品番 "
    gSL_Select = gSL_Select & "   ,CASE WHEN ISNULL(TR4.略称,'') <> '' Then ISNULL(ZT.加工先,'') + '：' + TR4.略称 Else ISNULL(ZT.加工先,'') End 表示加工先"
    gSL_Select = gSL_Select & "   ,CASE WHEN ISNULL(TR5.略称,'') <> '' Then ISNULL(ZT.得意先,'') + '：' + TR5.略称 Else ISNULL(ZT.得意先,'') End 表示得意先"
'---------------------------------------------------------------' 2009.02.27 add end
...
'---------------------------------------------------------------' 2009.02.27 add start
    ...
    gSL_Select = gSL_Select & "    ON  TR4.取引先区分 = 1"
    gSL_Select = gSL_Select & "    AND TR4.取引先CD = ISNULL(ZT.加工先,'')"
    gSL_Select = gSL_Select & "    Left Outer Join 取引先マスタ TR5"
    gSL_Select = gSL_Select & "    ON  TR5.取引先区分 = 0"
    gSL_Select = gSL_Select & "    AND TR5.取引先CD = ISNULL(ZT.得意先,'')"
'---------------------------------------------------------------' 2009.02.27 add end
...
'-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 2010/01/21 INSERT START
        .Where_Create Numeric, "ISNULL(ZM.板厚,0)", iH1_Itatu
        .Where_Create Numeric, "ISNULL(ZM.幅,0)", iH1_Width
        .Where_Create Numeric, "ISNULL(ZM.長さ,0)", iH1_Size
        .Where_Create AlphaMe, "ISNULL(ZT.自社品番,'')", iH1_Hinbn
