...
Begin VB.Form CKK0025 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0025.frx":0000
         Height          =   5550
         Left            =   90
         OleObjectBlob   =   "CKK0025.frx":0015
         TabIndex        =   3
         Top             =   60
         Width           =   6870
      End
...
'    Timer1.Enabled = True    ' 時刻表示開始
    If RV_TorKb = 1 Then
       FormCap = "【受注】納入先検索": HLB_Title = "納入先 検索"
       TorWhere = " 取引先区分 = 0  AND 契約先区分 = 0"
    End If
    
    If RV_TorKb = 2 Then
       FormCap = "【受注】契約先検索": HLB_Title = "[受注] 契約先 検索"
       TorWhere = " 取引先区分 = 0 AND 契約先区分 = 1"
    End If
    If RV_TorKb = 3 Then
       FormCap = "【受注】納入先/契約先 検索": HLB_Title = "納入先/契約先 検索"
       TorWhere = " 取引先区分 = 0 "
    End If
    If RV_TorKb = 4 Then
       FormCap = "【発注】手配先検索": HLB_Title = "手配先 検索"
       TorWhere = " 取引先区分 = 1 AND 契約先区分 = 0 AND 社内区分 = 0"
    End If
    If RV_TorKb = 5 Then
       FormCap = "【発注】契約先検索": HLB_Title = "[発注] 契約先 検索"
       TorWhere = " 取引先区分 = 1 AND 契約先区分 = 1 AND 社内区分 = 0"
    End If
    If RV_TorKb = 6 Then
       FormCap = "【発注】社内検索": HLB_Title = "社内 検索"
       TorWhere = " 取引先区分 = 1 AND 契約先区分 = 0 AND 社内区分 = 1"
    End If
    If RV_TorKb = 7 Then
       FormCap = "【発注】手配先/社内検索": HLB_Title = "手配先/社内 検索"
       TorWhere = " 取引先区分 = 1 AND 契約先区分 = 0"
    End If
    If RV_TorKb = 8 Then
       FormCap = "【発注】手配先/契約先検索": HLB_Title = "手配先/契約先 検索"
       TorWhere = " 取引先区分 = 1 AND 社内区分 = 0"
    End If
    If RV_TorKb = 9 Then
       FormCap = "【発注】手配先/契約先/社内検索": HLB_Title = "手配先 検索"
       TorWhere = " 取引先区分 = 1 "
    End If
    Me.Caption = FormCap
'   #------------------#
...
Private Function BodySet() As Boolean
    ...
    gSL_Select = gSL_Select & "略称,工場名    "
    gSL_Select = gSL_Select & " FROM 取引先マスタ "
    mSL_OrderA = " Order By 取引先CD "
    mSL_OrderD = " Order By 取引先CD Desc"
' 選択条件セット
    mSL_SelectW = ""
    With CisFun
