...
Begin VB.Form CKH0030 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKH0030.frx":0000
         Height          =   5550
         Left            =   90
         OleObjectBlob   =   "CKH0030.frx":0015
         TabIndex        =   0
         Top             =   60
         Width           =   7125
      End
...
Private Function BodySet() As Boolean
    ...
    If Op_Sort(0).Value Then
        gSL_Select = "Select CASE WHEN 発行時間 <> '0000' Then substring(発行時間,1,2) + ':' + substring(発行時間,3,2) Else '' END 編集発行時間,発行時間 "
        gSL_Select = gSL_Select & " From 発注サイクルマスタ  "
        gSL_Select = gSL_Select & " Where 行識別 = 0 group by 発行時間 "
        mSL_OrderA = " Order By 発行時間 "
        
        DBGrid1.Splits(0).Columns(3).Merge = True
        DBGrid1.Splits(0).Columns(4).Merge = True
        
    Else
        gSL_Select = "Select CASE WHEN 発行時間 <> '0000' Then substring(発行時間,1,2) + ':' + substring(発行時間,3,2) Else '' END 編集発行時間,発行時間, "
        gSL_Select = gSL_Select & " 取引先,工場CD,CASE WHEN RTRIm(工場CD) = '' THEN 取引先 ELSE 取引先 + '-' + 工場CD END 表示取引先,  "
        gSL_Select = gSL_Select & "略称,工場名,受入,    "
        gSL_Select = gSL_Select & "Substring(サイクル,1,1) + '-' + Substring(サイクル,2,2) + '-' + Substring(サイクル,4,2) 表示サイクル "
        gSL_Select = gSL_Select & " From 発注サイクルマスタ HS "
        gSL_Select = gSL_Select & " Left Outer Join 取引先マスタ TR "
        gSL_Select = gSL_Select & "   ON  TR.取引先CD = HS.仕入先 "
        gSL_Select = gSL_Select & "   And TR.取引先区分 = 1 "
        gSL_Select = gSL_Select & " Where HS.行識別 = 0"
        mSL_OrderA = " Order By HS.発行時間,TR.取引先CD,HS.受入 "
        
        DBGrid1.Splits(0).Columns(3).Merge = False
        DBGrid1.Splits(0).Columns(4).Merge = False
    
    End If
' 選択条件セット
    Adodc1.RecordSource = gSL_Select & mSL_OrderA
    Adodc1.Refresh
...
Private Sub RtnSet()
    With Adodc1
        ...
        Else
           ...
           RV_Ukeir = ""
        End If
        RV_SYmd = .Recordset("発行時間")
        If RV_SYmd = "0000" Then
           RV_SYmd = ""
        End If
    End With
    Adodc1.Recordset.Close
