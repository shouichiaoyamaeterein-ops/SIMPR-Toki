...
Begin VB.Form CKK0078 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0078.frx":0000
         Height          =   3150
         Left            =   60
         OleObjectBlob   =   "CKK0078.frx":0015
         TabIndex        =   7
         Top             =   60
         Width           =   10575
      End
...
Private Function BodySet(SetType As String) As Boolean
    ...
    gSL_Select = ""
    gSL_Select = gSL_Select & " Select ZT.背番号, ZT.取引先, ZM.材質, ZT.材料管理番号,"
    gSL_Select = gSL_Select & "        ZM.板厚, ZM.幅, ZM.長さ, ISNULL(ZT.発注品番,'') 発注品番, ISNULL(ZT.受給品番,'') 受給品番,"
    gSL_Select = gSL_Select & "        CASE WHEN ISNULL(TR.略称,'') <> '' "
    gSL_Select = gSL_Select & "             Then ISNULL(ZT.取引先,'') + '：' + TR.略称"
    gSL_Select = gSL_Select & "             Else ISNULL(ZT.取引先,'') End 表示取引先,"
    gSL_Select = gSL_Select & "        ZT.材質 + ' ' +"
    gSL_Select = gSL_Select & "        CAST(ZM.板厚 AS VARCHAR) + 'X' +"
    gSL_Select = gSL_Select & "        CAST(ZM.幅   AS VARCHAR) + 'X' +"
    gSL_Select = gSL_Select & "        CAST(ZM.長さ AS VARCHAR) 材料"
...
'           mSL_SelectW = mSL_SelectW & " WHERE (品目 = 0 OR (品目 = 1 AND 手配区分 = 1))"
           mSL_SelectW = mSL_SelectW & " WHERE 手配区分 = 1"
        Else
'           mSL_SelectW = mSL_SelectW & "   AND (品目 = 0 OR (品目 = 1 AND 手配区分 = 1))"
           mSL_SelectW = mSL_SelectW & "   AND 手配区分 = 1"
        End If
    End With
