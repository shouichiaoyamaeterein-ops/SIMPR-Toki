...
Begin VB.Form CKK0010 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0010.frx":0000
         Height          =   5475
         Left            =   45
         OleObjectBlob   =   "CKK0010.frx":0015
         TabIndex        =   0
         Top             =   60
         Width           =   4695
      End
...
'End Sub
    ...
    Dim mKubun          As String
    Dim mAtai           As String
    Dim mAtaiNM         As String
...
Private Sub Form_Activate()
    ...
    RV_Rtn = False
'
    If Trim(RVI_Kubun) = "" Then
        FormCap = "区分名検索"
        HLB_Title = "区  分  名  検  索"
        DBGrid1.Columns(0).Caption = "桁数"
        DBGrid1.Columns(0).Alignment = dbgCenter
        DBGrid1.Columns(1).Caption = "区    分    名"
        DBGrid1.Columns(1).Order = 0
    Else
        gSL_Select = "SELECT * FROM 名称マスタ"
        gSL_Select = gSL_Select & " WHERE 区分名称 = '名称検索'"
        gSL_Select = gSL_Select & "   AND 値名称   = '" & RTrim(RVI_Kubun) & "'"
        ...
        Else
            With SYM
                gStr = Replace(.英数字1, "　", "")
                FormCap = Replace(gStr, " ", "")
                HLB_Title = RTrim(.英数字1)
                mKubun = RTrim(.値名称)
                mAtai = RTrim(.英数字2)
                mAtaiNM = RTrim(.英数字3)
                DBGrid1.Columns(0).Caption = mAtai
                DBGrid1.Columns(1).Caption = mAtaiNM
                If .数字1 <> 0 Then
                   gLong = DBGrid1.Columns(0).Width
                   DBGrid1.Columns(0).Width = .数字1 * 130
                   DBGrid1.Columns(1).Width = DBGrid1.Columns(1).Width - (.数字1 * 130 - gLong)
                End If
            End With
        End If
    End If
...
Private Function BodySet() As Boolean
    BodySet = False
    If Trim(RVI_Kubun) = "" Then
        gSL_Select = "SELECT 区分桁数 AS 値,区分名称 AS 値名称 FROM 名称マスタ"
        gSL_Select = gSL_Select & " WHERE 区分タイプ = 'K'"
        gSL_Select = gSL_Select & "   AND  制御区分 = '" & RVI_Seigyo & "'"
        mSL_OrderA = " Order By 区分名称"
        mSL_OrderD = " Order By 区分名称 Desc"
    Else
        gSL_Select = "SELECT 値,値名称 FROM 名称マスタ"
        gSL_Select = gSL_Select & " WHERE 区分名称 = '" & mKubun & "'"
        gSL_Select = gSL_Select & "   AND 区分タイプ  = ''"
        mSL_OrderA = " Order By 値"
        mSL_OrderD = " Order By 値 Desc"
    End If
' 選択条件セット
    mSL_SelectW = RTrim(Rv_Select)
    If Op_Sort(0) Then
