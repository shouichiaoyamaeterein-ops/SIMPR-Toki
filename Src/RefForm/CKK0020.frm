...
Begin VB.Form CKK0020 
   ...
   Begin VB.PictureBox B1_Area1 
      ...
      Begin TrueOleDBGrid60.TDBGrid DBGrid1 
         Bindings        =   "CKK0020.frx":0000
         Height          =   5580
         Left            =   90
         OleObjectBlob   =   "CKK0020.frx":0015
         TabIndex        =   5
         Top             =   60
         Width           =   8445
      End
...
'契約先区分
    If RV_TorKb1 = 1 Then
       Chk_Keiyaku.Value = 1
       Call Chk_Keiyaku_Click
    End If
'社内区分
    If RV_TorKb2 = 1 Then
       Chk_Syanai.Value = 1
       Call Chk_Syanai_Click
    End If
...
Private Sub Chk_Keiyaku_Click()
    If Chk_Keiyaku.Value = 1 Then
        Chk_Keiyaku.Caption = "限定"
    Else
        Chk_Keiyaku.Caption = "非限定"
    End If
End Sub
Private Sub Chk_Syanai_Click()
    If Chk_Syanai.Value = 1 Then
        Chk_Syanai.Caption = "限定"
    Else
        Chk_Syanai.Caption = "非限定"
    End If
End Sub
'****************************
'*                          *
...
Private Function BodySet() As Boolean
    ...
    BodySet = False
    gSL_Select = "SELECT 取引先,工場CD,CASE WHEN RTRIm(工場CD) = '' THEN 取引先 ELSE 取引先 + '-' + 工場CD END 表示取引先,  "
    gSL_Select = gSL_Select & "略称,取引先区分,"
    gSL_Select = gSL_Select & "契約先区分,社内区分,工場名    "
    gSL_Select = gSL_Select & " FROM 取引先マスタ "
    mSL_OrderA = " Order By 取引先区分,取引先CD "
    mSL_OrderD = " Order By 取引先区分 Desc,取引先CD Desc"
' 選択条件セット
    mSL_SelectW = ""
    With CisFun
        .Where_CreateKB = NewDocument
        .Where_Create AlphaMe, "索引カナ", iH1_SKana
        .Where_Create AlphaMe, "取引先", iH1_Torcd
        .Where_Create AlphaMe, "工場CD", iH1_Kojyo
        If Chk_Torkb(0).Value = 1 Or Chk_Torkb(1).Value = 1 Then
            If Chk_Torkb(0).Value = 0 Then
               .Where_Create Numeric, "取引先区分", 1
            End If
            If Chk_Torkb(1).Value = 0 Then
               .Where_Create Numeric, "取引先区分", 0, , Create
            End If
        End If
        If Chk_Keiyaku.Value = 1 Then
           .Where_Create Numeric, "契約先区分", 1
        End If
        If Chk_Syanai.Value = 1 Then
           .Where_Create Numeric, "社内区分", 1
        End If
