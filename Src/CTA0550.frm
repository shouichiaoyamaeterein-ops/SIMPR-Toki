...
Private Sub PB_Look_Click()
    ...
    If PB_LOOK.Tag = "iB1_Tanto" Then Call Look_Kubun: Exit Sub         ' 2019/09/10 ADD
...
'+       : 他(2) 担当         +
'+----------------------------+
Private Sub Look_Kubun(Optional KB As Integer = 2)
    RV_Left = 0
    RV_Top = 0
    Select Case KB
        Case 2: RVI_Kubun = "担当管理"
...
' DB更新確認
    With CisFun
        .MB_Lines = 4
        .MB_Title = wStr + "処理"
        .MB_MSG(2) = "     棚卸データメンテナンス　" + wStr + "処理を行います      "
        .MB_MSG(3) = "     よろしいですか？      "
        .MB_Button = OK_CAN
        If Not .MBOX Then Exit Function
    End With
...
Private Function DBInput() As Boolean
    ...
    gSL_Select = gSL_Select & vbCrLf & " ORDER BY TT.処理日, TT.読取連番"
    Call Str_Monitor(gSL_Select)
    If HHRRead(gSL_Select) Then
        Call GridSet
        DBInput = True
